-- Kimqetras HC v2.49 • restored v2.19 weapon-extras core + isolated wing flight
...
local KIMQ_BUILD = "v2.49-restored-v219-extras"
...
do
    ...
    if running and runningBuild == KIMQ_BUILD then
        ...
        pcall(function()
            StarterGui0:SetCore("SendNotification",{
                Title="Kimqetras HC",
                Text=(runtimeState=="ready") and "v2.49 is already running ♡" or "v2.49 is already starting ♡",
                Duration=4,
            })
        end)
        return
    ...
    if oldRuntimeDetected then
        pcall(function()
            StarterGui0:SetCore("SendNotification",{
                Title="Kimqetras HC",
                Text="An older Kimqetras HC runtime is still active. Rejoin once, then run v2.49. v2.49 will not stack on top of it.",
                Duration=9,
            })
        end)
        warn("[Kimqetras HC v2.49] Older runtime detected. Refusing to stack; rejoin once.")
        return
    end
    ...
    local knownRenderBindings = {
        "KimqMacroSpeedV240","KimqMacroSpeedV241","KimqMacroSpeedV242","KimqMacroSpeedV249",
        ...
        "KimqPermanentTimeV241","KimqPermanentTimeV242","KimqPermanentTimeV249",
    }
...
pcall(function()
    RunService:BindToRenderStep("KimqMacroSpeedV249",Enum.RenderPriority.Last.Value,function()
        enforceMacroSpeed()
    end)
end)
...
local function addCleanSection(title, subtitle)
    ...
    -- v2.49: pageHead already displays the section name.
...
task.spawn(function()
    ...
    -- v2.49: pages are grouped by what they actually do so the sidebar is
    ...
    local pageDefs = {
        ...
        -- RAGE
        {"ragecam", "rage camlock", "persistent target lock with a live player list", "RAGE"},
        {"rageorbit", "target orbit", "teleport to or circle your selected target", "RAGE"},
        {"ragecombat", "rage combat", "auto shoot and force-hit controls for the selected target", "RAGE"},
    ...
    do
        ...
        if rageCamPage and rageOrbitPage and rageCombatPage then
            ...
            local rageOrbitSpeed=2.5
            local rageOrbitRadius=9
            local rageOrbitHeight=5
            local rageOrbitAngle=0
            local rageWingAttempt=0

            local rageAutoShoot=false
            local rageShootInterval=.12
            ...
            local function selectedPlayer()
                ...
                return PlayersR:GetPlayerByUserId(selectedUserId)
            end

            local function targetName()
                local p=selectedPlayer()
                if not p then return "No target selected" end
            ...
            local function ensureAngelWings()
                ...
                if os.clock()-rageWingAttempt<1.5 then return end
            ...
            local function teleportToSelected()
                ...
                ensureAngelWings()
                local pos=targetRoot.Position + Vector3.new(0,math.max(rageOrbitHeight,3),rageOrbitRadius)
                pcall(function()
            ...
            local function forceHitSelected()
                local pl=selectedPlayer()
                local part=targetPartFor(pl,rageShootPart)
                local backend=_G.KimpetrasKIMBackend
                if backend and type(backend.ForceHitFire)=="function" and part then
                    local ok=pcall(backend.ForceHitFire,part)
                    return ok
                end
                return false
            end
            ...
            local function makeToggle(page,label,defaultValue,callback)
                ...
                local b=makeButton(c,state and "ON" or "OFF",UDim2.new(1,-68,.5,-15),UDim2.fromOffset(56,30),function()
                    state=not state
                ...
                if state then b.BackgroundColor3=T.hot; b.TextColor3=T.white end
                return function(v)
                    state=not not v
                    b.Text=state and "ON" or "OFF"
                    b.BackgroundColor3=state and T.hot or T.card2
            ...
            local function makeSlider(page,label,min,max,defaultValue,callback,suffix)
                ...
                local lbl=rageText(c,label,UDim2.new(.65,-12,0,24),UDim2.fromOffset(12,5),Enum.Font.GothamSemibold,12,T.text)
                ...
                local function setValue(v)
                    ...
                    val.Text=(suffix=="s" and string.format("%.2fs",value))
                        or (suffix=="x" and string.format("%.1fx",value))
                        or tostring(math.floor(value*10+.5)/10)..(suffix or "")
                    callback(value)
                end
            ...
            -- ---------------- RAGE CAMLOCK ----------------
            local camTargetCard=rageCard(rageCamPage,278)
            rageText(camTargetCard,"Selected Target",UDim2.new(.43,-18,0,22),UDim2.fromOffset(12,8),Enum.Font.GothamBold,13,T.hot)
            local camSelected=rageText(camTargetCard,targetName(),UDim2.new(.43,-18,0,36),UDim2.fromOffset(12,31),Enum.Font.Gotham,11,T.sub)
            camSelected.TextWrapped=true
            table.insert(selectedLabels,camSelected)

            local left=Instance.new("Frame",camTargetCard)
            left.Size=UDim2.new(.43,-14,0,184)
            left.Position=UDim2.fromOffset(8,76)
            left.BackgroundTransparency=1

            makeButton(left,"Lock",UDim2.fromOffset(0,0),UDim2.new(.5,-4,0,34),function()
                if selectedPlayer() then rageLockEnabled=true end
            end)
            makeButton(left,"Unlock",UDim2.new(.5,4,0,0),UDim2.new(.5,-4,0,34),function()
                rageLockEnabled=false
            end)
            makeButton(left,"View",UDim2.fromOffset(0,42),UDim2.new(.5,-4,0,34),function()
                if selectedPlayer() then rageViewEnabled=true; ensureViewTarget() end
            end)
            makeButton(left,"Unview",UDim2.new(.5,4,0,42),UDim2.new(.5,-4,0,34),function()
                restoreOwnCamera()
            end)
            makeButton(left,"Teleport",UDim2.fromOffset(0,84),UDim2.new(1,0,0,34),function()
                teleportToSelected()
            end)

            local persistence=rageText(left,"Lock stays selected through your respawn and resumes on the same player.",UDim2.new(1,0,0,54),UDim2.fromOffset(0,126),Enum.Font.Gotham,10,T.sub)
            persistence.TextWrapped=true

            local right=Instance.new("Frame",camTargetCard)
            right.Size=UDim2.new(.57,-18,1,-20)
            right.Position=UDim2.new(.43,8,0,10)
            right.BackgroundColor3=T.bg2
            right.BorderSizePixel=0
            corner(right,11)
            stroke(right,T.stroke,.3,1)

            rageText(right,"Players",UDim2.new(1,-76,0,26),UDim2.fromOffset(10,5),Enum.Font.GothamBold,12,T.text)
            local refreshPlayers=makeButton(right,"refresh",UDim2.new(1,-70,0,5),UDim2.fromOffset(60,26),function() end)

            playerList=Instance.new("ScrollingFrame",right)
            playerList.Position=UDim2.fromOffset(8,38)
            playerList.Size=UDim2.new(1,-16,1,-46)
            ...
            local function refreshPlayersList()
                ...
                for i,p in ipairs(list) do
                    ...
                    b.BackgroundColor3=(p.UserId==selectedUserId) and T.hot or T.panel
                    b.BorderSizePixel=0
                    b.Text=""
                    b.AutoButtonColor=false
                    corner(b,9)
                    stroke(b,(p.UserId==selectedUserId) and T.hot or T.stroke,.3,1)

                    local dn=rageText(b,p.DisplayName,UDim2.new(1,-12,0,19),UDim2.fromOffset(8,4),Enum.Font.GothamBold,11,(p.UserId==selectedUserId) and T.white or T.text)
                    local un=rageText(b,"@"..p.Name,UDim2.new(1,-12,0,15),UDim2.fromOffset(8,22),Enum.Font.Gotham,9,(p.UserId==selectedUserId) and T.white or T.sub)
            ...
            PlayersR.PlayerAdded:Connect(function() task.defer(refreshPlayersList) end)
            ...
            makeCycle(rageCamPage,"Target Part",{"Head","UpperTorso","HumanoidRootPart","Closest Part"},rageTargetPart,function(v)
                rageTargetPart=v
            end)
            makeSlider(rageCamPage,"Camlock Smoothness",.03,1,rageSmoothness,function(v) rageSmoothness=v end,"")

            -- ---------------- TARGET ORBIT ----------------
            local orbitTargetCard=rageCard(rageOrbitPage,72)
            rageText(orbitTargetCard,"Selected Target",UDim2.new(1,-24,0,22),UDim2.fromOffset(12,8),Enum.Font.GothamBold,13,T.hot)
            local orbitSelected=rageText(orbitTargetCard,targetName(),UDim2.new(1,-24,0,28),UDim2.fromOffset(12,34),Enum.Font.Gotham,11,T.sub)
            table.insert(selectedLabels,orbitSelected)

            local orbitAction=rageCard(rageOrbitPage,54)
            makeButton(orbitAction,"♥  Teleport To Target With Angel Wings",UDim2.fromOffset(10,10),UDim2.new(1,-20,0,34),function()
                teleportToSelected()
            end)
            makeToggle(rageOrbitPage,"Auto Orbit Target",false,function(v)
                rageOrbitEnabled=v
                if v then
                    ensureAngelWings()
                    rageOrbitAngle=0
            ...
            makeSlider(rageOrbitPage,"Orbit Speed",.15,8,rageOrbitSpeed,function(v) rageOrbitSpeed=v end,"x")
            makeSlider(rageOrbitPage,"Orbit Radius",3,28,rageOrbitRadius,function(v) rageOrbitRadius=v end,"")
            makeSlider(rageOrbitPage,"Orbit Height",-2,18,rageOrbitHeight,function(v) rageOrbitHeight=v end,"")

            local orbitNote=rageCard(rageOrbitPage,56)
            local note=rageText(orbitNote,"When Auto Orbit is on, the script tries to wear Angel Wings, flies you around the selected player, and keeps facing them.",UDim2.new(1,-24,1,-12),UDim2.fromOffset(12,6),Enum.Font.Gotham,10,T.sub)
            note.TextWrapped=true

            -- ---------------- RAGE COMBAT ----------------
            local combatTargetCard=rageCard(rageCombatPage,72)
            rageText(combatTargetCard,"Selected Target",UDim2.new(1,-24,0,22),UDim2.fromOffset(12,8),Enum.Font.GothamBold,13,T.hot)
            local combatSelected=rageText(combatTargetCard,targetName(),UDim2.new(1,-24,0,28),UDim2.fromOffset(12,34),Enum.Font.Gotham,11,T.sub)
            table.insert(selectedLabels,combatSelected)

            makeCycle(rageCombatPage,"Hit Part",{"Head","UpperTorso","HumanoidRootPart","Closest Part"},rageShootPart,function(v)
                rageShootPart=v
            end)
            makeToggle(rageCombatPage,"Auto Shoot Selected Target",false,function(v)
                rageAutoShoot=v
                rageShootAccumulator=0
            end)
            makeSlider(rageCombatPage,"Auto Shoot Delay",.03,1,rageShootInterval,function(v)
                rageShootInterval=v
            end,"s")

            local oneShotCard=rageCard(rageCombatPage,70)
            local oneShot=makeButton(oneShotCard,"♥  One-Shot Force Hit",UDim2.fromOffset(10,8),UDim2.new(1,-20,0,34),function()
                forceHitSelected()
            end)
            local oneShotNote=rageText(oneShotCard,"Sends one Force Hit request at the selected target. Actual damage still depends on the game's server rules.",UDim2.new(1,-20,0,20),UDim2.fromOffset(10,46),Enum.Font.Gotham,9,T.sub)
            oneShotNote.TextWrapped=true

            -- Persistent target lock / view / orbit / auto fire.
            RunServiceR.RenderStepped:Connect(function(dt)
                ...
                if rageLockEnabled and pl then
                    ...
                    if part and cameraR then
                        ...
                        cameraR.CFrame=cameraR.CFrame:Lerp(wanted,math.clamp(rageSmoothness,0,1))
                    end
                end
                ...
                if rageOrbitEnabled and pl then
            ...
            RunServiceR.Heartbeat:Connect(function(dt)
                if rageAutoShoot and selectedPlayer() then
            ...
            -- Your death/respawn does NOT clear the selected Rage target.
            lp.CharacterAdded:Connect(function()
                task.delay(.35,function()
                    if rageViewEnabled then ensureViewTarget() end
                    if rageOrbitEnabled then ensureAngelWings() end
                end)
            end)
...
-- v2.49 module: local catalog / limited accessory try-on.
task.spawn(function()
    _G.KimqAccessoryUIReady=false
    local Players = game:GetService("Players")
    local CoreGui = game:GetService("CoreGui")
...
-- v2.49 module: Environment + Weapon Skins (canonical sidebar pages).
task.spawn(function()
    local pageReadyStart=tick()
    while not _G.KimqBasePagesReady and tick()-pageReadyStart<8 do task.wait(.02) end
    ...
    -- v2.49: Environment and Weapon Skins are already real canonical pages/buttons.
    ...
    pcall(function()
        RunService:BindToRenderStep("KimqPermanentTimeV249",Enum.RenderPriority.Last.Value,function()
    ...
    if not envBuildOk then
        warn("[Kimqetras HC v2.49] Environment page fallback: "..tostring(envBuildErr))
    ...
    if not extrasOk then
        ...
        warn("[Kimqetras HC v2.49] restored v2.19 Weapon Extras: "..tostring(extrasErr))
    ...
    -- ========================================================
    -- v2.49 ISOLATED ANGEL WINGS FLIGHT
    ...
    skinsBtn.MouseButton1Click:Connect(function()
        task.defer(function()
            ...
            if extras and type(extras.Refresh)=="function" then
                ...
                if not ok then
                    warn("[Kimqetras HC v2.49] extras refresh: "..tostring(err))
...
task.spawn(function()
    ...
    -- v2.49: preserve the grouped sidebar order from pageDefs.
    ...
    local function applyTheme(name,settlePass)
        ...
        -- v2.49 hard guarantee: old controls created with literal pink/yellow
    ...
    -- v2.49: section navigation does not repaint/rescan the entire GUI.
    ...
    -- v2.49 reveal happens only after this initial theme pass is complete.
end)


-- v2.49: delayed post-show cleanup passes removed.
...
-- ========================================================
-- v2.49 branding is deterministic at construction time; no delayed scrub pass.

-- ========================================================
-- v2.49 DETERMINISTIC REVEAL
...
task.spawn(function()
    ...
    -- v2.49: remove the old one-page section divider labels from the
