-- Kimqetras HC v2.47 • restored v2.19 weapon-extras core + isolated wing flight
...
local KIMQ_BUILD = "v2.47-restored-v219-extras"
...
do
    ...
    if running and runningBuild == KIMQ_BUILD then
        ...
        pcall(function()
            StarterGui0:SetCore("SendNotification",{
                Title="Kimqetras HC",
                Text=(runtimeState=="ready") and "v2.47 is already running ♡" or "v2.47 is already starting ♡",
                Duration=4,
            })
        end)
        return
    ...
    if oldRuntimeDetected then
        pcall(function()
            StarterGui0:SetCore("SendNotification",{
                Title="Kimqetras HC",
                Text="An older Kimqetras HC runtime is still active. Rejoin once, then run v2.47. v2.47 will not stack on top of it.",
                Duration=9,
            })
        end)
        warn("[Kimqetras HC v2.47] Older runtime detected. Refusing to stack; rejoin once.")
        return
    end
    ...
    local knownRenderBindings = {
        "KimqMacroSpeedV240","KimqMacroSpeedV241","KimqMacroSpeedV242","KimqMacroSpeedV247",
        ...
        "KimqPermanentTimeV241","KimqPermanentTimeV242","KimqPermanentTimeV247",
    }
...
pcall(function()
    RunService:BindToRenderStep("KimqMacroSpeedV247",Enum.RenderPriority.Last.Value,function()
        enforceMacroSpeed()
    end)
end)
...
local function addCleanSection(title, subtitle)
    ...
    -- v2.47: pageHead already displays the section name.
...
task.spawn(function()
    ...
    -- v2.47: pages are grouped by what they actually do so the sidebar is
...
-- v2.47 module: local catalog / limited accessory try-on.
task.spawn(function()
    _G.KimqAccessoryUIReady=false
    local Players = game:GetService("Players")
    local CoreGui = game:GetService("CoreGui")
...
-- v2.47 module: Environment + Weapon Skins (canonical sidebar pages).
task.spawn(function()
    local pageReadyStart=tick()
    while not _G.KimqBasePagesReady and tick()-pageReadyStart<8 do task.wait(.02) end
    ...
    -- v2.47: Environment and Weapon Skins are already real canonical pages/buttons.
    ...
    pcall(function()
        RunService:BindToRenderStep("KimqPermanentTimeV247",Enum.RenderPriority.Last.Value,function()
    ...
    if not envBuildOk then
        warn("[Kimqetras HC v2.47] Environment page fallback: "..tostring(envBuildErr))
    ...
    if not extrasOk then
        ...
        warn("[Kimqetras HC v2.47] restored v2.19 Weapon Extras: "..tostring(extrasErr))
    ...
    -- ========================================================
    -- v2.47 ISOLATED ANGEL WINGS FLIGHT
    ...
    skinsBtn.MouseButton1Click:Connect(function()
        task.defer(function()
            ...
            if extras and type(extras.Refresh)=="function" then
                ...
                if not ok then
                    warn("[Kimqetras HC v2.47] extras refresh: "..tostring(err))
...
task.spawn(function()
    ...
    -- v2.47: preserve the grouped sidebar order from pageDefs.
    ...
    local function applyTheme(name,settlePass)
        ...
        -- v2.47 hard guarantee: old controls created with literal pink/yellow
    ...
    -- v2.47: section navigation does not repaint/rescan the entire GUI.
    ...
    -- v2.47 reveal happens only after this initial theme pass is complete.
end)


-- v2.47: delayed post-show cleanup passes removed.
...
-- ========================================================
-- v2.47 branding is deterministic at construction time; no delayed scrub pass.

-- ========================================================
-- v2.47 DETERMINISTIC REVEAL
...
task.spawn(function()
    ...
    -- v2.47: remove the old one-page section divider labels from the
