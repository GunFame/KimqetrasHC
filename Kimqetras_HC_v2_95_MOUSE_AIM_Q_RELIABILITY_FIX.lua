-- Kimqetras HC v2.95 • v2.94 base + reliable Q Mouse Aim activation
...
local KIMQ_BUILD = "v2.95-mouse-aim-q-reliability"
...
Hello.Text = "v2.94  •  body-true smooth mouse aim ♡"
...
local function currentInset()
    local u=UIS:GetMouseLocation()
    if mouseObj then
        local mx,my=tonumber(mouseObj.X),tonumber(mouseObj.Y)
        if mx and my then
            local d=Vector2.new(u.X-mx,u.Y-my)
            if math.abs(d.X)<=120 and math.abs(d.Y)<=120 then return d end
        end
    end
...
local function toScreen(pos)
    ...
    return Vector2.new(v.X+inset.X,v.Y+inset.Y),on and v.Z>0
end
...
local function candidateDistance(pl)
    ...
    if d>math.max(10,tonumber(cfg.mouseAimFOV) or 180) then return math.huge,nil,nil end
    return d,part,point
end
...
UIS.InputBegan:Connect(function(input,gpe)
    -- Do not let the game's own Q binding swallow Mouse Aim. We still ignore
    -- keyboard input while the user is actually typing in a TextBox/chat.
    if not cfg.mouseAimEnabled or UIS:GetFocusedTextBox() then return end
...
addKeybind("Mouse Aim Key",cfg.mouseAimKey,function(v) cfg.mouseAimKey=normalizeMouseAimKey(v) end)
...
task.spawn(function()
    ...
    pcall(function()
        game:GetService("StarterGui"):SetCore("SendNotification",{
            Title="Kimqetras HC",
            Text="v2.94 ACTIVE ♡ • body-true mouse aim + existing systems ready",
            Duration=7,
        })
    end)
