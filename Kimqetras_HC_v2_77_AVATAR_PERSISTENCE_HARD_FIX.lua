-- Kimqetras HC v2.77 • v2.76 base + isolated hard avatar persistence/respawn repair
...
local KIMQ_BUILD = "v2.77-avatar-persistence-hard-fix"
...
do
    ...
    if running and runningBuild == KIMQ_BUILD then
        if runningGui then
            ...
            notifyBoot((runtimeState=="ready") and "v2.77 is already running ♡" or "v2.77 is already starting ♡",4)
            return
        end
        ...
        notifyBoot("Recovering a failed v2.77 boot ♡",4)
    end
    ...
    if oldRuntimeDetected then
        ...
        if staleBoot then
            ...
            notifyBoot("STOP: another Kimqetras HC build is already active. v2.77 DID NOT LOAD. Rejoin once, then run only v2.77.",9)
            warn("[Kimqetras HC v2.77] Another build is active. v2.77 DID NOT LOAD; rejoin once before testing.")
            return
        end
    end
...
Hello.Text = "v2.77  •  hard avatar persistence fix ♡"
...
local function restoreKeptAvatar(char)
    ...
    avatarSuppressRepairUntil=os.clock()+1.20
    ...
    task.delay(restored and .30 or .55,function()
...
local function startKeptAvatarRespawnGuard(char)
    ...
    task.spawn(function()
        ...
        -- Absolute-ish settle coverage for games that load appearance late.
        local waits={.15,.35,.65,1.0,1.6,2.4,3.4}
        ...
        -- Lifetime integrity monitor: cheap checks only.  It does NOT constantly
        -- ApplyDescription; it repairs only if the copied visual markers disappear.
        while serial==avatarRespawnGuardSerial and AvatarEnabled and char==lp.Character and char.Parent do
            task.wait(1.25)
            if not avatarVisualLooksIntact(char) then restoreKeptAvatar(char) end
        end
    end)
    ...
    char.ChildRemoved:Connect(function(obj)
        ...
        task.delay(.12,function()
            if serial==avatarRespawnGuardSerial and AvatarEnabled and char==lp.Character
...
lp.CharacterAdded:Connect(function(char)
    if AvatarEnabled then
        -- New character = old monitor is invalid immediately.
...
lp.CharacterAppearanceLoaded:Connect(function(char)
    ...
    task.delay(.08,function()
        if AvatarEnabled and char==lp.Character and not avatarVisualLooksIntact(char) then
            restoreKeptAvatar(char)
        end
...
task.spawn(function()
    ...
    pcall(function()
        game:GetService("StarterGui"):SetCore("SendNotification",{
            Title="Kimqetras HC",
            Text="v2.77 ACTIVE ♡ • clean runtime • avatar persistence armed after Apply",
            Duration=7,
        })
    end)
