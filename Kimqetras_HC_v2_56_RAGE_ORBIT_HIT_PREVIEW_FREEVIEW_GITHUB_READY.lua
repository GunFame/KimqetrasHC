-- Kimqetras HC v2.56 • restored v2.19 weapon-extras core + isolated wing flight
...
local KIMQ_BUILD = "v2.56-restored-v219-extras"
...
do
    ...
    if running and runningBuild == KIMQ_BUILD then
        ...
        pcall(function()
            StarterGui0:SetCore("SendNotification",{
                Title="Kimqetras HC",
                Text=(runtimeState=="ready") and "v2.56 is already running ♡" or "v2.56 is already starting ♡",
                Duration=4,
            })
        end)
        return
    ...
    if oldRuntimeDetected then
        pcall(function()
            StarterGui0:SetCore("SendNotification",{
                Title="Kimqetras HC",
                Text="An older Kimqetras HC runtime is still active. Rejoin once, then run v2.56. v2.56 will not stack on top of it.",
                Duration=9,
            })
        end)
        warn("[Kimqetras HC v2.56] Older runtime detected. Refusing to stack; rejoin once.")
        return
    end
    ...
    local knownRenderBindings = {
        "KimqMacroSpeedV240","KimqMacroSpeedV241","KimqMacroSpeedV242","KimqMacroSpeedV256",
        ...
        "KimqPermanentTimeV241","KimqPermanentTimeV242","KimqPermanentTimeV256",
    }
...
pcall(function()
    RunService:BindToRenderStep("KimqMacroSpeedV256",Enum.RenderPriority.Last.Value,function()
        enforceMacroSpeed()
    end)
end)
...
LocalPlayer.CharacterAdded:Connect(function()
    -- v2.56: dying no longer clears the COMBAT Camlock target.
...
local function addCleanSection(title, subtitle)
    ...
    -- v2.56: pageHead already displays the section name.
...
task.spawn(function()
    ...
    -- v2.56: pages are grouped by what they actually do so the sidebar is
    ...
    task.defer(function()
        local rageOK,rageERR=pcall(function()
            ...
            local function button(parent,text,pos,size,fn)
                ...
                b.MouseButton1Click:Connect(function()
                    ...
                    if not ok then warn("[Kimqetras HC v2.56 RAGE button] "..tostring(err)) end
                end)
                return b
            end
            ...
            local function resolvePreviewPoseId()
                ...
                local direct="rbxassetid://80004965633316"
            ...
            local function showPreview(pl)
                ...
                task.spawn(function()
                    ...
                    playPreviewPose(model)

                    -- Let the requested pose actually change the bounds before fitting.
                    task.wait(.10)
                    ...
                    -- Some pose animations include root rotation. Re-normalize yaw while
                    -- leaving limb animation intact.
                    if root then
                        previewPoseConn=RunServiceR.RenderStepped:Connect(function()
                            if token~=previewToken or not root.Parent then return end
                            pcall(function()
            ...
            local function startFreeView(pl)
                ...
                local cam=workspace.CurrentCamera
                if not tr or not cam then return false end
                ...
                local focus=tr.Position+Vector3.new(0,2.4,0)
                local offset=cam.CFrame.Position-focus
                local mag=offset.Magnitude
                if mag<1 then
                    offset=Vector3.new(0,2,12)
                    mag=offset.Magnitude
                end

                viewDistance=math.clamp(mag,5,35)
                viewPitch=math.asin(math.clamp(offset.Y/viewDistance,-.98,.98))
            ...
            local function startOrbitMotion()
                ...
                orbitMotionConn=RunServiceR.Heartbeat:Connect(function(dt)
                    local ok,err=pcall(function()
                        ...
                        orbitAngle=(orbitAngle + dt*math.max(orbitSpeed,.05)*math.pi*2)%(math.pi*2)

                        local focus=tr.Position+Vector3.new(0,1.45,0)
                        local pos=Vector3.new(
                            tr.Position.X+math.cos(orbitAngle)*orbitRadius,
                            tr.Position.Y+orbitHeight,
                            tr.Position.Z+math.sin(orbitAngle)*orbitRadius
                        )
                    ...
                    if not ok then
                        warn("[Kimqetras HC v2.56 RAGE orbit] "..tostring(err))
                    end
                end)
            end
            ...
            local function normalGunShoot(pl)
                ...
                -- Normal Gun is one normal trigger + one targeted Shoot request.
                pcall(function() tool:Activate() end)
                pcall(ForceHit_Fire,part)
                return true
            end
            ...
            makeToggle(rageCombatPage,"Rage Combat Master",false,function(v)
                ...
                teleportClock=0
                if v then
                    -- One switch: orbit + auto-fire + KO/stomp + repeat after respawn.
                    tryEquipRageWings()
                    teleportTo(currentPlayer(),false)
                    startOrbitAnimation()
                    startOrbitMotion()
            ...
            RunServiceR.RenderStepped:Connect(function(dt)
                ...
                if not ok then
                    warn("[Kimqetras HC v2.56 RAGE camera] "..tostring(err))
                end
            end)
            ...
            RunServiceR.Heartbeat:Connect(function(dt)
                ...
                if not ok then
                    warn("[Kimqetras HC v2.56 RAGE heartbeat] "..tostring(err))
                end
            end)
        ...
        if not rageOK then
            warn("[Kimqetras HC v2.56] RAGE module isolated error: "..tostring(rageERR))
        end
    end)
...
-- v2.56 module: local catalog / limited accessory try-on.
task.spawn(function()
    _G.KimqAccessoryUIReady=false
    local Players = game:GetService("Players")
    local CoreGui = game:GetService("CoreGui")
...
-- v2.56 module: Environment + Weapon Skins (canonical sidebar pages).
task.spawn(function()
    local pageReadyStart=tick()
    while not _G.KimqBasePagesReady and tick()-pageReadyStart<8 do task.wait(.02) end
    ...
    -- v2.56: Environment and Weapon Skins are already real canonical pages/buttons.
    ...
    pcall(function()
        RunService:BindToRenderStep("KimqPermanentTimeV256",Enum.RenderPriority.Last.Value,function()
    ...
    if not envBuildOk then
        warn("[Kimqetras HC v2.56] Environment page fallback: "..tostring(envBuildErr))
    ...
    if not extrasOk then
        ...
        warn("[Kimqetras HC v2.56] restored v2.19 Weapon Extras: "..tostring(extrasErr))
    ...
    -- ========================================================
    -- v2.56 ISOLATED ANGEL WINGS FLIGHT
    ...
    skinsBtn.MouseButton1Click:Connect(function()
        task.defer(function()
            ...
            if extras and type(extras.Refresh)=="function" then
                ...
                if not ok then
                    warn("[Kimqetras HC v2.56] extras refresh: "..tostring(err))
...
task.spawn(function()
    ...
    -- v2.56: preserve the grouped sidebar order from pageDefs.
    ...
    local function applyTheme(name,settlePass)
        ...
        -- v2.56 hard guarantee: old controls created with literal pink/yellow
    ...
    -- v2.56: section navigation does not repaint/rescan the entire GUI.
    ...
    -- v2.56 reveal happens only after this initial theme pass is complete.
end)


-- v2.56: delayed post-show cleanup passes removed.
...
-- ========================================================
-- v2.56 branding is deterministic at construction time; no delayed scrub pass.

-- ========================================================
-- v2.56 DETERMINISTIC REVEAL
...
task.spawn(function()
    ...
    -- v2.56: remove the old one-page section divider labels from the
