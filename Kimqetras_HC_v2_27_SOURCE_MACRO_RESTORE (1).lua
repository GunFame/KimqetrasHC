-- Kimqetras HC v2.27 • source macro restore + v2.26 features
-- Keeps the v2.26 feature set and replaces the Macro page with the exact Speed behavior from the supplied source script.
-- No other feature system is replaced in this build.
-- Re-running this SAME build only reopens the current GUI instead of stacking another copy.
-- If another build is already active, v2.27 refuses to stack and asks for one clean rejoin.
do
    local SINGLE_KEY = "KimqHC_v21_SingleRuntimeOptimized"
    local BUILD = "v2.27-source-macro-restore"
    ...
    -- Older builds did not expose a complete disconnect/cleanup handle. Starting v2.27
    ...
    if rawget(_G, SINGLE_KEY) and rawget(_G, "KimqHC_CurrentBuild") ~= BUILD then
        pcall(function()
            game:GetService("StarterGui"):SetCore("SendNotification", {
                Title = "Kimqetras HC",
                Text = "Older runtime detected. Rejoin once, then run v2.27 so only one version is active.",
                Duration = 7,
            })
        end)
        warn("[Kimqetras HC v2.27] Older runtime detected. Rejoin once before running v2.27; refusing to stack versions.")
        return
    end
...
-- v2.27 keeps the v2.26 base and replaces only the Macro/Speed implementation.
...
-- ========================================================
-- MACRO / SPEED - restored from the user's supplied source
-- ========================================================
-- The supplied script uses ONE shared state table through _G:
--   SpeedMaster, SpeedActive, SpeedValue, SpeedKey
-- Its key handler + Heartbeat live in the original backend later in this file.
-- This page only controls those exact values so there is no second macro system
-- competing with the source behavior.

_G.SpeedMaster = _G.SpeedMaster == true
_G.SpeedActive = _G.SpeedActive == true
_G.SpeedValue = tonumber(_G.SpeedValue) or 50
if typeof(_G.SpeedKey) ~= "EnumItem" then
...
-- Same behavior as the source's Speed Master toggle.
...
-- Same key/value controls as the source script.
...
-- Original Speed/Macro loop from the supplied script.
-- Do not add a second key listener here: the original backend InputBegan above
-- already toggles _G.SpeedActive from _G.SpeedKey.
