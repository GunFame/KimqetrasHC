-- Kimqetras HC v2.26 • original macro restore + wing animation/theme/halo + day/night polish
-- Keeps the stable v2.25 feature set, restores the original WalkSpeed macro behavior,
-- strengthens wing animation playback, theme-syncs the wing mini GUI, and adds a head halo from weapon skins.
-- Re-running this SAME build only reopens the current GUI instead of stacking another copy.
-- If another build is already active, v2.26 refuses to stack and asks for one clean rejoin.
do
    local SINGLE_KEY = "KimqHC_v21_SingleRuntimeOptimized"
    local BUILD = "v2.26-original-macro-wing-anim-theme-halo"

    if rawget(_G, SINGLE_KEY) and rawget(_G, "KimqHC_CurrentBuild") == BUILD then
        pcall(function()
            ...
            if root and root:IsA("ScreenGui") then root.Enabled = true end
    ...
    -- Older builds did not expose a complete disconnect/cleanup handle. Starting v2.26
    -- on top of one could leave invisible old RenderStepped/Input connections alive.
    -- Refuse that unsafe hot-stack and require one clean rejoin instead.
    if rawget(_G, SINGLE_KEY) and rawget(_G, "KimqHC_CurrentBuild") ~= BUILD then
        pcall(function()
            game:GetService("StarterGui"):SetCore("SendNotification", {
                Title = "Kimqetras HC",
                Text = "Older runtime detected. Rejoin once, then run v2.26 so only one version is active.",
                Duration = 7,
            })
        end)
        warn("[Kimqetras HC v2.26] Older runtime detected. Rejoin once before running v2.26; refusing to stack versions.")
        return
    end
...
-- v2.26 keeps the stable base and replaces only the requested systems.
-- The broken experimental target-jump camera patches are not carried forward.
...
-- Primary Silent Aim page controls. These directly edit the core cfg table used by
-- the existing Silent Aim targeting loop, and every card is stamped as Silent Aim.
...
addDropdown("Hit Part", silentParts, cfg.silentAimPart, function(v) cfg.silentAimPart = v end)
addSlider("Max Target Distance", 50, 5000, cfg.silentAimMaxDist, function(v) cfg.silentAimMaxDist = v end)
addDecimalSlider("Prediction X", 0, 0.5, cfg.silentAimPredX, 3, function(v) cfg.silentAimPredX = v end)
addDecimalSlider("Prediction Y", 0, 0.5, cfg.silentAimPredY, 3, function(v) cfg.silentAimPredY = v end)
...
-- ========================================================
-- MACRO / SPEED - original behavior, reliability-cleaned
-- ========================================================
...
-- Keep the separate I/O option the user already had, but do not change the
-- movement macro itself: movement is the original Humanoid.WalkSpeed method.
local MacroIOSpam = false
local MacroIOInterval = 0.028
...
local MacroWalkSpeedConn = nil
local MacroHumanoid = nil
local applyingMacroSpeed = false
...
local function disconnectMacroHumanoid()
    if MacroWalkSpeedConn then pcall(function() MacroWalkSpeedConn:Disconnect() end) end
    MacroWalkSpeedConn=nil
    MacroHumanoid=nil
end

local function forceMacroSpeed()
    if not MacroMaster or not MacroActive then return end
    local hum=currentHumanoid()
    if not hum or hum.Health<=0 then return end
    if MacroHumanoid~=hum then
        disconnectMacroHumanoid()
        MacroHumanoid=hum
        if MacroOriginalWalkSpeed==nil then MacroOriginalWalkSpeed=hum.WalkSpeed end
        MacroWalkSpeedConn=hum:GetPropertyChangedSignal("WalkSpeed"):Connect(function()
            if MacroMaster and MacroActive and hum.Parent and not applyingMacroSpeed and hum.WalkSpeed~=MacroSpeed then
                applyingMacroSpeed=true
                pcall(function() hum.WalkSpeed=MacroSpeed end)
                applyingMacroSpeed=false
            end
        end)
    end
    if hum.WalkSpeed~=MacroSpeed then
        applyingMacroSpeed=true
        pcall(function() hum.WalkSpeed=MacroSpeed end)
        applyingMacroSpeed=false
    end
...
local function stopMacroMovement()
    local hum=MacroHumanoid or currentHumanoid()
    if hum and MacroOriginalWalkSpeed~=nil then
        pcall(function() hum.WalkSpeed=MacroOriginalWalkSpeed end)
    end
    disconnectMacroHumanoid()
...
addToggle("Macro / Speed Master", MacroMaster, function(v)
    ...
    if not MacroMaster then
        MacroActive = false
        stopMacroMovement()
        MacroIOAccumulator=0; MacroIOFlip=false
    end
end)
...
-- Original range and original WalkSpeed behavior.
addSlider("Macro Speed", 16, 1000, MacroSpeed, function(v)
    MacroSpeed = tonumber(v) or 50
    if MacroMaster and MacroActive then forceMacroSpeed() end
end)
...
local MacroHintCard = createCard(46)
...
MacroHint.Text = "Turn Master on, then press the Macro Key. I/O spam is optional and stops with the macro."
...
UIS.InputBegan:Connect(function(input)
    -- Keep the original key behavior, but don't let Roblox's gameProcessed flag
    -- swallow the macro key. Only ignore it while the user is actually typing.
    ...
    if input.UserInputType == Enum.UserInputType.Keyboard and input.KeyCode == MacroKey then
        if MacroMaster then
            ...
            if MacroActive then
                local hum=currentHumanoid()
                MacroOriginalWalkSpeed=hum and hum.WalkSpeed or 16
                forceMacroSpeed()
            else
                stopMacroMovement()
                MacroIOAccumulator=0; MacroIOFlip=false
...
local function sendMacroIO(keyCode)
    ...
    if VirtualInputManager then
        sent=pcall(function()
            ...
            task.delay(.006,function() pcall(function() VirtualInputManager:SendKeyEvent(false,keyCode,false,game) end) end)
        end)
    end
...
-- Heartbeat is the original implementation. The property hook above simply puts
-- the same WalkSpeed back if this game tries to overwrite it between heartbeats.
...
lp.CharacterAdded:Connect(function()
    disconnectMacroHumanoid()
    MacroOriginalWalkSpeed=nil
    if MacroMaster and MacroActive then task.delay(.25,forceMacroSpeed) end
end)
...
local function applyESPColorRGB()
    ...
    _G.KimqESPColor = ExtraESPColor
    if ESPColorPreview and ESPColorPreview.Parent then
        ESPColorPreview.BackgroundColor3 = ExtraESPColor
    end
end
...
espColorLabel.BackgroundTransparency=1; espColorLabel.Text="ESP Color  •  RGB gives you any color"
...
local espStroke=Instance.new("UIStroke",ESPColorPreview); espStroke.Color=Color3.fromRGB(255,255,255); espStroke.Transparency=.15; espStroke.Thickness=1
...
-- Avatar target is a real preset value too, not just the toggle beside it.
...
_G.KimqAvatarController = {
    ...
    ApplySaved = function()
        AvatarTarget = AvatarTargetBox.Text
        if AvatarEnabled and AvatarTarget ~= "" and lp.Character then
            task.spawn(function()
                task.wait(0.08)
                applyFullOutfit(lp.Character)
            end)
...
local function SaveConfig(configName)
    ...
    local configData = {
        format = "KimqetrasHC-v2.6-all-settings",
        name = name,
...
local function LoadConfig(configName)
    ...
    -- Avatar target is restored as a control; apply it only after every toggle has
    -- been restored so Enable Avatar / Visual Headless are already in final state.
    if _G.KimqAvatarController and type(_G.KimqAvatarController.ApplySaved)=="function" then
        task.defer(function() pcall(_G.KimqAvatarController.ApplySaved) end)
    end
...
local function addCleanSection(title, subtitle)
    ...
    if key then card:SetAttribute("KimqSection", key) end
...
-- COMBAT -------------------------------------------------
addCleanSection("Combat", "Extra combat controls")
...
addCleanSection("Force Hit", "Adjust force-hit settings")
...
-- AVATAR -------------------------------------------------
addCleanSection("Headless", "Toggle the headless look")
local KIMHeadless = false
addToggle("Headless Mode", KIMHeadless, function(v)
    KIMHeadless = v
...
savedTitle.Position = UDim2.fromOffset(10,5)
...
savedHint.Size = UDim2.new(1,-20,0,18)
savedHint.Position = UDim2.fromOffset(10,28)
savedHint.BackgroundTransparency = 1
savedHint.Text = "pick a preset, then load it ♡"
savedHint.TextColor3 = Color3.fromRGB(184,100,125)
savedHint.Font = Enum.Font.Gotham
savedHint.TextSize = 11
...
savedList.Size = UDim2.new(1,-20,0,124)
savedList.Position = UDim2.fromOffset(10,52)
...
local _, saveConfigButton = addButton("Save Current Config", function()
...
local _, updateConfigButton = addButton("Update Selected Config", function()
    if not selectedConfig or selectedConfig == "" then
        pcall(function()
            game:GetService("StarterGui"):SetCore("SendNotification", {
                Title="KIM ♡", Text="Select a saved config first", Duration=3
            })
        end)
        return
    end

    -- The Config Name box can now rename the selected config while updating it.
    ...
    if KIM.SaveConfig and KIM.SaveConfig(wantedName) then
        if wantedName ~= oldName and KIM.DeleteConfig then
            pcall(function() KIM.DeleteConfig(oldName) end)
        end
        selectedConfig = wantedName
        configName = wantedName
        configBox.Text = wantedName
        _G.KimqSelectedConfig = wantedName
        refreshConfigList()
    end
end)
updateConfigButton.Font = Enum.Font.GothamSemibold
updateConfigButton.TextSize = 13

local _, loadConfigButton = addButton("Load Selected Config", function()
...
local _, deleteConfigButton = addButton("Delete Selected Config", function()
    local name = selectedConfig or configName
...
deleteConfigButton.Font = Enum.Font.GothamSemibold
deleteConfigButton.TextSize = 13
local _, refreshConfigButton = addButton("Refresh Config List", function() refreshConfigList() end)
refreshConfigButton.Font = Enum.Font.GothamSemibold
refreshConfigButton.TextSize = 13

-- v2.3: keep the original GUI typography, but make these four config actions
-- visually identical. Two could inherit a text stroke from an older repaint path.
for _,b in ipairs({saveConfigButton,updateConfigButton,loadConfigButton,deleteConfigButton,refreshConfigButton}) do
    b.Font = Enum.Font.GothamSemibold
    b.TextSize = 13
    b.TextStrokeColor3 = Color3.fromRGB(255,255,255)
    b.TextStrokeTransparency = 0.42
end

addSmallNote("Configs save your setup, including the exact fog color and amount, so it comes back the same when loaded.")
...
task.spawn(function()
    ...
    local pageDefs = {
        ...
        {"silent", "silent aim", "adjust your targeting settings"},
        {"macro", "macro", "control how your macro behaves"},
        {"whitelist", "whitelist", "choose players you want to ignore"},
        {"protection", "protection", "extra protection options"},
        {"antifall", "anti fall", "helps with unwanted falling states"},
        {"delay", "delay changer", "adjust weapon delay settings"},
        {"esp", "esp", "customize player ESP"},
        {"avatar", "avatar", "change your local avatar look"},
        {"fog", "fog / atmosphere", "customize the atmosphere and colors"},
        {"hcsilent", "HC silent aim", "extra targeting settings"},
        {"forcehit", "force hit", "adjust force-hit settings"},
        {"hitbox", "hitbox expander", "adjust hitbox settings"},
        {"flamelock", "flamelock", "customize flamelock settings"},
        {"camlock", "camlock", "customize camlock settings"},
        {"headless", "headless", "toggle the headless look"},
        {"antimod", "anti mod", "extra anti-mod options"},
        {"spawn", "spawn point", "choose where you return after respawning"},
        {"settings", "settings", "performance and saved setup options"},
        {"theme", "theme", "change the colors of the whole interface"},
        {"info", "information", "credits and roles for this build"},
    }
    ...
    do
        ...
        if spawnPage then
            ...
            local function setRespawnLocationProperty(obj)
                pcall(function()
                    if useSelectedSpawn and obj and obj:IsA("SpawnLocation") then
                        lp.RespawnLocation = obj
                    elseif not useSelectedSpawn then
            ...
            local function moveCharacter(char, forcePhysicalMove)
                ...
                -- Prefer RespawnLocation for real Roblox spawns. Spawn Here Now and the
                -- respawn fallback can still request one safe physical move when needed.
                ...
                -- Generic map markers need a local move. Freeze the rig for only a couple
                -- frames, disable collisions during placement, and clear all velocity so
                -- the fresh character cannot be launched by intersecting map geometry.
                local savedCollide = {}
                ...
                local lift = math.max(5.0, (hum and hum.HipHeight or 2) + 2.75)
                ...
                pcall(function() root.Anchored = oldAnchored end)
            ...
            nowBtn.MouseButton1Click:Connect(function()
                ...
                local old = useSelectedSpawn
                useSelectedSpawn = true
                local ok = moveCharacter(lp.Character,true)
                useSelectedSpawn = old
                behaviorStatus.Text = ok and ("Moved to: "..tostring(selectedName)) or "Could not move to that point."
            ...
            lp.CharacterAdded:Connect(function(char)
                ...
                local obj = selectedSpawn
                if not (obj and obj.Parent) then obj = resolvePath(selectedPath); selectedSpawn = obj end
                if obj and obj:IsA("SpawnLocation") then setRespawnLocationProperty(obj) end

                -- Never move a half-built character. Wait for the root/humanoid and let the
                -- game's own spawn code settle first. If RespawnLocation already placed us
                -- correctly we do nothing; otherwise perform exactly one collision-safe move.
                task.spawn(function()
                    local root = char:WaitForChild("HumanoidRootPart",5)
                    local hum = char:FindFirstChildOfClass("Humanoid") or char:WaitForChild("Humanoid",5)
                    if not root or not hum then return end
                    task.wait(1.05)
                    if not (char==lp.Character and char.Parent and useSelectedSpawn) then return end
                    local liveObj=selectedSpawn
                    if not (liveObj and liveObj.Parent) then liveObj=resolvePath(selectedPath); selectedSpawn=liveObj end
                    if liveObj and liveObj:IsA("SpawnLocation") then
                        local dist=(root.Position-liveObj.Position).Magnitude
                        if dist>14 then moveCharacter(char,true) end
                    else
...
task.spawn(function()
    ...
    -- Permanent Day / Night -------------------------------------------------
    -- The game is allowed to keep running its own clock script, but whenever it
    -- tries to change ClockTime we immediately put the locally selected time back.
    -- This avoids the constant day/night sky flicker from competing loops.
    local timeMode = "Game Default"
    local nightLights = false
    -- If the script is loaded during the game's normal daylight, keep that exact
    -- daylight hour. If it was loaded at night, use neutral noon as the day preset.
    local DAY_CLOCK = tonumber(original.ClockTime) or 12.0
    if DAY_CLOCK < 6 or DAY_CLOCK > 18 then DAY_CLOCK = 12.0 end
    local NIGHT_CLOCK = 0.0
    local forcingClock = false
    local timeFallbackAccum = 0
    local lightOriginal = setmetatable({}, {__mode="k"})
    local lightHooks = setmetatable({}, {__mode="k"})

    local function isMapLight(obj)
        if not (obj:IsA("PointLight") or obj:IsA("SpotLight") or obj:IsA("SurfaceLight")) then return false end
        -- Do not turn muzzle flashes / bullet glow back up when Night Lights is enabled.
        for _,plr in ipairs(Players:GetPlayers()) do
            if plr.Character and obj:IsDescendantOf(plr.Character) then return false end
        end
    ...
    local function rememberLight(light)
        if not isMapLight(light) then return end
        if lightOriginal[light]==nil then lightOriginal[light]=light.Enabled end
        if not lightHooks[light] then
            lightHooks[light]=light:GetPropertyChangedSignal("Enabled"):Connect(function()
    ...
    for _,d in ipairs(workspace:GetDescendants()) do if isMapLight(d) then rememberLight(d) end end
    workspace.DescendantAdded:Connect(function(d) if isMapLight(d) then task.defer(function() if d.Parent then rememberLight(d) end end) end end)

    local function applyNightLights()
        local shouldForce = timeMode=="Night" and nightLights
        for light,wasEnabled in pairs(lightOriginal) do
            if light and light.Parent then
                pcall(function() light.Enabled = shouldForce and true or wasEnabled end)
            end
        end
    end

    local function desiredClock()
        if timeMode=="Day" then return DAY_CLOCK end
        if timeMode=="Night" then return NIGHT_CLOCK end
        return nil
    end
    local function forceSelectedClock()
        local wanted=desiredClock()
        if not wanted then return end
        if math.abs((Lighting.ClockTime or 0)-wanted)>.01 then
            forcingClock=true
    ...
    local timeCard=card(envPage,184); timeCard.Name="KimqPermanentTimeCard"
    txt(timeCard,"♥  Day / Night",UDim2.new(1,-24,0,24),UDim2.fromOffset(12,8),Enum.Font.GothamBold,14,textColor)
    local timeSub=txt(timeCard,"Lock the map to one time so the game's changing clock cannot flicker your sky.",UDim2.new(1,-24,0,34),UDim2.fromOffset(12,31),Enum.Font.Gotham,10,subColor); timeSub.TextWrapped=true

    local function timeChoiceButton(label,x,w)
    ...
    local defaultTimeBtn=timeChoiceButton("game default",0,.34)
    local dayTimeBtn=timeChoiceButton("day",.34,.33)
    local nightTimeBtn=timeChoiceButton("night",.67,.33)

    local nightLightLabel=txt(timeCard,"Night Lights",UDim2.new(0,180,0,24),UDim2.fromOffset(12,116),Enum.Font.GothamSemibold,11,textColor)
    local nightLightToggle=Instance.new("TextButton",timeCard); nightLightToggle.Size=UDim2.fromOffset(48,24); nightLightToggle.Position=UDim2.fromOffset(196,116); nightLightToggle.Text=""; nightLightToggle.AutoButtonColor=false; nightLightToggle.BorderSizePixel=0; corner(nightLightToggle,999)
    local nightKnob=Instance.new("Frame",nightLightToggle); nightKnob.Size=UDim2.fromOffset(18,18); nightKnob.Position=UDim2.new(0,3,.5,-9); nightKnob.BackgroundColor3=Color3.new(1,1,1); nightKnob.BorderSizePixel=0; corner(nightKnob,999)
    ...
    local function refreshTimeUI()
        local function style(b,on) b.BackgroundColor3=on and hotColor or lightColor; b.TextColor3=on and Color3.new(1,1,1) or textColor end
        style(defaultTimeBtn,timeMode=="Game Default"); style(dayTimeBtn,timeMode=="Day"); style(nightTimeBtn,timeMode=="Night")
        nightLightToggle.BackgroundColor3=nightLights and hotColor or lightColor
        ...
        elseif timeMode=="Night" then timeStatus.Text=nightLights and "Night is locked at 12:00 AM • map lights forced on ♡" or "Night is locked at 12:00 AM."
        else timeStatus.Text="Game clock is unchanged." end
        timeStatus.TextColor3=timeMode=="Game Default" and subColor or hotColor
    end
    ...
    nightLightToggle.MouseButton1Click:Connect(function() nightLights=not nightLights; applyNightLights(); refreshTimeUI() end)

    Lighting:GetPropertyChangedSignal("ClockTime"):Connect(function()
        if not forcingClock and desiredClock() then forceSelectedClock() end
    end)
    -- Put the chosen clock back immediately before the frame renders. This makes
    -- a game-side day/night loop much less likely to visibly flash the sky.
    pcall(function()
        RunService:BindToRenderStep("KimqPermanentTimeV226", Enum.RenderPriority.Last.Value, function()
            if desiredClock() then forceSelectedClock() end
        end)
    end)
    refreshTimeUI()

    -- Extend the existing environment controller instead of creating another one.
    ...
    local function applySkin(w,s,tool,quiet)
        ...
        local mirrorParts=(clientScripts==0 and embedded==0 and mirrored==0)
        if mirrorParts then
            -- Live-mirror mode owns every cosmetic part CFrame, so remove only the
            -- helper welds we created and anchor the clone locally.  This avoids the
            -- physics solver fighting the animation and keeps the cosmetic from
            -- affecting the character/gun at all.
            for _,d in ipairs(visual:GetDescendants()) do
                if (d:IsA("WeldConstraint") and (d.Name=="KimqV21SkinRootWeld" or d.Name=="KimqV21LooseVisualWeld")) then
                    d:Destroy()
                elseif d:IsA("BasePart") then
        ...
        -- Only use the per-frame source mirror as a last resort. If an Animation ID
        -- successfully started, the cloned rig owns its own pose and does not need a
        -- RenderStepped copier (which also keeps performance much smoother).
        if mirrorParts then
            startSourceMirror(gun,sourceContainer,sourceRoot,visual,cloneRoot,sourcePairs,true)
        end
        ...
        if not quiet then
            ...
            elseif animated then detail=mirrorParts and " • live animation mirror active" or " • animated effects preserved" end
    ...
    local function setupWeaponExtras()
        ...
        local function softenBulletGlow(d)
            ...
            if d:IsA("PointLight") or d:IsA("SpotLight") or d:IsA("SurfaceLight") then
                pcall(function() d.Brightness=math.min(d.Brightness,0.65) end)
                pcall(function() d.Range=math.min(d.Range,8) end)
            elseif d:IsA("Beam") or d:IsA("Trail") or d:IsA("ParticleEmitter") then
                pcall(function() d.LightEmission=math.min(d.LightEmission,0.30) end)
                pcall(function() d.LightInfluence=math.max(d.LightInfluence,0.25) end)
            end
        end
        ...
        -- Smooth Angel Wings flight -----------------------------------------
        -- Double-tap Space while the wings are worn to start/stop. Flight is
        -- completely local and cleans itself up on wing removal or respawn.
        local wingFlightEnabled=true
        local wingFlightActive=false
        local wingFlightSpeed=46
        local wingFlightVerticalSpeed=30
        local wingFlightVelocity=nil
        local wingFlightGyro=nil
        local wingFlightConn=nil
        local wingFlightHum=nil
        local wingFlightOldAutoRotate=true
        local wingFlightLastSpace=0
        local wingFlightStatus=nil
        local wingFlightToggleButton=nil
        local wingFlightAnimationTrack=nil
        local wingFlightAnimationObject=nil
        local wingFlightAnimationId="rbxassetid://108343889270403"
        local wingFlightAnimationAssetId=108343889270403
        local wingFlightAnimationMode="none"
        local wingFlightAnimateScript=nil
        local wingFlightAnimateWasDisabled=nil
        local wingFlightPoseMotors={}
        local activeWingHeadHalo=nil
        local wingMiniGui=nil
        local wingMiniFrame=nil
        local wingMiniTop=nil
        local wingMiniMask=nil
        local wingMiniTitle=nil
        local wingMiniClose=nil
        local wingMiniBar=nil
        local wingMiniSpeedLabel=nil
        local wingMiniFill=nil
        local wingMiniKnob=nil
        local wingMiniStatus=nil
        local wingMiniHelp=nil

        local function wingsAreWorn()
            return activeWornEquippable and activeWornEquippable.Parent and activeWornEquippable.Name=="KimqWornAngelWings"
        end

        local function clearWingHeadHalo()
            if activeWingHeadHalo and activeWingHeadHalo.Parent then
                pcall(function() activeWingHeadHalo:Destroy() end)
            end
            activeWingHeadHalo=nil
            local char=lp.Character
            if char then
                local old=char:FindFirstChild("KimqAngelWingHeadHalo")
                if old then pcall(function() old:Destroy() end) end
            end
        end

        local function haloCandidateScore(obj)
            if not obj then return -1 end
            local n=tostring(obj.Name):lower()
            local score=0
            if n:find("halo",1,true) then score+=100 end
            if n:find("ring",1,true) then score+=25 end
            if n:find("angel",1,true) then score+=10 end
            if score==0 then return -1 end
            if obj:IsA("Accessory") then score+=30
            elseif obj:IsA("Model") then score+=20
            elseif obj:IsA("MeshPart") then score+=18
            elseif obj:IsA("BasePart") then score+=12
            else return -1 end
            return score
        end

        local function bestHaloInside(root)
            if not root then return nil end
            local best,bestScore=nil,-1
            local function consider(o)
                local s=haloCandidateScore(o)
        ...
        local function findWeaponHaloSource()
            -- First use the exact currently-applied local weapon visual if it exposes
            -- a Halo/AngelHalo/Ring object. This best matches what the user sees.
            for _,container in ipairs({lp.Character,lp:FindFirstChildOfClass("Backpack")}) do
                if container then
                    for _,tool in ipairs(container:GetChildren()) do
                        if tool:IsA("Tool") then
                            for _,d in ipairs(tool:GetDescendants()) do
                                if d.Name=="KimqV26SkinVisual" or d.Name=="KimqV21AnimatedSkinVisual" then
                                    local h=bestHaloInside(d)
            ...
            -- Otherwise check the selected Wrap sources directly, even if the Weapon
            -- Skins page has never been opened this session.
            local root=wrapRoot or locateWraps()
            if root then
                for weaponName,skinName in pairs(selectedByWeapon or {}) do
                    local wf=folderByName[weaponName] or root:FindFirstChild(weaponName)
                    local sf=wf and wf:FindFirstChild(skinName)
                    local h=bestHaloInside(sf)
                    if h then return h end
                end
                -- Last resort: find an explicitly named halo anywhere in Wraps.
                local h=bestHaloInside(root)
                if h then return h end
            end
            return nil
        end

        local function scaleHaloClone(obj,scale)
            scale=scale or 1.45
            if obj:IsA("Model") then
                pcall(function() obj:ScaleTo((obj:GetScale() or 1)*scale) end)
            elseif obj:IsA("Accessory") then
                local h=obj:FindFirstChild("Handle")
                if h and h:IsA("BasePart") then
                    local sm=h:FindFirstChildOfClass("SpecialMesh")
                    if sm then
                        pcall(function() sm.Scale=sm.Scale*scale; sm.Offset=sm.Offset*scale end)
                    else
                        pcall(function() h.Size=h.Size*scale end)
                    end
                    for _,a in ipairs(h:GetDescendants()) do if a:IsA("Attachment") then pcall(function() a.Position=a.Position*scale end) end end
                end
        ...
        local function attachWeaponHaloToHead()
            clearWingHeadHalo()
            if not wingsAreWorn() then return false end
            local char=lp.Character
            local head=char and char:FindFirstChild("Head")
            if not (head and head:IsA("BasePart")) then return false end
            local source=findWeaponHaloSource()
            if not source then return false end

            local clone=source:Clone()
            clone.Name="KimqAngelWingHeadHalo"
            clone:SetAttribute("KimqWornEquippable",true)
            for _,d in ipairs(clone:GetDescendants()) do
                if d:IsA("LocalScript") or d:IsA("Script") then pcall(function() d.Disabled=true end)
                elseif d:IsA("ParticleEmitter") or d:IsA("Trail") or d:IsA("Beam") or d:IsA("PointLight") or d:IsA("SpotLight") or d:IsA("SurfaceLight") then pcall(function() d.Enabled=true end)
                elseif d:IsA("BasePart") then d.Anchored=false; d.CanCollide=false; d.CanTouch=false; d.CanQuery=false; d.Massless=true end
            end
            if clone:IsA("BasePart") then clone.Anchored=false; clone.CanCollide=false; clone.CanTouch=false; clone.CanQuery=false; clone.Massless=true end
            scaleHaloClone(clone,1.45)
            clone.Parent=char

            local parts={}
            if clone:IsA("BasePart") then table.insert(parts,clone) end
            for _,d in ipairs(clone:GetDescendants()) do if d:IsA("BasePart") then table.insert(parts,d) end end
            if #parts==0 then clone:Destroy(); return false end

            local sourceCf
            pcall(function()
                if source:IsA("Model") then sourceCf=source:GetPivot()
                elseif source:IsA("Accessory") then local h=source:FindFirstChild("Handle"); if h then sourceCf=h.CFrame end
                elseif source:IsA("BasePart") then sourceCf=source.CFrame end
            end)
            local sourceRot=sourceCf and (sourceCf-sourceCf.Position) or CFrame.new()
            local desired=CFrame.new(head.Position+Vector3.new(0,1.25,0))*sourceRot

            if clone:IsA("Model") then
                pcall(function() clone:PivotTo(desired) end)
            elseif clone:IsA("Accessory") then
                local h=clone:FindFirstChild("Handle")
                if h and h:IsA("BasePart") then h.CFrame=desired end
            elseif clone:IsA("BasePart") then
                clone.CFrame=desired
            end

            -- Weld every halo part to Head after positioning so the weapon-skin ring
            -- stays above the player instead of following the gun.
            for _,part in ipairs(parts) do
                local w=Instance.new("WeldConstraint")
                w.Name="KimqAngelHaloHeadWeld"; w.Part0=head; w.Part1=part; w.Parent=part
            end
            activeWingHeadHalo=clone
            return true
        end

        local function destroyWingMiniGui()
            if wingMiniGui then pcall(function() wingMiniGui:Destroy() end) end
            wingMiniGui=nil; wingMiniFrame=nil; wingMiniTop=nil; wingMiniMask=nil; wingMiniTitle=nil; wingMiniClose=nil; wingMiniBar=nil
            wingMiniSpeedLabel=nil; wingMiniFill=nil; wingMiniKnob=nil; wingMiniStatus=nil; wingMiniHelp=nil
        end

        local function refreshWingMiniTheme()
            if not wingMiniGui or not wingMiniGui.Parent then return end
            local bg=pcolor("bg",Color3.fromRGB(255,245,250))
            local soft=pcolor("soft",lightColor)
            local hot=pcolor("hot",hotColor)
            local tx=pcolor("text",textColor)
            local sub=pcolor("sub",subColor)
            local line=pcolor("line",lineColor)
            if wingMiniFrame then
                wingMiniFrame.BackgroundColor3=bg
                local st=wingMiniFrame:FindFirstChildOfClass("UIStroke"); if st then st.Color=line end
            end
            if wingMiniTop then wingMiniTop.BackgroundColor3=soft end
            if wingMiniMask then wingMiniMask.BackgroundColor3=soft end
            if wingMiniTitle then wingMiniTitle.TextColor3=tx end
            if wingMiniClose then wingMiniClose.TextColor3=hot end
            if wingMiniSpeedLabel then wingMiniSpeedLabel.TextColor3=tx end
            if wingMiniBar then wingMiniBar.BackgroundColor3=soft end
            if wingMiniFill then wingMiniFill.BackgroundColor3=hot end
            if wingMiniKnob then
                wingMiniKnob.BackgroundColor3=Color3.new(1,1,1)
                local st=wingMiniKnob:FindFirstChildOfClass("UIStroke"); if st then st.Color=hot end
            end
            if wingMiniStatus then wingMiniStatus.TextColor3=wingFlightActive and hot or sub end
            if wingMiniHelp then wingMiniHelp.TextColor3=sub end
        end
        _G.KimqRefreshWingMiniTheme=refreshWingMiniTheme

        local function refreshWingMiniGui()
            if not wingMiniGui or not wingMiniGui.Parent then return end
            refreshWingMiniTheme()
            if wingMiniSpeedLabel then wingMiniSpeedLabel.Text="Flight Speed: "..tostring(math.floor(wingFlightSpeed+.5)) end
            local alpha=math.clamp((wingFlightSpeed-16)/(140-16),0,1)
            if wingMiniFill then wingMiniFill.Size=UDim2.new(alpha,0,1,0) end
            if wingMiniKnob then wingMiniKnob.Position=UDim2.new(alpha,0,.5,0) end
            if wingMiniStatus then
                wingMiniStatus.Text=wingFlightActive and "flying ♡  •  double jump to stop" or "double jump to fly ♡"
                wingMiniStatus.TextColor3=wingFlightActive and pcolor("hot",hotColor) or pcolor("sub",subColor)
            end
        end

        local function makeWingMiniGui()
            if wingMiniGui and wingMiniGui.Parent then refreshWingMiniGui(); return end
            destroyWingMiniGui()
            ...
            if not parented then gui.Parent=pg end
            wingMiniGui=gui

            local f=Instance.new("Frame",gui)
            f.Name="Main"; f.Size=UDim2.fromOffset(274,148); f.Position=UDim2.new(1,-294,.55,-74)
            f.BackgroundColor3=pcolor("bg",Color3.fromRGB(255,245,250)); f.BorderSizePixel=0
            corner(f,14); stroke(f,pcolor("line",lineColor),.22,1.2)
            wingMiniFrame=f

            local top=Instance.new("Frame",f)
            wingMiniTop=top
            top.Size=UDim2.new(1,0,0,38); top.BackgroundColor3=pcolor("soft",lightColor); top.BorderSizePixel=0
            corner(top,14)
            local mask=Instance.new("Frame",top); wingMiniMask=mask; mask.Size=UDim2.new(1,0,0,14); mask.Position=UDim2.new(0,0,1,-14); mask.BackgroundColor3=top.BackgroundColor3; mask.BorderSizePixel=0
            local title=txt(top,"♥  Angel Wings",UDim2.new(1,-54,1,0),UDim2.fromOffset(12,0),Enum.Font.GothamBold,13,pcolor("text",textColor))
            wingMiniTitle=title
            local x=Instance.new("TextButton",top); wingMiniClose=x; x.Size=UDim2.fromOffset(30,28); x.Position=UDim2.new(1,-36,.5,-14); x.BackgroundTransparency=1; x.Text="×"; x.TextColor3=pcolor("hot",hotColor); x.Font=Enum.Font.GothamBold; x.TextSize=20
            x.MouseButton1Click:Connect(function() gui.Enabled=false end)

            wingMiniSpeedLabel=txt(f,"Flight Speed: "..tostring(math.floor(wingFlightSpeed+.5)),UDim2.new(1,-24,0,22),UDim2.fromOffset(12,48),Enum.Font.GothamSemibold,11,pcolor("text",textColor))
            local bar=Instance.new("Frame",f); wingMiniBar=bar; bar.Size=UDim2.new(1,-24,0,14); bar.Position=UDim2.fromOffset(12,76); bar.BackgroundColor3=pcolor("soft",lightColor); bar.BorderSizePixel=0; corner(bar,7)
            local fill=Instance.new("Frame",bar); fill.BackgroundColor3=pcolor("hot",hotColor); fill.BorderSizePixel=0; fill.Size=UDim2.new(0,0,1,0); corner(fill,7); wingMiniFill=fill
            local knob=Instance.new("Frame",bar); knob.Size=UDim2.fromOffset(18,18); knob.AnchorPoint=Vector2.new(.5,.5); knob.Position=UDim2.new(0,.5,.5,0); knob.BackgroundColor3=Color3.new(1,1,1); knob.BorderSizePixel=0; corner(knob,9); stroke(knob,pcolor("hot",hotColor),.15,1); wingMiniKnob=knob

            wingMiniStatus=txt(f,"double jump to fly ♡",UDim2.new(1,-24,0,22),UDim2.fromOffset(12,104),Enum.Font.GothamSemibold,10,pcolor("sub",subColor))
            local help=txt(f,"WASD move  •  Space up  •  Shift/Ctrl down",UDim2.new(1,-24,0,18),UDim2.fromOffset(12,124),Enum.Font.Gotham,9,pcolor("sub",subColor))
            wingMiniHelp=help

            local draggingSlider=false
            local function setFromX(xpos)
                local a=math.clamp((xpos-bar.AbsolutePosition.X)/math.max(bar.AbsoluteSize.X,1),0,1)
                wingFlightSpeed=math.floor((16+(140-16)*a)+.5)
                wingFlightVerticalSpeed=math.max(18,math.floor(wingFlightSpeed*.65+.5))
                knob.Position=UDim2.new(a,0,.5,0)
                refreshWingMiniGui()
            end
            bar.InputBegan:Connect(function(i)
                if i.UserInputType==Enum.UserInputType.MouseButton1 or i.UserInputType==Enum.UserInputType.Touch then draggingSlider=true; setFromX(i.Position.X) end
            end)
            knob.InputBegan:Connect(function(i)
                if i.UserInputType==Enum.UserInputType.MouseButton1 or i.UserInputType==Enum.UserInputType.Touch then draggingSlider=true; setFromX(i.Position.X) end
            end)
            UIS.InputChanged:Connect(function(i)
                if draggingSlider and (i.UserInputType==Enum.UserInputType.MouseMovement or i.UserInputType==Enum.UserInputType.Touch) then setFromX(i.Position.X) end
            end)
            ...
            top.InputBegan:Connect(function(i)
                if i.UserInputType==Enum.UserInputType.MouseButton1 or i.UserInputType==Enum.UserInputType.Touch then
                    drag=true; dragStart=i.Position; startPos=f.Position
                end
            end)
            UIS.InputChanged:Connect(function(i)
                if drag and dragStart and (i.UserInputType==Enum.UserInputType.MouseMovement or i.UserInputType==Enum.UserInputType.Touch) then
            ...
            refreshWingMiniGui()
            local a=math.clamp((wingFlightSpeed-16)/(140-16),0,1)
            knob.Position=UDim2.new(a,0,.5,0)
        end
        local function refreshWingFlightUI(message)
            if wingFlightToggleButton and wingFlightToggleButton.Parent then
                wingFlightToggleButton.Text=wingFlightEnabled and "enabled ♥" or "disabled"
                wingFlightToggleButton.BackgroundColor3=wingFlightEnabled and pcolor("hot",hotColor) or pcolor("soft",lightColor)
                wingFlightToggleButton.TextColor3=wingFlightEnabled and Color3.new(1,1,1) or pcolor("text",textColor)
            end
            if wingFlightStatus and wingFlightStatus.Parent then
                if message then wingFlightStatus.Text=message
                elseif wingFlightActive then wingFlightStatus.Text="Flying ♡  •  WASD move  •  Space up  •  Shift/Ctrl down"
                elseif wingsAreWorn() then wingFlightStatus.Text="Double jump to fly ♡"
                else wingFlightStatus.Text="Wear Angel Wings, then double jump to fly." end
                wingFlightStatus.TextColor3=wingFlightActive and pcolor("hot",hotColor) or pcolor("sub",subColor)
            end
        end


        local function restoreWingFlightPose()
            for motor,oldTransform in pairs(wingFlightPoseMotors) do
                if motor and motor.Parent then pcall(function() motor.Transform=oldTransform end) end
            end
            table.clear(wingFlightPoseMotors)
            if wingFlightAnimateScript and wingFlightAnimateScript.Parent and wingFlightAnimateWasDisabled~=nil then
                pcall(function() wingFlightAnimateScript.Disabled=wingFlightAnimateWasDisabled end)
            end
            wingFlightAnimateScript=nil
            wingFlightAnimateWasDisabled=nil
        end

        local function beginCuteFlightPose(hum)
            wingFlightAnimationMode="pose"
            local char=hum and hum.Parent
            if not char then return end
            local animate=char:FindFirstChild("Animate")
            if animate and animate:IsA("LocalScript") then
                wingFlightAnimateScript=animate
                wingFlightAnimateWasDisabled=animate.Disabled
            ...
            if animator then
                pcall(function()
                    for _,t in ipairs(animator:GetPlayingAnimationTracks()) do
                        if t~=wingFlightAnimationTrack then t:Stop(.12) end
                    end
                end)
            end
            for _,m in ipairs(char:GetDescendants()) do
                if m:IsA("Motor6D") then
                    local n=tostring(m.Name):lower():gsub("%s+","")
                    if n=="rightshoulder" or n=="leftshoulder" or n=="righthip" or n=="lefthip" or n=="waist" or n=="rootjoint" then
                        wingFlightPoseMotors[m]=m.Transform
                    end
                end
            end
        end

        local function applyCuteFlightPose()
            if wingFlightAnimationMode~="pose" then return end
            local bob=math.sin(os.clock()*2.2)
            for m,base in pairs(wingFlightPoseMotors) do
                if m and m.Parent then
                    local n=tostring(m.Name):lower():gsub("%s+","")
                    local t=base
                    if n=="rightshoulder" then t=base*CFrame.Angles(math.rad(-18+bob*2),0,math.rad(38))
                    elseif n=="leftshoulder" then t=base*CFrame.Angles(math.rad(-18+bob*2),0,math.rad(-38))
                    elseif n=="righthip" then t=base*CFrame.Angles(math.rad(10),0,math.rad(5))
                    elseif n=="lefthip" then t=base*CFrame.Angles(math.rad(10),0,math.rad(-5))
                    elseif n=="waist" or n=="rootjoint" then t=base*CFrame.Angles(math.rad(-6+bob*1.5),0,0) end
                    pcall(function() m.Transform=t end)
                end
            end
        end

        local function stopWingFlightAnimation()
            if wingFlightAnimationTrack then
                pcall(function() wingFlightAnimationTrack:Stop(.18) end)
                wingFlightAnimationTrack=nil
            end
            if wingFlightAnimationObject then
                pcall(function() wingFlightAnimationObject:Destroy() end)
                wingFlightAnimationObject=nil
            end
            restoreWingFlightPose()
            wingFlightAnimationMode="none"
        end

        local function startWingFlightAnimation(hum)
            stopWingFlightAnimation()
            if not hum then return end

            local track=nil
            -- The supplied ID comes from the Roblox shop. Shop animations can be
            -- EmoteAnimation assets rather than raw Animation clips, so try the
            -- Humanoid emote path first. That is the main reason v2.25 could show
            -- the normal freefall pose even though the ID was valid.
            pcall(function()
                if type(hum.PlayEmoteAndGetAnimTrackById)=="function" then
                    track=hum:PlayEmoteAndGetAnimTrackById(wingFlightAnimationAssetId)
                end
            end)
            ...
            if not track then
                ...
                local anim=Instance.new("Animation")
                anim.Name="KimqAngelWingFlightAnimation"
                anim.AnimationId=wingFlightAnimationId
                wingFlightAnimationObject=anim
                local ok,loaded=pcall(function() return animator:LoadAnimation(anim) end)
                if ok then track=loaded end
                if not track then
            ...
            if track then
                wingFlightAnimationTrack=track
                wingFlightAnimationMode="track"
                pcall(function()
                ...
                task.delay(.28,function()
                    if not wingFlightActive or wingFlightAnimationTrack~=track then return end
                    local playing=false
                    pcall(function() playing=track.IsPlaying end)
                    if not playing then
                        pcall(function() track:Stop(0) end)
                        wingFlightAnimationTrack=nil
                        beginCuteFlightPose(hum)
                    end
                end)
            else
                -- If the experience blocks that catalog animation, still give flight
                -- a deliberate cute floating pose instead of the default falling pose.
                beginCuteFlightPose(hum)
            end
        end

        local function stopWingFlight(message)
            wingFlightActive=false
            stopWingFlightAnimation()
            if wingFlightConn then pcall(function() wingFlightConn:Disconnect() end); wingFlightConn=nil end
            if wingFlightVelocity then pcall(function() wingFlightVelocity:Destroy() end); wingFlightVelocity=nil end
            if wingFlightGyro then pcall(function() wingFlightGyro:Destroy() end); wingFlightGyro=nil end
            if wingFlightHum and wingFlightHum.Parent then
                pcall(function() wingFlightHum.AutoRotate=wingFlightOldAutoRotate end)
                pcall(function() wingFlightHum:ChangeState(Enum.HumanoidStateType.Freefall) end)
            end
            ...
            wingFlightHum=nil
            refreshWingFlightUI(message)
            refreshWingMiniGui()
        end

        local function startWingFlight()
            if not wingFlightEnabled then refreshWingFlightUI("Enable Angel Wings Flight first."); return false end
            if not wingsAreWorn() then refreshWingFlightUI("Wear Angel Wings first."); return false end
            local char=lp.Character
            local hum=char and char:FindFirstChildOfClass("Humanoid")
            ...
            stopWingFlight()
            wingFlightActive=true
            wingFlightHum=hum
            wingFlightOldAutoRotate=hum.AutoRotate
            hum.AutoRotate=false

            local bv=Instance.new("BodyVelocity")
            bv.Name="KimqAngelWingFlightVelocity"; bv.MaxForce=Vector3.new(1e6,1e6,1e6); bv.P=9000; bv.Velocity=Vector3.zero; bv.Parent=root
            local bg=Instance.new("BodyGyro")
            bg.Name="KimqAngelWingFlightGyro"; bg.MaxTorque=Vector3.new(1e6,1e6,1e6); bg.P=9500; bg.D=650; bg.CFrame=root.CFrame; bg.Parent=root
            wingFlightVelocity=bv; wingFlightGyro=bg
            pcall(function() hum:ChangeState(Enum.HumanoidStateType.Freefall) end)
            startWingFlightAnimation(hum)
            refreshWingFlightUI()
            refreshWingMiniGui()

            wingFlightConn=RunService.RenderStepped:Connect(function(dt)
                if not wingFlightActive or not wingFlightEnabled or not wingsAreWorn() or lp.Character~=char or hum.Health<=0 or not root.Parent then
                    stopWingFlight()
                    return
                end
                ...
                if flatLook.Magnitude<.05 then flatLook=Vector3.new(root.CFrame.LookVector.X,0,root.CFrame.LookVector.Z) end
                if flatLook.Magnitude<.05 then flatLook=Vector3.new(0,0,-1) end
                flatLook=flatLook.Unit
                local right=Vector3.new(camera.CFrame.RightVector.X,0,camera.CFrame.RightVector.Z)
                if right.Magnitude<.05 then right=Vector3.new(1,0,0) else right=right.Unit end
                ...
                local down=((UIS:IsKeyDown(Enum.KeyCode.LeftShift) or UIS:IsKeyDown(Enum.KeyCode.RightShift) or UIS:IsKeyDown(Enum.KeyCode.LeftControl) or UIS:IsKeyDown(Enum.KeyCode.RightControl)) and 1 or 0)
                local horizontal=flatLook*f+right*s
                if horizontal.Magnitude>1 then horizontal=horizontal.Unit end
                local idle=horizontal.Magnitude<.02 and up==0 and down==0
                local y=(up-down)*wingFlightVerticalSpeed
                if idle then y=math.sin(os.clock()*2.35)*1.35 end -- cute gentle hover bob
                local target=horizontal*wingFlightSpeed+Vector3.new(0,y,0)
                local alpha=math.clamp(dt*7.5,0,1)
                bv.Velocity=bv.Velocity:Lerp(target,alpha)

                if wingFlightAnimationMode=="pose" then
                    applyCuteFlightPose()
                elseif wingFlightAnimationMode=="track" and wingFlightAnimationTrack then
                    local playing=true
                    pcall(function() playing=wingFlightAnimationTrack.IsPlaying end)
                    if not playing then pcall(function() wingFlightAnimationTrack:Play(.08,1,1) end) end
                end

                -- Face the camera direction and softly tilt into movement for a floating look.
                local pitch=math.rad(-f*7)
                local roll=math.rad(-s*9)
                local desired=CFrame.lookAt(root.Position,root.Position+flatLook)*CFrame.Angles(pitch,0,roll)
                bg.CFrame=bg.CFrame:Lerp(desired,math.clamp(dt*5.5,0,1))
            end)
            return true
        end
        ...
        UIS.InputBegan:Connect(function(input)
            ...
            if UIS:GetFocusedTextBox() then return end
            local now=os.clock()
            if now-wingFlightLastSpace<=.34 then
                wingFlightLastSpace=0
                if wingsAreWorn() and wingFlightEnabled then
                    if wingFlightActive then stopWingFlight("Flight off — double jump to fly again.") else startWingFlight() end
                end
            else
                wingFlightLastSpace=now
            end
        end)
        lp.CharacterAdded:Connect(function() stopWingFlight() end)
        ...
        local function makeAngelWingsWearable(source,tool)
            ...
            makeWingMiniGui()
            pcall(attachWeaponHaloToHead)
            task.delay(.35,function() if visual.Parent and wingsAreWorn() and not activeWingHeadHalo then pcall(attachWeaponHaloToHead) end end)
            refreshWingFlightUI("Angel Wings are on ♡  •  double jump to fly")
            refreshWingMiniGui()
        ...
        local wingFlightCard=card(skinsPage,122); wingFlightCard.Name="KimqAngelWingsFlightCard"
        txt(wingFlightCard,"♥  Angel Wings Flight",UDim2.new(1,-150,0,24),UDim2.fromOffset(12,8),Enum.Font.GothamBold,14,pcolor("text",textColor))
        local wingFlightSub=txt(wingFlightCard,"Double jump while the wings are worn for smooth free flight + a soft floating hover.",UDim2.new(1,-24,0,34),UDim2.fromOffset(12,32),Enum.Font.Gotham,10,pcolor("sub",subColor)); wingFlightSub.TextWrapped=true
        wingFlightToggleButton=Instance.new("TextButton",wingFlightCard); wingFlightToggleButton.Size=UDim2.fromOffset(116,32); wingFlightToggleButton.Position=UDim2.new(1,-128,0,7); wingFlightToggleButton.BorderSizePixel=0; wingFlightToggleButton.Font=Enum.Font.GothamSemibold; wingFlightToggleButton.TextSize=11; wingFlightToggleButton.AutoButtonColor=false; corner(wingFlightToggleButton,9); stroke(wingFlightToggleButton,pcolor("line",lineColor),.28,1)
        wingFlightStatus=txt(wingFlightCard,"Wear Angel Wings, then double jump to fly.",UDim2.new(1,-24,0,36),UDim2.fromOffset(12,77),Enum.Font.GothamSemibold,10,pcolor("sub",subColor)); wingFlightStatus.TextWrapped=true
        wingFlightToggleButton.MouseButton1Click:Connect(function()
            wingFlightEnabled=not wingFlightEnabled
            if not wingFlightEnabled and wingFlightActive then stopWingFlight("Angel Wings Flight disabled.") else refreshWingFlightUI() end
        end)
        refreshWingFlightUI()

        if type(_G.KimqRegisterConfigControl)=="function" then
            _G.KimqRegisterConfigControl("Angel Wings Flight", "state",
                function()
                    return {Enabled=wingFlightEnabled, Speed=wingFlightSpeed}
                end,
                function(v)
                    if type(v)~="table" then return end
                    if v.Enabled~=nil then wingFlightEnabled=not not v.Enabled end
                    if tonumber(v.Speed) then
                        wingFlightSpeed=math.clamp(tonumber(v.Speed),16,140)
                        wingFlightVerticalSpeed=math.max(18,math.floor(wingFlightSpeed*.65+.5))
                    end
                    if not wingFlightEnabled and wingFlightActive then stopWingFlight() end
                    refreshWingFlightUI()
                    refreshWingMiniGui()
                end)
        end
        ...
        local function getExtraState()
            return {
                ...
                WingFlightEnabled=wingFlightEnabled,
            }
        end
        local function setExtraState(state)
            ...
            if state.WingFlightEnabled~=nil then wingFlightEnabled=not not state.WingFlightEnabled end
            if not wingFlightEnabled and wingFlightActive then stopWingFlight() end
            refreshWingFlightUI()
            task.defer(function()
                if skinsPage.Visible then scanAllExtras() end
                refreshBeamStyle(); refreshKnifeStyle(); refreshEquipStyle(); refreshBulletColorUI()
                applyBulletBeamOverride(extraState.BulletBeam or "None")
                local t=findKnifeTool()
                local needsKnife=(extraState.KnifeSkin~=oldKnife) or (extraState.KnifeSkin~="None" and t and not t:FindFirstChild("KimqKnifeSkinVisual"))
                if needsKnife then
    ...
    -- Page switching. Final theme engine remains authoritative after every section change.
    ...
    envBtn.MouseButton1Click:Connect(function() showSpecial(envPage,envBtn,"environment","seasonal styles, permanent day/night, and optional night lights") end)
    skinsBtn.MouseButton1Click:Connect(function() showSpecial(skinsPage,skinsBtn,"weapon skins","wraps, bullets, knives, Angel Wings, and smooth wing flight") end)
    for _,b in ipairs(normalButtons) do
        if b and b.Parent then
            b.MouseButton1Click:Connect(function()
    ...
    local function syncTheme()
        ...
        styleSpecial(envBtn,specialActive==envBtn); styleSpecial(skinsBtn,specialActive==skinsBtn); refreshPreset(); refreshWeaponStyle(); refreshSkinStyle(); if _G.KimqWeaponExtrasController and _G.KimqWeaponExtrasController.RefreshColor then _G.KimqWeaponExtrasController.RefreshColor() end
    end
...
task.spawn(function()
    ...
    local pageNames={overview=true,["silent aim"]=true,macro=true,whitelist=true,protection=true,["anti fall"]=true,["delay changer"]=true,esp=true,avatar=true,["fog / atmosphere"]=true,environment=true,["weapon skins"]=true,["hc silent aim"]=true,["force hit"]=true,["hitbox expander"]=true,flamelock=true,camlock=true,headless=true,["anti mod"]=true,["spawn point"]=true,settings=true,theme=true,information=true}
    ...
    -- Sidebar order: Overview first, A-Z feature pages, then Settings, then Information.
    if nav then
        local layout=nav:FindFirstChildOfClass("UIListLayout")
        if layout then layout.SortOrder=Enum.SortOrder.LayoutOrder end
        local arr={}
        for _,b in ipairs(nav:GetChildren()) do
            if b:IsA("TextButton") and pageNames[clean(b.Text)] then table.insert(arr,b) end
        end
        table.sort(arr,function(a,b)
            local an,bn=clean(a.Text),clean(b.Text)
            local function rank(n)
                if n=="overview" then return 0 end
                if n=="settings" then return 2 end
                if n=="information" then return 3 end
                return 1
            end
            local ar,br=rank(an),rank(bn)
            if ar~=br then return ar<br end
            if ar==1 then return an<bn end
            return false
        end)
    ...
    -- A page becoming visible is the single navigation refresh point.  Some legacy
    -- callbacks still change page/header/button colors while switching sections;
    -- re-applying the selected palette once after that callback finishes makes the
    -- theme persistent without any frame loop or stacked delayed repaint chain.
    for _,page in ipairs(main:GetDescendants()) do
        if page:IsA("ScrollingFrame") and page~=nav and page.Name:lower():match("page$") and not page:GetAttribute("KimqThemeVisibleHook") then
            page:SetAttribute("KimqThemeVisibleHook",true)
            page:GetPropertyChangedSignal("Visible"):Connect(function()
                if page.Visible then
                    task.defer(function()
                        if page.Parent then applyTheme(currentName) end
                    end)
                end
            end)
        end
    ...
    main.Visible=true; root.Enabled=true
    _G.KimqV21FullThemeActive=true

    -- v2.3: original typography restored. The GUI keeps its authored mix of
    -- Fredoka / Gotham / SourceSans exactly like the previous version.
end)


-- v2.1 final page polish: standalone legacy section-name labels are decoration only.
-- Remove them after every module has finished so Silent Aim contains actual controls,
-- never orphan headings such as Macro / Whitelist / Protection.
task.spawn(function()
    task.wait(2.8)
    local Players = game:GetService("Players")
    local CoreGui = game:GetService("CoreGui")
    ...
    local sectionNames = {
        ["silent aim"]=true,["macro"]=true,["whitelist"]=true,["protection"]=true,
        ["anti fall"]=true,["delay changer"]=true,["esp"]=true,["avatar"]=true,
        ["hc silent aim"]=true,["combat"]=true,["force hit"]=true,["hitbox expander"]=true,
        ["flamelock"]=true,["camlock"]=true,["visuals"]=true,["atmosphere presets"]=true,
        ["headless"]=true,["protection + anti mod"]=true,["anti mod"]=true,
        ["settings"]=true,["credits"]=true,["information"]=true,
    }
    local function cleanText(t)
        t=tostring(t or ""):gsub("^%s*[♡♥❤]%s*",""):gsub("%s+"," ")
        return t:gsub("^%s+",""):gsub("%s+$",""):lower()
    end
    for _, page in ipairs(main:GetDescendants()) do
        if page:IsA("ScrollingFrame") and page.Name:lower():match("page$") then
            for _, child in ipairs(page:GetChildren()) do
                if child:IsA("TextLabel") and sectionNames[cleanText(child.Text)] then
                    child:Destroy()
                end
            end
        end
...
-- ========================================================
-- v2.4 final polish: make all four config action labels visually identical.
-- ========================================================
task.spawn(function()
    task.wait(3.2)
    local Players=game:GetService("Players")
    ...
    local wanted={
        ...
        ["Update Selected Config"]=true,
        ["Load Selected Config"]=true,
        ["Delete Selected Config"]=true,
        ["Refresh Config List"]=true,
    }
    for _,d in ipairs(main:GetDescendants()) do
        if d:IsA("TextButton") and wanted[tostring(d.Text)] then
            d.Font=Enum.Font.FredokaOne
            d.TextSize=14
            d.TextStrokeColor3=Color3.fromRGB(255,255,255)
            d.TextStrokeTransparency=.42
        end
    end
end)
...
-- ========================================================
-- v2.5 Silent Aim page cleanup.
-- Some legacy section-heading labels can be reparented after the first cleanup pass.
-- Remove only the large decorative headings, never the actual control labels.
-- ========================================================
task.spawn(function()
    task.wait(4.25)
    local Players=game:GetService("Players")
    ...
    local silentPage=nil
    for _,d in ipairs(main:GetDescendants()) do
        if d:IsA("ScrollingFrame") and d.Name:lower()=="silentpage" then
            silentPage=d
            break
        end
    end
    if not silentPage then return end
    ...
    local function isLegacyHeading(lbl)
        if not lbl:IsA("TextLabel") or not names[clean(lbl.Text)] then return false end
        -- Original feature headings are large (18px+) standalone labels.
        -- Actual Silent Aim control labels are 15px and are therefore preserved.
        return tonumber(lbl.TextSize or 0) >= 17
    end
    local function cleanupTarget(lbl)
        local p=lbl.Parent
        if p and p:IsA("Frame") and p~=silentPage then
            local hasInteractive=p:FindFirstChildWhichIsA("TextButton",true) or p:FindFirstChildWhichIsA("TextBox",true)
            if not hasInteractive and p.Size.Y.Offset<=44 then return p end
        end
        return lbl
    end
    local function cleanNow()
        local doomed={}
        local seen={}
        for _,d in ipairs(silentPage:GetDescendants()) do
            if isLegacyHeading(d) then
                local target=cleanupTarget(d)
                if target and not seen[target] then seen[target]=true; table.insert(doomed,target) end
            end
        end
    ...
    cleanNow()
    task.delay(0.9,cleanNow)
    task.delay(2.0,cleanNow)

    -- Catch any late legacy label recreated by an older callback for a few seconds.
    local conn
    conn=silentPage.DescendantAdded:Connect(function(d)
        task.defer(function()
            if d and d.Parent and isLegacyHeading(d) then
                local target=cleanupTarget(d)
                pcall(function() if target and target.Parent then target:Destroy() end end)
            end
        end)
    end)
    task.delay(8,function() if conn then pcall(function() conn:Disconnect() end) end end)
end)
...
-- ========================================================
-- v2.24 owner / developer credit guard
-- Only Kimqetras may be presented as owner. Famesgun is the only developer/scripter credit.
-- This only cleans GUI branding text; technical projectile ownership metadata is untouched.
-- ========================================================
task.spawn(function()
    task.wait(3.0)
    local Players=game:GetService("Players")
    ...
    local function isAllowedRoleText(t)
        local low=cleanText(t)
        if low:find("kimqetras",1,true) or low:find("famesgun",1,true) then return true end
        -- These two role-only labels belong to the deterministic Information card.
        if low=="owner ♡" or low=="owner" then return true end
        if low=="original developer / scripter" or low=="developer / scripter" then return true end
        return false
    end

    local function isUnauthorizedBrandingText(t)
        local low=cleanText(t)
        local roleMention = low:find("developer",1,true) or low:find("owner",1,true) or low:find("creator",1,true) or low:find("author",1,true)
        return roleMention and not isAllowedRoleText(t)
    end

    local function scrubObject(d)
        if not d or not d.Parent then return end
        if d:IsA("TextLabel") or d:IsA("TextButton") or d:IsA("TextBox") then
            if isUnauthorizedBrandingText(d.Text) then
                pcall(function() d:Destroy() end)
            end
        end
    end

    for _,d in ipairs(root:GetDescendants()) do scrubObject(d) end

    -- Keep the Information card deterministic: exactly Kimqetras + famesgun roles.
    local infoCard=root:FindFirstChild("KimqInformationCard",true)
    if infoCard then
        local labels={}
        for _,d in ipairs(infoCard:GetDescendants()) do
            if d:IsA("TextLabel") then table.insert(labels,d) end
        end
        -- Existing card is already built with these exact credits; remove any late foreign role line.
        for _,d in ipairs(labels) do scrubObject(d) end
    end

    local conn
    conn=root.DescendantAdded:Connect(function(d)
        task.defer(function() scrubObject(d) end)
    end)
