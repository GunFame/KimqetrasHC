-- Kimqetras HC v2.41 • persistent selected-spawn correction
...
local KIMQ_BUILD = "v2.41-persistent-selected-spawn"
    ...
    if running and runningGui and runningBuild ~= KIMQ_BUILD then
        pcall(function()
            game:GetService("StarterGui"):SetCore("SendNotification", {
                Title = "Kimqetras HC",
                Text = "Another Kimqetras HC version is still active. Rejoin once, then run v2.41.",
                Duration = 8,
            })
        end)
        warn("[Kimqetras HC v2.41] Another runtime is active; rejoin once before switching builds.")
        return
    end
...
_G.KimqAccessoryUIReady = false
    ...
    pcall(function() game:GetService("RunService"):UnbindFromRenderStep("KimqPermanentTimeV241") end)
end
...
pcall(function()
    RunService:BindToRenderStep("KimqMacroSpeedV241",Enum.RenderPriority.Last.Value,function()
        enforceMacroSpeed()
    end)
end)
...
local function addCleanSection(title, subtitle)
    ...
    -- v2.41: pageHead already displays the section name.
...
task.spawn(function()
    ...
    -- v2.41: pages are grouped by what they actually do so the sidebar is
        ...
        if spawnPage then
            ...
            local function beginSpawnCorrectionWindow(char)
                ...
                task.spawn(function()
                    ...
                    -- Games can apply their own spawn/round teleport a little later
                    -- than CharacterAdded. Re-check for a few seconds, but only
                    -- move when the character has actually been pulled away from
                    -- the chosen point. This avoids a permanent teleport loop.
                    local checks={0.12,0.38,0.72,1.15,1.75,2.55,3.60,4.80}
                    local last=0

                    for _,atTime in ipairs(checks) do
                        local waitFor=atTime-last
                        last=atTime
                        ...
                        if not isNearSelectedSpawn(char,14) then
...
-- v2.41 module: local catalog / limited accessory try-on.
task.spawn(function()
    _G.KimqAccessoryUIReady=false
    local Players = game:GetService("Players")
    local CoreGui = game:GetService("CoreGui")
...
-- v2.41 module: Environment + Weapon Skins (canonical sidebar pages).
task.spawn(function()
    local pageReadyStart=tick()
    while not _G.KimqBasePagesReady and tick()-pageReadyStart<8 do task.wait(.02) end
    ...
    -- v2.41: Environment and Weapon Skins are already real canonical pages/buttons.
    ...
    pcall(function()
        RunService:BindToRenderStep("KimqPermanentTimeV241",Enum.RenderPriority.Last.Value,function()
    ...
    if not envBuildOk then
        warn("[Kimqetras HC v2.41] Environment page fallback: "..tostring(envBuildErr))
    ...
    local function setupWeaponExtras()
        ...
        -- This function is local to the register-saving bullet scope.
        -- Run it BEFORE leaving the scope.
        refreshAllLocalGunTemplates()
        end
        ...
        task.defer(function()
            -- v2.41: populate the lists once immediately. The page can already be
            ...
            if not ok then
                warn("[Kimqetras HC v2.41] initial extras scan: "..tostring(err))
            end
    ...
    if not extrasOk then
        _G.KimqWeaponExtrasInstalled=false
        warn("[Kimqetras HC v2.41] Weapon extras: "..tostring(extrasErr))
    ...
    skinsBtn.MouseButton1Click:Connect(function()
        task.defer(function()
            ...
            if extras and type(extras.Refresh)=="function" then
                ...
                if not ok then
                    warn("[Kimqetras HC v2.41] extras refresh: "..tostring(err))
...
task.spawn(function()
    ...
    -- v2.41: preserve the grouped sidebar order from pageDefs.
    ...
    local function applyTheme(name,settlePass)
        ...
        -- v2.41 hard guarantee: old controls created with literal pink/yellow
    ...
    -- v2.41: section navigation does not repaint/rescan the entire GUI.
    ...
    -- v2.41 reveal happens only after this initial theme pass is complete.
end)


-- v2.41: delayed post-show cleanup passes removed.
...
-- ========================================================
-- v2.41 branding is deterministic at construction time; no delayed scrub pass.

-- ========================================================
-- v2.41 DETERMINISTIC REVEAL
...
task.spawn(function()
    ...
    -- v2.41: remove the old one-page section divider labels from the
