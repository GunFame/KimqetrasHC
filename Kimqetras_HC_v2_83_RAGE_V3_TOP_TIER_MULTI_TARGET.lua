-- Kimqetras HC v2.83 • v2.82 base + full RAGE V3 multi-target combat rewrite
...
local KIMQ_BUILD = "v2.83-rage-v3-multitarget"
...
do
    ...
    if running and runningBuild == KIMQ_BUILD then
        if runningGui then
            ...
            notifyBoot((runtimeState=="ready") and "v2.83 is already running ♡" or "v2.83 is already starting ♡",4)
            return
        end
        ...
        notifyBoot("Recovering a failed v2.83 boot ♡",4)
    end
    ...
    if oldRuntimeDetected then
        ...
        if staleBoot then
            ...
            notifyBoot("STOP: another Kimqetras HC build is already active. v2.83 DID NOT LOAD. Rejoin once, then run only v2.83.",9)
            warn("[Kimqetras HC v2.83] Another build is active. v2.83 DID NOT LOAD; rejoin once before testing.")
            return
        end
    end
...
Hello.Text = "v2.83  •  RAGE V3 + reliable multi-target queue ♡"
...
task.spawn(function()
    ...
    local pageDefs = {
        ...
        -- RAGE
        {"ragecam", "rage targets", "multi-select target queue, camera lock, and queue controls", "RAGE"},
        {"rageorbit", "target orbit", "fast non-stiff orbit controls for the active RAGE target", "RAGE"},
        {"ragecombat", "rage combat", "attack, K.O detect, stomp, and advance through the selected queue", "RAGE"},
        {"ragepresets", "rage presets", "OP all-target, solo, close-hunt, and stop presets", "RAGE"},
    ...
    local function refreshPageSearch()
        ...
        for _, entry in ipairs(navButtons) do
            ...
            if entry.key=="ragecam" then aliases=" target targets multi queue player list select all view spectate" end
            ...
            if entry.key=="ragepresets" then aliases=" preset presets solo all targets queue wipe close hunt stop calm" end
    ...
    task.defer(function()
        local rageOK,rageERR=pcall(function()
            ...
            function R.card(page,height)
                ...
                accent.Size=UDim2.fromOffset(5,math.clamp(height-16,24,48))
                ...
                dots.Parent=f
                dots.Name="RageDots"
                dots.Size=UDim2.fromOffset(40,12)
                dots.Position=UDim2.new(1,-52,0,8)
                dots.BorderSizePixel=0
                R.role(dots,"lightBg")
                corner(dots,999)
                stroke(dots,p.line or T.stroke,.52,1)
                for i=0,2 do
                    local d=Instance.new("Frame")
                    d.Parent=dots
                    d.Size=UDim2.fromOffset(6,6)
                    d.Position=UDim2.fromOffset(7+i*11,3)
                    d.BorderSizePixel=0
                    R.role(d,i==1 and "hotBg" or "panel")
                    corner(d,999)
                end
                return f
            end
            ...
            function R.button(parent,text,pos,size,fn)
                ...
                b.MouseButton1Click:Connect(function()
                    ...
                    if not ok then warn("[Kimqetras HC v2.83 RAGE button] "..tostring(err)) end
                end)
                return b
            end
            ...
            function R.makeToggle(page,text,default,callback)
                ...
                R.role(R.label(c,text,UDim2.new(1,-88,1,0),UDim2.fromOffset(18,0),Enum.Font.GothamSemibold,12,R.pal().text or T.text),"textText")
                local b
                local function paint()
            ...
            function R.makeCycle(page,text,items,default,callback)
                ...
                local c=R.card(page,48)
                R.role(R.label(c,text,UDim2.new(.48,-12,1,0),UDim2.fromOffset(18,0),Enum.Font.GothamSemibold,12,R.pal().text or T.text),"textText")
                local b
                local function setValue(v,quiet)
            ...
            function R.makeSlider(page,text,min,max,default,callback,format)
                ...
                R.role(R.label(c,text,UDim2.new(.68,-12,0,22),UDim2.fromOffset(18,5),Enum.Font.GothamSemibold,12,R.pal().text or T.text),"textText")
            ...
            function R.setActiveById(id)
                local idx=R.queueIndex(id)
                if idx then R.activeIndex=idx; R.refreshQueueUI(); return true end
                return false
            end
            ...
            function R.placeForStomp(pl)
                ...
                local root=R.localRoot()
                local hum=char and char:FindFirstChildOfClass("Humanoid")
                if not (tr and char and root and hum) then return false end
                R.releaseMovement()
                pcall(function() hum.PlatformStand=false; hum.Sit=false; hum.AutoRotate=true end)
                local forward=Vector3.new(root.CFrame.LookVector.X,0,root.CFrame.LookVector.Z)
                if forward.Magnitude<.05 then forward=Vector3.new(0,0,-1) else forward=forward.Unit end
                local pos=tr.Position+Vector3.new(0,2.0,0)
            ...
            function R.finishTarget(pl)
                ...
                task.spawn(function()
                    ...
                    local attempts=0
                    local sawKnock=false
                    while R.master and token==R.runToken and pl.Parent and os.clock()<deadline and attempts<R.stompAttempts do
                        local down=R.stompable(pl)
                        sawKnock=sawKnock or down
                        if R.placeForStomp(pl) then
                            task.wait(.045)
                            R.placeForStomp(pl)
                        ...
                        local hum=char and char:FindFirstChildOfClass("Humanoid")
                        if sawKnock and attempts>=4 and (not char or not hum or hum.Health<=0) then break end
                    end
            ...
            function R.buildHud()
                ...
                f.Size=UDim2.fromOffset(310,164)
                f.BorderSizePixel=0
                f.Visible=false
                f.ZIndex=80
                R.role(f,"panel")
                ...
                R.ui.hudTitle=R.role(R.label(head,"♥  RAGE V3",UDim2.new(1,-130,0,24),UDim2.fromOffset(24,3),Enum.Font.FredokaOne,14,p.hot or T.hot),"hotText")
            ...
            do
                local intro=R.card(R.pages.cam,76)
                R.role(R.label(intro,"♥  Target Queue V3",UDim2.new(1,-24,0,26),UDim2.fromOffset(18,8),Enum.Font.FredokaOne,18,R.pal().hot or T.hot),"hotText")
                local sel=R.role(R.label(intro,"No targets selected",UDim2.new(1,-24,0,24),UDim2.fromOffset(18,39),Enum.Font.GothamSemibold,10,R.pal().sub or T.sub),"subText")
                table.insert(R.ui.selectedLabels,sel)

                local actions=R.card(R.pages.cam,98)
                R.button(actions,"Select All",UDim2.fromOffset(18,8),UDim2.new(.5,-23,0,34),R.selectAll)
                R.button(actions,"Clear",UDim2.new(.5,5,0,8),UDim2.new(.5,-17,0,34),R.clearQueue)
                R.button(actions,"◀ Prev",UDim2.fromOffset(18,52),UDim2.new(.5,-23,0,34),function() R.manualAdvance(-1) end)
                R.button(actions,"Next ▶",UDim2.new(.5,5,0,52),UDim2.new(.5,-17,0,34),function() R.manualAdvance(1) end)

                local listCard=R.card(R.pages.cam,360)
                R.role(R.label(listCard,"Choose as many players as you want ♡",UDim2.new(1,-24,0,26),UDim2.fromOffset(18,8),Enum.Font.GothamBold,12,R.pal().text or T.text),"textText")
                ...
                layout:GetPropertyChangedSignal("AbsoluteContentSize"):Connect(function() list.CanvasSize=UDim2.new(0,0,0,layout.AbsoluteContentSize.Y+6) end)
                ...
                local viewButton
                viewButton=R.button(R.card(R.pages.cam,48),"View Active Target",UDim2.fromOffset(18,9),UDim2.new(1,-36,0,30),function()
                    local pl=R.currentPlayer(); local hum=pl and pl.Character and pl.Character:FindFirstChildOfClass("Humanoid"); local cam=workspace.CurrentCamera
                    if not (hum and cam) then return end
                    R.freeView=not R.freeView
                    if R.freeView then R.camLock=false; cam.CameraType=Enum.CameraType.Custom; cam.CameraSubject=hum; viewButton.Text="Stop Viewing" else R.restoreCamera(); viewButton.Text="View Active Target" end
                end)
            end
            ...
            do
                ...
                R.role(R.label(head,"♥  Orbit Target",UDim2.new(1,-24,0,26),UDim2.fromOffset(18,8),Enum.Font.FredokaOne,18,R.pal().hot or T.hot),"hotText")
                local sel=R.role(R.label(head,"No active target",UDim2.new(1,-24,0,24),UDim2.fromOffset(18,39),Enum.Font.GothamSemibold,10,R.pal().sub or T.sub),"subText")
                table.insert(R.ui.selectedLabels,sel)
            ...
            do
                ...
                R.role(R.label(head,"♥  RAGE Combat V3",UDim2.new(1,-24,0,26),UDim2.fromOffset(18,8),Enum.Font.FredokaOne,18,R.pal().hot or T.hot),"hotText")
                local sel=R.role(R.label(head,"Select targets in RAGE Camlock first ♡",UDim2.new(1,-24,0,30),UDim2.fromOffset(18,39),Enum.Font.GothamSemibold,10,R.pal().sub or T.sub),"subText")
                ...
                local _,setAttempts=R.makeSlider(R.pages.combat,"Stomp Attempts",4,20,R.stompAttempts,function(v) R.stompAttempts=math.floor(v+.5) end,function(v) return tostring(math.floor(v+.5)) end)
            ...
            do
                ...
                R.role(R.label(c,"♥  OP RAGE V3",UDim2.new(1,-24,0,28),UDim2.fromOffset(18,8),Enum.Font.FredokaOne,19,R.pal().hot or T.hot),"hotText")
                local sub=R.role(R.label(c,"real multi-target run: acquire → orbit → fire → K.O → stop orbit → E stomp → next ♡",UDim2.new(1,-34,0,38),UDim2.fromOffset(18,38),Enum.Font.Gotham,9,R.pal().sub or T.sub),"subText")
                sub.TextWrapped=true

                R.button(c,"OP ALL TARGETS",UDim2.fromOffset(18,86),UDim2.new(.5,-23,0,40),function()
                    if #R.queue==0 then R.selectAll() end
                    R.applyOpSettings("all","health")
                    R.notify("OP ALL TARGETS armed • "..#R.queue.." selected ♡",3)
                end)
                R.button(c,"OP SOLO",UDim2.new(.5,5,0,86),UDim2.new(.5,-17,0,40),function()
                    R.applyOpSettings("solo","health")
                    R.notify("OP SOLO armed ♡",3)
                end)
                R.button(c,"OP CLOSE HUNT",UDim2.fromOffset(18,136),UDim2.new(.5,-23,0,40),function()
                    if #R.queue==0 then R.selectAll() end
                    R.applyOpSettings("all","close")
                    R.notify("OP CLOSE HUNT armed • "..#R.queue.." selected ♡",3)
                end)
                R.button(c,"STOP / CALM",UDim2.new(.5,5,0,136),UDim2.new(.5,-17,0,40),function()
                    R.stopRun()
                    R.autoShoot=false
                    R.orbitMaster=false
                    R.camLock=false
                ...
                local hint=R.role(R.label(c,"Tip: check multiple names in RAGE Camlock, then press OP ALL TARGETS. Your checked queue is never collapsed into one target.",UDim2.new(1,-36,0,44),UDim2.fromOffset(18,188),Enum.Font.Gotham,9,R.pal().sub or T.sub),"subText")
            ...
            R.RunService.RenderStepped:Connect(function(dt)
                ...
                if not ok then warn("[Kimqetras HC v2.83 RAGE camera] "..tostring(err)) end
            end)
            ...
            R.RunService.Heartbeat:Connect(function(dt)
                ...
                if not ok then warn("[Kimqetras HC v2.83 RAGE heartbeat] "..tostring(err)) end
            end)
        ...
        if not rageOK then
            warn("[Kimqetras HC v2.83] RAGE V3 isolated error: "..tostring(rageERR))
        end
    end)
...
task.spawn(function()
    ...
    pcall(function()
        game:GetService("StarterGui"):SetCore("SendNotification",{
            Title="Kimqetras HC",
            Text="v2.83 ACTIVE ♡ • RAGE V3 multi-target ready",
            Duration=7,
        })
    end)
