-- Kimqetras HC v2.18 • Kimqetras owner branding + famesgun developer credit + gun-only free camera swap + v2.15 bullet fix
-- Re-running this SAME build only reopens the current GUI instead of stacking another copy.
-- If an older build is already active, v2.18 refuses to stack and asks for one clean rejoin.
do
    local SINGLE_KEY = "KimqHC_v21_SingleRuntimeOptimized"
    local BUILD = "v2.18-owner-credits-clean"
    ...
    -- Older builds did not expose a complete disconnect/cleanup handle. Starting v2.18
    ...
    if rawget(_G, SINGLE_KEY) and rawget(_G, "KimqHC_CurrentBuild") ~= BUILD then
        pcall(function()
            game:GetService("StarterGui"):SetCore("SendNotification", {
                Title = "Kimqetras HC",
                Text = "Older runtime detected. Rejoin once, then run v2.18 so only one version is active.",
                Duration = 7,
            })
        end)
        warn("[Kimqetras HC v2.18] Older runtime detected. Rejoin once before running v2.18; refusing to stack versions.")
        return
    end
...
-- v2.18 keeps the v2.15 local bullet fix, tunable camera swap, and owner/credit cleanup.
...
-- ========================================================
-- v2.18 owner / developer credit guard
