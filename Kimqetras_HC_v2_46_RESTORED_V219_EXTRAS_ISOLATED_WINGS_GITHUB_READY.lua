-- Kimqetras HC v2.46 • restored v2.19 weapon-extras core + isolated wing flight
...
local KIMQ_BUILD = "v2.46-restored-v219-extras"
...
do
    ...
    if running and runningBuild == KIMQ_BUILD then
        ...
        pcall(function()
            StarterGui0:SetCore("SendNotification",{
                Title="Kimqetras HC",
                Text=(runtimeState=="ready") and "v2.46 is already running ♡" or "v2.46 is already starting ♡",
                Duration=4,
            })
        end)
        return
    ...
    if oldRuntimeDetected then
        pcall(function()
            StarterGui0:SetCore("SendNotification",{
                Title="Kimqetras HC",
                Text="An older Kimqetras HC runtime is still active. Rejoin once, then run v2.46. v2.46 will not stack on top of it.",
                Duration=9,
            })
        end)
        warn("[Kimqetras HC v2.46] Older runtime detected. Refusing to stack; rejoin once.")
        return
    end
    ...
    local knownRenderBindings = {
        "KimqMacroSpeedV240","KimqMacroSpeedV241","KimqMacroSpeedV242","KimqMacroSpeedV246",
        ...
        "KimqPermanentTimeV241","KimqPermanentTimeV242","KimqPermanentTimeV246",
    }
...
pcall(function()
    RunService:BindToRenderStep("KimqMacroSpeedV246",Enum.RenderPriority.Last.Value,function()
        enforceMacroSpeed()
    end)
end)
...
local function addCleanSection(title, subtitle)
    ...
    -- v2.46: pageHead already displays the section name.
...
task.spawn(function()
    ...
    -- v2.46: pages are grouped by what they actually do so the sidebar is
...
-- v2.46 module: local catalog / limited accessory try-on.
task.spawn(function()
    _G.KimqAccessoryUIReady=false
    local Players = game:GetService("Players")
    local CoreGui = game:GetService("CoreGui")
...
-- v2.46 module: Environment + Weapon Skins (canonical sidebar pages).
task.spawn(function()
    local pageReadyStart=tick()
    while not _G.KimqBasePagesReady and tick()-pageReadyStart<8 do task.wait(.02) end
    ...
    -- v2.46: Environment and Weapon Skins are already real canonical pages/buttons.
    ...
    pcall(function()
        RunService:BindToRenderStep("KimqPermanentTimeV246",Enum.RenderPriority.Last.Value,function()
    ...
    if not envBuildOk then
        warn("[Kimqetras HC v2.46] Environment page fallback: "..tostring(envBuildErr))
    ...
    if not extrasOk then
        ...
        warn("[Kimqetras HC v2.46] restored v2.19 Weapon Extras: "..tostring(extrasErr))
    ...
    -- ========================================================
    -- v2.46 ISOLATED ANGEL WINGS FLIGHT
        ...
        local function playFlightAnim()
            ...
            pcall(function()
                local ok,track=pcall(function()
                    return hum:PlayEmoteAndGetAnimTrackById(108343889270403)
                end)
            ...
            local resolved="rbxassetid://108343889270403"
            pcall(function()
                local objs=game:GetObjects("rbxassetid://108343889270403")
    ...
    skinsBtn.MouseButton1Click:Connect(function()
        task.defer(function()
            ...
            if extras and type(extras.Refresh)=="function" then
                ...
                if not ok then
                    warn("[Kimqetras HC v2.46] extras refresh: "..tostring(err))
...
task.spawn(function()
    ...
    -- v2.46: preserve the grouped sidebar order from pageDefs.
    ...
    local function applyTheme(name,settlePass)
        ...
        -- v2.46 hard guarantee: old controls created with literal pink/yellow
    ...
    -- v2.46: section navigation does not repaint/rescan the entire GUI.
    ...
    -- v2.46 reveal happens only after this initial theme pass is complete.
end)


-- v2.46: delayed post-show cleanup passes removed.
...
-- ========================================================
-- v2.46 branding is deterministic at construction time; no delayed scrub pass.

-- ========================================================
-- v2.46 DETERMINISTIC REVEAL
...
task.spawn(function()
    ...
    -- v2.46: remove the old one-page section divider labels from the
