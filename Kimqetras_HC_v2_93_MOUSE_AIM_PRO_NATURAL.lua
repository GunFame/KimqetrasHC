-- Kimqetras HC v2.93 • v2.92 base + Pro Natural Mouse Aim spring tracking revamp
...
local KIMQ_BUILD = "v2.93-mouse-aim-pro-natural"
...
do
    ...
    if running and runningBuild == KIMQ_BUILD then
        if runningGui then
            ...
            notifyBoot((runtimeState=="ready") and "v2.91 is already running ♡" or "v2.91 is already starting ♡",4)
            return
        end
        ...
        notifyBoot("Recovering a failed v2.91 boot ♡",4)
    end
    ...
    if oldRuntimeDetected then
        ...
        if staleBoot then
            ...
            notifyBoot("STOP: another Kimqetras HC build is already active. v2.91 DID NOT LOAD. Rejoin once, then run only v2.91.",9)
            warn("[Kimqetras HC v2.91] Another build is active. v2.91 DID NOT LOAD; rejoin once before testing.")
            return
        end
    end
...
Hello.Text = "v2.93  •  pro natural mouse aim ♡"
...
-- v2.93 Mouse Aim: buttery free-camera assist with on-body cursor placement.
-- The visible cursor is controlled by a damped spring instead of direct snapping.
-- It keeps a small amount of the user's own mouse movement, uses smooth target-motion
-- compensation, and keeps all assisted points safely inside the selected body part.
-- No Camera.CFrame writes are used here, so this remains separate from Camlock.

-- Natural stock profile. Existing custom values are preserved; only the previous
-- v2.92 stock values migrate automatically.
if cfg.mouseAimNaturalProfileVersion~="v2.93" then
    if cfg.mouseAimPart==nil or tostring(cfg.mouseAimPart)=="Closest Point" then cfg.mouseAimPart="UpperTorso" end
    if cfg.mouseAimFOV==nil or tonumber(cfg.mouseAimFOV)==205 then cfg.mouseAimFOV=190 end
    if cfg.mouseAimFollowStrength==nil or tonumber(cfg.mouseAimFollowStrength)==36 then cfg.mouseAimFollowStrength=52 end
    if cfg.mouseAimSoftness==nil or tonumber(cfg.mouseAimSoftness)==2 then cfg.mouseAimSoftness=5 end
    cfg.mouseAimNaturalProfileVersion="v2.93"
end

cfg.mouseAimEnabled = cfg.mouseAimEnabled == true
cfg.mouseAimKey = cfg.mouseAimKey or Enum.KeyCode.Q
cfg.mouseAimPart = tostring(cfg.mouseAimPart or "UpperTorso")
cfg.mouseAimFOV = tonumber(cfg.mouseAimFOV) or 190
cfg.mouseAimFollowStrength = tonumber(cfg.mouseAimFollowStrength) or 52
cfg.mouseAimSoftness = tonumber(cfg.mouseAimSoftness) or 5
...
local state={
    active=false,target=nil,part=nil,point=nil,worldAimPoint=nil,worldAimPart=nil,
    lastRenderClock=os.clock(),lastRawScreen=nil,filteredCenter=nil,
    targetVelocity=Vector2.zero,cursorVelocity=Vector2.zero,
    manualBias=Vector2.zero,lastMouse=nil,lastApplied=Vector2.zero,
    noiseSeedX=math.random()*800+100,noiseSeedY=math.random()*800+900,
    lockStartedAt=0,shotFocusUntil=0
}
...
local function hum(pl)
    local c=pl and pl.Character
    return c and c:FindFirstChildOfClass("Humanoid")
end
local function knocked(pl)
    ...
    if be then
        for _,name in ipairs({"K.O","KO","Knocked","Downed","Dead","Unconscious"}) do
            ...
            if v and ((v:IsA("BoolValue") and v.Value) or ((v:IsA("IntValue") or v:IsA("NumberValue")) and v.Value~=0)) then return true end
        end
    end
    for _,name in ipairs({"K.O","KO","Knocked","Downed","Dead","Unconscious"}) do
...
local function sameTeam(pl)
    return lp.Team and pl and pl.Team and lp.Team==pl.Team
end
...
local function valid(pl)
    if not pl or pl==lp or _G.KHWhitelist and _G.KHWhitelist[pl.UserId] then return false end
    local h=hum(pl)
    if not h or h.Health<=0 or not pl.Character then return false end
    if knocked(pl) then return false end
    if cfg.mouseAimTeamCheck and sameTeam(pl) then return false end
    return true
end
...
local function preferredPart(pl)
    local c=pl and pl.Character
    if not c then return nil,nil end
    local wanted=tostring(cfg.mouseAimPart or "UpperTorso")
    if wanted~="Closest Point" then
        local p=c:FindFirstChild(wanted)
        if not p and wanted=="UpperTorso" then p=c:FindFirstChild("Torso") end
        if not p and wanted=="LowerTorso" then p=c:FindFirstChild("Torso") end
        if p and p:IsA("BasePart") then return p,p.Position end
    end
    local mousePos=UIS:GetMouseLocation()
    local best,bestD=nil,math.huge
    for _,p in ipairs(usableParts(c)) do
    ...
    best=best or c:FindFirstChild("UpperTorso") or c:FindFirstChild("Torso") or c:FindFirstChild("HumanoidRootPart") or c:FindFirstChild("Head")
    return best,best and best.Position or nil
end

-- Screen-space half-extents from the actual part corners. This is more reliable
-- than guessing a fixed pixel radius at different camera distances/FOVs.
local function partScreenBox(part)
    local center,on=part and toScreen(part.Position) or nil,false
    if not part or not center or not on then return center,5,7 end
    local minX,maxX,minY,maxY=math.huge,-math.huge,math.huge,-math.huge
    local sx,sy,sz=part.Size.X*.46,part.Size.Y*.46,part.Size.Z*.46
    ...
    if minX==math.huge then return center,5,7 end
    local rx=math.clamp((maxX-minX)*.5,3,40)
    local ry=math.clamp((maxY-minY)*.5,4,48)
    return center,rx,ry
end

local function candidateDistance(pl)
    if not valid(pl) then return math.huge,nil,nil end
    local part,point=preferredPart(pl)
    ...
    if d>math.max(10,tonumber(cfg.mouseAimFOV) or 190) then return math.huge,nil,nil end
    return d,part,point
end
local function resetMotionState()
    state.lastRenderClock=os.clock();state.lastRawScreen=nil;state.filteredCenter=nil
    state.targetVelocity=Vector2.zero;state.cursorVelocity=Vector2.zero
    state.manualBias=Vector2.zero;state.lastMouse=nil;state.lastApplied=Vector2.zero
    state.worldAimPoint=nil;state.worldAimPart=nil;state.shotFocusUntil=0
end
local function acquire()
    local best,bestD,bestPart,bestPoint=nil,math.huge,nil,nil
    for _,pl in ipairs(Players:GetPlayers()) do
        local d,p,pt=candidateDistance(pl)
        if d<bestD then best,bestD,bestPart,bestPoint=pl,d,p,pt end
    end
    if best then
        state.active=true;state.target=best;state.part=bestPart;state.point=bestPoint
        state.lockStartedAt=os.clock();resetMotionState()
        _G.KimqMouseAimTarget=best
        return true
    end
    state.active=false;state.target=nil;state.part=nil;state.point=nil;resetMotionState()
    _G.KimqMouseAimTarget=nil
    return false
end
local function unlock()
    state.active=false;state.target=nil;state.part=nil;state.point=nil
    resetMotionState();_G.KimqMouseAimTarget=nil
end

local function resolveMoveRel()
    local fn=rawget(_G,"mousemoverel")
    if type(fn)=="function" then return fn end
    if type(getgenv)=="function" then
        local ok,env=pcall(getgenv)
        if ok and type(env)=="table" and type(env.mousemoverel)=="function" then return env.mousemoverel end
    end
    local direct=nil
    pcall(function() direct=mousemoverel end)
    return type(direct)=="function" and direct or nil
end
local moveRel=resolveMoveRel()
local VIM=nil
pcall(function() VIM=game:GetService("VirtualInputManager") end)

local function clampVec(v,maxMag)
    local m=v.Magnitude
    if m>maxMag and m>0 then return v*(maxMag/m) end
    return v
end

local function worldPointFromScreenBias(part,screenBias,rx,ry)
    if not part then return nil end
    local nx=math.clamp(screenBias.X/math.max(rx,1),-.34,.34)
    local ny=math.clamp(screenBias.Y/math.max(ry,1),-.30,.30)
    -- Positive screen Y is downward, opposite a part's local UpVector.
    return part.Position
        + part.CFrame.RightVector*(part.Size.X*.34*nx)
        - part.CFrame.UpVector*(part.Size.Y*.30*ny)
end

-- Silent Aim uses the exact same Q-locked person. The correction point stays
-- inside the selected body part and follows the visible assisted aim location.
_G.KimqMouseAimOverride=function()
    if not cfg.mouseAimEnabled or not cfg.mouseAimSilentCorrection or not state.active or not valid(state.target) then return nil,nil end
    local part,point=preferredPart(state.target)
    if not part or not point or wallBlocked(state.target,point) then return nil,nil end
    local sp,on=toScreen(point)
    if not on then return nil,nil end
    local leash=math.max(90,(tonumber(cfg.mouseAimFOV) or 190)*1.75)
    if (sp-UIS:GetMouseLocation()).Magnitude>leash then return nil,nil end
    local sameAimPart=state.worldAimPart==part
    state.part,state.point=part,point
    local aimPoint=(sameAimPart and state.worldAimPoint) or point
    return part,aimPoint
end
_G.KimqMouseAimForceCorrection=function()
    if not (cfg.mouseAimEnabled and cfg.mouseAimSilentCorrection and state.active and valid(state.target)) then return false end
    local part,point=preferredPart(state.target)
    if not part or not point then return false end
    local sp,on=toScreen(point)
    if not on then return false end
    return (sp-UIS:GetMouseLocation()).Magnitude<=math.max(90,(tonumber(cfg.mouseAimFOV) or 190)*1.75)
end

UIS.InputBegan:Connect(function(input,gpe)
    if gpe or not cfg.mouseAimEnabled or UIS:GetFocusedTextBox() then return end
    if input.UserInputType==Enum.UserInputType.Keyboard and input.KeyCode==cfg.mouseAimKey then
        if state.active then unlock() else acquire() end
    elseif state.active and input.UserInputType==Enum.UserInputType.MouseButton1 then
        -- Short settle window: faster response + smaller drift while firing, but no snap.
        state.shotFocusUntil=os.clock()+.16
    end
end)
Players.PlayerRemoving:Connect(function(pl) if state.target==pl then unlock() end end)
lp.CharacterAdded:Connect(function() unlock() end)

RunService.RenderStepped:Connect(function()
    if not cfg.mouseAimEnabled then
        if state.active then unlock() end
        return
    end
    if not state.active then return end
    if not valid(state.target) then
        if cfg.mouseAimAutoUnlock then unlock() end
        return
    end

    local part,point=preferredPart(state.target)
    if not part or not point or wallBlocked(state.target,point) then
        if cfg.mouseAimAutoUnlock and (not valid(state.target)) then unlock() end
        return
    end
    state.part,state.point=part,point

    -- Keep first-person/center-lock camera completely free from cursor injection.
    if not cfg.mouseAimVisibleFollow or UIS.MouseBehavior==Enum.MouseBehavior.LockCenter then
        state.worldAimPoint=point
        state.worldAimPart=part
        return
    end
    if not moveRel and not VIM then return end

    local center,rx,ry=partScreenBox(part)
    if not center then return end
    local now=os.clock()
    local dt=math.clamp(now-(state.lastRenderClock or now),1/240,1/18)
    state.lastRenderClock=now

    -- Smooth target screen motion. This removes the tiny one-frame twitch caused
    -- by animation/physics while remaining quick enough for strafing players.
    local rawVel=Vector2.zero
    if state.lastRawScreen then rawVel=(center-state.lastRawScreen)/math.max(dt,1/240) end
    state.lastRawScreen=center
    state.targetVelocity=state.targetVelocity:Lerp(rawVel,1-math.exp(-dt*9.5))
    if not state.filteredCenter then state.filteredCenter=center end
    local targetSpeed=state.targetVelocity.Magnitude
    local centerResponse=14+math.clamp(targetSpeed/180,0,1)*8
    state.filteredCenter=state.filteredCenter:Lerp(center,1-math.exp(-dt*centerResponse))

    local mp=UIS:GetMouseLocation()

    -- Preserve a little of the player's own mouse input. We subtract the movement
    -- injected on the previous frame, so deliberate manual nudges shift the assist
    -- point naturally instead of being instantly erased by the lock.
    if state.lastMouse then
        local observed=mp-state.lastMouse
        local external=observed-(state.lastApplied or Vector2.zero)
        if external.Magnitude<90 then
            state.manualBias=state.manualBias+external*.58
        end
    end
    state.lastMouse=mp
    state.lastApplied=Vector2.zero

    local softness=math.clamp(tonumber(cfg.mouseAimSoftness) or 5,0,18)
    local softFactor=softness/18
    local shotFocus=now<(state.shotFocusUntil or 0)

    -- Manual influence decays slowly instead of snapping back to center.
    local biasDecay=shotFocus and 4.4 or 1.8
    state.manualBias=state.manualBias*math.exp(-dt*biasDecay)
    state.manualBias=Vector2.new(
        math.clamp(state.manualBias.X,-rx*.27,rx*.27),
        math.clamp(state.manualBias.Y,-ry*.23,ry*.23)
    )

    -- Smooth low-amplitude drift from math.noise, never a circle/repeating jitter.
    -- It is intentionally tiny and clamped inside the selected part.
    local driftScale=shotFocus and .28 or 1
    local drift=Vector2.new(
        math.noise(state.noiseSeedX,now*.55)*rx*.075*softFactor,
        math.noise(state.noiseSeedY,now*.48)*ry*.065*softFactor
    )*driftScale

    -- A small screen-velocity lead prevents the cursor from visibly sitting behind
    -- a fast strafe. It is reduced while firing and never leaves the body part.
    local leadScale=shotFocus and .28 or 1
    local lead=Vector2.new(
        math.clamp(state.targetVelocity.X*.0042,-rx*.11,rx*.11),
        math.clamp(state.targetVelocity.Y*.0035,-ry*.09,ry*.09)
    )*leadScale

    local manual=shotFocus and state.manualBias*.32 or state.manualBias
    local offset=manual+drift+lead
    local desired=state.filteredCenter+offset

    -- Final target is clamped relative to the CURRENT part center, not the lagged
    -- center, which keeps the cursor visually on the avatar even during fast movement.
    desired=Vector2.new(
        math.clamp(desired.X,center.X-rx*.34,center.X+rx*.34),
        math.clamp(desired.Y,center.Y-ry*.31,center.Y+ry*.31)
    )
    state.worldAimPoint=worldPointFromScreenBias(part,desired-center,rx,ry) or point
    state.worldAimPart=part

    local delta=desired-mp
    local dist=delta.Magnitude
    local strength=math.clamp(tonumber(cfg.mouseAimFollowStrength) or 52,1,100)/100
    local distanceBoost=math.clamp(dist/145,0,1)
    local insideBody=math.abs(mp.X-center.X)<=rx*.34 and math.abs(mp.Y-center.Y)<=ry*.31
    local engage=math.clamp((now-(state.lockStartedAt or now))/.14,0,1)
    local motionBoost=math.clamp(targetSpeed/1100,0,1)

    -- Once the cursor is already on the selected body part, loosen the magnetism.
    -- That lets normal hand movement remain visible instead of pinning one pixel.
    local insideEase=(insideBody and not shotFocus) and .78 or 1

    -- Critically-near-damped spring. Unlike direct Lerp/max-step tracking, this has
    -- smooth acceleration/deceleration, giving a much more natural mouse path.
    local omega=(9.0+(strength*8.5)+(distanceBoost*7.0)+(motionBoost*2.2)+(shotFocus and 4.0 or 0))*insideEase*(.72+.28*engage)
    local dampingRatio=.96+(1-distanceBoost)*.025
    local accel=delta*(omega*omega)-state.cursorVelocity*(2*omega*dampingRatio)
    local maxAccel=8500+(strength*11500)+(distanceBoost*6500)+(shotFocus and 4500 or 0)
    accel=clampVec(accel,maxAccel)
    state.cursorVelocity=state.cursorVelocity+accel*dt

    local maxSpeed=(560+(strength*1350)+(distanceBoost*650)+(motionBoost*260)+(shotFocus and 420 or 0))*insideEase*(.78+.22*engage)
    state.cursorVelocity=clampVec(state.cursorVelocity,maxSpeed)
    local step=state.cursorVelocity*dt

    -- Never overshoot wildly at close range. A tiny under/oversettle from the
    -- spring is preserved at long range, but close corrections stay clean.
    if dist<28 and step.Magnitude>dist*.82 and dist>0 then step=delta*.82 end
    if dist<10 and step.Magnitude>dist*.68 and dist>0 then step=delta*.68 end

    local deadZone=shotFocus and .32 or math.max(.55,1.05-(strength*.38))
    if dist<deadZone then
        state.cursorVelocity=state.cursorVelocity*.72
        return
    end

    if math.abs(step.X)<.035 then step=Vector2.new(0,step.Y) end
    if math.abs(step.Y)<.035 then step=Vector2.new(step.X,0) end
    if step.X~=0 or step.Y~=0 then
        if moveRel then
            local ok=pcall(moveRel,step.X,step.Y)
            if ok then state.lastApplied=step end
        elseif VIM then
            local ok=pcall(function() VIM:SendMouseMoveEvent(mp.X+step.X,mp.Y+step.Y,game) end)
            if ok then state.lastApplied=step end
        end
    end
end)
...
addToggle("Mouse Aim Enabled",cfg.mouseAimEnabled,function(v)
    ...
    if not cfg.mouseAimEnabled then unlock() end
end)
addKeybind("Mouse Aim Key",cfg.mouseAimKey,function(v) cfg.mouseAimKey=v end)
addToggle("Visible Mouse Follow",cfg.mouseAimVisibleFollow,function(v) cfg.mouseAimVisibleFollow=v==true end)
addToggle("Silent Correction",cfg.mouseAimSilentCorrection,function(v) cfg.mouseAimSilentCorrection=v==true end)
addDropdown("Mouse Aim Part",parts,cfg.mouseAimPart,function(v) cfg.mouseAimPart=tostring(v) end)
addSlider("Mouse Follow Strength",1,100,cfg.mouseAimFollowStrength,function(v) cfg.mouseAimFollowStrength=v end)
addSlider("Aim Softness",0,18,cfg.mouseAimSoftness,function(v) cfg.mouseAimSoftness=v end)
addSlider("Mouse Aim FOV",20,800,cfg.mouseAimFOV,function(v) cfg.mouseAimFOV=v end)
addToggle("Mouse Aim Wall Check",cfg.mouseAimWallCheck,function(v) cfg.mouseAimWallCheck=v==true end)
addToggle("Mouse Aim Team Check",cfg.mouseAimTeamCheck,function(v) cfg.mouseAimTeamCheck=v==true end)
addToggle("Mouse Aim Knock Check",cfg.mouseAimKnockCheck,function(v) cfg.mouseAimKnockCheck=v==true end)
addToggle("Auto Unlock On K.O.",cfg.mouseAimAutoUnlock,function(v) cfg.mouseAimAutoUnlock=v==true end)
]=====], false) then return end
...
task.spawn(function()
    ...
    pcall(function()
        game:GetService("StarterGui"):SetCore("SendNotification",{
            Title="Kimqetras HC",
            Text="v2.93 ACTIVE ♡ • pro natural mouse aim + existing systems ready",
            Duration=7,
        })
    end)
