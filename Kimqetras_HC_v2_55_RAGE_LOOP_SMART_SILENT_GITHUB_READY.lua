-- Kimqetras HC v2.55 • restored v2.19 weapon-extras core + isolated wing flight
...
local KIMQ_BUILD = "v2.55-restored-v219-extras"
...
do
    ...
    if running and runningBuild == KIMQ_BUILD then
        ...
        pcall(function()
            StarterGui0:SetCore("SendNotification",{
                Title="Kimqetras HC",
                Text=(runtimeState=="ready") and "v2.55 is already running ♡" or "v2.55 is already starting ♡",
                Duration=4,
            })
        end)
        return
    ...
    if oldRuntimeDetected then
        pcall(function()
            StarterGui0:SetCore("SendNotification",{
                Title="Kimqetras HC",
                Text="An older Kimqetras HC runtime is still active. Rejoin once, then run v2.55. v2.55 will not stack on top of it.",
                Duration=9,
            })
        end)
        warn("[Kimqetras HC v2.55] Older runtime detected. Refusing to stack; rejoin once.")
        return
    end
    ...
    local knownRenderBindings = {
        "KimqMacroSpeedV240","KimqMacroSpeedV241","KimqMacroSpeedV242","KimqMacroSpeedV255",
        ...
        "KimqPermanentTimeV241","KimqPermanentTimeV242","KimqPermanentTimeV255",
    }
...
local function getSmoothedAimVelocity(part)
    ...
    local smooth=old and old:Lerp(now,.38) or now
...
-- Screen coordinates match UserInputService:GetMouseLocation more closely
-- than viewport coordinates, which removes the small GUI-inset offset.
local function safeWorldToScreenPoint(pos)
    local sp, on = Vector3.new(), false
    pcall(function()
        sp, on = cam:WorldToScreenPoint(pos)
    end)
    return sp, on
end
...
local function getClosestBodyPart(char)
    ...
    for _, pName in ipairs(bodyPartsList) do
        ...
        if part then
            local screenPos, onScreen = safeWorldToScreenPoint(part.Position)
...
local function getClosestPlayerToCursor()
    ...
    local mousePos = UIS:GetMouseLocation()
    local bestPart=nil
    local bestScore=cfg.silentAimFOV

    -- Keep the current target unless another target is clearly closer.
    -- This stops rapid target switching when players overlap on screen.
    ...
    if silentAimCachedPart and silentAimCachedPart.Parent then
        ...
        if stickyPlayer
            and stickyPlayer~=lp
            and not _G.KHWhitelist[stickyPlayer.UserId]
            and isAlive(stickyPlayer)
            and (not cfg.silentAimTeamCheck or not sameTeam(stickyPlayer))
        then
            local d3=(silentAimCachedPart.Position-cam.CFrame.Position).Magnitude
            local sp,on=safeWorldToScreenPoint(silentAimCachedPart.Position)
            if d3<=cfg.silentAimMaxDist
                and on
                and not wallBetween(silentAimCachedPart.Position,stickyChar)
            then
                local d2=(Vector2.new(sp.X,sp.Y)-mousePos).Magnitude
                if d2<=cfg.silentAimFOV*1.10 then
                    stickyPart=silentAimCachedPart
                    stickyScore=d2
                end
            end
        end
    ...
    for _,p in ipairs(Players:GetPlayers()) do
        if p~=lp and not _G.KHWhitelist[p.UserId] and isAlive(p) then
            ...
            if part then
                ...
                if dist3D<=cfg.silentAimMaxDist
                    and not wallBetween(part.Position,char)
                then
                    -- Score the ACTUAL selected hit part instead of the HRP.
                    local sp,onScreen=safeWorldToScreenPoint(part.Position)
                    if onScreen then
    ...
    if stickyPart then
        -- New target must be about 22% better to steal the lock.
        if not bestPart or bestScore>stickyScore*.78 then
            return stickyPart
        end
    end
    return bestPart
        ...
        if not checkcaller() and self == mouse and cfg.silentAim and not isHoldingRevolver() then
            if (key == "Hit" or key == "Target" or key == "UnitRay") and silentAimCachedPart then
                if math.random(1, 100) <= cfg.silentAimHitChance then
                    ...
                    local aimVelocity=getSmoothedAimVelocity(silentAimCachedPart)
                    local hitPos = silentAimCachedPart.Position + Vector3.new(
                        aimVelocity.X * cfg.silentAimPredX,
                        aimVelocity.Y * cfg.silentAimPredY,
                        aimVelocity.Z * cfg.silentAimPredX
                    )
                    if key == "UnitRay" then
...
pcall(function()
    RunService:BindToRenderStep("KimqMacroSpeedV255",Enum.RenderPriority.Last.Value,function()
        enforceMacroSpeed()
    end)
end)
...
local function addCleanSection(title, subtitle)
    ...
    -- v2.55: pageHead already displays the section name.
...
task.spawn(function()
    ...
    -- v2.55: pages are grouped by what they actually do so the sidebar is
    ...
    task.defer(function()
        local rageOK,rageERR=pcall(function()
            ...
            local function button(parent,text,pos,size,fn)
                ...
                b.MouseButton1Click:Connect(function()
                    ...
                    if not ok then warn("[Kimqetras HC v2.55 RAGE button] "..tostring(err)) end
                end)
                return b
            end
            ...
            local function advance(removeCurrent)
                ...
                if #queue>1 then currentIndex=(currentIndex % #queue)+1 end
                refreshSelected()
                if refreshPlayerList then refreshPlayerList() end
            end
            ...
            local function getSourceAnimationId(pl)
                local char=pl and pl.Character
                local hum=char and char:FindFirstChildOfClass("Humanoid")
                if hum then
                    ...
                    if animator then
                        ...
                        if ok and tracks then
                            ...
                            for _,tr in ipairs(tracks) do
                                ...
                                if id~="" then
                                    local w=tonumber(tr.WeightCurrent) or 0
                ...
                -- Fallback: read their idle animation from the live Animate tree.
                local animate=char and char:FindFirstChild("Animate")
                if animate then
                    for _,d in ipairs(animate:GetDescendants()) do
                        if d:IsA("Animation") and d.Name:lower():find("animation") then
                            local id=tostring(d.AnimationId or "")
            ...
            local function makePreviewModel(pl)
                -- Live-character clone is fastest and preserves custom avatars better.
                ...
                -- Avatar service can occasionally fail on the first request.
                for attempt=1,3 do
                    ...
                    task.wait(.12*attempt)
                end
                return nil
            end

            local function playPreviewAnimation(model,animationId)
            ...
            local function showPreview(pl)
                ...
                local animationId="rbxassetid://80004965633316"
                ...
                task.spawn(function()
                    ...
                    -- Keep the Humanoid rig movable for animation; only anchor the root.
                    ...
                    -- Put the avatar upright, straight, and facing the camera.
                    -- Roblox characters face -Z at identity rotation; the preview camera
                    -- sits on -Z looking toward the origin, so identity faces it directly.
                    pcall(function()
                        model:PivotTo(CFrame.new(0,(size.Y*.5)+.05,0))
                    end)

                    -- Re-read bounds after normalizing the model orientation.
                    local okFront,frontCF,frontSize=pcall(function()
                        return model:GetBoundingBox()
                    end)
                    if okFront then
                        cf=frontCF
                        size=frontSize
                        local bottomY=cf.Position.Y-(size.Y*.5)
                        model:PivotTo(
                            CFrame.new(-cf.Position.X,-bottomY+.05,-cf.Position.Z)
                            * model:GetPivot()
                        )
                    end
                    ...
                    -- Platform directly under the avatar.
                    ...
                    -- Their live animation/idle gets mirrored when Roblox permits it.
                    playPreviewAnimation(model,animationId)
                    ...
                    -- Straight-on full-body framing with extra breathing room.
                    local dist=math.max(size.Y*1.92,math.max(size.X,size.Z)*3.35,11.5)
                    local lookY=math.max(size.Y*.48,2.2)
            ...
            local function forceHit(pl,count)
                ...
                task.spawn(function()
                    ...
                    for i=1,n do
                        local part=getPart(pl,hitPart)
                        if not part then break end
                        pcall(ForceHit_Fire,part)
                        if i<n then task.wait(.015) end
                    end
                end)
            end
            ...
            local function normalGunShoot(pl)
                ...
                -- Aim the virtual cursor at the selected body part, then use the
                -- game's normal equipped Tool instead of ForceHit_Fire.
                local cam=workspace.CurrentCamera
                local screen,onScreen=cam:WorldToScreenPoint(part.Position)

                if VIMR and onScreen then
                    pcall(function()
                        if VIMR.SendMouseMoveEvent then
                            VIMR:SendMouseMoveEvent(screen.X,screen.Y,game)
                        end
                    end)
                    pcall(function()
                        VIMR:SendMouseButtonEvent(screen.X,screen.Y,0,true,game,0)
                        task.delay(.018,function()
                            pcall(function()
                                VIMR:SendMouseButtonEvent(screen.X,screen.Y,0,false,game,0)
                            end)
                        end)
                    end)
                end
            ...
            local function rageShoot(pl)
                ...
                if combatMode=="One Shot" then
                    -- One trigger action; burst strength is adjustable because the
                    -- server may reject/limit individual hit requests.
            ...
            local function pressE()
                if VIMR then
                    pcall(function()
                        VIMR:SendKeyEvent(true,Enum.KeyCode.E,false,game)
                        task.wait(.03)
                        VIMR:SendKeyEvent(false,Enum.KeyCode.E,false,game)
                    end)
                    return
                end
                if type(keypress)=="function" then
                    pcall(keypress,0x45)
                    task.wait(.03)
                    if type(keyrelease)=="function" then pcall(keyrelease,0x45) end
                end
            end
            ...
            local function finish(pl)
                ...
                task.spawn(function()
                    if autoStomp then
                        for _=1,stompRepeats do
                            if not pl.Parent then break end
                            pcall(teleportTo,pl,true)
                            task.wait(.06)
                            pressE()
                            task.wait(.08)
                        end
                    ...
                    -- Do NOT delete the target. If there are other queued players,
                    -- temporarily move to the next one. This one becomes eligible
                    -- again automatically after their next respawn.
            ...
            makeSlider(rageCamPage,"Camlock Smoothness",.03,1,camSmooth,function(v) camSmooth=v end,function(v) return string.format("%.2f",v) end)
            ...
            makeToggle(rageOrbitPage,"Auto Teleport",false,function(v) autoTeleport=v; teleportClock=0 end)
            ...
            makeToggle(rageOrbitPage,"Target Orbit Master",false,function(v)
                orbitEnabled=v
                orbitAngle=0
                teleportClock=0
                if v then
                    -- No extra click needed: enabling Target Orbit immediately
                    -- moves to the selected player and starts the continuous flight.
                    tryEquipRageWings()
                    teleportTo(currentPlayer(),false)
                    startOrbitAnimation()
                else
                    if not autoHunt then stopOrbitAnimation() end
                end
            end)
            makeSlider(rageOrbitPage,"Flight / Orbit Speed",.25,20,orbitSpeed,function(v) orbitSpeed=v end,function(v) return string.format("%.1fx",v) end)
            makeSlider(rageOrbitPage,"Orbit Radius",3,30,orbitRadius,function(v) orbitRadius=v end,function(v) return string.format("%.1f",v) end)
            makeSlider(rageOrbitPage,"Orbit Height",-2,18,orbitHeight,function(v) orbitHeight=v end,function(v) return string.format("%.1f",v) end)
            ...
            makeCycle(rageCombatPage,"Hit Part",{"Head","UpperTorso","HumanoidRootPart","Closest Part"},hitPart,function(v) hitPart=v end)

            makeCycle(rageCombatPage,"Combat Mode",{"Normal Gun","One Shot"},combatMode,function(v)
                combatMode=v
            ...
            makeSlider(rageCombatPage,"Auto Shoot Delay",.03,1,shootDelay,function(v) shootDelay=v end,function(v) return string.format("%.2fs",v) end)

            makeSlider(rageCombatPage,"One-Shot Power",1,10,burstCount,function(v)
            ...
            makeToggle(rageCombatPage,"Rage Combat Master",false,function(v)
                autoHunt=v
                orbitAngle=0
                shootClock=0
                teleportClock=0
                if v then
                    -- One switch starts the attack loop. Combat Mode decides whether
                    -- this uses the normal equipped gun or the One Shot path.
                    tryEquipRageWings()
                    teleportTo(currentPlayer(),false)
                    startOrbitAnimation()
                    if combatMode=="Normal Gun" then ensureGunEquipped() end
                else
                    if not orbitEnabled then stopOrbitAnimation() end
                end
            end)
            makeToggle(rageCombatPage,"Auto Stomp With E",true,function(v) autoStomp=v end)
            makeSlider(rageCombatPage,"Stomp E Repeats",1,6,stompRepeats,function(v) stompRepeats=math.floor(v+.5) end,function(v) return tostring(math.floor(v+.5)) end)
            makeToggle(rageCombatPage,"Auto Advance After Finish",true,function(v) autoAdvance=v end)

            local note=card(rageCombatPage,64)
            local noteText=role(label(
                note,
                "Rage Combat Master runs by itself: fly/orbit the current target → auto-fire your selected Combat Mode → detect K.O → stomp with E → continue through the queue. Targets stay queued and are attacked again after they respawn.",
            ...
            RunServiceR.RenderStepped:Connect(function(dt)
                local ok,err=pcall(function()
                    ...
                    if camEnabled and pl then
                        local part=getPart(pl,camPart)
                        if part and cam then
                            local wanted=CFrame.lookAt(cam.CFrame.Position,part.Position)
                            cam.CFrame=cam.CFrame:Lerp(wanted,math.clamp(camSmooth,0,1))
                        end
                    end

                    if viewEnabled and pl then
                    ...
                    if (orbitEnabled or autoHunt) and pl and not finishedWaiting[pl.UserId] then
                        tryEquipRageWings()
                        ...
                        local char=lp.Character
                        local mr=char and char:FindFirstChild("HumanoidRootPart")
                        if tr and char and mr then
                            orbitAngle+=dt*orbitSpeed*math.pi*2
                            local off=Vector3.new(
                                math.cos(orbitAngle)*orbitRadius,
                                orbitHeight,
                                math.sin(orbitAngle)*orbitRadius
                            )
                            local pos=tr.Position+off
                            char:PivotTo(CFrame.lookAt(pos,tr.Position))
                            mr.AssemblyLinearVelocity=Vector3.zero
                            mr.AssemblyAngularVelocity=Vector3.zero
                        end
                    elseif orbitAnimTrack then
                        stopOrbitAnimation()
                    end
                end)
                if not ok then
                    -- Do not kill the rest of the script if RAGE has a runtime issue.
                    warn("[Kimqetras HC v2.55 RAGE frame] "..tostring(err))
                end
            end)
            ...
            RunServiceR.Heartbeat:Connect(function(dt)
                local ok,err=pcall(function()
                    ...
                    if pl and not waiting and (autoShoot or autoHunt) then
                        shootClock+=dt
                        if shootClock>=shootDelay then
                            shootClock=0
                            rageShoot(pl)
                        end
                    else
                ...
                if not ok then
                    warn("[Kimqetras HC v2.55 RAGE heartbeat] "..tostring(err))
                end
            end)
        ...
        if not rageOK then
            warn("[Kimqetras HC v2.55] RAGE module isolated error: "..tostring(rageERR))
        end
    end)
...
-- v2.55 module: local catalog / limited accessory try-on.
task.spawn(function()
    _G.KimqAccessoryUIReady=false
    local Players = game:GetService("Players")
    local CoreGui = game:GetService("CoreGui")
...
-- v2.55 module: Environment + Weapon Skins (canonical sidebar pages).
task.spawn(function()
    local pageReadyStart=tick()
    while not _G.KimqBasePagesReady and tick()-pageReadyStart<8 do task.wait(.02) end
    ...
    -- v2.55: Environment and Weapon Skins are already real canonical pages/buttons.
    ...
    pcall(function()
        RunService:BindToRenderStep("KimqPermanentTimeV255",Enum.RenderPriority.Last.Value,function()
    ...
    if not envBuildOk then
        warn("[Kimqetras HC v2.55] Environment page fallback: "..tostring(envBuildErr))
    ...
    if not extrasOk then
        ...
        warn("[Kimqetras HC v2.55] restored v2.19 Weapon Extras: "..tostring(extrasErr))
    ...
    -- ========================================================
    -- v2.55 ISOLATED ANGEL WINGS FLIGHT
    ...
    skinsBtn.MouseButton1Click:Connect(function()
        task.defer(function()
            ...
            if extras and type(extras.Refresh)=="function" then
                ...
                if not ok then
                    warn("[Kimqetras HC v2.55] extras refresh: "..tostring(err))
...
task.spawn(function()
    ...
    -- v2.55: preserve the grouped sidebar order from pageDefs.
    ...
    local function applyTheme(name,settlePass)
        ...
        -- v2.55 hard guarantee: old controls created with literal pink/yellow
    ...
    -- v2.55: section navigation does not repaint/rescan the entire GUI.
    ...
    -- v2.55 reveal happens only after this initial theme pass is complete.
end)


-- v2.55: delayed post-show cleanup passes removed.
...
-- ========================================================
-- v2.55 branding is deterministic at construction time; no delayed scrub pass.

-- ========================================================
-- v2.55 DETERMINISTIC REVEAL
...
task.spawn(function()
    ...
    -- v2.55: remove the old one-page section divider labels from the
