-- Kimqetras HC v2.90 • v2.89 base + isolated Mouse Aim soft-lock section
...
local KIMQ_BUILD = "v2.90-mouse-aim-soft-lock"
...
do
    ...
    if running and runningBuild == KIMQ_BUILD then
        if runningGui then
            ...
            notifyBoot((runtimeState=="ready") and "v2.90 is already running ♡" or "v2.90 is already starting ♡",4)
            return
        end
        ...
        notifyBoot("Recovering a failed v2.90 boot ♡",4)
    end
    ...
    if oldRuntimeDetected then
        ...
        if staleBoot then
            ...
            notifyBoot("STOP: another Kimqetras HC build is already active. v2.90 DID NOT LOAD. Rejoin once, then run only v2.90.",9)
            warn("[Kimqetras HC v2.90] Another build is active. v2.90 DID NOT LOAD; rejoin once before testing.")
            return
        end
    end
...
Hello.Text = "v2.90  •  mouse aim soft-lock ♡"
...
RunService.RenderStepped:Connect(function()
    ...
    -- v2.90 Mouse Aim bridge.  This only overrides WHO Silent Aim resolves while
...
-- v2.90 Mouse Aim is intentionally separate from Camlock. It never writes the
...
cfg.mouseAimFollowStrength = tonumber(cfg.mouseAimFollowStrength) or 34
...
local state={active=false,target=nil,part=nil,point=nil,lastScreen=nil}
...
RunService.RenderStepped:Connect(function()
    ...
    if softness>0 then
        sp+=Vector2.new(math.sin(now*3.1)*softness,math.cos(now*2.55)*softness*.65)
    end
    local mp=UIS:GetMouseLocation()
    local delta=sp-mp
    local strength=math.clamp(tonumber(cfg.mouseAimFollowStrength) or 34,1,100)/100
    -- Soft, finite movement prevents a robotic one-frame snap.
    local maxStep=8+strength*46
    local dx=math.clamp(delta.X*strength,-maxStep,maxStep)
    local dy=math.clamp(delta.Y*strength,-maxStep,maxStep)
...
task.spawn(function()
    ...
    pcall(function()
        game:GetService("StarterGui"):SetCore("SendNotification",{
            Title="Kimqetras HC",
            Text="v2.90 ACTIVE ♡ • mouse aim + existing systems ready",
            Duration=7,
        })
    end)
