...
-- Uses the user's uploaded Roblox decals and contains no stitched-border overlay.
-- Redundant V20/V21 layout/environment/weapon passes were removed for performance.
...
local LIME = Color3.fromRGB(244, 255, 216)
local LIME2 = Color3.fromRGB(217, 251, 95)
local PINK = Color3.fromRGB(255, 123, 181)
local PINK2 = Color3.fromRGB(255, 184, 213)
local INK = Color3.fromRGB(74, 88, 57)
local SOFT = Color3.fromRGB(118, 132, 92)
...
ShadeGradient.Color = ColorSequence.new({
    ColorSequenceKeypoint.new(0, Color3.fromRGB(251,255,239)),
    ColorSequenceKeypoint.new(.55, LIME),
    ColorSequenceKeypoint.new(1, Color3.fromRGB(255,235,246)),
})
...
-- simple decorations only
for _, data in ipairs({
    {22,22,26,LIME2,.18}, {420,24,20,PINK2,.22}, {28,184,18,PINK2,.30}, {423,182,24,LIME2,.22}
}) do
    local b = Instance.new("Frame", Panel)
    b.Size = UDim2.fromOffset(data[3], data[3])
    b.Position = UDim2.fromOffset(data[1], data[2])
    b.BackgroundColor3 = data[4]
    b.BackgroundTransparency = data[5]
    b.BorderSizePixel = 0
...
BarBack.BackgroundColor3 = LIME2
...
task.spawn(function()
    while Heart.Parent do
        heartUp = not heartUp
        TweenService:Create(Heart, TweenInfo.new(0.7, Enum.EasingStyle.Sine, Enum.EasingDirection.InOut), {
            TextTransparency = heartUp and 0.12 or 0
        }):Play()
        task.wait(0.72)
    end
end)
...
task.spawn(function()
    ...
    local THEMES = {
        Blue = {
            hot = Color3.fromRGB(255, 92, 164), hot2 = Color3.fromRGB(255, 151, 199),
            bg = Color3.fromRGB(239, 255, 211), bg2 = Color3.fromRGB(224, 249, 186),
            panel = Color3.fromRGB(250, 255, 242), card = Color3.fromRGB(255, 255, 255),
            text = Color3.fromRGB(83, 76, 86), sub = Color3.fromRGB(129, 111, 122),
            stroke = Color3.fromRGB(255, 174, 209), white = Color3.fromRGB(255, 255, 255),
        },
        Purple = {
    ...
    if pages.theme then
        ...
        for i, name in ipairs(themeNames) do
            ...
            label(card, ((name == "Blue") and "Lime Pink" or name) .. " Theme", UDim2.new(0.48,0,1,0), UDim2.fromOffset(12,0), Enum.Font.GothamSemibold, 14, p.text)
...
task.spawn(function()
    ...
    local THEMES = {
        Blue = {
            hot=Color3.fromRGB(255,123,181), hot2=Color3.fromRGB(255,184,213),
            bg=Color3.fromRGB(244,255,216), bg2=Color3.fromRGB(233,250,199),
            panel=Color3.fromRGB(250,255,238), card=Color3.fromRGB(255,255,255),
            text=Color3.fromRGB(74,88,57), sub=Color3.fromRGB(112,128,83),
            stroke=Color3.fromRGB(255,181,214), white=Color3.fromRGB(255,255,255),
        },
        Purple = {
...
-- v2.1 final GUI: lime + pink + white, no mascots, no stitching.
task.spawn(function()
    local Players=game:GetService("Players")
    local CoreGui=game:GetService("CoreGui")
    local TweenService=game:GetService("TweenService")
    ...
    local function paw(parent,pos,size,color,rotation,z)
        ...
        img.Image=KIMQ_V26_ASSETS.Paw
        img.ScaleType=Enum.ScaleType.Fit
        img.Rotation=rotation or 0
        img.ZIndex=z or parent.ZIndex+2
    ...
    local function palette()
        local hot=badge and badge.BackgroundColor3 or Color3.fromRGB(255,123,181)
        local defaultHot=Color3.fromRGB(255,123,181)
        local isDefault=math.abs(hot.R-defaultHot.R)<.03 and math.abs(hot.G-defaultHot.G)<.03 and math.abs(hot.B-defaultHot.B)<.03
        local cream=isDefault and Color3.fromRGB(244,255,216) or hot:Lerp(Color3.new(1,1,1),.90)
        local cream2=isDefault and Color3.fromRGB(233,250,199) or hot:Lerp(Color3.new(1,1,1),.94)
        local light=isDefault and Color3.fromRGB(250,255,238) or hot:Lerp(Color3.new(1,1,1),.82)
        local line=isDefault and Color3.fromRGB(255,181,214) or hot:Lerp(Color3.new(1,1,1),.58)
        return {
            hot=hot,
            hot2=hot:Lerp(Color3.new(1,1,1),.34),
            light=light,
            line=line,
            cream=cream,
            cream2=cream2,
            panel=Color3.fromRGB(255,255,255),
            text=Color3.fromRGB(83,76,86),
            sub=Color3.fromRGB(129,111,122),
            white=Color3.fromRGB(255,255,255),
    ...
    local brandSub=label(shell,"silent hc  ♡",UDim2.fromOffset(200,24),UDim2.fromOffset(28,45),Enum.Font.FredokaOne,16,Color3.fromRGB(121,155,84)); brandSub.ZIndex=24; role(brandSub,"limeText")
    ...
    styleNav()
    for _,b in ipairs(navButtons) do
        b.MouseButton1Click:Connect(function() task.delay(.05,styleNav) end)
        b:GetPropertyChangedSignal("BackgroundColor3"):Connect(function() task.defer(styleNav) end)
    end
    ...
    if overview and overview:IsA("ScrollingFrame") then
        ...
        local l2=label(welcome,"Pick a tool on the left, or press Right Shift to hide / reopen the GUI.",UDim2.new(1,-215,0,20),UDim2.fromOffset(112,104),Enum.Font.GothamSemibold,12,P.text); role(l2,"textText")
        ...
        local cd=label(controls,"Right Shift = hide / show the GUI",UDim2.new(1,-80,0,24),UDim2.fromOffset(18,43),Enum.Font.GothamSemibold,12,P.text); role(cd,"textText")
    end
        ...
        for _,d in ipairs(main:GetDescendants()) do
            ...
            if r then
                ...
                if d:IsA("TextLabel") or d:IsA("TextButton") or d:IsA("TextBox") then
                    if r=="hotText" then d.TextColor3=P.hot elseif r=="subText" then d.TextColor3=P.sub elseif r=="textText" then d.TextColor3=P.text elseif r=="lineText" then d.TextColor3=P.line elseif r=="whiteText" then d.TextColor3=P.white elseif r=="limeText" then d.TextColor3=(P.defaultLime and Color3.fromRGB(121,155,84) or P.hot2) end
                end
...
-- ========================================================
-- v2.1 CUTE LIME HEARTS FINAL POLISH
-- Fresh Lime #D9FB5F + Candy Pink #FF7BB5
-- Keeps feature callbacks intact; this block only handles visuals/theme selection.
-- ========================================================
task.spawn(function()
    local Players = game:GetService("Players")
    local CoreGui = game:GetService("CoreGui")
    ...
    local started = tick()
    while not _G.KimqV26Ready and tick() - started < 35 do task.wait(.1) end
    ...
    local PAW_ID = "rbxassetid://138088505213748"
    local THEMES = {
        ["Lime Pink"] = {
            hot=Color3.fromRGB(255,123,181), hot2=Color3.fromRGB(255,184,213),
            lime=Color3.fromRGB(217,251,95), lime2=Color3.fromRGB(233,250,199),
            bg=Color3.fromRGB(244,255,216), panel=Color3.fromRGB(250,255,238),
            card=Color3.fromRGB(255,255,255), text=Color3.fromRGB(74,88,57),
            sub=Color3.fromRGB(112,128,83), line=Color3.fromRGB(255,181,214), white=Color3.fromRGB(255,255,255),
        },
        Purple = {
            hot=Color3.fromRGB(154,72,255), hot2=Color3.fromRGB(205,164,255),
            lime=Color3.fromRGB(223,205,255), lime2=Color3.fromRGB(239,229,255),
            bg=Color3.fromRGB(247,240,255), panel=Color3.fromRGB(252,248,255),
            card=Color3.fromRGB(255,255,255), text=Color3.fromRGB(86,58,127),
            sub=Color3.fromRGB(129,102,158), line=Color3.fromRGB(211,181,242), white=Color3.fromRGB(255,255,255),
        },
        Red = {
            hot=Color3.fromRGB(255,64,95), hot2=Color3.fromRGB(255,151,170),
            lime=Color3.fromRGB(255,205,215), lime2=Color3.fromRGB(255,229,235),
            bg=Color3.fromRGB(255,242,245), panel=Color3.fromRGB(255,248,250),
            card=Color3.fromRGB(255,255,255), text=Color3.fromRGB(139,58,72),
            sub=Color3.fromRGB(177,101,113), line=Color3.fromRGB(242,174,187), white=Color3.fromRGB(255,255,255),
        },
        Pink = {
            hot=Color3.fromRGB(255,30,148), hot2=Color3.fromRGB(255,137,196),
            lime=Color3.fromRGB(255,202,226), lime2=Color3.fromRGB(255,229,242),
            bg=Color3.fromRGB(255,242,249), panel=Color3.fromRGB(255,248,252),
            card=Color3.fromRGB(255,255,255), text=Color3.fromRGB(151,63,108),
            sub=Color3.fromRGB(187,110,148), line=Color3.fromRGB(241,176,207), white=Color3.fromRGB(255,255,255),
        },
        Aqua = {
            hot=Color3.fromRGB(9,180,226), hot2=Color3.fromRGB(121,218,242),
            lime=Color3.fromRGB(194,241,251), lime2=Color3.fromRGB(222,249,255),
            bg=Color3.fromRGB(239,252,255), panel=Color3.fromRGB(247,254,255),
            card=Color3.fromRGB(255,255,255), text=Color3.fromRGB(48,108,128),
            sub=Color3.fromRGB(88,143,161), line=Color3.fromRGB(153,220,235), white=Color3.fromRGB(255,255,255),
        },
        Green = {
            hot=Color3.fromRGB(46,190,116), hot2=Color3.fromRGB(133,224,174),
            lime=Color3.fromRGB(199,241,215), lime2=Color3.fromRGB(225,249,234),
            bg=Color3.fromRGB(240,253,245), panel=Color3.fromRGB(248,255,251),
            card=Color3.fromRGB(255,255,255), text=Color3.fromRGB(53,115,81),
            sub=Color3.fromRGB(92,151,117), line=Color3.fromRGB(157,219,181), white=Color3.fromRGB(255,255,255),
        },
    }

    local currentName = "Lime Pink"
    ...
    local function outline(obj,color,trans,thick)
        local s=obj:FindFirstChildOfClass("UIStroke") or Instance.new("UIStroke",obj)
        s.Color=color; s.Transparency=trans or .25; s.Thickness=thick or 1
        return s
    end
    ...
    -- Remove every remaining paw image. Hearts are the only decorative motif now.
    for _,d in ipairs(main:GetDescendants()) do
        if (d:IsA("ImageLabel") or d:IsA("ImageButton")) then
            local nm=tostring(d.Name):lower()
            if nm:find("paw",1,true) or d.Image==PAW_ID then
                pcall(function() d:Destroy() end)
            end
        end
    end

    -- Remove any old decorative heart labels we created previously so this remains minimal.
    for _,d in ipairs(main:GetDescendants()) do
        if d:GetAttribute("KimqCuteDecor") then pcall(function() d:Destroy() end) end
    end

    local nav, themePage, pageHead, badge
    for _,d in ipairs(shell:GetDescendants()) do
        if d:IsA("ScrollingFrame") then
        ...
        elseif d:IsA("TextLabel") and tostring(d.Text):lower():match("^v2%.1") then
            badge=d
        end
    end
    themePage = themePage or shell:FindFirstChild("themePage",true)

    -- Cute, restrained hearts: sidebar title + current page title only.
    if nav and nav.Parent then
        for _,d in ipairs(nav.Parent:GetChildren()) do
            if d:IsA("TextLabel") and cleanText(d.Text)=="features" then
                d.Text="♥  FEATURES  ♥"; d.TextColor3=current.hot; d.Font=Enum.Font.FredokaOne; d.TextSize=18
            end
        end
    end
    for _,d in ipairs(shell:GetDescendants()) do
        if d:IsA("TextLabel") and cleanText(d.Text)=="theme" and d.TextSize>=18 then
            local p=d.Parent
            if p and p:IsA("Frame") then pageHead=p break end
        end
    end

    local function setRoleColors(p)
        main.BackgroundColor3=p.bg
        local ms=main:FindFirstChildOfClass("UIStroke"); if ms then ms.Color=p.hot end

        for _,d in ipairs(main:GetDescendants()) do
            local role=d:GetAttribute("KimqV26Role")
            if role then
                if d:IsA("Frame") or d:IsA("TextButton") or d:IsA("TextBox") or d:IsA("ImageLabel") then
                    if role=="cream" then d.BackgroundColor3=p.bg
                    elseif role=="cream2" then d.BackgroundColor3=p.lime2
                    elseif role=="panel" then d.BackgroundColor3=p.card
                    elseif role=="lightBg" then d.BackgroundColor3=p.panel
                    elseif role=="hotBg" then d.BackgroundColor3=p.hot
                    elseif role=="whiteBg" then d.BackgroundColor3=p.white end
                end
                if d:IsA("TextLabel") or d:IsA("TextButton") or d:IsA("TextBox") then
                    if role=="hotText" then d.TextColor3=p.hot
                    elseif role=="subText" then d.TextColor3=p.sub
                    elseif role=="textText" then d.TextColor3=p.text
                    elseif role=="lineText" then d.TextColor3=p.line
                    elseif role=="whiteText" then d.TextColor3=p.white
                    elseif role=="limeText" then d.TextColor3=p.text end
                end
                local st=d:FindFirstChildOfClass("UIStroke")
                if st and role~="hotBg" then st.Color=p.line end
            end
        end

        -- Major containers, even if an older pass forgot to tag them.
        if nav and nav.Parent then
            nav.Parent.BackgroundColor3=p.lime2; nav.Parent.BackgroundTransparency=0
            outline(nav.Parent,p.line,.25,1)
            nav.ScrollBarImageColor3=p.hot
        end
        if pageHead then
            pageHead.BackgroundColor3=p.card; outline(pageHead,p.line,.28,1)
        end
        if badge then badge.BackgroundColor3=p.hot; badge.TextColor3=p.white end
    end

    local function restyleNav(p)
        if not nav then return end
        for _,b in ipairs(nav:GetChildren()) do
            if b:IsA("TextButton") then
                local selected = b.BackgroundTransparency < .5 and (b.TextColor3.R>.8 and b.TextColor3.G>.8 and b.TextColor3.B>.8)
                b.BackgroundColor3 = selected and p.hot or p.card
                b.TextColor3 = selected and p.white or p.text
                corner(b,11); outline(b, selected and p.hot or p.line, selected and .05 or .55, 1)
                b.Font=Enum.Font.FredokaOne
                b.TextSize=math.max(b.TextSize,12)
                if not tostring(b.Text):find("♥",1,true) then b.Text="♥  "..b.Text end
            end
        end
    end

    local function styleStandardCards(p)
        for _,d in ipairs(main:GetDescendants()) do
            if d:IsA("Frame") and d ~= main then
                local sz=d.AbsoluteSize
                if sz.X>250 and sz.Y>=42 and sz.Y<=190 and d.BackgroundTransparency<.95 and not d:FindFirstAncestor("ThemeChoiceCute") then
                    -- Do not touch color picker surfaces.
                    local n=tostring(d.Name):lower()
                    if not n:find("color",1,true) and not n:find("picker",1,true) then
                        if d:GetAttribute("KimqV26Role")~="hotBg" then
                            d.BackgroundColor3=p.card
            ...
            elseif d:IsA("TextLabel") then
                local t=cleanText(d.Text)
                if t=="about" or t=="controls" or t=="theme" or t=="overview" then d.TextColor3=p.hot end
            end
        end
    end

    local applyCuteTheme

    local function rebuildThemePage()
        if not themePage then return end
        for _,ch in ipairs(themePage:GetChildren()) do
            if not ch:IsA("UIListLayout") and not ch:IsA("UIPadding") then ch:Destroy() end
        end
        local list=themePage:FindFirstChildOfClass("UIListLayout") or Instance.new("UIListLayout",themePage)
        list.SortOrder=Enum.SortOrder.LayoutOrder; list.Padding=UDim.new(0,9)
        local pad=themePage:FindFirstChildOfClass("UIPadding") or Instance.new("UIPadding",themePage)
        pad.PaddingTop=UDim.new(0,4); pad.PaddingBottom=UDim.new(0,8); pad.PaddingLeft=UDim.new(0,2); pad.PaddingRight=UDim.new(0,4)
        list:GetPropertyChangedSignal("AbsoluteContentSize"):Connect(function() themePage.CanvasSize=UDim2.new(0,0,0,list.AbsoluteContentSize.Y+16) end)

        local order={"Lime Pink","Purple","Red","Pink","Aqua","Green"}
        for _,name in ipairs(order) do
            local p=THEMES[name]
            local row=Instance.new("Frame",themePage)
            row.Name="ThemeChoiceCute"
            row.Size=UDim2.new(1,-8,0,58)
            row.BackgroundColor3=p.card; row.BorderSizePixel=0
            corner(row,13); outline(row,p.line,currentName==name and .02 or .25,currentName==name and 2 or 1)

            local txt=Instance.new("TextLabel",row)
            txt.BackgroundTransparency=1; txt.Size=UDim2.new(1,-205,1,0); txt.Position=UDim2.fromOffset(14,0)
            txt.Text=(name=="Lime Pink" and "Fresh Lime + Candy Pink" or name.." Theme")
            txt.Font=Enum.Font.GothamSemibold; txt.TextSize=14; txt.TextColor3=p.text; txt.TextXAlignment=Enum.TextXAlignment.Left

            local sw1=Instance.new("Frame",row); sw1.Size=UDim2.fromOffset(30,30); sw1.Position=UDim2.new(1,-148,.5,-15); sw1.BackgroundColor3=p.lime; sw1.BorderSizePixel=0; corner(sw1,9); outline(sw1,p.line,.25,1)
            local sw2=Instance.new("Frame",row); sw2.Size=UDim2.fromOffset(30,30); sw2.Position=UDim2.new(1,-109,.5,-15); sw2.BackgroundColor3=p.hot; sw2.BorderSizePixel=0; corner(sw2,9); outline(sw2,p.hot,.18,1)

            local choose=Instance.new("TextButton",row)
            choose.Size=UDim2.fromOffset(54,32); choose.Position=UDim2.new(1,-66,.5,-16)
            choose.BackgroundColor3=p.hot; choose.BorderSizePixel=0; choose.Text="♥"; choose.TextColor3=p.white; choose.Font=Enum.Font.FredokaOne; choose.TextSize=18; choose.AutoButtonColor=false
            corner(choose,10); outline(choose,p.hot,.05,1)
            choose.MouseButton1Click:Connect(function() applyCuteTheme(name) end)
        end
    end

    applyCuteTheme=function(name)
        local p=THEMES[name]; if not p then return end
        currentName=name; current=p
        _G.KimqCuteTheme=name
        if badge then badge.BackgroundColor3=p.hot end -- keeps older theme listeners synchronized too
        task.defer(function()
            setRoleColors(p)
            restyleNav(p)
            styleStandardCards(p)
            rebuildThemePage()
        end)
    end

    -- Make the header/profile/banner feel like one coherent Fresh Lime + Candy Pink design.
    for _,d in ipairs(shell:GetDescendants()) do
        if d:IsA("TextLabel") then
            local t=cleanText(d.Text)
            if t=="kimqetras hc" and d.AbsolutePosition.Y < main.AbsolutePosition.Y+105 then
                d.TextColor3=current.hot; d.Font=Enum.Font.FredokaOne
            elseif t:find("silent hc",1,true) then
                d.TextColor3=current.text
            end
        end
    end

    -- Update after nav clicks because the original page system changes selected-button colors.
    if nav then
        for _,b in ipairs(nav:GetChildren()) do
            if b:IsA("TextButton") then
                b.MouseButton1Click:Connect(function() task.delay(.03,function() restyleNav(current) end) end)
            end
        end
    end

    applyCuteTheme("Lime Pink")
    task.delay(.35,function() applyCuteTheme(_G.KimqCuteTheme or "Lime Pink") end)
    task.delay(1,function() applyCuteTheme(_G.KimqCuteTheme or "Lime Pink") end)
end)
