...
task.spawn(function()
    ...
    local function setupWeaponExtras()
        ...
        txt(beamCard,"Choose the local beam / tracer style used by your shots.",UDim2.new(1,-24,0,20),UDim2.fromOffset(12,32),Enum.Font.Gotham,11,pcolor("sub",subColor))
        ...
        local function normalizeKnifeScale(visual,source,knifeFolder)
            if not visual:IsA("Model") then return 1 end
            local base=knifeFolder and knifeFolder:FindFirstChild("None")
            if not base or base==source then return 1 end
            local baseSize=modelExtent(base); local srcSize=modelExtent(source)
            if not baseSize or not srcSize then return 1 end
            local bm=baseSize.Magnitude; local sm=srcSize.Magnitude
            if bm<=.01 or sm<=.01 then return 1 end
            local ratio=math.clamp(bm/sm,.58,1.72)
            -- Only correct clearly oversized/undersized templates. Normal intentional
            -- differences are left alone.
            if ratio<.78 or ratio>1.28 then
                pcall(function() visual:ScaleTo(ratio) end)
                return ratio
            end
            return 1
        end
        ...
        local function applyKnifeSkin(name,quiet)
            ...
            local sourceRoot=firstPart(source); local target=firstPart(tool)
            if not sourceRoot or not target then return false,"Knife skin has no usable Handle" end
            local visual=source:Clone(); visual.Name="KimqKnifeSkinVisual"
            sanitizeClone(visual)
            visual.Parent=tool
            local knifeFolder=findFolder({"Knives","KnifeSkins","Knife Skins"})
            normalizeKnifeScale(visual,source,knifeFolder)
            local cloneRoot=firstPart(visual); if not cloneRoot then visual:Destroy(); return false,"Knife clone has no visual root" end

            -- Match the skin's grip attachment to the real knife grip whenever possible.
            -- This fixes skins whose Handle origin/rotation is different from the default knife.
            local srcGrip,targetGrip=matchingGripAttachment(cloneRoot,target)
            local desiredRoot=target.CFrame
            if srcGrip and targetGrip then
                desiredRoot=target.CFrame*targetGrip.CFrame*srcGrip.CFrame:Inverse()
            end
            local delta=desiredRoot*cloneRoot.CFrame:Inverse()
            if visual:IsA("BasePart") then visual.CFrame=delta*visual.CFrame end
            for _,d in ipairs(visual:GetDescendants()) do if d:IsA("BasePart") then d.CFrame=delta*d.CFrame end end
            ...
            local rootWeld=Instance.new("WeldConstraint",cloneRoot); rootWeld.Name="KimqKnifeRootWeld"; rootWeld.Part0=cloneRoot; rootWeld.Part1=target
            for _,d in ipairs(visual:GetDescendants()) do
                if d:IsA("BasePart") and d~=cloneRoot and not jointed[d] then
                    local w=Instance.new("WeldConstraint",d); w.Name="KimqKnifeLooseWeld"; w.Part0=d; w.Part1=cloneRoot
                end
            end
            for _,d in ipairs(tool:GetDescendants()) do
        ...
        txt(knifeCard,"Choose a knife style. Animated skins keep their local animation setup.",UDim2.new(1,-24,0,20),UDim2.fromOffset(12,32),Enum.Font.Gotham,11,pcolor("sub",subColor))
