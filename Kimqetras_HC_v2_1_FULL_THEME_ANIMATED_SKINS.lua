-- KIMQETRAS HC v2.1 - FULL THEME SYNC + ANIMATED SKINS
...
-- v2.1 PERFORMANCE SINGLE-RUN GUARD
-- Re-running the loader in the same server no longer stacks another copy of all
-- RenderStepped / Heartbeat / input connections. Rejoin once when upgrading to this build.
do
    local already = rawget(_G, "KimqHC_v21_PerformanceLoaded")
    if already then
        pcall(function()
            local Players = game:GetService("Players")
            local CoreGui = game:GetService("CoreGui")
            local lp = Players.LocalPlayer
            local root = CoreGui:FindFirstChild("KimpetrasHC") or (lp and lp:FindFirstChildOfClass("PlayerGui") and lp.PlayerGui:FindFirstChild("KimpetrasHC"))
...
local function addToggle(text, default, callback)
    ...
    btn.MouseButton1Click:Connect(function()
        ...
        local offColor=(live and live.bg2) or Color3.fromRGB(236, 255, 243)
...
local mainESPWasEnabled = false
RunService.RenderStepped:Connect(function()
    if _G.FPSUnlocker and type(setfpscap) == "function" then setfpscap(_G.FPSTarget) end
    local needFov = _G.ShowFOV == true
    local needFlame = _G.FlamelockEnabled and _G.FlameActive
    local needESP = _G.ESP_Enabled == true
    if not needFov then fovCircle.Visible = false end
    if not needESP and mainESPWasEnabled then
    ...
    mainESPWasEnabled = needESP
    if not needFov and not needFlame and not needESP then return end
    fovCircle.Radius = _G.FOV_RADIUS
...
task.spawn(function()
    ...
    local function hasAnimatedVisuals(obj)
        ...
        for _,d in ipairs(obj:GetDescendants()) do
            if d:IsA("ParticleEmitter") or d:IsA("Trail") or d:IsA("Beam") or d:IsA("AnimationController") or d:IsA("Animator") or d:IsA("Animation") then return true end
        end
        return false
    end
    local function mirrorPlayingAnimations(sourceObj,cloneObj)
        local cloneAnimator=cloneObj and cloneObj:FindFirstChildWhichIsA("Animator",true)
    ...
    local function applySkin(w,s,tool,quiet)
        ...
        -- Script logic is not duplicated. Roblox-native visual animation objects are kept.
        ...
        -- Preserve existing Motor6D/Weld animation rigs. Only loose parts get a safety weld.
        local jointed={}
        ...
        -- If the source preview already has Roblox AnimationTracks playing, mirror them.
        -- ParticleEmitter/Trail/Beam animations work automatically because the full tree is cloned.
        ...
        if not quiet then
            ...
            setStatus(displayWeapon(w).." • "..s..(animated and (mirrored>0 and " • animation synced" or " • animated effects kept") or " • applied locally"),true)
        end
        return true
    end
...
-- ========================================================
-- ========================================================
-- v2.1 PERFORMANCE THEME ENGINE
-- One registry, one click handler per theme, one repaint per click.
-- No delayed 3x repaint passes and no full descendant rescans on every click.
-- ========================================================
task.spawn(function()
    local Players=game:GetService("Players")
    local CoreGui=game:GetService("CoreGui")
    ...
    local function outline(o,c,tr,th)
        local s=o:FindFirstChildOfClass("UIStroke") or Instance.new("UIStroke",o); s.Color=c; s.Transparency=tr or .42; s.Thickness=th or 1; return s
    end
    local function isExcluded(o)
        local x=o
    ...
    local function findNav()
        local best,scoreBest=nil,0
        for _,d in ipairs(main:GetDescendants()) do
            if d:IsA("ScrollingFrame") then
                local score=0
                for _,b in ipairs(d:GetChildren()) do if b:IsA("TextButton") and pageNames[clean(b.Text)] then score+=1 end end
                if score>scoreBest then best,scoreBest=d,score end
            end
        end
        return best
    end
    ...
    -- A-Z only once. No ongoing sorting loops.
    if nav then
        local list=nav:FindFirstChildOfClass("UIListLayout"); if list then list.SortOrder=Enum.SortOrder.LayoutOrder end
        local arr={}
        for _,b in ipairs(nav:GetChildren()) do if b:IsA("TextButton") and pageNames[clean(b.Text)] then table.insert(arr,b) end end
    ...
    -- Build ONE stable registry. Besides explicit roles, classify the original
    -- Matcha/Pink colors once so every old section control joins the theme system.
    -- Theme clicks use this cached registry and do not rescan descendants.
    local registry={}
    local seen={}
    local function add(o,role)
        if not o or isExcluded(o) then return end
        local perObject=seen[o]
        if not perObject then perObject={}; seen[o]=perObject end
        if perObject[role] then return end
        perObject[role]=true
        table.insert(registry,{o=o,role=role})
    end
    ...
    local function closest(color,choices,maxDist)
        local bestRole,bestDist=nil,math.huge
        for _,pair in ipairs(choices) do
            local dist=colorDistance(color,pair[1])
            if dist<bestDist then bestRole,bestDist=pair[2],dist end
        end
        if bestDist<=(maxDist or .08) then return bestRole end
        return nil
    end
    local bgChoices={{BASE.bg,"bg"},{BASE.bg2,"bg2"},{BASE.panel,"panel"},{BASE.soft,"soft"},{BASE.hot,"hot"},{BASE.hot2,"hot2"}}
    local textChoices={{BASE.hot,"hotText"},{BASE.text,"textText"},{BASE.sub,"subText"},{BASE.hot2,"lineText"},{BASE.white,"whiteText"}}
    local imageChoices={{BASE.hot,"hotImage"},{BASE.text,"textImage"},{BASE.sub,"subImage"},{BASE.hot2,"lineImage"},{BASE.white,"whiteImage"}}
    for _,d in ipairs(main:GetDescendants()) do
        if d:IsA("GuiObject") then
            d.BorderSizePixel=0
            if d.BackgroundTransparency<.98 then
                local rr=closest(d.BackgroundColor3,bgChoices,.12)
                if rr then add(d,rr) end
            end
        end
        local r=d:GetAttribute("KimqV26Role")
        if r and roleMap[r] then add(d,roleMap[r]) end
        if d:IsA("TextLabel") or d:IsA("TextButton") or d:IsA("TextBox") then
            local rr=closest(d.TextColor3,textChoices,.14); if rr then add(d,rr) end
            if d:IsA("TextBox") then
                local pr=closest(d.PlaceholderColor3,textChoices,.14)
                if pr then add(d,pr:gsub("Text$","Placeholder")) end
            end
        elseif d:IsA("ImageLabel") or d:IsA("ImageButton") then
            local rr=closest(d.ImageColor3,imageChoices,.14); if rr then add(d,rr) end
        end
        if d:IsA("ScrollingFrame") then add(d,"scroll") end
        if d:IsA("UIStroke") then
            local sr=closest(d.Color,{{BASE.hot,"hotStroke"},{BASE.hot2,"stroke"}},.14)
            add(d,sr or "stroke")
        end
        if d:IsA("UIGradient") and d.Name=="V26BannerGradient" then add(d,"gradient") end
    end
    add(main,"bg")
    ...
    local function styleNav(p)
        ...
        if nav.Parent and nav.Parent:IsA("GuiObject") then nav.Parent.BackgroundColor3=p.bg2; local st=nav.Parent:FindFirstChildOfClass("UIStroke"); if st then st.Color=p.line end end
        local active=activePageName()
        for _,b in ipairs(nav:GetChildren()) do
            if b:IsA("TextButton") and pageNames[clean(b.Text)] then
                local sel=clean(b.Text)==active
                b:SetAttribute("KimqSelected",sel)
                b.BackgroundColor3=sel and p.hot or p.panel
                b.TextColor3=sel and p.white or p.text
                local st=b:FindFirstChildOfClass("UIStroke"); if st then st.Color=sel and p.hot or p.line; st.Transparency=sel and .05 or .58 end
                for _,c in ipairs(b:GetChildren()) do if c:IsA("TextLabel") and (c.Text=="♡" or c.Text=="♥") then c.TextColor3=sel and p.white or p.hot end end
            end
        end
    end
    ...
    for _,d in ipairs(main:GetDescendants()) do
        if d:IsA("TextLabel") or d:IsA("TextButton") then
            ...
            if low:match("^v2%.1") or low:find("kimqetras hc",1,true) or clean(d.Text)=="silent hc" then table.insert(specialTexts,d) end
        end
    end

    local function applyTheme(name)
        local p=THEMES[name]; if not p then return end
        currentName=name; _G.KimqCuteTheme=name
        _G.KimqThemeLivePalette=p

        -- Update the live T tables used by older page/toggle/button callbacks.
        -- This is what stops a click after theming from restoring Matcha/Pink.
        ...
        for _,e in ipairs(registry) do
            ...
            if o and o.Parent then
                local r=e.role
                if r=="bg" and o:IsA("GuiObject") then o.BackgroundColor3=p.bg
                elseif r=="bg2" and o:IsA("GuiObject") then o.BackgroundColor3=p.bg2
                elseif r=="panel" and o:IsA("GuiObject") then o.BackgroundColor3=p.panel
                elseif r=="soft" and o:IsA("GuiObject") then o.BackgroundColor3=p.soft
                elseif r=="hot" and o:IsA("GuiObject") then o.BackgroundColor3=p.hot
                elseif r=="hot2" and o:IsA("GuiObject") then o.BackgroundColor3=p.hot2
                elseif r=="white" and o:IsA("GuiObject") then o.BackgroundColor3=p.white
                elseif r=="hotText" and (o:IsA("TextLabel") or o:IsA("TextButton") or o:IsA("TextBox")) then o.TextColor3=p.hot
                elseif r=="subText" and (o:IsA("TextLabel") or o:IsA("TextButton") or o:IsA("TextBox")) then o.TextColor3=p.sub
                elseif r=="textText" and (o:IsA("TextLabel") or o:IsA("TextButton") or o:IsA("TextBox")) then o.TextColor3=p.text
                elseif r=="lineText" and (o:IsA("TextLabel") or o:IsA("TextButton") or o:IsA("TextBox")) then o.TextColor3=p.hot2
                elseif r=="whiteText" and (o:IsA("TextLabel") or o:IsA("TextButton") or o:IsA("TextBox")) then o.TextColor3=p.white
                elseif r=="hotPlaceholder" and o:IsA("TextBox") then o.PlaceholderColor3=p.hot
                elseif r=="subPlaceholder" and o:IsA("TextBox") then o.PlaceholderColor3=p.sub
                elseif r=="textPlaceholder" and o:IsA("TextBox") then o.PlaceholderColor3=p.text
                elseif r=="linePlaceholder" and o:IsA("TextBox") then o.PlaceholderColor3=p.hot2
                elseif r=="whitePlaceholder" and o:IsA("TextBox") then o.PlaceholderColor3=p.white
                elseif r=="hotImage" and (o:IsA("ImageLabel") or o:IsA("ImageButton")) then o.ImageColor3=p.hot
                elseif r=="subImage" and (o:IsA("ImageLabel") or o:IsA("ImageButton")) then o.ImageColor3=p.sub
                elseif r=="textImage" and (o:IsA("ImageLabel") or o:IsA("ImageButton")) then o.ImageColor3=p.text
                elseif r=="lineImage" and (o:IsA("ImageLabel") or o:IsA("ImageButton")) then o.ImageColor3=p.hot2
                elseif r=="whiteImage" and (o:IsA("ImageLabel") or o:IsA("ImageButton")) then o.ImageColor3=p.white
                elseif r=="scroll" and o:IsA("ScrollingFrame") then o.ScrollBarImageColor3=p.hot
                elseif r=="hotStroke" and o:IsA("UIStroke") then o.Color=p.hot
                elseif r=="stroke" and o:IsA("UIStroke") then o.Color=p.line
                elseif r=="gradient" and o:IsA("UIGradient") then o.Color=ColorSequence.new({ColorSequenceKeypoint.new(0,p.hot2),ColorSequenceKeypoint.new(.52,p.hot),ColorSequenceKeypoint.new(1,p.hot:Lerp(p.white,.12))})
                end
            end
        end
        -- Explicit small set outside role registry, cached once at startup.
        for _,d in ipairs(specialTexts) do
            if d and d.Parent then
                local low=tostring(d.Text or ""):lower()
                if low:match("^v2%.1") then d.BackgroundColor3=p.hot; d.BackgroundTransparency=0; d.TextColor3=p.white; local s=d:FindFirstChildOfClass("UIStroke"); if s then s.Color=p.hot end
                elseif low:find("kimqetras hc",1,true) or clean(d.Text)=="silent hc" then if d.BackgroundTransparency>.8 then d.TextColor3=p.hot end end
            end
        end
    ...
    -- Keep nav highlight synced after a page click, without repainting the whole GUI.
    if nav then
        for _,b in ipairs(nav:GetChildren()) do
            if b:IsA("TextButton") and pageNames[clean(b.Text)] and not b:GetAttribute("KimqPerfNavHook") then
                b:SetAttribute("KimqPerfNavHook",true)
                b.MouseButton1Click:Connect(function() task.defer(function() styleNav(THEMES[currentName]) end) end)
            end
        end
    end

    -- Rebuild Theme page once. Swatches are intentionally excluded from recoloring.
    if themePage then
        for _,ch in ipairs(themePage:GetChildren()) do if not ch:IsA("UIListLayout") and not ch:IsA("UIPadding") then ch:Destroy() end end
        themePage.BackgroundColor3=THEMES[currentName].bg
        ...
        for i,name in ipairs(order) do row(name,i) end
        layout:GetPropertyChangedSignal("AbsoluteContentSize"):Connect(function() themePage.CanvasSize=UDim2.new(0,0,0,layout.AbsoluteContentSize.Y+18) end)
        themePage.CanvasSize=UDim2.new(0,0,0,layout.AbsoluteContentSize.Y+18)
    end

    -- The base rebuild already owns the F1 InputBegan connection. Do not add another.
