...
local KIMQ_BUILD = "v2.70-executor-safe-boot"
local KIMQ_SHARED = (type(getgenv)=="function" and getgenv()) or _G
if type(KIMQ_SHARED)~="table" then KIMQ_SHARED=_G end
local function kimqRoots()
local roots={}
local CoreGui0=game:GetService("CoreGui")
table.insert(roots,CoreGui0)
local lp0=game:GetService("Players").LocalPlayer
local pg0=lp0 and lp0:FindFirstChildOfClass("PlayerGui")
if pg0 then table.insert(roots,pg0) end
...
for _,root in ipairs(kimqRoots()) do
local g=root and root:FindFirstChild("KimpetrasHC")
if g then return g end
end
return nil
end
local function kimqClearRuntimeClaim()
for _,env in ipairs({KIMQ_SHARED,_G}) do
if type(env)=="table" then
env[KIMQ_SINGLE_KEY]=nil
env.KimqHC_CurrentBuild=nil
env.KimqHC_RuntimeState=nil
env.KimqHC_RuntimeStartedAt=nil
env.KimqHC_v21_PerformanceLoaded=nil
end
end
end
do
...
local running=rawget(KIMQ_SHARED,KIMQ_SINGLE_KEY)
local runningBuild=rawget(KIMQ_SHARED,"KimqHC_CurrentBuild")
local runningGui=kimqFindExistingGui()
if runningGui and running then
pcall(function()
...
Title="Kimqetras HC",
Text="Kimqetras HC is already running ♡",
Duration=4,
})
end)
return
end
if not runningGui then
kimqClearRuntimeClaim()
end
for _,root in ipairs(kimqRoots()) do
for _,name in ipairs({"KimpetrasHC","KimpetrasHC_Boot","KimqetrasHC_Error","KimpetrasHC_Error"}) do
local g=root and root:FindFirstChild(name)
...
local playerGui = lp:FindFirstChildOfClass("PlayerGui") or lp:WaitForChild("PlayerGui",8)
pcall(function()
...
BootGui.ZIndexBehavior = Enum.ZIndexBehavior.Global
if type(gethui)=="function" then
pcall(function()
local hui=gethui()
if hui then BootGui.Parent=hui end
end)
end
if not BootGui.Parent then pcall(function() BootGui.Parent = CoreGui end) end
if not BootGui.Parent then
playerGui=playerGui or (lp and lp:WaitForChild("PlayerGui",8))
if playerGui then BootGui.Parent=playerGui end
end
if BootGui.Parent then
KIMQ_SHARED[KIMQ_SINGLE_KEY]=true
KIMQ_SHARED.KimqHC_CurrentBuild=KIMQ_BUILD
KIMQ_SHARED.KimqHC_RuntimeState="booting"
KIMQ_SHARED.KimqHC_RuntimeStartedAt=os.clock()
_G[KIMQ_SINGLE_KEY]=true
_G.KimqHC_CurrentBuild=KIMQ_BUILD
_G.KimqHC_RuntimeState="booting"
_G.KimqHC_RuntimeStartedAt=KIMQ_SHARED.KimqHC_RuntimeStartedAt
else
kimqClearRuntimeClaim()
warn("[Kimqetras HC v2.70] Could not parent the boot GUI.")
end
...
Hello.Text = "v2.70  •  compatibility boot + full feature build ♡"
...
if not quiet then setAvatarStatus("Saved avatar snapshot applied ♡",true) end
elseif not quiet then
setAvatarStatus("Saved avatar snapshot could not be applied",false)
end
return ok
end
...
applyAvatarSnapshot(activeAvatarSnapshot,char,true)
return
end
AvatarTarget=AvatarTargetBox.Text
if tostring(AvatarTarget or "")~="" then
local userId=resolveAvatarUserId(AvatarTarget)
...
local setQueueOrderCycle=nil
local setQueueSkipDownedToggle=nil
local setQueueSkipWhitelistToggle=nil
local setAutoTeleportToggle=nil
local setOrbitMasterToggle=nil
local setAutoWeaponProfilesToggle=nil
local setRageHudToggle=nil
local setAutoShootToggle=nil
local setAutoReloadToggle=nil
local setRageMasterToggle=nil
local setAntiLockToggle=nil
local setAutoStompToggle=nil
local setAutoAdvanceToggle=nil
...
rageHudHeader.BackgroundTransparency=1
end
...
if rageHudStatusPill then rageHudStatusPill.BackgroundColor3=p.soft or p.bg2 or T.bg2 end
if rageHudStatusPillStroke then rageHudStatusPillStroke.Color=p.line or p.stroke or T.stroke end
if rageHudStatus then rageHudStatus.TextColor3=p.hot or T.hot end
end
...
rageHudFrame.Size=UDim2.fromOffset(292,142)
rageHudFrame.BorderSizePixel=0
rageHudFrame.Visible=false
rageHudFrame.ZIndex=80
corner(rageHudFrame,14)
rageHudStroke=stroke(rageHudFrame,pal().line or T.stroke,.22,1)
rageHudHeader=Instance.new("Frame")
rageHudHeader.Name="Header"
rageHudHeader.Parent=rageHudFrame
rageHudHeader.Size=UDim2.new(1,-16,0,34)
rageHudHeader.Position=UDim2.fromOffset(8,5)
rageHudHeader.BackgroundTransparency=1
...
rageHudAccent.Size=UDim2.fromOffset(4,22)
rageHudAccent.Position=UDim2.fromOffset(2,6)
...
UDim2.new(1,-100,1,0),UDim2.fromOffset(14,0),
Enum.Font.FredokaOne,13,pal().hot or T.hot
)
rageHudTitle.ZIndex=82
rageHudStatusPill=Instance.new("Frame")
rageHudStatusPill.Parent=rageHudHeader
rageHudStatusPill.Size=UDim2.fromOffset(78,24)
rageHudStatusPill.Position=UDim2.new(1,-80,.5,-12)
rageHudStatusPill.BorderSizePixel=0
rageHudStatusPill.ZIndex=82
corner(rageHudStatusPill,8)
rageHudStatusPillStroke=stroke(rageHudStatusPill,pal().line or T.stroke,.45,1)
rageHudStatus=label(
rageHudStatusPill,"READY",
UDim2.fromScale(1,1),UDim2.new(),
Enum.Font.GothamBold,9,pal().hot or T.hot,
...
rageHudBody.Position=UDim2.fromOffset(8,40)
rageHudBody.Size=UDim2.new(1,-16,0,94)
rageHudBody.BorderSizePixel=0
rageHudBody.ZIndex=81
corner(rageHudBody,10)
rageHudBodyStroke=stroke(rageHudBody,pal().line or T.stroke,.55,1)
rageHudTarget=label(
rageHudBody,"TARGET  •  none",
UDim2.new(1,-20,0,20),UDim2.fromOffset(10,7),
...
UDim2.new(1,-20,0,18),UDim2.fromOffset(10,28),
Enum.Font.Gotham,9,pal().sub or T.sub
)
rageHudQueue.ZIndex=82
rageHudWeapon=label(
rageHudBody,"WEAPON  •  none",
UDim2.new(1,-20,0,19),UDim2.fromOffset(10,49),
...
UDim2.new(1,-20,0,18),UDim2.fromOffset(10,70),
Enum.Font.Gotham,9,pal().sub or T.sub
)
...
local pl=currentPlayer()
local status="READY"
if stompBusy then status="STOMPING"
...
queueOrderButton,setQueueOrderCycle=makeCycle(rageCamPage,"Queue Order",{"Manual","Closest","Lowest Health","Random"},queueOrderMode,function(v)
queueOrderMode=v
applyQueueOrdering(v=="Random")
end)
local queueSkipDownedButton
queueSkipDownedButton,setQueueSkipDownedToggle=makeToggle(rageCamPage,"Skip Downed / Dead On Advance",true,function(v)
queueSkipDowned=v
end)
local queueSkipWhitelistButton
queueSkipWhitelistButton,setQueueSkipWhitelistToggle=makeToggle(rageCamPage,"Skip Whitelisted On Advance",true,function(v)
...
makeSlider(rageCamPage,"Camlock Follow Strength",.10,1,camSmooth,function(v) camSmooth=v end,function(v) return string.format("%.2f",v) end)
...
autoTeleportButton,setAutoTeleportToggle=makeToggle(rageOrbitPage,"Auto Teleport",false,function(v) autoTeleport=v; teleportClock=0 end)
...
orbitMasterButton,setOrbitMasterToggle=makeToggle(rageOrbitPage,"Target Orbit Master",false,function(v)
orbitEnabled=v
orbitAngle=0
teleportClock=0
if v then
...
autoWeaponProfilesButton,setAutoWeaponProfilesToggle=makeToggle(rageCombatPage,"Auto Weapon Profiles",true,function(v)
autoWeaponProfiles=v
end)
local rageHudButton
rageHudButton,setRageHudToggle=makeToggle(rageCombatPage,"Rage Mini HUD",true,function(v)
rageHudEnabled=v
updateRageHud()
end)
...
autoShootButton,setAutoShootToggle=makeToggle(rageCombatPage,"Auto Shoot Current Target",false,function(v)
autoShoot=v
shootClock=0
...
autoReloadButton,setAutoReloadToggle=makeToggle(rageCombatPage,"Auto Reload When Empty",false,function(v)
autoReload=v
reloadBusy=false
lastReloadAt=0
end)
...
rageMasterButton,setRageMasterToggle=makeToggle(rageCombatPage,"Rage Combat Master",false,function(v)
autoHunt=v
orbitAngle=0
shootClock=0
teleportClock=0
...
antiLockButton,setAntiLockToggle=makeToggle(rageCombatPage,"Anti Lock / Evasive",false,function(v)
antiLockEnabled=v
end)
makeSlider(
rageCombatPage,
...
autoStompButton,setAutoStompToggle=makeToggle(rageCombatPage,"Auto Stomp With E",true,function(v) autoStomp=v end)
...
autoAdvanceButton,setAutoAdvanceToggle=makeToggle(rageCombatPage,"Auto Advance After Finish",true,function(v) autoAdvance=v end)
local ragePresetCard=card(rageCombatPage,150)
role(label(ragePresetCard,"♥  RAGE Presets",UDim2.new(1,-24,0,24),UDim2.fromOffset(12,7),Enum.Font.GothamBold,13,pal().hot or T.hot),"hotText")
local ragePresetSub=role(label(
ragePresetCard,"one-tap setups • your other Kimqetras sections stay untouched",
UDim2.new(1,-24,0,18),UDim2.fromOffset(12,30),Enum.Font.Gotham,9,pal().sub or T.sub
),"subText")
...
if type(setQueueOrderCycle)=="function" then setQueueOrderCycle("Manual",true) end
setRageToggle(setQueueSkipDownedToggle,true)
setRageToggle(setQueueSkipWhitelistToggle,true)
setRageToggle(setAutoWeaponProfilesToggle,true)
setRageToggle(setRageHudToggle,true)
setRageToggle(setAutoReloadToggle,true)
setRageToggle(setAutoStompToggle,true)
setRageToggle(setAutoAdvanceToggle,false)
setRageToggle(setAntiLockToggle,true)
setRageToggle(setAutoTeleportToggle,false)
setRageToggle(setOrbitMasterToggle,false)
setRageToggle(setAutoShootToggle,false)
setRageToggle(setRageMasterToggle,true)
elseif name=="Queue Sweep" then
if type(setQueueOrderCycle)=="function" then setQueueOrderCycle("Closest",true) end
setRageToggle(setQueueSkipDownedToggle,true)
setRageToggle(setQueueSkipWhitelistToggle,true)
setRageToggle(setAutoWeaponProfilesToggle,true)
setRageToggle(setRageHudToggle,true)
setRageToggle(setAutoReloadToggle,true)
setRageToggle(setAutoStompToggle,true)
setRageToggle(setAutoAdvanceToggle,true)
setRageToggle(setAntiLockToggle,true)
setRageToggle(setAutoTeleportToggle,false)
setRageToggle(setOrbitMasterToggle,false)
setRageToggle(setAutoShootToggle,false)
setRageToggle(setRageMasterToggle,true)
elseif name=="Orbit + Evasive" then
setRageToggle(setRageMasterToggle,false)
setRageToggle(setAutoShootToggle,false)
setRageToggle(setAutoTeleportToggle,false)
setRageToggle(setRageHudToggle,true)
setRageToggle(setAntiLockToggle,true)
setRageToggle(setOrbitMasterToggle,true)
elseif name=="Manual / Calm" then
setRageToggle(setRageMasterToggle,false)
setRageToggle(setOrbitMasterToggle,false)
setRageToggle(setAutoShootToggle,false)
setRageToggle(setAutoTeleportToggle,false)
setRageToggle(setAntiLockToggle,false)
reloadBusy=false
stopStableLock()
updateRageHud()
end
...
button(ragePresetCard,"Solo Full Send",UDim2.fromOffset(10,56),UDim2.new(.5,-15,0,34),function() applyRagePreset("Solo Full Send") end)
button(ragePresetCard,"Queue Sweep",UDim2.new(.5,5,0,56),UDim2.new(.5,-15,0,34),function() applyRagePreset("Queue Sweep") end)
button(ragePresetCard,"Orbit + Evasive",UDim2.fromOffset(10,100),UDim2.new(.5,-15,0,34),function() applyRagePreset("Orbit + Evasive") end)
button(ragePresetCard,"Manual / Calm",UDim2.new(.5,5,0,100),UDim2.new(.5,-15,0,34),function() applyRagePreset("Manual / Calm") end)
local note=card(rageCombatPage,64)
local noteText=role(label(
note,
"Rage Combat Master runs by itself: stable target camera → orbit → auto-fire → detect K.O → stomp → automatically continue to the next queued target. The camera follows the target at a fixed offset, so your orbit cannot spin it.",
...
if not rageOK then
warn("[Kimqetras HC v2.70] RAGE module isolated error: "..tostring(rageERR))
end
end)
do
...
if mainW then mainW.Visible=true end
end
end)
end)
task.spawn(function()
