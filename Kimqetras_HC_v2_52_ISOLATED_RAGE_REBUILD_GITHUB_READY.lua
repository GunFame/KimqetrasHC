-- Kimqetras HC v2.52 • restored v2.19 weapon-extras core + isolated wing flight
...
local KIMQ_BUILD = "v2.52-restored-v219-extras"
...
do
    ...
    if running and runningBuild == KIMQ_BUILD then
        ...
        pcall(function()
            StarterGui0:SetCore("SendNotification",{
                Title="Kimqetras HC",
                Text=(runtimeState=="ready") and "v2.52 is already running ♡" or "v2.52 is already starting ♡",
                Duration=4,
            })
        end)
        return
    ...
    if oldRuntimeDetected then
        pcall(function()
            StarterGui0:SetCore("SendNotification",{
                Title="Kimqetras HC",
                Text="An older Kimqetras HC runtime is still active. Rejoin once, then run v2.52. v2.52 will not stack on top of it.",
                Duration=9,
            })
        end)
        warn("[Kimqetras HC v2.52] Older runtime detected. Refusing to stack; rejoin once.")
        return
    end
    ...
    local knownRenderBindings = {
        "KimqMacroSpeedV240","KimqMacroSpeedV241","KimqMacroSpeedV242","KimqMacroSpeedV252",
        ...
        "KimqPermanentTimeV241","KimqPermanentTimeV242","KimqPermanentTimeV252",
    }
...
pcall(function()
    RunService:BindToRenderStep("KimqMacroSpeedV252",Enum.RenderPriority.Last.Value,function()
        enforceMacroSpeed()
    end)
end)
...
local function addCleanSection(title, subtitle)
    ...
    -- v2.52: pageHead already displays the section name.
...
task.spawn(function()
    ...
    -- v2.52: pages are grouped by what they actually do so the sidebar is
    ...
    task.defer(function()
        local rageOK,rageERR=pcall(function()
            ...
            local function button(parent,text,pos,size,fn)
                ...
                b.MouseButton1Click:Connect(function()
                    ...
                    if not ok then warn("[Kimqetras HC v2.52 RAGE button] "..tostring(err)) end
                end)
                return b
            end
            ...
            local function showPreview(pl)
                ...
                task.spawn(function()
                    ...
                    if not ok or not model or token~=previewToken or not previewViewport or not previewViewport.Parent then
                    ...
                    platform.Size=Vector3.new(.45,5.2,5.2)
                    platform.Material=Enum.Material.SmoothPlastic
                    platform.Color=p.soft or T.bg2
                    platform.Anchored=true
                    platform.CanCollide=false
                    platform.CastShadow=false
                    platform.CFrame=CFrame.Angles(0,0,math.rad(90))*CFrame.new(0,-size.Y*.5-.3,0)
                    platform.Parent=world

                    local ring=Instance.new("Part")
                    ring.Shape=Enum.PartType.Cylinder
                    ring.Size=Vector3.new(.12,5.8,5.8)
                    ...
                    ring.CFrame=CFrame.Angles(0,0,math.rad(90))*CFrame.new(0,-size.Y*.5-.24,0)
                    ring.Parent=world

                    pcall(function()
                        local pivot=model:GetPivot()
                        model:PivotTo(CFrame.new(-cf.Position)*pivot)
                    end)
                    ...
                    local dist=math.max(size.Y*1.28,math.max(size.X,size.Z)*2.5,7)
                    cam.CFrame=CFrame.lookAt(Vector3.new(0,size.Y*.05,-dist),Vector3.new(0,0,0))
                end)
            end
            ...
            local function forceHit(pl,count)
                ...
                task.spawn(function()
                    for i=1,math.clamp(math.floor(count or 1),1,12) do
                        local part=getPart(pl,hitPart)
                        if not part then break end
                        pcall(ForceHit_Fire,part)
                        if i<count then task.wait(.015) end
                    end
                end)
            end
            ...
            makeToggle(rageOrbitPage,"Auto Orbit",false,function(v) orbitEnabled=v; orbitAngle=0 end)
            makeSlider(rageOrbitPage,"Orbit Speed",.25,20,orbitSpeed,function(v) orbitSpeed=v end,function(v) return string.format("%.1fx",v) end)
            ...
            makeToggle(rageCombatPage,"Auto Shoot",false,function(v) autoShoot=v; shootClock=0 end)
            makeSlider(rageCombatPage,"Shoot Delay",.03,1,shootDelay,function(v) shootDelay=v end,function(v) return string.format("%.2fs",v) end)
            makeSlider(rageCombatPage,"Force Hit Burst",1,10,burstCount,function(v) burstCount=math.floor(v+.5) end,function(v) return tostring(math.floor(v+.5)) end)

            local burstCard=card(rageCombatPage,52)
            button(burstCard,"♥  Force Hit Current Target",UDim2.fromOffset(10,9),UDim2.new(1,-20,0,34),function()
                forceHit(currentPlayer(),burstCount)
            end)
            ...
            local noteText=role(label(
                note,
                "Auto Hunt only affects the RAGE target queue: orbit current target → fire burst → detect KO/down → move close and press E → continue.",
            ...
            RunServiceR.RenderStepped:Connect(function(dt)
                ...
                if not ok then
                    ...
                    warn("[Kimqetras HC v2.52 RAGE frame] "..tostring(err))
                end
            end)
            ...
            RunServiceR.Heartbeat:Connect(function(dt)
                local ok,err=pcall(function()
                    ...
                    if pl and (autoShoot or autoHunt) then
                        ...
                        if shootClock>=shootDelay then
                            shootClock=0
                            forceHit(pl,autoHunt and burstCount or 1)
                        end
                    else
                        shootClock=0
                    end
                ...
                if not ok then
                    warn("[Kimqetras HC v2.52 RAGE heartbeat] "..tostring(err))
                end
            end)
        ...
        if not rageOK then
            warn("[Kimqetras HC v2.52] RAGE module isolated error: "..tostring(rageERR))
        end
    end)
...
-- v2.52 module: local catalog / limited accessory try-on.
task.spawn(function()
    _G.KimqAccessoryUIReady=false
    local Players = game:GetService("Players")
    local CoreGui = game:GetService("CoreGui")
...
-- v2.52 module: Environment + Weapon Skins (canonical sidebar pages).
task.spawn(function()
    local pageReadyStart=tick()
    while not _G.KimqBasePagesReady and tick()-pageReadyStart<8 do task.wait(.02) end
    ...
    -- v2.52: Environment and Weapon Skins are already real canonical pages/buttons.
    ...
    pcall(function()
        RunService:BindToRenderStep("KimqPermanentTimeV252",Enum.RenderPriority.Last.Value,function()
    ...
    if not envBuildOk then
        warn("[Kimqetras HC v2.52] Environment page fallback: "..tostring(envBuildErr))
    ...
    if not extrasOk then
        ...
        warn("[Kimqetras HC v2.52] restored v2.19 Weapon Extras: "..tostring(extrasErr))
    ...
    -- ========================================================
    -- v2.52 ISOLATED ANGEL WINGS FLIGHT
    ...
    skinsBtn.MouseButton1Click:Connect(function()
        task.defer(function()
            ...
            if extras and type(extras.Refresh)=="function" then
                ...
                if not ok then
                    warn("[Kimqetras HC v2.52] extras refresh: "..tostring(err))
...
task.spawn(function()
    ...
    -- v2.52: preserve the grouped sidebar order from pageDefs.
    ...
    local function applyTheme(name,settlePass)
        ...
        -- v2.52 hard guarantee: old controls created with literal pink/yellow
    ...
    -- v2.52: section navigation does not repaint/rescan the entire GUI.
    ...
    -- v2.52 reveal happens only after this initial theme pass is complete.
end)


-- v2.52: delayed post-show cleanup passes removed.
...
-- ========================================================
-- v2.52 branding is deterministic at construction time; no delayed scrub pass.

-- ========================================================
-- v2.52 DETERMINISTIC REVEAL
...
task.spawn(function()
    ...
    -- v2.52: remove the old one-page section divider labels from the
