-- Kimqetras HC v2.89 • v2.88 base + MAX RAGE loop kills + preset auto-shoot + clean feature UI + no mini hubs
...
local KIMQ_BUILD = "v2.89-rage-max-loop-autoshoot-clean"
...
do
    ...
    if running and runningBuild == KIMQ_BUILD then
        if runningGui then
            ...
            notifyBoot((runtimeState=="ready") and "v2.89 is already running ♡" or "v2.89 is already starting ♡",4)
            return
        end
        ...
        notifyBoot("Recovering a failed v2.89 boot ♡",4)
    end
    ...
    if oldRuntimeDetected then
        ...
        if staleBoot then
            ...
            notifyBoot("STOP: another Kimqetras HC build is already active. v2.89 DID NOT LOAD. Rejoin once, then run only v2.89.",9)
            warn("[Kimqetras HC v2.89] Another build is active. v2.89 DID NOT LOAD; rejoin once before testing.")
            return
        end
    end
...
Hello.Text = "v2.89  •  MAX loop rage + clean UI ♡"
...
task.spawn(function()
    ...
    pcall(function()
        game:GetService("StarterGui"):SetCore("SendNotification",{
            Title="Kimqetras HC",
            Text="v2.89 ACTIVE ♡ • MAX loop rage + clean UI ready",
            Duration=7,
        })
    end)
