...
do
    ...
    _G.KimqHC_CurrentBuild = "v2.1-single-runtime-optimized"
end
...
task.spawn(function()
    ...
    if info then
        ...
        local function profileRow(userId, displayName, role, x)
            ...
            -- rbxthumb works immediately in executors where GetUserThumbnailAsync can
            -- stay blank for users who are not currently in the server.
            pic.Image = ("rbxthumb://type=AvatarHeadShot&id=%d&w=150&h=150"):format(userId)
            task.spawn(function()
                local ok,img=pcall(function()
                    local content = PlayersSvc:GetUserThumbnailAsync(userId,Enum.ThumbnailType.HeadShot,Enum.ThumbnailSize.Size150x150)
                    return content
                end)
                if ok and type(img)=="string" and img~="" and pic.Parent then pic.Image=img end
            end)
    ...
    local function setupWeaponExtras()
        ...
        local function applyBulletBeamOverride(name)
            ...
            if name=="None" then
                ...
                return true,"Bullet color reset"
            end
            local selected=bulletBeamRoot:FindFirstChild(name)
            if not selected then return false,"Bullet preset was not found" end
            -- Do not recolor the gun or rewrite ReplicatedStorage templates.  The game
            -- owns those.  We draw a short-lived LOCAL tracer from the gun muzzle using
            -- this preset whenever the local player fires.
            setLocalBeamSelectionHints(name)
            return true,"Bullet color: "..name
        end
        ...
        txt(beamCard,"Shows the selected BulletBeams preset as your local shot tracer. Does not recolor the gun.",UDim2.new(1,-24,0,20),UDim2.fromOffset(12,32),Enum.Font.Gotham,11,pcolor("sub",subColor))
        ...
        -- Local tracer rendering below is authoritative; do not recolor arbitrary Workspace descendants.

        local Debris=game:GetService("Debris")
        local localMouse=lp:GetMouse()
        local lastTracerDraw=0

        local function sourcePartForBeam()
            local src=selectedBeamSource()
            if not src then return nil,nil end
            local part=src:FindFirstChildWhichIsA("BasePart",true)
            return src,part
        end

        local function muzzlePosition(tool)
            if tool then
                local preferred={"MuzzleAttachment","Muzzle","BarrelAttachment","GunMuzzle","ShootAttachment","FireAttachment"}
                for _,n in ipairs(preferred) do
                    local a=tool:FindFirstChild(n,true)
                    if a and a:IsA("Attachment") then return a.WorldPosition end
                end
                for _,n in ipairs({"Muzzle","Barrel","Handle"}) do
                    local part=tool:FindFirstChild(n,true)
                    if part and part:IsA("BasePart") then return part.Position end
                end
                local p=tool:FindFirstChildWhichIsA("BasePart",true)
                if p then return p.Position end
            end
            return cam.CFrame.Position
        end

        local function shotTarget(origin)
            local ray=cam:ScreenPointToRay(localMouse.X,localMouse.Y)
            local params=RaycastParams.new()
            params.FilterType=Enum.RaycastFilterType.Exclude
            params.FilterDescendantsInstances=lp.Character and {lp.Character} or {}
            params.IgnoreWater=true
            local result=workspace:Raycast(origin,ray.Direction*1500,params)
            return result and result.Position or (origin+ray.Direction*900)
        end

        local function copyTracerExtras(preset,tracer)
            -- Hallows/Kurumi-style presets can carry an Outline/Highlight or lights.
            for _,d in ipairs(preset:GetDescendants()) do
                if d:IsA("Highlight") then
                    local c=d:Clone(); c.Adornee=tracer; c.Parent=tracer
                elseif d:IsA("PointLight") or d:IsA("SpotLight") or d:IsA("SurfaceLight") then
                    local c=d:Clone(); c.Parent=tracer; pcall(function() c.Enabled=true end)
                elseif d:IsA("ParticleEmitter") then
                    local c=d:Clone(); c.Parent=tracer; pcall(function() c.Enabled=true end); pcall(function() c:Emit(math.max(1,c:GetAttribute("EmitCount") or 2)) end)
                end
            end
        end

        local function drawLocalBulletTracer(tool)
            if extraState.BulletBeam=="None" then return end
            local now=os.clock()
            if now-lastTracerDraw<0.045 then return end
            lastTracerDraw=now
            local preset,srcPart=sourcePartForBeam()
            if not preset or not srcPart then return end

            local origin=muzzlePosition(tool)
            local finish=shotTarget(origin)
            local delta=finish-origin
            local distance=delta.Magnitude
            if distance<1 then return end
            distance=math.min(distance,1500)
            finish=origin+delta.Unit*distance

            local tracer=Instance.new("Part")
            tracer.Name="KimqLocalBulletTracer"
            tracer.Anchored=true
            tracer.CanCollide=false; tracer.CanTouch=false; tracer.CanQuery=false
            tracer.CastShadow=false
            tracer.Color=srcPart.Color
            tracer.Material=srcPart.Material
            tracer.Reflectance=srcPart.Reflectance
            local tr=srcPart.Transparency
            tracer.Transparency=(tr>=.92) and .08 or math.clamp(tr,0,.72)
            local ss=srcPart.Size
            local thickness=math.clamp(math.min(ss.X,ss.Y,ss.Z),.035,.22)
            tracer.Size=Vector3.new(thickness,thickness,distance)
            tracer.CFrame=CFrame.lookAt((origin+finish)*.5,finish)
            tracer.Parent=workspace
            copyTracerExtras(preset,tracer)
            Debris:AddItem(tracer,.11)
        end

        local function markLocalShot(tool)
            lastLocalShot=os.clock()
            drawLocalBulletTracer(tool)
        end
        local function hookShotTool(tool)
            if not tool or not tool:IsA("Tool") or tool:GetAttribute("KimqBulletBeamHook3") then return end
            tool:SetAttribute("KimqBulletBeamHook3",true)
        ...
        local function matchingGripAttachment(sourceRoot,targetRoot)
            ...
            local preferred={"RightGripAttachment","GripAttachment","HandleAttachment","KnifeAttachment"}
            for _,n in ipairs(preferred) do
            ...
            for _,a in ipairs(sourceRoot:GetDescendants()) do
                if a:IsA("Attachment") then
                    local b=targetRoot:FindFirstChild(a.Name,true)
                    if b and b:IsA("Attachment") then return a,b end
                end
            end
            return nil,nil
        end
        local function buildKnifeVisual(source,tool)
            -- Clone the complete skin exactly as it exists in ReplicatedStorage.  The
            -- Knives folder stores the visible knife under Handle, with extra meshes,
            -- unions, trails, beams and (for some skins) an AnimationController.
            ...
            local function functionalName(n)
                n=tostring(n):upper()
                return n:find("HITBOX",1,true) or n=="HUMANOIDROOTPART" or n:find("PARTICLE_PART",1,true)
            end
            local function looksVisual(part)
                return part:IsA("MeshPart") or part:IsA("UnionOperation") or part:FindFirstChildWhichIsA("SpecialMesh")~=nil
            end
            for _,part in ipairs(visual:GetDescendants()) do
                if part:IsA("BasePart") then
                    ...
                    -- Display meshes/unions in these knife templates can be stored at
                    -- Transparency 1 because the game's own equip code reveals them.
                    -- Reveal visual geometry, but leave hitboxes/particle carrier parts hidden.
                    if functionalName(part.Name) then
                        part.Transparency=1
                    elseif looksVisual(part) then
                        if part.Transparency>=.95 then part.Transparency=0 end
                        if part.Transparency<.95 then visibleCount+=1 end
                    elseif part.Name=="Handle" then
                        if part.Transparency>=.95 then part.Transparency=0 end
                        if part.Transparency<.95 then visibleCount+=1 end
                    elseif part.Transparency<.95 then
                        visibleCount+=1
                    end
                end
            end
        ...
        applyKnifeSkin=function(name,quiet)
            ...
            if not (cloneRoot and cloneRoot:IsA("BasePart")) then cloneRoot=visual:FindFirstChild("Handle",true) end
            ...
            -- The knife templates are authored around their Handle.  Align Handle to
            -- the real Knife Handle first; generic Attachment names in skins are often
            -- VFX anchors, not grip points, and using them can put the knife off-screen.
            local delta=target.CFrame*cloneRoot.CFrame:Inverse()
            ...
            -- Make sure there is at least one visible cosmetic part before hiding the
            -- original knife.  This is especially important for Beta/Bitcoin/Nightblade.
            if visibleCount==0 then
                cloneRoot.Transparency=0
                cloneRoot.LocalTransparencyModifier=0
                visibleCount=1
            end
            ...
            return true,"Knife skin: "..name..detail.."  •  "..tostring(visibleCount).." visual part(s)"
        end
        ...
        local function hookKnifeContainer(container)
            ...
            container.ChildAdded:Connect(function(ch)
                hookShotTool(ch)
                if ch:IsA("Tool") and ch.Name:lower():find("knife",1,true) and extraState.KnifeSkin and extraState.KnifeSkin~="None" then
