-- Kimqetras HC v2.98 • v2.97 base + rebuilt Mouse Aim 2.0 actual-pointer assist
...
local KIMQ_BUILD = "v2.98-mouse-aim-2-actual-pointer"
...
do
    ...
    if running and runningBuild == KIMQ_BUILD then
        if runningGui then
            ...
            notifyBoot((runtimeState=="ready") and "v2.98 is already running ♡" or "v2.98 is already starting ♡",4)
            return
        end
        ...
        notifyBoot("Recovering a failed v2.98 boot ♡",4)
    end
    ...
    if oldRuntimeDetected then
        ...
        if staleBoot then
            ...
            notifyBoot("STOP: another Kimqetras HC build is already active. v2.98 DID NOT LOAD. Rejoin once, then run only v2.98.",9)
            warn("[Kimqetras HC v2.98] Another build is active. v2.98 DID NOT LOAD; rejoin once before testing.")
            return
        end
    end
...
Hello.Text = "v2.98  •  Mouse Aim 2.0 + actual pointer ♡"
...
-- v2.98 Mouse Aim 2.0
-- Actual pointer assistance again: the real Roblox mouse is moved with the
-- executor's native relative-mouse API when available. The camera is never
-- rotated by this section. First-person / LockCenter stays camera-free and
-- uses the screen center only for the shot-alignment check.

if cfg.mouseAimEngineVersion~="v2.98" then
    cfg.mouseAimEnabled=true
    cfg.mouseAimKey=Enum.KeyCode.Q
    cfg.mouseAimPart="UpperTorso"
    cfg.mouseAimFOV=220
    cfg.mouseAimAssistStrength=72
    cfg.mouseAimSmoothness=76
    cfg.mouseAimCatchUp=82
    cfg.mouseAimOnBodyAssist=14
    cfg.mouseAimVisibleFollow=true
    cfg.mouseAimSilentCorrection=true
    cfg.mouseAimWallCheck=true
    cfg.mouseAimTeamCheck=false
    cfg.mouseAimKnockCheck=true
    cfg.mouseAimAutoUnlock=true
    cfg.mouseAimEngineVersion="v2.98"
end
...
local function screenCenter()
    ...
    return inset + cam.ViewportSize*.5
end

local function currentAimPosition()
    if mouseFree() then return UIS:GetMouseLocation() end
    return screenCenter()
end
...
local function toScreen(pos)
    ...
    local v,on=cam:WorldToScreenPoint(pos)
...
local function partScreenBox(part)
    ...
    if minX==math.huge then return center,5,7,true end
    return center,math.clamp((maxX-minX)*.5,3,55),math.clamp((maxY-minY)*.5,4,65),true
end
...
local function desiredPoint(part,mousePos)
    ...
    local desired=Vector2.new(center.X+rx*state.anchorX,center.Y+ry*state.anchorY)
    -- Hard guarantee: the target point itself is always well inside the part.
    if not insideEllipse(desired,center,rx,ry,.72,0) then desired=center end
...
local function worldPointFromCursor(part,cursor,center,rx,ry)
    ...
    local nx=math.clamp((cursor.X-center.X)/math.max(rx,1),-.42,.42)
    local ny=math.clamp((cursor.Y-center.Y)/math.max(ry,1),-.42,.42)
...
local function alignedShotPoint(pl,part,point)
    ...
    -- No more impossible behind-you / off-target shots: Mouse Aim correction only
    -- exists while the visible cursor (or first-person center) is actually on body.
    if not insideEllipse(aimPos,center,rx,ry,1.02,3) then return nil end
...
local lastToggle=0
local actionName="KimqMouseAimToggle_v298"
...
RunService.RenderStepped:Connect(function()
    ...
    local insideBody=insideEllipse(mp,center,rx,ry,.94,1)
    ...
    -- Once the real cursor is genuinely inside the target part, stop micro-chasing
    -- the exact center. This keeps it ON the player without looking pixel-glued.
    if insideCore and dist<math.max(3,math.min(rx,ry)*.30) then
...
task.spawn(function()
    ...
    pcall(function()
        game:GetService("StarterGui"):SetCore("SendNotification",{
            Title="Kimqetras HC",
            Text="v2.98 ACTIVE ♡ • Mouse Aim 2.0 + actual pointer ready",
            Duration=7,
        })
    end)
