-- KIMQETRAS HC v2.1 - AUTHORITATIVE THEME + REAL SKIN ANIMATION FIX
...
task.spawn(function()
    ...
    local function hasAnimatedVisuals(obj)
        ...
        for _,d in ipairs(obj:GetDescendants()) do
            if d:IsA("ParticleEmitter") or d:IsA("Trail") or d:IsA("Beam") or d:IsA("AnimationController") or d:IsA("Animator") or d:IsA("Animation") or d:IsA("Constraint") or d:IsA("BodyMover") then return true end
        end
        return false
    end
    ...
    local function animationLooksCosmetic(name)
        ...
        return name:find("idle",1,true) or name:find("loop",1,true) or name:find("spin",1,true) or name:find("float",1,true) or name:find("hover",1,true) or name:find("aura",1,true) or name:find("effect",1,true) or name:find("animated",1,true) or name:find("skin",1,true)
    end
    ...
    local function playEmbeddedAnimations(cloneObj)
        ...
        for _,anim in ipairs(toPlay) do
            ...
            if ok and track then
                local lname=anim.Name:lower()
                if lname:find("idle",1,true) or lname:find("loop",1,true) or lname:find("spin",1,true) or lname:find("float",1,true) or lname:find("hover",1,true) or lname:find("aura",1,true) or lname:find("effect",1,true) then
    ...
    local function applySkin(w,s,tool,quiet)
        ...
        for _,d in ipairs(visual:GetDescendants()) do
            ...
            elseif d:IsA("ParticleEmitter") or d:IsA("Trail") or d:IsA("Beam") or d:IsA("Fire") or d:IsA("Smoke") or d:IsA("Sparkles") then
                -- Wrap storage often keeps VFX disabled until the skin is equipped.
                -- The local visual clone is the equipped copy, so let its VFX run.
        ...
        if not quiet then
            ...
            setStatus(displayWeapon(w).." • "..s..(animated and (animCount>0 and (" • "..animCount.." animation"..(animCount==1 and "" or "s").." running") or " • physics/effects preserved") or " • applied locally"),true)
        end
        return true
    end
...
task.spawn(function()
    ...
    -- Only the nav highlight is refreshed after navigation; this does not repaint the GUI.
    if nav then
        for _,b in ipairs(nav:GetChildren()) do
            if b:IsA("TextButton") and pageNames[clean(b.Text)] and not b:GetAttribute("KimqThemeNavHook") then
                b:SetAttribute("KimqThemeNavHook",true)
                b.MouseButton1Click:Connect(function()
                    task.defer(function() styleNav(THEMES[currentName]) end)
                end)
            end
        end
    end
