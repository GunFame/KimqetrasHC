...
task.spawn(function()
        ...
        for _,entry in ipairs(roleObjects) do
            ...
            if d and d.Parent then
                if d:IsA("Frame") or d:IsA("TextButton") or d:IsA("TextBox") then
                    if r=="cream" then d.BackgroundColor3=p.bg elseif r=="cream2" then d.BackgroundColor3=p.bg2 elseif r=="panel" then d.BackgroundColor3=p.panel elseif r=="lightBg" then d.BackgroundColor3=p.soft elseif r=="hotBg" then d.BackgroundColor3=p.hot elseif r=="whiteBg" then d.BackgroundColor3=p.white end
                end
                if d:IsA("TextLabel") or d:IsA("TextButton") or d:IsA("TextBox") then
                    if r=="hotText" then d.TextColor3=p.hot elseif r=="subText" then d.TextColor3=p.sub elseif r=="textText" or r=="limeText" then d.TextColor3=p.text elseif r=="lineText" then d.TextColor3=p.line elseif r=="whiteText" then d.TextColor3=p.white end
                end
                local st=d:FindFirstChildOfClass("UIStroke"); if st and r~="hotBg" then st.Color=p.line end
            end
        end

        -- Standard rows/cards + core toggles/sliders that predate role tagging.
        for _,d in ipairs(main:GetDescendants()) do
            if not isColorSurface(d) then
                if d:IsA("Frame") and d.BackgroundTransparency<.95 then
                    local sz=d.AbsoluteSize
                    if sz.X>240 and sz.Y>=36 and sz.Y<=190 and not d:GetAttribute("KimqPreserveColor") then
                        if d:GetAttribute("KimqV26Role")~="hotBg" then d.BackgroundColor3=p.panel; local st=d:FindFirstChildOfClass("UIStroke"); if st then st.Color=p.line end end
                    end
                elseif d:IsA("TextButton") then
                    styleToggle(d,p)
                elseif d:IsA("TextBox") then
                    d.BackgroundColor3=p.soft; d.TextColor3=p.text; d.PlaceholderColor3=p.sub; local st=d:FindFirstChildOfClass("UIStroke"); if st then st.Color=p.line end
                end
            end
        end
    ...
    local function buildThemePage()
        ...
        local order={"Matcha Pink","Purple","Red","Pink","Aqua","Green"}
