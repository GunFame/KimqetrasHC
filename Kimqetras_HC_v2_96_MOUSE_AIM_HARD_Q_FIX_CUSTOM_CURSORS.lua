-- Kimqetras HC v2.96 • v2.95 base + hard Q Mouse Aim binding + custom cursors
...
local KIMQ_BUILD = "v2.96-mouse-aim-hard-q-custom-cursors"
...
do
    ...
    if running and runningBuild == KIMQ_BUILD then
        if runningGui then
            ...
            notifyBoot((runtimeState=="ready") and "v2.96 is already running ♡" or "v2.96 is already starting ♡",4)
            return
        end
        ...
        notifyBoot("Recovering a failed v2.96 boot ♡",4)
    end
    ...
    if oldRuntimeDetected then
        ...
        if staleBoot then
            ...
            notifyBoot("STOP: another Kimqetras HC build is already active. v2.96 DID NOT LOAD. Rejoin once, then run only v2.96.",9)
            warn("[Kimqetras HC v2.96] Another build is active. v2.96 DID NOT LOAD; rejoin once before testing.")
            return
        end
    end
...
Hello.Text = "v2.96  •  hard Q lock + custom cursors ♡"
...
-- v2.94 Mouse Aim: body-true soft assist.
-- Free camera is preserved. The cursor is kept inside a forgiving region of the
-- selected body part instead of being pinned to one exact pixel.

if cfg.mouseAimNaturalProfileVersion~="v2.94" then
    cfg.mouseAimPart="UpperTorso"
    cfg.mouseAimFOV=180
    cfg.mouseAimFollowStrength=48
    cfg.mouseAimSoftness=8
    cfg.mouseAimNaturalProfileVersion="v2.94"
end

-- v2.95: Mouse Aim should be usable immediately on a fresh execute.
-- Older builds initialized this to false, so the Q handler could silently return
-- before acquisition even ran. Preserve an explicit saved boolean, otherwise ON.
if cfg.mouseAimEnabled == nil then
    cfg.mouseAimEnabled = true
else
    cfg.mouseAimEnabled = cfg.mouseAimEnabled == true
end

-- Normalize config-loaded keybinds. Configs store keybinds as strings, while the
-- runtime input event gives an Enum.KeyCode. Without normalization, Q can never
-- compare equal after a saved config has supplied "Q".
local function normalizeMouseAimKey(v)
    if typeof(v)=="EnumItem" and v.EnumType==Enum.KeyCode then return v end
...
cfg.mouseAimKey = normalizeMouseAimKey(cfg.mouseAimKey)
cfg.mouseAimPart = tostring(cfg.mouseAimPart or "UpperTorso")
cfg.mouseAimFOV = tonumber(cfg.mouseAimFOV) or 180
cfg.mouseAimFollowStrength = tonumber(cfg.mouseAimFollowStrength) or 48
cfg.mouseAimSoftness = tonumber(cfg.mouseAimSoftness) or 8
...
local fallbackInset=Vector2.zero
pcall(function()
    local topLeft=GuiService:GetGuiInset()
    if typeof(topLeft)=="Vector2" then fallbackInset=topLeft end
end)

local state={
    active=false,target=nil,part=nil,point=nil,
    worldAimPoint=nil,worldAimPart=nil,
    lastClock=os.clock(),lastCenter=nil,targetVelocity=Vector2.zero,
    lastMouse=nil,lastApplied=Vector2.zero,
}
...
local function cursorViewportPos()
    if mouseObj then
        local mx,my=tonumber(mouseObj.X),tonumber(mouseObj.Y)
        if mx and my then return Vector2.new(mx,my) end
    end
    local u=UIS:GetMouseLocation()
    return Vector2.new(u.X-fallbackInset.X,u.Y-fallbackInset.Y)
end
...
local function preferredPart(pl)
    ...
    if wanted~="Closest Point" then
        ...
        if not p and (wanted=="UpperTorso" or wanted=="LowerTorso") then p=c:FindFirstChild("Torso") end
        if p and p:IsA("BasePart") then return p,p.Position end
    end
    local mousePos=cursorViewportPos()
...
local function partScreenBox(part)
    ...
    if not part or not center or not on then return center,5,7,false end
    local minX,maxX,minY,maxY=math.huge,-math.huge,math.huge,-math.huge
    local sx,sy,sz=part.Size.X*.49,part.Size.Y*.49,part.Size.Z*.49
    ...
    local rx=math.clamp((maxX-minX)*.5,3,44)
    local ry=math.clamp((maxY-minY)*.5,4,52)
    return center,rx,ry,true
end

local function candidateDistance(pl,limit)
    ...
    local d=(sp-cursorViewportPos()).Magnitude
    if limit and d>limit then return math.huge,nil,nil end
    return d,part,point
end
local function playerFromMouseTarget()
    local inst=mouseObj and mouseObj.Target
    if not inst then return nil end
    for _,pl in ipairs(Players:GetPlayers()) do
        local c=pl.Character
        if c and inst:IsDescendantOf(c) and valid(pl) then return pl end
    end
    return nil
end
local function resetMotion()
    state.lastClock=os.clock();state.lastCenter=nil;state.targetVelocity=Vector2.zero
    state.lastMouse=nil;state.lastApplied=Vector2.zero
    state.worldAimPoint=nil;state.worldAimPart=nil
end
local function acquire()
    -- First choice: if the physical cursor is actually over a player, lock that
    -- exact player immediately. This makes Q feel deterministic instead of FOV-only.
    local direct=playerFromMouseTarget()
    if direct then
        local p,pt=preferredPart(direct)
        local sp,on=pt and toScreen(pt) or nil,false
        if p and pt and sp and on then
            state.active=true;state.target=direct;state.part=p;state.point=pt
            resetMotion();_G.KimqMouseAimTarget=direct
            return true
        end
    end

    local best,bestD,bestPart,bestPoint=nil,math.huge,nil,nil
    local fov=math.max(20,tonumber(cfg.mouseAimFOV) or 180)
    for _,pl in ipairs(Players:GetPlayers()) do
        local d,p,pt=candidateDistance(pl,fov)
        if d<bestD then best,bestD,bestPart,bestPoint=pl,d,p,pt end
    end

    -- If nobody is inside the configured FOV, still let Q work on the nearest
    -- visible player in a generous screen radius. The FOV remains the preference,
    -- but pressing Q never feels silently dead just because the cursor is 10 px out.
    if not best then
        local fallback=math.max(500,math.min(900,fov*2.75))
        for _,pl in ipairs(Players:GetPlayers()) do
            local d,p,pt=candidateDistance(pl,fallback)
            if d<bestD then best,bestD,bestPart,bestPoint=pl,d,p,pt end
        end
    end
    ...
    if best then
        ...
        resetMotion();_G.KimqMouseAimTarget=best
        return true
    end
    state.active=false;state.target=nil;state.part=nil;state.point=nil
    resetMotion();_G.KimqMouseAimTarget=nil
    return false
end
...
local function cursorInsidePart(part,marginScale,extraPx)
    local center,rx,ry,on=partScreenBox(part)
    if not center or not on then return false,nil,rx,ry end
    local mp=cursorViewportPos()
    local sx=rx*(marginScale or .92)+(extraPx or 0)
    local sy=ry*(marginScale or .92)+(extraPx or 0)
    local dx=(mp.X-center.X)/math.max(sx,1)
    local dy=(mp.Y-center.Y)/math.max(sy,1)
    return (dx*dx+dy*dy)<=1.0,center,rx,ry
end

local function worldPointFromCursor(part,center,rx,ry)
    if not part or not center then return nil end
    local mp=cursorViewportPos()
    local nx=math.clamp((mp.X-center.X)/math.max(rx,1),-.46,.46)
    local ny=math.clamp((mp.Y-center.Y)/math.max(ry,1),-.46,.46)
    return part.Position
        + part.CFrame.RightVector*(part.Size.X*nx)
        - part.CFrame.UpVector*(part.Size.Y*ny)
end

local function alignedShotPoint(pl,part,point)
    if not part or not point or not valid(pl) or wallBlocked(pl,point) then return nil end
    local center,rx,ry,on=partScreenBox(part)
    if not center or not on then return nil end
    local inside=cursorInsidePart(part,.98,3)
    if not inside then return nil end
    return worldPointFromCursor(part,center,rx,ry) or point
end

-- While Q-lock is active, Mouse Aim owns the Silent Aim decision. If the visible
-- cursor is not actually on the selected body part, there is no hidden fallback
-- shot to a different player or to somebody off-screen.
...
_G.KimqMouseAimOverride=function()
    ...
    local hitPoint=alignedShotPoint(state.target,part,point)
    if not hitPoint then return nil,nil end
    state.part,state.point=part,point
    state.worldAimPart,state.worldAimPoint=part,hitPoint
    return part,hitPoint
end
_G.KimqMouseAimForceCorrection=function()
    if not cfg.mouseAimEnabled or not cfg.mouseAimSilentCorrection or not state.active or not valid(state.target) then return false end
    local part,point=preferredPart(state.target)
    return alignedShotPoint(state.target,part,point)~=nil
end

local lastToggleAt=0
local actionName="KimqMouseAimToggle_v296"
local function mouseAimNotify(text)
    pcall(function()
        StarterGui:SetCore("SendNotification",{Title="Mouse Aim ♡",Text=text,Duration=2})
    end)
end
local function requestMouseAimToggle()
    if UIS:GetFocusedTextBox() then return end
    local now=os.clock()
    if now-lastToggleAt<.16 then return end
    lastToggleAt=now

    -- Q itself is the user's activation intent. If a saved config left the UI
    -- toggle OFF, pressing the bound key turns Mouse Aim back on instead of doing nothing.
    if not cfg.mouseAimEnabled then cfg.mouseAimEnabled=true end
    if state.active then
        unlock();mouseAimNotify("unlocked")
    else
        if acquire() then
            local pl=state.target
            mouseAimNotify("locked → "..tostring((pl and (pl.DisplayName or pl.Name)) or "target"))
        else
            mouseAimNotify("no visible target near your cursor")
        end
    end
end
local function bindMouseAimAction()
    pcall(function() CAS:UnbindAction(actionName) end)
    local key=normalizeMouseAimKey(cfg.mouseAimKey)
    cfg.mouseAimKey=key
    pcall(function()
        CAS:BindActionAtPriority(actionName,function(_,inputState)
            if inputState==Enum.UserInputState.Begin then requestMouseAimToggle() end
...
-- UIS is kept as a fallback for executors/games where ContextActionService does
-- not receive the key. The debounce prevents the two paths from double-toggling.
UIS.InputBegan:Connect(function(input)
    if UIS:GetFocusedTextBox() then return end
    if input.UserInputType==Enum.UserInputType.Keyboard and input.KeyCode==normalizeMouseAimKey(cfg.mouseAimKey) then
        requestMouseAimToggle()
    end
end)
...
RunService.RenderStepped:Connect(function()
    ...
    local center,rx,ry,on=partScreenBox(part)
    if not center or not on or wallBlocked(state.target,point) then
        state.worldAimPoint=nil;state.worldAimPart=nil
        state.lastCenter=nil;state.targetVelocity=Vector2.zero
        return
    end

    -- In first person Roblox owns the centered mouse. We never inject cursor
    -- movement there; the same on-body gate is used for shot correction instead.
    if not cfg.mouseAimVisibleFollow or UIS.MouseBehavior==Enum.MouseBehavior.LockCenter then
        local hitPoint=alignedShotPoint(state.target,part,point)
        state.worldAimPart=hitPoint and part or nil
        state.worldAimPoint=hitPoint
        return
    end
    ...
    state.lastClock=now
    if state.lastCenter then
        local rawVel=(center-state.lastCenter)/math.max(dt,1/240)
        local a=1-math.exp(-dt*10)
        state.targetVelocity=state.targetVelocity:Lerp(rawVel,a)
    end
    state.lastCenter=center

    local mp=cursorViewportPos()
    local softness=math.clamp(tonumber(cfg.mouseAimSoftness) or 8,0,18)/18
    local strength=math.clamp(tonumber(cfg.mouseAimFollowStrength) or 48,1,100)/100

    -- The inner region is deliberately larger than a pixel. Once the cursor is
    -- actually on the selected part, assistance mostly gets out of the way.
    local innerX=rx*(.34+.18*softness)
    local innerY=ry*(.32+.18*softness)
    local edgeX=math.clamp(mp.X,center.X-innerX,center.X+innerX)
    local edgeY=math.clamp(mp.Y,center.Y-innerY,center.Y+innerY)
    local desired=Vector2.new(edgeX,edgeY)
    local delta=desired-mp
    local dist=delta.Magnitude

    -- If already inside the comfortable body region, do not pin the cursor to a
    -- center point. This is what removes the obvious robotic one-pixel lock.
    if dist<.35 then
    ...
    local distanceFactor=math.clamp(dist/115,0,1)
    local motionFactor=math.clamp(targetSpeed/900,0,1)
    local response=7.5+(strength*8.5)+(distanceFactor*10)+(motionFactor*4.5)-(softness*1.8)
    response=math.max(5.5,response)
    local alpha=1-math.exp(-dt*response)
    local step=delta*alpha

    -- Far corrections catch up quickly, close corrections ease off. No overshoot,
    -- no circular noise, and no artificial shot-time snap.
    local maxSpeed=620+(strength*1450)+(distanceFactor*1100)+(motionFactor*700)
    step=clampVec(step,maxSpeed*dt)
    if dist<18 then step=step*(.62+.38*math.clamp(dist/18,0,1)) end
    if step.Magnitude>dist and dist>0 then step=delta end

    if math.abs(step.X)<.02 then step=Vector2.new(0,step.Y) end
    if math.abs(step.Y)<.02 then step=Vector2.new(step.X,0) end
    state.lastApplied=Vector2.zero
    if step.X~=0 or step.Y~=0 then
        ...
        elseif VIM then
            local ok=pcall(function() local abs=UIS:GetMouseLocation(); VIM:SendMouseMoveEvent(abs.X+step.X,abs.Y+step.Y,game) end)
            if ok then state.lastApplied=step end
        end
    end
...
addKeybind("Mouse Aim Key",cfg.mouseAimKey,function(v)
    cfg.mouseAimKey=normalizeMouseAimKey(v)
    bindMouseAimAction()
end)
...
do
    ...
    RunService.RenderStepped:Connect(function()
        ...
        -- Other recovery code can restore the stock cursor; re-assert only the
        -- visual cursor state here without touching MouseBehavior/camera input.
...
task.spawn(function()
    ...
    pcall(function()
        game:GetService("StarterGui"):SetCore("SendNotification",{
            Title="Kimqetras HC",
            Text="v2.96 ACTIVE ♡ • hard Q Mouse Aim + custom cursors ready",
            Duration=7,
        })
    end)
