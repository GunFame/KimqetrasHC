-- Kimqetras HC v2.103 • v2.102 base + Environment shadow controls
...
local KIMQ_BUILD = "v2.103-environment-shadows"
...
Hello.Text = "v2.103  •  Environment shadows ♡"
...
-- v2.100: Mouse Aim was removed. Clear its public bridges for clean fresh runtimes.
_G.KimqMouseAimBackend=nil
_G.KimqMouseAimState=nil
_G.KimqMouseAimTarget=nil
_G.KimqMouseAimOverride=nil
_G.KimqMouseAimSuppressFallback=nil
_G.KimqMouseAimForceCorrection=nil
_G.KimqMouseAimCursorOverride=nil
...
task.spawn(function()
    ...
    -- Shadow controls are layered on top of whichever Environment preset is active.
    -- Keeping them separate prevents Christmas/Halloween/Normal from fighting the slider.
    local shadowEnabled=false
    local shadowDarkness=55
    local shadowSoftness=35
    local function presetAmbientBase()
        if activePreset=="Christmas" then
            return original.Ambient:Lerp(Color3.fromRGB(222,234,247),.18), original.OutdoorAmbient:Lerp(Color3.fromRGB(235,244,252),.22)
        elseif activePreset=="Halloween" then
            return original.Ambient:Lerp(Color3.fromRGB(190,135,132),.12), original.OutdoorAmbient:Lerp(Color3.fromRGB(218,158,121),.13)
        end
        return original.Ambient, original.OutdoorAmbient
    end
    local function applyShadowLook()
        local baseAmbient,baseOutdoor=presetAmbientBase()
        if shadowEnabled then
            local d=math.clamp(shadowDarkness/100,0,1)
            Lighting.GlobalShadows=true
            Lighting.ShadowSoftness=math.clamp(shadowSoftness/100,0,1)
            -- Ambient controls how much fill light reaches dark faces. Lowering it
            -- deepens actual shaded areas without globally lowering Brightness.
            Lighting.Ambient=baseAmbient:Lerp(Color3.new(0,0,0),d*.88)
            Lighting.OutdoorAmbient=baseOutdoor:Lerp(Color3.new(0,0,0),d*.76)
        else
    ...
    local function restoreEnvBase()
        ...
        Lighting.GlobalShadows=original.GlobalShadows; Lighting.ShadowSoftness=original.ShadowSoftness
    ...
    do
        ...
        local shadowTitle=txt(shadowCard,"♥  Shadows",UDim2.new(1,-24,0,24),UDim2.fromOffset(12,8),Enum.Font.GothamBold,14,textColor)
        shadowTitle:SetAttribute("KimqV26Role","hotText")

        local shadowToggleLabel=txt(shadowCard,"Enhanced Shadows",UDim2.fromOffset(180,24),UDim2.fromOffset(12,38),Enum.Font.GothamSemibold,11,textColor)
        ...
        local refreshDark=makeShadowSlider(70,"Shadow Darkness",0,100,function() return shadowDarkness end,function(v) shadowDarkness=math.clamp(v,0,100); applyShadowLook() end)
        local refreshSoft=makeShadowSlider(116,"Shadow Softness",0,100,function() return shadowSoftness end,function(v) shadowSoftness=math.clamp(v,0,100); applyShadowLook() end)
        ...
        local function setShadowDarkness(v) shadowDarkness=math.clamp(tonumber(v) or 55,0,100); applyShadowLook(); refreshShadowUI() end
        local function setShadowSoftness(v) shadowSoftness=math.clamp(tonumber(v) or 35,0,100); applyShadowLook(); refreshShadowUI() end

        shadowToggle.MouseButton1Click:Connect(function() setShadowEnabled(not shadowEnabled) end)
        resetShadow.MouseButton1Click:Connect(function() shadowEnabled=false; shadowDarkness=55; shadowSoftness=35; applyShadowLook(); refreshShadowUI() end)
        refreshShadowUI()
        ...
        if type(_G.KimqRegisterConfigControl)=="function" then
            _G.KimqRegisterConfigControl("Enhanced Shadows","toggle",function() return shadowEnabled end,function(v) setShadowEnabled(v) end)
            _G.KimqRegisterConfigControl("Shadow Darkness","slider",function() return shadowDarkness end,function(v) setShadowDarkness(v) end)
            _G.KimqRegisterConfigControl("Shadow Softness","slider",function() return shadowSoftness end,function(v) setShadowSoftness(v) end)
        end
    end
...
task.spawn(function()
    ...
    pcall(function()
        game:GetService("StarterGui"):SetCore("SendNotification",{
            Title="Kimqetras HC",
            Text="v2.103 ACTIVE ♡ • Environment shadows ready",
            Duration=7,
        })
    end)
