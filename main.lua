-- Kimqetras HC v2.1 • Humanoid-rig animation fix
...
task.spawn(function()
    ...
    local function getOrCreateAnimator(root)
        if not root then return nil,nil,"none" end

        -- First prefer the exact driver name used by several Wraps skins.
        local humanoid=root:FindFirstChild("ANIMATE_HUMANOID",true)
        if not (humanoid and humanoid:IsA("Humanoid")) then
            humanoid=root:FindFirstChildWhichIsA("Humanoid",true)
        end
        if humanoid then
            -- Cosmetic mini-rigs frequently have no Neck, so do not let the
            -- Humanoid invalidate/break the cloned rig.
            pcall(function() humanoid.RequiresNeck=false end)
            pcall(function() humanoid.BreakJointsOnDeath=false end)
            pcall(function()
                if humanoid.MaxHealth<=0 then humanoid.MaxHealth=100 end
        ...
        if animator then return animator,nil,"existing animator" end
        local controller=root:FindFirstChildWhichIsA("AnimationController",true)
    ...
    local function animationLooksCosmetic(name)
        name=tostring(name or ""):lower()
        if name:find("reload",1,true) or name:find("shoot",1,true) or name:find("fire",1,true) or name:find("equip",1,true) or name:find("unequip",1,true) then
            return false
        end
        -- Skin creators often leave cosmetic Animation objects named simply
        -- "Animation" or with an asset-like name. Treat every non-combat animation
        -- as cosmetic instead of requiring idle/loop/spin in the name.
        return true
    end

    local function playEmbeddedAnimations(cloneObj)
        if not cloneObj then return 0,"none" end
        local animations={}
        for _,d in ipairs(cloneObj:GetDescendants()) do
            if d:IsA("Animation") and d.AnimationId~="" then table.insert(animations,d) end
        end
        if #animations==0 then return 0,"none" end

        local animator,humanoid,driverMode=getOrCreateAnimator(cloneObj)
        if not animator then return 0,"none" end

        local preferred={}
        for _,anim in ipairs(animations) do
            if animationLooksCosmetic(anim.Name) then table.insert(preferred,anim) end
        end
        local toPlay=(#preferred>0 and preferred) or (#animations==1 and animations or {})
        local count=0
        for _,anim in ipairs(toPlay) do
            local track=nil

            -- Mimic skins such as Ascension as closely as possible: their
            -- ANIMATE script normally calls Humanoid:LoadAnimation on the
            -- ANIMATE_HUMANOID. Use that path first, then Animator as fallback.
            if humanoid then
                local okHum,result=pcall(function() return humanoid:LoadAnimation(anim) end)
                if okHum then track=result end
            end
            if not track then
                local okAnim,result=pcall(function() return animator:LoadAnimation(anim) end)
                if okAnim then track=result end
            end
    ...
    local function applySkin(w,s,tool,quiet)
        ...
        local embedded,embeddedDriver=playEmbeddedAnimations(visual)
        ...
        startSourceMirror(gun,sourceContainer,sourceRoot,visual,cloneRoot,sourcePairs,mirrorParts)
        ...
        if not quiet then
            ...
            if animCount>0 and embeddedDriver=="humanoid" then detail=" • "..animCount.." rig animation"..(animCount==1 and "" or "s").." running"
            elseif clientScripts>0 then detail=" • client animation script"..(clientScripts==1 and "" or "s").." kept"
            elseif animCount>0 then detail=" • "..animCount.." animation"..(animCount==1 and "" or "s").." running"
