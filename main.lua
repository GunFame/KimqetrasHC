-- Kimqetras HC v2.20 • target jump zoom + softer local bullet glow + Kimqetras owner branding + famesgun developer credit + gun-only free camera swap
-- Re-running this SAME build only reopens the current GUI instead of stacking another copy.
-- If an older build is already active, v2.20 refuses to stack and asks for one clean rejoin.
do
    local SINGLE_KEY = "KimqHC_v21_SingleRuntimeOptimized"
    local BUILD = "v2.20-target-jump-zoom"
    ...
    -- Older builds did not expose a complete disconnect/cleanup handle. Starting v2.20
    ...
    if rawget(_G, SINGLE_KEY) and rawget(_G, "KimqHC_CurrentBuild") ~= BUILD then
        pcall(function()
            game:GetService("StarterGui"):SetCore("SendNotification", {
                Title = "Kimqetras HC",
                Text = "Older runtime detected. Rejoin once, then run v2.20 so only one version is active.",
                Duration = 7,
            })
        end)
        warn("[Kimqetras HC v2.20] Older runtime detected. Rejoin once before running v2.20; refusing to stack versions.")
        return
    end
...
-- v2.20 keeps the v2.15 local bullet fix, tunable camera swap, owner/credit cleanup, and softens only bullet/tracer glow.
...
-- Target Jump Zoom ---------------------------------------------------------
-- Picks the player nearest the crosshair using the same general FOV/distance
-- idea as Silent Aim, but it works independently of the Silent Aim toggle.
-- It only watches while Shot Camera Swap is enabled AND your gun is equipped.
local jumpZoomTrackedUserId = nil
...
local function getJumpZoomTarget()
    local mousePos = UIS:GetMouseLocation()
    local radius = math.max(40, tonumber(cfg.silentAimFOV) or 120)
    local maxWorld = math.max(50, tonumber(cfg.silentAimMaxDist) or 5000)
    local bestPlayer, bestScreen = nil, radius

    for _,plr in ipairs(Players:GetPlayers()) do
        if plr ~= lp and not (_G.KHWhitelist and _G.KHWhitelist[plr.UserId]) and isAlive(plr) then
            if cfg.silentAimTeamCheck and sameTeam(plr) then continue end
            local char = plr.Character
            local hrp = char and char:FindFirstChild("HumanoidRootPart")
            local hum = char and char:FindFirstChildOfClass("Humanoid")
            if hrp and hum and hum.Health > 0 then
                local worldDist = (hrp.Position - cam.CFrame.Position).Magnitude
                if worldDist <= maxWorld then
                    local sp,onScreen = safeWorldToViewportPoint(hrp.Position)
                    if onScreen then
                        local screenDist = (Vector2.new(sp.X,sp.Y)-mousePos).Magnitude
                        if screenDist < bestScreen then
                            -- Respect the user's wall-check preference without forcing it on.
                            local blocked = cfg.silentAimWallCheck and wallBetween(hrp.Position)
                            if not blocked then
...
local function humanoidIsAirborne(hum)
...
local function triggerJumpZoom()
    ...
    local now = os.clock()
    if now - jumpZoomLastTrigger < 0.18 then return end
    ...
    local d = cameraDistance()
    if d > 1.1 then
        lastThirdDistance = math.clamp(d,zmin,zmax)
    end

    -- Use the existing camera tween so the new feature cannot create a second
    -- competing camera loop. It simply asks the current system to zoom inward.
    cameraSwapToken += 1
    local token = cameraSwapToken
...
RunService.RenderStepped:Connect(function(dt)
    ...
    if jumpZoomAccumulator < 0.05 then return end -- ~20 Hz; very light
    ...
    local airborne = humanoidIsAirborne(hum)

    if jumpZoomTrackedUserId ~= target.UserId then
        jumpZoomTrackedUserId = target.UserId
        jumpZoomWasAirborne = airborne
        return
    end

    -- Trigger once on the grounded -> airborne edge, not continuously in midair.
    ...
    do
        ...
        if spawnPage then
            ...
            local function moveCharacter(char)
                ...
                local dest = cf * CFrame.new(0, 3.25, 0)
            ...
            lp.CharacterAdded:Connect(function(char)
                ...
                task.delay(.16,function() if char and char.Parent then moveCharacter(char) end end)
                task.delay(.48,function() if char and char.Parent then moveCharacter(char) end end)
            end)
