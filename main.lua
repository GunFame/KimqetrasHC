-- Kimqetras HC v2.7 • camera swap hotkey • low-lag local bullets • single-runtime guard
-- Re-running this SAME build only reopens the current GUI instead of stacking another copy.
-- If an older build is already active, v2.7 refuses to stack and asks for one clean rejoin.
do
    local SINGLE_KEY = "KimqHC_v21_SingleRuntimeOptimized"
    local BUILD = "v2.7-camera-bullet-performance-single-runtime"
    ...
    -- Older builds did not expose a complete disconnect/cleanup handle. Starting v2.7
    ...
    if rawget(_G, SINGLE_KEY) and rawget(_G, "KimqHC_CurrentBuild") ~= BUILD then
        pcall(function()
            game:GetService("StarterGui"):SetCore("SendNotification", {
                Title = "Kimqetras HC",
                Text = "Older runtime detected. Rejoin once, then run v2.7 so only one version is active.",
                Duration = 7,
            })
        end)
        warn("[Kimqetras HC v2.7] Older runtime detected. Rejoin once before running v2.7; refusing to stack versions.")
        return
    end
...
-- v2.7 keeps v2.6 config/fog persistence and adds camera/bullet performance fixes.
...
-- Third person -> snap fully into first person -> return to the exact prior zoom.
-- First person -> snap out -> return to first person.
-- Shift+4 toggles the same setting without opening the GUI.
...
local function inFirstPerson()
    ...
    return cameraDistance() <= 1.05
end

local function forceZoomDistance(distance)
    distance = math.clamp(tonumber(distance) or 0.5, 0.5, 128)
...
local function doShotCameraSwap(tool)
    ...
    local oldDistance = math.clamp(cameraDistance(), 0.5, 128)

    -- Do the actual visible swap by changing zoom distance, rather than leaving
    -- CameraMode locked. This gives the quick OUT -> FIRST PERSON -> OUT motion.
    pcall(function()
        if wasFirst then
            forceZoomDistance(math.clamp(math.max(oldDistance, 9), 9, 13))
        else
            forceZoomDistance(0.5)
        end
    end)
...
-- Shift+4 toggles Shot Camera Swap and also updates the visible toggle/config value.
UIS.InputBegan:Connect(function(input, gpe)
...
task.spawn(function()
    ...
    local function setupWeaponExtras()
        ...
        -- v2.7 low-lag local path:
        --   * only styles rays verified as this player's shot
        --   * styles each effect once and watches only that effect for later resets
        --   * no per-frame GetDescendants scans
        --   * rainbow updates are capped to ~30 FPS and only touch live local effects
        local rainbowObjects=setmetatable({}, {__mode="k"})
        local colorLocks=setmetatable({}, {__mode="k"})
        local shotDescConns=setmetatable({}, {__mode="k"})
        ...
        local function lockBulletColor(d)
            if not d or colorLocks[d] then return end
            local signal=nil
            if d:IsA("Beam") or d:IsA("Trail") or d:IsA("ParticleEmitter") or d:IsA("PointLight") or d:IsA("SpotLight") or d:IsA("SurfaceLight") or d:IsA("BasePart") then
                local ok,res=pcall(function() return d:GetPropertyChangedSignal("Color") end)
                if ok then signal=res end
            elseif d:IsA("Color3Value") then
                local ok,res=pcall(function() return d:GetPropertyChangedSignal("Value") end)
                if ok then signal=res end
            end
            if not signal then return end
            local busy=false
            colorLocks[d]=signal:Connect(function()
                if busy or not d.Parent then return end
        ...
        local function applyBulletModeTo(d)
            ...
            if c then
                applyColorOnly(d,c)
                lockBulletColor(d)
                if extraState.BulletColorMode=="Rainbow" then rainbowObjects[d]=true end
            end
        end
        ...
        local function rayStartsAtOrigin(root,origin)
            ...
            if root:IsA("BasePart") and closePos(root.Position) then return true end
            -- Verification only needs a small sample; stop as soon as a start point is found.
            local checked=0
            for _,d in ipairs(root:GetDescendants()) do
                checked+=1
        ...
        local function styleVerifiedRoot(shotRoot)
            if not shotRoot or not shotRoot.Parent then return end
            applyBulletModeTo(shotRoot)
            for _,d in ipairs(shotRoot:GetDescendants()) do applyBulletModeTo(d) end

            -- New children of THIS verified local shot get styled immediately. This is
            -- much cheaper than rescanning the whole shot every RenderStepped.
            if not shotDescConns[shotRoot] then
                shotDescConns[shotRoot]=shotRoot.DescendantAdded:Connect(function(d)
                    applyBulletModeTo(d)
                    -- The game sometimes assigns its default color just after parenting.
                    -- Reapply on the next scheduler slice; property locks handle later resets.
                    task.defer(function()
                        if d and d.Parent then applyBulletModeTo(d) end
                    end)
                end)
            end
            pcall(function()
        ...
        local function styleLocalBulletRay(rootObj)
            ...
            local now=os.clock()
            if activeShotSerial==0 or (now-activeShotAt)>.22 then return end

            local shotRoot=bulletRayShotRoot(rootObj)
            if not shotRoot then return end
            if claimedShotRoots[shotRoot]==activeShotSerial then
                applyBulletModeTo(rootObj)
                return
            end
        ...
        local function hookShotTool(tool)
            if not tool or not tool:IsA("Tool") or tool:GetAttribute("KimqBulletBeamHook4") then return end
            tool:SetAttribute("KimqBulletBeamHook4",true)
        ...
        local function hookShotContainer(container)
            ...
            if container:GetAttribute("KimqBulletContainerHook4") then return end
            container:SetAttribute("KimqBulletContainerHook4",true)
        ...
        lp.CharacterAdded:Connect(function()
            activeShotSerial=0; activeShotOrigin=nil
            task.delay(.20,hookAllShotContainers)
        end)

        -- Keep one very small global listener only for the brief local-shot capture window.
        -- There is no per-frame descendant scan anymore.
        workspace.DescendantAdded:Connect(function(d)
            if activeShotSerial==0 or (os.clock()-activeShotAt)>.22 then return end
            local low=tostring(d.Name):lower()
            local shotRoot=bulletRayShotRoot(d)
            if not shotRoot and low~="bullet_rays" then return end
        ...
        RunService.Heartbeat:Connect(function(dt)
            ...
            for d,_ in pairs(rainbowObjects) do
                if d and d.Parent then
                    applyColorOnly(d,c)
                    touched+=1
                    if touched>=120 then break end
                else
                    rainbowObjects[d]=nil
                    local conn=colorLocks[d]
                    if conn then pcall(function() conn:Disconnect() end); colorLocks[d]=nil end
                end
            end
        end)
