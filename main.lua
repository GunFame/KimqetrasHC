-- Kimqetras HC v2.61 • restored v2.19 weapon-extras core + isolated wing flight
...
local KIMQ_BUILD = "v2.61-restored-v219-extras"
...
do
    ...
    if running and runningBuild == KIMQ_BUILD then
        ...
        pcall(function()
            StarterGui0:SetCore("SendNotification",{
                Title="Kimqetras HC",
                Text=(runtimeState=="ready") and "v2.61 is already running ♡" or "v2.61 is already starting ♡",
                Duration=4,
            })
        end)
        return
    ...
    if oldRuntimeDetected then
        pcall(function()
            StarterGui0:SetCore("SendNotification",{
                Title="Kimqetras HC",
                Text="An older Kimqetras HC runtime is still active. Rejoin once, then run v2.61. v2.61 will not stack on top of it.",
                Duration=9,
            })
        end)
        warn("[Kimqetras HC v2.61] Older runtime detected. Refusing to stack; rejoin once.")
        return
    end
...
LocalPlayer.CharacterAdded:Connect(function()
    -- v2.61: dying no longer clears the COMBAT Camlock target.
...
local function addCleanSection(title, subtitle)
    ...
    -- v2.61: pageHead already displays the section name.
...
task.spawn(function()
    ...
    -- v2.61: pages are grouped by what they actually do so the sidebar is
    ...
    task.defer(function()
        local rageOK,rageERR=pcall(function()
            ...
            local function button(parent,text,pos,size,fn)
                ...
                b.MouseButton1Click:Connect(function()
                    ...
                    if not ok then warn("[Kimqetras HC v2.61 RAGE button] "..tostring(err)) end
                end)
                return b
            end
            ...
            local function startOrbitMotion()
                ...
                orbitMotionConn=RunServiceR.Heartbeat:Connect(function(dt)
                    ...
                    if not ok then
                        warn("[Kimqetras HC v2.61 RAGE orbit] "..tostring(err))
                    end
                end)
            end
            ...
            local function finish(pl)
                ...
                task.spawn(function()
                    if autoStomp then
                        -- Give the E interaction a real window instead of one tiny
                        -- instant. This keeps us beside the knocked body and presses
                        -- E several times while the body is still stompable.
                        local presses=0
                        local wantedPresses=math.max(stompRepeats,5)
                    ...
                    -- Keep the target queued. If multiple targets exist, continue
                    -- through the queue and re-arm this player after their respawn.
            ...
            makeToggle(rageCombatPage,"Anti Lock / Evasive",false,function(v)
                antiLockEnabled=v
            end)
            makeSlider(
                rageCombatPage,
            ...
            local noteText=role(label(
                note,
                "Rage Combat Master runs by itself: orbit → auto-fire → detect K.O → move beside the body and press E → continue. Anti Lock adds unpredictable evasive orbit movement; server-side hits can still apply.",
            ...
            RunServiceR.RenderStepped:Connect(function(dt)
                ...
                if not ok then
                    warn("[Kimqetras HC v2.61 RAGE camera] "..tostring(err))
                end
            end)
            ...
            RunServiceR.Heartbeat:Connect(function(dt)
                ...
                if not ok then
                    warn("[Kimqetras HC v2.61 RAGE heartbeat] "..tostring(err))
                end
            end)
        ...
        if not rageOK then
            warn("[Kimqetras HC v2.61] RAGE module isolated error: "..tostring(rageERR))
        end
    end)
...
-- v2.61 module: local catalog / limited accessory try-on.
task.spawn(function()
    _G.KimqAccessoryUIReady=false
    local Players = game:GetService("Players")
    local CoreGui = game:GetService("CoreGui")
...
-- v2.61 module: Environment + Weapon Skins (canonical sidebar pages).
task.spawn(function()
    local pageReadyStart=tick()
    while not _G.KimqBasePagesReady and tick()-pageReadyStart<8 do task.wait(.02) end
    ...
    -- v2.61: Environment and Weapon Skins are already real canonical pages/buttons.
    ...
    if not envBuildOk then
        warn("[Kimqetras HC v2.61] Environment page fallback: "..tostring(envBuildErr))
    ...
    if not extrasOk then
        ...
        warn("[Kimqetras HC v2.61] restored v2.19 Weapon Extras: "..tostring(extrasErr))
    ...
    -- ========================================================
    -- v2.61 ISOLATED ANGEL WINGS FLIGHT
    ...
    skinsBtn.MouseButton1Click:Connect(function()
        task.defer(function()
            ...
            if extras and type(extras.Refresh)=="function" then
                ...
                if not ok then
                    warn("[Kimqetras HC v2.61] extras refresh: "..tostring(err))
...
task.spawn(function()
    ...
    -- v2.61: preserve the grouped sidebar order from pageDefs.
    ...
    local function applyTheme(name,settlePass)
        ...
        -- v2.61 hard guarantee: old controls created with literal pink/yellow
    ...
    -- v2.61: section navigation does not repaint/rescan the entire GUI.
    ...
    -- v2.61 reveal happens only after this initial theme pass is complete.
end)


-- v2.61: delayed post-show cleanup passes removed.
...
-- ========================================================
-- v2.61 branding is deterministic at construction time; no delayed scrub pass.

-- ========================================================
-- v2.61 DETERMINISTIC REVEAL
...
task.spawn(function()
    ...
    -- v2.61: remove the old one-page section divider labels from the
    ...
    -- ========================================================
    -- v2.61 FINAL SECTION OWNERSHIP GUARD
    --
