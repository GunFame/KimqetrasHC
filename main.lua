-- Kimqetras HC v2.10 • instant persistent local bullet color • slightly stronger macro I/O
-- Re-running this SAME build only reopens the current GUI instead of stacking another copy.
-- If an older build is already active, v2.10 refuses to stack and asks for one clean rejoin.
do
    local SINGLE_KEY = "KimqHC_v21_SingleRuntimeOptimized"
    local BUILD = "v2.10-instant-persistent-bullets-macro"
    ...
    -- Older builds did not expose a complete disconnect/cleanup handle. Starting v2.10
    ...
    if rawget(_G, SINGLE_KEY) and rawget(_G, "KimqHC_CurrentBuild") ~= BUILD then
        pcall(function()
            game:GetService("StarterGui"):SetCore("SendNotification", {
                Title = "Kimqetras HC",
                Text = "Older runtime detected. Rejoin once, then run v2.10 so only one version is active.",
                Duration = 7,
            })
        end)
        warn("[Kimqetras HC v2.10] Older runtime detected. Rejoin once before running v2.10; refusing to stack versions.")
        return
    end
...
-- v2.10 keeps the low-lag v2.9 path, but claims the first local shot immediately so its selected color is present before the first visible frame.
