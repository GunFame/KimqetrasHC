-- Kimqetras HC v2.45 • direct live-folder extras + early controller + retry
...
local KIMQ_BUILD = "v2.45-direct-live-extras"
...
do
    ...
    -- Same build already owns this session. Never start a second copy, even
    -- while the first copy is still loading.
    if running and runningBuild == KIMQ_BUILD then
        ...
        pcall(function()
            StarterGui0:SetCore("SendNotification",{
                Title="Kimqetras HC",
                Text=(runtimeState=="ready") and "v2.45 is already running ♡" or "v2.45 is already starting ♡",
                Duration=4,
            })
        end)
        return
    ...
    -- If an older Kimqetras runtime is alive, do NOT load over it. Destroying
    -- only the old GUI would leave its hidden input/heartbeat/respawn callbacks alive.
    ...
    if oldRuntimeDetected then
        pcall(function()
            StarterGui0:SetCore("SendNotification",{
                Title="Kimqetras HC",
                Text="An older Kimqetras HC runtime is still active. Rejoin once, then run v2.45. v2.45 will not stack on top of it.",
                Duration=9,
            })
        end)
        warn("[Kimqetras HC v2.45] Older runtime detected. Refusing to stack; rejoin once.")
        return
    end
    ...
    local knownRenderBindings = {
        "KimqMacroSpeedV240","KimqMacroSpeedV241","KimqMacroSpeedV242","KimqMacroSpeedV245",
        ...
        "KimqPermanentTimeV241","KimqPermanentTimeV242","KimqPermanentTimeV245",
    }
...
local initialSilentPart = cfg.silentAimClosestPart and "Closest Part" or cfg.silentAimPart
addDropdown("Hit Part", silentParts, initialSilentPart, function(v)
    if v=="Closest Part" then
...
pcall(function()
    RunService:BindToRenderStep("KimqMacroSpeedV245",Enum.RenderPriority.Last.Value,function()
        enforceMacroSpeed()
    end)
end)
...
local function applyAvatarUser(userId, char, quiet)
    ...
    task.spawn(function()
        if not quiet then setAvatarStatus("Loading avatar...",false) end
        ...
        local okDummy,dummy=pcall(function()
            return Players:CreateHumanoidModelFromUserId(userId)
        end)
        if not okDummy or not dummy or serial~=avatarApplySerial then
            if dummy then pcall(function() dummy:Destroy() end) end
            if not quiet then setAvatarStatus("Avatar could not be loaded",false) end
            return
        end

        -- Remove only visual avatar pieces. Keep gameplay folders/scripts/tools.
        ...
        -- Explicit classic clothing/body fallback.
        ...
        -- Accessories from the chosen Roblox avatar.
        for _,item in ipairs(dummy:GetChildren()) do
        ...
        if not quiet then setAvatarStatus("Avatar applied ♡",true) end
    end)
end
...
lp.CharacterAdded:Connect(function(char)
    if AvatarEnabled and tostring(AvatarTargetBox.Text or "")~="" then
        task.spawn(function()
            char:WaitForChild("Humanoid",6)
            task.wait(.45)
            AvatarTarget=AvatarTargetBox.Text
            local userId=resolveAvatarUserId(AvatarTarget)
...
_G.KimqAvatarController = {
    ...
    ApplySaved=function()
        ...
        if AvatarEnabled and AvatarTarget~="" and lp.Character then
            local userId=resolveAvatarUserId(AvatarTarget)
            if userId then applyAvatarUser(userId,lp.Character,true) end
        end
    end,
    Apply=function(v)
        if v~=nil then
...
local function addCleanSection(title, subtitle)
    ...
    -- v2.45: pageHead already displays the section name.
...
task.spawn(function()
    ...
    -- v2.45: pages are grouped by what they actually do so the sidebar is
...
-- v2.45 module: local catalog / limited accessory try-on.
task.spawn(function()
    _G.KimqAccessoryUIReady=false
    local Players = game:GetService("Players")
    local CoreGui = game:GetService("CoreGui")
...
-- v2.45 module: Environment + Weapon Skins (canonical sidebar pages).
task.spawn(function()
    local pageReadyStart=tick()
    while not _G.KimqBasePagesReady and tick()-pageReadyStart<8 do task.wait(.02) end
    ...
    -- v2.45: Environment and Weapon Skins are already real canonical pages/buttons.
    ...
    pcall(function()
        RunService:BindToRenderStep("KimqPermanentTimeV245",Enum.RenderPriority.Last.Value,function()
    ...
    if not envBuildOk then
        warn("[Kimqetras HC v2.45] Environment page fallback: "..tostring(envBuildErr))
    ...
    local function setupWeaponExtras()
        if _G.KimqWeaponExtrasInstalled then
            local existing=skinsPage:FindFirstChild("KimqBulletBeamsCard")
            if existing and existing.Parent then return true end
            _G.KimqWeaponExtrasInstalled=false
            _G.KimqWeaponExtrasController=nil
        end
        ...
        _G.KimqBulletLightBrightness=math.clamp(tonumber(_G.KimqWeaponExtrasState.BulletLightBrightness) or tonumber(_G.KimqBulletLightBrightness) or 0.65,0,2.5)
        ...
        local function findFolder(names)
            local function wantedName(objName)
                local lower=tostring(objName or ""):lower()
                for _,name in ipairs(names) do
                    if lower==tostring(name):lower() then return true end
                end
                return false
            end

            -- Exact hierarchy used by this game.
            local assets=ReplicatedStorage:FindFirstChild("Assets")
            if assets then
                for _,child in ipairs(assets:GetChildren()) do
                    if wantedName(child.Name) then return child end
                end
            end

            -- Direct ReplicatedStorage fallback.
            for _,child in ipairs(ReplicatedStorage:GetChildren()) do
                if wantedName(child.Name) then return child end
            end

            -- Case-insensitive recursive ReplicatedStorage fallback.
            for _,obj in ipairs(ReplicatedStorage:GetDescendants()) do
                if wantedName(obj.Name) then return obj end
            end

            -- A few games move cosmetic source folders after round load.
            local roots={workspace}
            local okStarter,starter=pcall(function() return game:GetService("StarterPack") end)
            if okStarter and starter then table.insert(roots,starter) end
        ...
        selectedBeamSource=function()
            ...
            local root=ReplicatedStorage:FindFirstChild("BulletBeams")
        ...
        local function scanBeams()
            ...
            if not root then
                beamStatus.Text="BulletBeams folder not found • showing None"
                beamStatus.TextColor3=pcolor("sub",subColor)
            end
            ...
            refreshBeamStyle()
            if root and #names>0 then
                beamStatus.Text="Bullet beams found: "..tostring(math.max(#names-1,0)).."  •  selected: "..tostring(extraState.BulletBeam or "None")
            end
        end
        ...
        -- v2.45: this refresh sits immediately before the old stable
        -- Knife / Equippable / Angel Wings builder. If a round-specific
        -- gun object errors here, skip only this refresh and keep building
        -- the old working weapon-extra sections.
        local refreshOk,refreshErr=pcall(refreshAllLocalGunTemplates)
        if not refreshOk then
            warn("[Kimqetras HC v2.45] bullet refresh skipped: "..tostring(refreshErr))
        end
        end
        ...
        local function scanKnives()
            ...
            local root=ReplicatedStorage:FindFirstChild("Knives"); local names={"None"}
            if root then
                for _,ch in ipairs(root:GetChildren()) do
                    if ch.Name~="None" and (ch:IsA("Model") or ch:IsA("Tool") or ch:IsA("Folder")) then
            ...
            refreshKnifeStyle()
            if root then
                knifeStatus.Text="Knife skins found: "..tostring(math.max(#names-1,0)).."  •  selected: "..tostring(extraState.KnifeSkin or "None")
            else
                knifeStatus.Text="Knives folder not found • showing None"
            end
        end
        ...
        if type(_G.KimqRegisterConfigControl)=="function" then
            pcall(function()
                _G.KimqRegisterConfigControl("Angel Wings Flight","toggle",
                    function() return wingFlightEnabled end,
                    function(v)
                        wingFlightEnabled=not not v
                        if not wingFlightEnabled then stopWingFlight() end
                    end)
            end)
            pcall(function()
                _G.KimqRegisterConfigControl("Angel Wings Flight Speed","slider",
                    function() return wingFlightSpeed end,
                    function(v)
                        wingFlightSpeed=math.clamp(tonumber(v) or 46,16,140)
                        wingFlightVerticalSpeed=math.clamp(wingFlightSpeed*.66,18,80)
                        updateWingMiniSpeedVisual()
                    end)
            end)
        ...
        local function scanEquipables()
            ...
            local root=ReplicatedStorage:FindFirstChild("EquipableItem"); local names={"None"}
            ...
            refreshEquipStyle()
            if root then
                equipStatus.Text="Equippable items found: "..tostring(math.max(#names-1,0)).."  •  selected: "..tostring(extraState.EquipableItem or "None")
            else
                equipStatus.Text="EquippableItem folder not found • showing None"
            end
        end

        -- ========================================================
        -- v2.45 EARLY WEAPON-EXTRAS CORE
        -- The live game paths are:
        --   ReplicatedStorage.BulletBeams
        --   ReplicatedStorage.Knives
        --   ReplicatedStorage.EquipableItem
        -- Populate NOW, before any optional wing-fit/settings UI.
        -- ========================================================
        local function kimqDirectExtrasRefresh()
            local beamOK,beamErr=pcall(bulletAPI.scanBeams)
            local knifeOK,knifeErr=pcall(scanKnives)
            local equipOK,equipErr=pcall(scanEquipables)

            if not beamOK then
                beamStatus.Text="Bullet Beam list error"
                warn("[Kimqetras HC v2.45] BulletBeams list: "..tostring(beamErr))
            end
            if not knifeOK then
                knifeStatus.Text="Knife list error"
                warn("[Kimqetras HC v2.45] Knives list: "..tostring(knifeErr))
            end
            if not equipOK then
                equipStatus.Text="Equippable list error"
                warn("[Kimqetras HC v2.45] EquipableItem list: "..tostring(equipErr))
            end

            return beamOK,knifeOK,equipOK
        end

        -- Expose the useful actions before the optional remainder of the
        -- Weapon Extras builder runs.
        _G.KimqWeaponExtrasController={
            GetState=function()
                return {
                    ...
                    BulletLightBrightness=tonumber(_G.KimqBulletLightBrightness) or .65,
                    KnifeSkin=tostring(extraState.KnifeSkin or "None"),
                    EquipableItem=tostring(extraState.EquipableItem or "None"),
                }
            end,
            Refresh=kimqDirectExtrasRefresh,
            RefreshColor=bulletAPI.refreshBulletColorUI,
        ...
        pcall(function()
            if type(getgenv)=="function" then
                local env=getgenv()
                env.KimqWeaponExtrasController=_G.KimqWeaponExtrasController
                env.KimqWeaponExtrasInstalled=true
                env.KimqWeaponExtrasState=extraState
            end
        end)

        -- First population happens synchronously.
        kimqDirectExtrasRefresh()

        -- ReplicatedStorage can finish cosmetic replication after the main GUI
        -- appears. Retry for a short bounded window; stop once all three folders
        -- exist and each list has at least one actual choice.
        task.spawn(function()
            for _=1,24 do
                task.wait(.5)
                if not skinsPage or not skinsPage.Parent then return end

                local beamRoot=ReplicatedStorage:FindFirstChild("BulletBeams")
                local knifeRoot=ReplicatedStorage:FindFirstChild("Knives")
                local equipRoot=ReplicatedStorage:FindFirstChild("EquipableItem")

                kimqDirectExtrasRefresh()

                local beamCount=beamRoot and #beamRoot:GetChildren() or 0
                local knifeCount=knifeRoot and #knifeRoot:GetChildren() or 0
                local equipCount=equipRoot and #equipRoot:GetChildren() or 0

                if beamCount>1 and knifeCount>1 and equipCount>0 then
                    break
                end
            end
        end)

        -- If the game swaps/recreates any of the three live folders, refresh.
        ReplicatedStorage.ChildAdded:Connect(function(ch)
            if ch.Name=="BulletBeams" or ch.Name=="Knives" or ch.Name=="EquipableItem" then
                task.defer(kimqDirectExtrasRefresh)
            end
        end)
        ...
        local function scanAllExtras()
            local okBeam,okKnife,okEquip=kimqDirectExtrasRefresh()

            if bulletAPI.beamList then
                bulletAPI.beamList.ScrollBarImageColor3=pcolor("hot",hotColor)
            end
            knifeList.ScrollBarImageColor3=pcolor("hot",hotColor)
            equipList.ScrollBarImageColor3=pcolor("hot",hotColor)

            local rootGui=skinsPage:FindFirstAncestorOfClass("ScreenGui")
            if rootGui then
                rootGui:SetAttribute("KimqWeaponScanBeamOK",okBeam)
                rootGui:SetAttribute("KimqWeaponScanKnifeOK",okKnife)
                rootGui:SetAttribute("KimqWeaponScanEquipOK",okEquip)
            end

            return okBeam,okKnife,okEquip
        end
        skinsPage:GetPropertyChangedSignal("Visible"):Connect(function()
            if skinsPage.Visible then task.defer(scanAllExtras) end
        end)

        -- Build the buttons NOW. No dependency on a later visibility transition.
        local initBeamOK,initKnifeOK,initEquipOK=scanAllExtras()
        pcall(function()
        ...
        local function getExtraState()
            return {
                ...
                BulletColorMode=tostring(extraState.BulletColorMode or "Preset"),
                BulletColorHex=tostring(extraState.BulletColorHex or "#FF69B4"),
                BulletLightBrightness=tonumber(_G.KimqBulletLightBrightness) or 0.65,
                KnifeSkin=tostring(extraState.KnifeSkin or "None"),
                EquipableItem=tostring(extraState.EquipableItem or "None"),
        ...
        local function setExtraState(state)
            ...
            if tonumber(state.BulletLightBrightness) then
                _G.KimqBulletLightBrightness=math.clamp(tonumber(state.BulletLightBrightness),0,2.5)
                extraState.BulletLightBrightness=_G.KimqBulletLightBrightness
            end
        ...
        pcall(function()
            if type(getgenv)=="function" then
                ...
                e.KimqWeaponExtrasState=extraState
            end
        end)

        local rootGui=skinsPage:FindFirstAncestorOfClass("ScreenGui")
        if rootGui then
            rootGui:SetAttribute("KimqWeaponExtrasReady",true)
            rootGui:SetAttribute("KimqWeaponBeamButtonCount",#(bulletAPI.beamList and bulletAPI.beamList:GetChildren() or {}))
            rootGui:SetAttribute("KimqWeaponKnifeButtonCount",#knifeList:GetChildren())
            rootGui:SetAttribute("KimqWeaponEquipButtonCount",#equipList:GetChildren())
        end
    ...
    if not extrasOk then
        ...
        pcall(function()
            if type(getgenv)=="function" then
                local e=getgenv()
                e.KimqWeaponExtrasInstalled=false
                e.KimqWeaponExtrasLastError=tostring(extrasErr)
            end
        end)
        local rootGui=skinsPage:FindFirstAncestorOfClass("ScreenGui")
        if rootGui then
            rootGui:SetAttribute("KimqWeaponExtrasReady",false)
            rootGui:SetAttribute("KimqWeaponExtrasError",tostring(extrasErr))
        end
        warn("[Kimqetras HC v2.45] Weapon extras: "..tostring(extrasErr))
        setStatus("Weapon extras could not build • "..tostring(extrasErr):sub(1,90),false)
    else
        task.defer(function()
            task.wait(.05)
            ...
            if extras and type(extras.Refresh)=="function" then
                ...
                if not ok then
                    warn("[Kimqetras HC v2.45] extras refresh: "..tostring(err))
                end
            end
        end)
    end
...
task.spawn(function()
    ...
    -- v2.45: preserve the grouped sidebar order from pageDefs.
    ...
    local function applyTheme(name,settlePass)
        ...
        -- v2.45 hard guarantee: old controls created with literal pink/yellow
    ...
    -- v2.45: section navigation does not repaint/rescan the entire GUI.
    ...
    -- v2.45 reveal happens only after this initial theme pass is complete.
end)


-- v2.45: delayed post-show cleanup passes removed.
...
-- ========================================================
-- v2.45 branding is deterministic at construction time; no delayed scrub pass.

-- ========================================================
-- v2.45 DETERMINISTIC REVEAL
...
task.spawn(function()
    ...
    -- v2.45: remove the old one-page section divider labels from the
