-- Kimqetras HC v2.76 • v2.75 base + persistent copied avatar + organized feature pages + OP RAGE presets
...
local KIMQ_BUILD = "v2.76-avatar-stay-organized-op-rage"
...
do
    ...
    if running and runningBuild == KIMQ_BUILD then
        if runningGui then
            ...
            notifyBoot((runtimeState=="ready") and "v2.76 is already running ♡" or "v2.76 is already starting ♡",4)
            return
        end
        ...
        notifyBoot("Recovering a failed v2.76 boot ♡",4)
    end
    ...
    if oldRuntimeDetected then
        ...
        if staleBoot then
            ...
            notifyBoot("Another Kimqetras HC runtime is still active. Rejoin once, then run v2.76.",9)
            warn("[Kimqetras HC v2.76] Older/live runtime detected. Refusing to stack; rejoin once.")
            return
        end
    end
...
Hello.Text = "v2.76  •  avatar stay + organized pages + OP RAGE ♡"
...
local function restoreKeptAvatar(char)
    ...
    -- Ignore ChildRemoved events caused by our own ApplyDescription/visual rebuild.
    avatarSuppressRepairUntil=os.clock()+1.15
    if activeAvatarSnapshot then
        pcall(applyAvatarSnapshot,activeAvatarSnapshot,char,true)
    else
        AvatarTarget=AvatarTargetBox.Text
        if tostring(AvatarTarget or "")~="" then
            local userId=resolveAvatarUserId(AvatarTarget)
            if userId then pcall(applyAvatarUser,userId,char,true) end
        end
    end
    task.delay(.18,function() avatarVisualRepairBusy=false end)
end
...
local function startKeptAvatarRespawnGuard(char)
    ...
    task.spawn(function()
        ...
        -- Games often re-apply the default appearance more than once after CharacterAdded.
        -- These are bounded retries across the first ~9 seconds only, so there is no
        -- permanent full-avatar ApplyDescription loop running during normal play.
        local waits={.20,.45,.80,1.35,2.10,3.20}
        for _,delayTime in ipairs(waits) do
            task.wait(delayTime)
            if serial~=avatarRespawnGuardSerial or not AvatarEnabled or char~=lp.Character then return end
            restoreKeptAvatar(char)
        end
    end)

    -- If the game removes one of the copied visual pieces after its own appearance
    -- loader finishes, repair once after a short debounce. Tools/gameplay folders are ignored.
    char.ChildRemoved:Connect(function(obj)
        if serial~=avatarRespawnGuardSerial or not AvatarEnabled or char~=lp.Character then return end
        if os.clock()<avatarSuppressRepairUntil then return end
        local visual = obj:IsA("Accessory") or obj:IsA("Shirt") or obj:IsA("Pants")
            or obj:IsA("ShirtGraphic") or obj:IsA("BodyColors") or obj:IsA("CharacterMesh")
        if not visual then return end
        task.delay(.18,function()
            if serial==avatarRespawnGuardSerial and AvatarEnabled and char==lp.Character then
                restoreKeptAvatar(char)
            end
        end)
    end)
...
lp.CharacterAdded:Connect(function(char)
    if AvatarEnabled then startKeptAvatarRespawnGuard(char) end
end)

-- Roblox/custom games can finish loading the default character appearance AFTER
-- CharacterAdded. Re-apply at that exact point, then one more time shortly after.
lp.CharacterAppearanceLoaded:Connect(function(char)
    ...
    task.delay(.10,function()
        ...
        task.delay(.55,function()
            if AvatarEnabled and char==lp.Character then restoreKeptAvatar(char) end
        end)
    end)
end)
...
_G.KimqAvatarController = {
    ...
    ApplySnapshot=function(state,quiet)
        ...
        if activeAvatarSnapshot then setAvatarKeepEnabled(true) end
