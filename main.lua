-- Kimqetras HC v2.43 • strict single-runtime / no stacking
...
local KIMQ_BUILD = "v2.43-strict-single-runtime"
...
do
    ...
    local runtimeState = rawget(_G, "KimqHC_RuntimeState")
    ...
    if running and runningBuild == KIMQ_BUILD then
        ...
        pcall(function()
            StarterGui0:SetCore("SendNotification",{
                Title="Kimqetras HC",
                Text=(runtimeState=="ready") and "v2.43 is already running ♡" or "v2.43 is already starting ♡",
                Duration=4,
            })
        end)
        return
    ...
    local oldRuntimeDetected =
        ...
        or (rawget(_G,"KimqHC_v21_PerformanceLoaded")==true and runningBuild~=KIMQ_BUILD)

    if oldRuntimeDetected then
        pcall(function()
            StarterGui0:SetCore("SendNotification",{
                Title="Kimqetras HC",
                Text="An older Kimqetras HC runtime is still active. Rejoin once, then run v2.43. v2.43 will not stack on top of it.",
                Duration=9,
            })
        end)
        warn("[Kimqetras HC v2.43] Older runtime detected. Refusing to stack; rejoin once.")
        return
    end
    ...
    local knownRenderBindings = {
        "KimqMacroSpeedV240","KimqMacroSpeedV241","KimqMacroSpeedV242","KimqMacroSpeedV243",
        ...
        "KimqPermanentTimeV241","KimqPermanentTimeV242","KimqPermanentTimeV243",
    }
    ...
    _G.KimqHC_RuntimeStartedAt = os.clock()
...
pcall(function()
    RunService:BindToRenderStep("KimqMacroSpeedV243",Enum.RenderPriority.Last.Value,function()
        enforceMacroSpeed()
    end)
end)
...
local function addCleanSection(title, subtitle)
    ...
    -- v2.43: pageHead already displays the section name.
...
task.spawn(function()
    ...
    -- v2.43: pages are grouped by what they actually do so the sidebar is
...
-- v2.43 module: local catalog / limited accessory try-on.
task.spawn(function()
    _G.KimqAccessoryUIReady=false
    local Players = game:GetService("Players")
    local CoreGui = game:GetService("CoreGui")
...
-- v2.43 module: Environment + Weapon Skins (canonical sidebar pages).
task.spawn(function()
    local pageReadyStart=tick()
    while not _G.KimqBasePagesReady and tick()-pageReadyStart<8 do task.wait(.02) end
    ...
    -- v2.43: Environment and Weapon Skins are already real canonical pages/buttons.
    ...
    pcall(function()
        RunService:BindToRenderStep("KimqPermanentTimeV243",Enum.RenderPriority.Last.Value,function()
    ...
    if not envBuildOk then
        warn("[Kimqetras HC v2.43] Environment page fallback: "..tostring(envBuildErr))
    ...
    local function setupWeaponExtras()
        ...
        -- v2.43: this refresh sits immediately before the old stable
        ...
        if not refreshOk then
            warn("[Kimqetras HC v2.43] bullet refresh skipped: "..tostring(refreshErr))
        end
        end
        ...
        task.defer(function()
            -- v2.43: populate the lists once immediately. The page can already be
            -- visible before its Visible listener attaches, which used to leave
            -- Bullet Beams / Knives / Equippables blank forever.
            local ok,err=pcall(scanAllExtras)
            if not ok then
                warn("[Kimqetras HC v2.43] initial extras scan: "..tostring(err))
            end
    ...
    if not extrasOk then
        _G.KimqWeaponExtrasInstalled=false
        warn("[Kimqetras HC v2.43] Weapon extras: "..tostring(extrasErr))
        ...
        task.defer(function()
            ...
            if extras and type(extras.Refresh)=="function" then
                ...
                if not ok then
                    warn("[Kimqetras HC v2.43] extras refresh: "..tostring(err))
                end
            end
        end)
    end
...
task.spawn(function()
    ...
    -- v2.43: preserve the grouped sidebar order from pageDefs.
    ...
    local function applyTheme(name,settlePass)
        ...
        -- v2.43 hard guarantee: old controls created with literal pink/yellow
    ...
    -- v2.43: section navigation does not repaint/rescan the entire GUI.
    ...
    -- v2.43 reveal happens only after this initial theme pass is complete.
end)


-- v2.43: delayed post-show cleanup passes removed.
...
-- ========================================================
-- v2.43 branding is deterministic at construction time; no delayed scrub pass.

-- ========================================================
-- v2.43 DETERMINISTIC REVEAL
...
task.spawn(function()
    ...
    -- v2.43: remove the old one-page section divider labels from the
