-- Kimqetras HC v2.42 • old working weapon-extras path + single spawn correction
...
local KIMQ_BUILD = "v2.42-old-extras-single-spawn"
    ...
    if running and runningGui and runningBuild ~= KIMQ_BUILD then
        pcall(function()
            game:GetService("StarterGui"):SetCore("SendNotification", {
                Title = "Kimqetras HC",
                Text = "Another Kimqetras HC version is still active. Rejoin once, then run v2.42.",
                Duration = 8,
            })
        end)
        warn("[Kimqetras HC v2.42] Another runtime is active; rejoin once before switching builds.")
        return
    end
...
_G.KimqAccessoryUIReady = false
    ...
    pcall(function() game:GetService("RunService"):UnbindFromRenderStep("KimqPermanentTimeV242") end)
end
...
pcall(function()
    RunService:BindToRenderStep("KimqMacroSpeedV242",Enum.RenderPriority.Last.Value,function()
        enforceMacroSpeed()
    end)
end)
...
local function addCleanSection(title, subtitle)
    ...
    -- v2.42: pageHead already displays the section name.
...
task.spawn(function()
    ...
    -- v2.42: pages are grouped by what they actually do so the sidebar is
...
-- v2.42 module: local catalog / limited accessory try-on.
task.spawn(function()
    _G.KimqAccessoryUIReady=false
    local Players = game:GetService("Players")
    local CoreGui = game:GetService("CoreGui")
...
-- v2.42 module: Environment + Weapon Skins (canonical sidebar pages).
task.spawn(function()
    local pageReadyStart=tick()
    while not _G.KimqBasePagesReady and tick()-pageReadyStart<8 do task.wait(.02) end
    ...
    -- v2.42: Environment and Weapon Skins are already real canonical pages/buttons.
    ...
    pcall(function()
        RunService:BindToRenderStep("KimqPermanentTimeV242",Enum.RenderPriority.Last.Value,function()
    ...
    if not envBuildOk then
        warn("[Kimqetras HC v2.42] Environment page fallback: "..tostring(envBuildErr))
    ...
    local function setupWeaponExtras()
        ...
        -- v2.42: this refresh sits immediately before the old stable
        ...
        if not refreshOk then
            warn("[Kimqetras HC v2.42] bullet refresh skipped: "..tostring(refreshErr))
        end
        end
        ...
        task.defer(function()
            -- v2.42: populate the lists once immediately. The page can already be
            ...
            if not ok then
                warn("[Kimqetras HC v2.42] initial extras scan: "..tostring(err))
            end
    ...
    if not extrasOk then
        _G.KimqWeaponExtrasInstalled=false
        warn("[Kimqetras HC v2.42] Weapon extras: "..tostring(extrasErr))
        ...
        task.defer(function()
            ...
            if extras and type(extras.Refresh)=="function" then
                ...
                if not ok then
                    warn("[Kimqetras HC v2.42] extras refresh: "..tostring(err))
                end
            end
        end)
    end
...
task.spawn(function()
    ...
    -- v2.42: preserve the grouped sidebar order from pageDefs.
    ...
    local function applyTheme(name,settlePass)
        ...
        -- v2.42 hard guarantee: old controls created with literal pink/yellow
    ...
    -- v2.42: section navigation does not repaint/rescan the entire GUI.
    ...
    -- v2.42 reveal happens only after this initial theme pass is complete.
end)


-- v2.42: delayed post-show cleanup passes removed.
...
-- ========================================================
-- v2.42 branding is deterministic at construction time; no delayed scrub pass.

-- ========================================================
-- v2.42 DETERMINISTIC REVEAL
...
task.spawn(function()
    ...
    -- v2.42: remove the old one-page section divider labels from the
