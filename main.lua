-- Kimqetras HC v2.91 • v2.90 base + isolated natural Mouse Aim tracking refinement
...
local KIMQ_BUILD = "v2.91-mouse-aim-natural-tracking"
...
Hello.Text = "v2.91  •  natural mouse aim ♡"
...
-- v2.91 Mouse Aim is intentionally separate from Camlock. It never writes the
-- camera CFrame. In free-cursor third person it can softly move the cursor;
-- when Roblox locks the cursor to center (first person / shift-lock style), it
-- keeps only the Q target + Silent Correction so the camera remains yours.
cfg.mouseAimEnabled = cfg.mouseAimEnabled == true
cfg.mouseAimKey = cfg.mouseAimKey or Enum.KeyCode.Q
cfg.mouseAimPart = tostring(cfg.mouseAimPart or "Closest Point")
cfg.mouseAimFOV = tonumber(cfg.mouseAimFOV) or 260
cfg.mouseAimFollowStrength = tonumber(cfg.mouseAimFollowStrength) or 22
cfg.mouseAimSoftness = tonumber(cfg.mouseAimSoftness) or 3
cfg.mouseAimVisibleFollow = cfg.mouseAimVisibleFollow ~= false
cfg.mouseAimSilentCorrection = cfg.mouseAimSilentCorrection ~= false
cfg.mouseAimWallCheck = cfg.mouseAimWallCheck == true
...
local state={active=false,target=nil,part=nil,point=nil,lastScreen=nil,humanOffset=Vector2.zero,humanTarget=Vector2.zero,nextHumanizeAt=0,lastRenderClock=os.clock()}
...
local bodyParts={
    ...
    -- R6 fallbacks are resolved dynamically below.
}
...
local function preferredPart(pl)
    ...
    local wanted=tostring(cfg.mouseAimPart or "Closest Point")
    ...
    best=best or c:FindFirstChild("HumanoidRootPart") or c:FindFirstChild("Head")
...
local function candidateDistance(pl)
    ...
    if d>math.max(10,tonumber(cfg.mouseAimFOV) or 260) then return math.huge,nil,nil end
    return d,part,point
end
...
local function unlock()
    ...
    state.humanOffset=Vector2.zero;state.humanTarget=Vector2.zero;state.nextHumanizeAt=0;state.lastRenderClock=os.clock()
    _G.KimqMouseAimTarget=nil
end
...
-- Silent Aim asks this bridge for the Q-locked target. Returning nil restores
-- normal Silent Aim immediately; no camera code is shared between the systems.
...
RunService.RenderStepped:Connect(function()
    ...
    -- Do NOT move the physical cursor while Roblox has it locked to screen
    -- center. That would rotate the camera and become Camlock. Silent Correction
    -- remains active in first person, so shots still follow the Q-locked target.
    ...
    -- v2.91: humanization no longer draws a perfect sine-wave circle around the
    -- body part.  That was the reason the cursor looked like it was "hovering"
    -- beside/around the selected part.  Instead it chooses a tiny irregular hold
    -- offset, eases into it, then naturally recenters/changes after a short delay.
    local softness=math.clamp(tonumber(cfg.mouseAimSoftness) or 0,0,18)
    if softness<=0 then
        state.humanTarget=Vector2.zero
        state.humanOffset=Vector2.zero
    else
        if now>=(state.nextHumanizeAt or 0) then
            local radius=softness*.52
            local angle=math.random()*math.pi*2
            local mag=math.sqrt(math.random())*radius
            state.humanTarget=Vector2.new(math.cos(angle)*mag,math.sin(angle)*mag*.72)
            -- Uneven timing looks like hand correction instead of an animation loop.
            state.nextHumanizeAt=now+.30+math.random()*.52
        end
        local offsetAlpha=1-math.exp(-dt*4.2)
        state.humanOffset=state.humanOffset:Lerp(state.humanTarget,offsetAlpha)
    end

    local desired=sp+(state.humanOffset or Vector2.zero)
    state.lastScreen=desired
    local mp=UIS:GetMouseLocation()
    local delta=desired-mp
    local strength=math.clamp(tonumber(cfg.mouseAimFollowStrength) or 22,1,100)/100

    -- Frame-rate independent easing: low settings visibly trail the player while
    -- higher values catch up faster, without becoming a one-frame robotic snap.
    local response=2.0+(strength*8.5)
    local followAlpha=1-math.exp(-response*dt)
    local maxStep=4+(strength*34)
    local dx=math.clamp(delta.X*followAlpha,-maxStep,maxStep)
    local dy=math.clamp(delta.Y*followAlpha,-maxStep,maxStep)

    -- Tiny dead-zone avoids pixel vibrating once the cursor reaches the selected
    -- body part. Silent Correction remains exact underneath the visible movement.
    if delta.Magnitude<.75 then return end
    if math.abs(dx)>.08 or math.abs(dy)>.08 then
        if moveRel then
            pcall(moveRel,dx,dy)
        elseif VIM then
...
task.spawn(function()
    ...
    pcall(function()
        game:GetService("StarterGui"):SetCore("SendNotification",{
            Title="Kimqetras HC",
            Text="v2.91 ACTIVE ♡ • natural mouse aim + existing systems ready",
            Duration=7,
        })
    end)
