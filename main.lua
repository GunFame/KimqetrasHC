-- Kimqetras HC v2.25 • stable macro rebuild + config rename + ESP color + wing flight animation/minigui
-- Keeps the stable v2.24 base, rebuilds Macro / Speed, adds config renaming,
-- full RGB ESP color control, and upgrades Angel Wings flight with animation + mini speed GUI.
-- Re-running this SAME build only reopens the current GUI instead of stacking another copy.
-- If another build is already active, v2.25 refuses to stack and asks for one clean rejoin.
do
    local SINGLE_KEY = "KimqHC_v21_SingleRuntimeOptimized"
    local BUILD = "v2.25-macro-config-esp-wing-ui"
    ...
    -- Older builds did not expose a complete disconnect/cleanup handle. Starting v2.25
    ...
    if rawget(_G, SINGLE_KEY) and rawget(_G, "KimqHC_CurrentBuild") ~= BUILD then
        pcall(function()
            game:GetService("StarterGui"):SetCore("SendNotification", {
                Title = "Kimqetras HC",
                Text = "Older runtime detected. Rejoin once, then run v2.25 so only one version is active.",
                Duration = 7,
            })
        end)
        warn("[Kimqetras HC v2.25] Older runtime detected. Rejoin once before running v2.25; refusing to stack versions.")
        return
    end
...
-- v2.25 keeps the v2.24 stable base and replaces only the requested systems.
...
local function refreshMacroStatus()
    if not MacroStatus or not MacroStatus.Parent then return end
    if not MacroMaster then
        MacroStatus.Text = "Macro: OFF  •  turn Master on first"
        MacroStatus.TextColor3 = Color3.fromRGB(184,100,125)
    elseif MacroActive then
        MacroStatus.Text = "Macro: ACTIVE ♥  •  press "..MacroKey.Name.." to stop"
        MacroStatus.TextColor3 = Color3.fromRGB(225,73,140)
    else
        MacroStatus.Text = "Macro: READY  •  press "..MacroKey.Name.." to start"
        MacroStatus.TextColor3 = Color3.fromRGB(82,116,94)
    end
end

local function restoreWalkSpeed()
    local hum = currentHumanoid()
    if hum and MacroOriginalWalkSpeed then
        pcall(function() hum.WalkSpeed = MacroOriginalWalkSpeed end)
    end
    MacroOriginalWalkSpeed = nil
end

local function setMacroActive(v)
    v = not not v
    if v and not MacroMaster then v = false end
    if MacroActive == v then refreshMacroStatus(); return end
    if v then
        local hum = currentHumanoid()
...
addToggle("Macro / Speed Master", MacroMaster, function(v)
    ...
    if not MacroMaster then setMacroActive(false) end
    refreshMacroStatus()
end)
...
-- The old 1000 cap was unstable in games that reset WalkSpeed. This version uses
-- a horizontal CFrame assist, so 16-250 stays quick without launching the character.
addSlider("Macro Speed", 16, 250, MacroSpeed, function(v)
    MacroSpeed = math.clamp(tonumber(v) or 50,16,250)
end)
...
local MacroHintCard = createCard(62)
MacroStatus = Instance.new("TextLabel", MacroHintCard)
MacroStatus.Size = UDim2.new(1,-20,0,24)
MacroStatus.Position = UDim2.fromOffset(10,5)
MacroStatus.BackgroundTransparency = 1
MacroStatus.Font = Enum.Font.GothamBold
MacroStatus.TextSize = 11
MacroStatus.TextXAlignment = Enum.TextXAlignment.Left

local MacroHint = Instance.new("TextLabel", MacroHintCard)
MacroHint.Size = UDim2.new(1,-20,0,26)
MacroHint.Position = UDim2.fromOffset(10,30)
MacroHint.BackgroundTransparency = 1
MacroHint.Text = "Moves in your normal WASD direction. I/O zoom spam is optional and only runs while Macro is ACTIVE."
...
UIS.InputBegan:Connect(function(input)
    ...
    if input.UserInputType == Enum.UserInputType.Keyboard and input.KeyCode == MacroKey then
        if MacroMaster then setMacroActive(not MacroActive) end
    end
end)

local function sendMacroIO(keyCode)
    local vk=(keyCode==Enum.KeyCode.I) and 0x49 or 0x4F
    -- Executor keyboard functions are generally more reliable than VirtualInputManager.
    if type(keypress)=="function" and type(keyrelease)=="function" then
        local ok=pcall(function()
            keypress(vk)
            task.delay(.008,function() pcall(function() keyrelease(vk) end) end)
        end)
...
-- Direct horizontal assist instead of depending on Humanoid.WalkSpeed, which many
-- Hood-style games overwrite every frame. Y is never changed, which avoids fling.
RunService.Heartbeat:Connect(function(dt)
    if MacroMaster and MacroActive then
        ...
        if hum and root and hum.Health>0 then
            if not MacroOriginalWalkSpeed then MacroOriginalWalkSpeed=hum.WalkSpeed end
            ...
            if dir.Magnitude>.01 then
                ...
                local extra=math.max(0,MacroSpeed-16)
                local step=horizontal*extra*math.min(dt,1/20)
                if step.Magnitude>0 then
                    pcall(function() root.CFrame=root.CFrame+step end)
                end
            end
        end
    end
    ...
    if MacroMaster and MacroActive and MacroIOSpam then
        ...
        if MacroIOAccumulator >= MacroIOInterval then
            MacroIOAccumulator = 0
...
lp.CharacterAdded:Connect(function()
    MacroOriginalWalkSpeed=nil
    if not MacroMaster then MacroActive=false end
    task.delay(.4,refreshMacroStatus)
end)
...
task.spawn(function()
    ...
    local DAY_CLOCK, NIGHT_CLOCK = 14.0, 0.0
    ...
    local function refreshTimeUI()
        ...
        if timeMode=="Day" then timeStatus.Text="Day is locked at 2:00 PM."
    ...
    RunService.Heartbeat:Connect(function(dt)
        if not desiredClock() then return end
        timeFallbackAccum+=dt
        if timeFallbackAccum>=.20 then timeFallbackAccum=0; forceSelectedClock() end
    end)
    refreshTimeUI()
    ...
    local function setupWeaponExtras()
        ...
        local function destroyWingMiniGui()
            ...
            wingMiniGui=nil; wingMiniFrame=nil; wingMiniSpeedLabel=nil; wingMiniFill=nil; wingMiniKnob=nil; wingMiniStatus=nil
        end
        ...
        local function makeWingMiniGui()
            ...
            corner(top,14)
            local mask=Instance.new("Frame",top); mask.Size=UDim2.new(1,0,0,14); mask.Position=UDim2.new(0,0,1,-14); mask.BackgroundColor3=top.BackgroundColor3; mask.BorderSizePixel=0
            local title=txt(top,"♥  Angel Wings",UDim2.new(1,-54,1,0),UDim2.fromOffset(12,0),Enum.Font.GothamBold,13,pcolor("text",textColor))
            local x=Instance.new("TextButton",top); x.Size=UDim2.fromOffset(30,28); x.Position=UDim2.new(1,-36,.5,-14); x.BackgroundTransparency=1; x.Text="×"; x.TextColor3=pcolor("hot",hotColor); x.Font=Enum.Font.GothamBold; x.TextSize=20
            ...
            local bar=Instance.new("Frame",f); bar.Size=UDim2.new(1,-24,0,14); bar.Position=UDim2.fromOffset(12,76); bar.BackgroundColor3=pcolor("soft",lightColor); bar.BorderSizePixel=0; corner(bar,7)
