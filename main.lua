...
task.spawn(function()
    ...
    local function setupWeaponExtras()
        ...
        local function bulletish(obj)
            ...
            for _=1,7 do
                ...
                if n:find("bullet",1,true) or n:find("beam",1,true) or n:find("tracer",1,true)
                    or n:find("projectile",1,true) or n:find("laser",1,true) or n:find("shot",1,true) then
                    return true
                end
                x=x.Parent
            end
        ...
        local function selectedBeamSource()
        ...
        txt(beamCard,"♥  bullet beams",UDim2.new(1,-24,0,24),UDim2.fromOffset(12,8),Enum.Font.FredokaOne,18,pcolor("hot",hotColor))
        txt(beamCard,"Pick one of the game's ReplicatedStorage beam styles for your local shots.",UDim2.new(1,-24,0,22),UDim2.fromOffset(12,34),Enum.Font.Gotham,11,pcolor("sub",subColor))
        ...
        -- Restyle bullet visuals as they are created. The shot-time gate keeps this
        -- from touching unrelated world VFX unless its ancestry already looks like a bullet.
        workspace.DescendantAdded:Connect(function(d)
            ...
            if d:IsA("BasePart") and not looksBullet then return end
            local recent=(os.clock()-lastLocalShot)<=.9
        ...
        local function hookShotTool(tool)
            ...
            tool.Activated:Connect(function() lastLocalShot=os.clock() end)
        end
        ...
        local function applyKnifeSkin(name,quiet)
            ...
            local detail=""
            if embedded+mirrored>0 then detail=" • animation running via "..tostring(driver)
            elseif found and found>0 then detail=" • animation ID found"
            elseif hasAnimatedVisuals(source) then detail=" • animated VFX preserved" end
            return true,"Knife • "..name..detail
        end

        local knifeCard=card(skinsPage,205); knifeCard.Name="KimqKnifeSkinsCard"
        txt(knifeCard,"♥  knife skins",UDim2.new(1,-24,0,24),UDim2.fromOffset(12,8),Enum.Font.FredokaOne,18,pcolor("hot",hotColor))
        txt(knifeCard,"ReplicatedStorage.Knives • click a style to apply it locally.",UDim2.new(1,-24,0,22),UDim2.fromOffset(12,34),Enum.Font.Gotham,11,pcolor("sub",subColor))
        ...
        txt(equipCard,"♥  equippable items",UDim2.new(1,-24,0,24),UDim2.fromOffset(12,8),Enum.Font.FredokaOne,18,pcolor("hot",hotColor))
        txt(equipCard,"ReplicatedStorage.EquipableItem • local visuals/animations only.",UDim2.new(1,-24,0,22),UDim2.fromOffset(12,34),Enum.Font.Gotham,11,pcolor("sub",subColor))
