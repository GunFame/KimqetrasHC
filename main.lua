...
task.spawn(function()
    ...
    if info then
        ...
        local function profileRow(userId, displayName, role, x)
            ...
            -- Reliable profile thumbnail: set a direct rbxthumb URI immediately, then
            -- retry GetUserThumbnailAsync a few times and preload it. This also works
            -- for users who are not currently in the server.
            pic.ImageTransparency=0
            pic.Image=("rbxthumb://type=AvatarHeadShot&id=%d&w=420&h=420"):format(userId)
            task.spawn(function()
                local ContentProvider=game:GetService("ContentProvider")
                for attempt=1,6 do
                    if not pic.Parent then break end
                    local ok,img=pcall(function()
                        local content=PlayersSvc:GetUserThumbnailAsync(userId,Enum.ThumbnailType.HeadShot,Enum.ThumbnailSize.Size420x420)
                        return content
                    end)
                    if ok and type(img)=="string" and img~="" and pic.Parent then
                        pic.Image=img
                        pcall(function() ContentProvider:PreloadAsync({pic}) end)
                        break
                    end
                    task.wait(.35)
                end
...
task.spawn(function()
    ...
    local function setupWeaponExtras()
        ...
        -- BulletBeams in this game are MODELS whose visible style is stored in a
        -- child Part (some presets also have Outline/Highlight children). The previous
        -- fake-tracer approach never touched what the game's own gun code actually
        -- clones. This version redirects/patches the real local BulletBeams templates.
        ...
        _G.KimqBulletBeamChoice=extraState.BulletBeam or "None"

        local BULLET_VISUAL_PROPS={
            "Color","Material","Transparency","Reflectance","CastShadow",
            "Shape","TopSurface","BottomSurface"
        }
        local function copyPartAppearance(dst,src,copySize)
            if not (dst and src and dst:IsA("BasePart") and src:IsA("BasePart")) then return end
            safeCopy(dst,src,BULLET_VISUAL_PROPS)
            if copySize then pcall(function() dst.Size=src.Size end) end
            if dst:IsA("MeshPart") and src:IsA("MeshPart") then
                safeCopy(dst,src,{"MeshId","TextureID","DoubleSided","RenderFidelity"})
            end
            local dm=dst:FindFirstChildWhichIsA("SpecialMesh")
            local sm=src:FindFirstChildWhichIsA("SpecialMesh")
            if dm and sm then safeCopy(dm,sm,{"MeshId","TextureId","MeshType","Scale","Offset","VertexColor"}) end
        end
        local function clearInjectedBulletVisuals(root)
            if not root then return end
            for _,d in ipairs(root:GetDescendants()) do
                if d:GetAttribute("KimqBulletInjected") then pcall(function() d:Destroy() end) end
            end
        end
        local function cloneBulletExtras(srcPreset,targetPart)
            if not srcPreset or not targetPart then return end
            local function addClone(obj)
                local ok,c=pcall(function() return obj:Clone() end)
                if ok and c then
                    c.Name="KimqBulletExtra_"..obj.Name
                    c:SetAttribute("KimqBulletInjected",true)
                    c.Parent=targetPart
                    if c:IsA("Beam") or c:IsA("Trail") or c:IsA("ParticleEmitter") or c:IsA("PointLight") or c:IsA("SpotLight") or c:IsA("SurfaceLight") then
                        pcall(function() c.Enabled=true end)
                    elseif c:IsA("Highlight") then
                        pcall(function() c.Adornee=targetPart end)
                    end
                end
            end
            local srcPart=firstPart(srcPreset)
            if srcPart then
                for _,ch in ipairs(srcPart:GetChildren()) do
                    if ch:IsA("Attachment") or ch:IsA("SpecialMesh") or ch:IsA("SurfaceAppearance") or ch:IsA("Decal") or ch:IsA("Texture")
                        or ch:IsA("Beam") or ch:IsA("Trail") or ch:IsA("ParticleEmitter") or ch:IsA("PointLight") or ch:IsA("SpotLight") or ch:IsA("SurfaceLight") then
                        addClone(ch)
                    end
                end
            end
            for _,ch in ipairs(srcPreset:GetChildren()) do
                if ch~=srcPart and (ch:IsA("Highlight") or ch:IsA("SurfaceAppearance") or ch:IsA("ParticleEmitter") or ch:IsA("PointLight")) then addClone(ch) end
            end
        end
        local function pristinePreset(name)
            if bulletBeamBackup then return bulletBeamBackup:FindFirstChild(name) end
            return bulletBeamRoot and bulletBeamRoot:FindFirstChild(name)
        end
        local function patchOneBulletPreset(targetPreset,sourcePreset)
            if not targetPreset or not sourcePreset then return end
            clearInjectedBulletVisuals(targetPreset)
            local tp=firstPart(targetPreset); local sp=firstPart(sourcePreset)
            if tp and sp then
                copyPartAppearance(tp,sp,true)
                cloneBulletExtras(sourcePreset,tp)
            end
            local scol=templateColor(sourcePreset)
            if scol then
                for _,d in ipairs(targetPreset:GetDescendants()) do
                    if d:IsA("Beam") or d:IsA("Trail") or d:IsA("ParticleEmitter") then
                        pcall(function() d.Color=ColorSequence.new(scol) end)
                    elseif d:IsA("Color3Value") then pcall(function() d.Value=scol end) end
                end
            end
        end
        local function restoreBulletTemplates()
            bulletBeamRoot=findFolder({"BulletBeams","Bullet Beams","BulletBeam"})
            if not bulletBeamRoot or not bulletBeamBackup then return end
            for _,live in ipairs(bulletBeamRoot:GetChildren()) do
                if not live:IsA("Sound") then
                    clearInjectedBulletVisuals(live)
                    local back=bulletBeamBackup:FindFirstChild(live.Name)
                    if back then
                        local lp0=firstPart(live); local bp0=firstPart(back)
                        if lp0 and bp0 then copyPartAppearance(lp0,bp0,true) end
                    end
                end
            end
        end
        local function patchAllBulletTemplates(name)
            bulletBeamRoot=findFolder({"BulletBeams","Bullet Beams","BulletBeam"})
            if not bulletBeamRoot then return false,"BulletBeams folder was not found" end
            if name=="None" then
                restoreBulletTemplates()
                _G.KimqBulletBeamChoice="None"
                return true,"Bullet beam reset"
            end
            local source=pristinePreset(name)
            if not source then return false,"Bullet preset was not found" end
            _G.KimqBulletBeamChoice=name
            -- Patch every local preset. The gun can choose any BulletBeams child; no
            -- matter which one its LocalScript clones, the clone now has our style.
            for _,targetPreset in ipairs(bulletBeamRoot:GetChildren()) do
                if not targetPreset:IsA("Sound") then patchOneBulletPreset(targetPreset,source) end
            end
            return true,"Bullet beam: "..name
        end

        -- Intercept local Clone() calls originating from ReplicatedStorage.BulletBeams
        -- as a second path. If the weapon script clones the default/None model or its
        -- Part directly, return the selected preset's matching object instead.
        if not _G.KimqBulletCloneHookInstalled and type(hookmetamethod)=="function" and type(getnamecallmethod)=="function" then
            _G.KimqBulletCloneHookInstalled=true
            local oldBulletNC
            oldBulletNC=hookmetamethod(game,"__namecall",newcclosure(function(self,...)
                local method=getnamecallmethod()
                if method=="Clone" then
                    local choice=_G.KimqBulletBeamChoice
                    local root=findFolder({"BulletBeams","Bullet Beams","BulletBeam"})
                    if root and choice and choice~="None" and typeof(self)=="Instance" then
                        local ok,inside=pcall(function() return self:IsDescendantOf(root) end)
                        if ok and inside then
                            local preset=self
                            while preset and preset.Parent~=root do preset=preset.Parent end
                            local selected=root:FindFirstChild(choice)
                            if preset and selected and preset~=selected then
                                local replacement=nil
                                if self==preset then
                                    replacement=selected
                                else
                                    local rel={}; local x=self
                                    while x and x~=preset do table.insert(rel,1,x.Name); x=x.Parent end
                                    local y=selected
                                    for _,nm in ipairs(rel) do y=y and y:FindFirstChild(nm) end
                                    replacement=y
                                    if not replacement and self:IsA("BasePart") then replacement=firstPart(selected) end
                                end
                                if replacement then return oldBulletNC(replacement,...) end
                            end
                        end
                    end
                end
                return oldBulletNC(self,...)
            end))
        end
        ...
        local function applyBulletBeamOverride(name)
            ...
            return patchAllBulletTemplates(name)
        end
        ...
        txt(beamCard,"Uses the game's real ReplicatedStorage > BulletBeams Part presets for your local shots.",UDim2.new(1,-24,0,20),UDim2.fromOffset(12,32),Enum.Font.Gotham,11,pcolor("sub",subColor))
        ...
        local function scanBeams()
            ...
            bulletBeamRoot=findFolder({"BulletBeams","Bullet Beams","BulletBeam"})
            local names={}
            if bulletBeamRoot then
                if not bulletBeamBackup then pcall(function() bulletBeamBackup=bulletBeamRoot:Clone() end) end
                for _,ch in ipairs(bulletBeamRoot:GetChildren()) do
                    if not ch:IsA("Sound") and firstPart(ch) then table.insert(names,ch.Name) end
                end
            end
            ...
            for i,name in ipairs(names) do
                ...
                b.MouseButton1Click:Connect(function()
                    ...
                    refreshBeamStyle(); beamStatus.Text=msg
        ...
        -- Last-resort live recolor for client-created projectile Parts. We only touch
        -- small new world parts during the short window after YOUR equipped Tool fires.
        local shotUntil=0
        local function selectedPristinePart()
            local p=pristinePreset(extraState.BulletBeam)
            return p and firstPart(p),p
        end
        local function applyLiveBulletPart(part)
            if extraState.BulletBeam=="None" or os.clock()>shotUntil then return end
            if not part:IsA("BasePart") or insideToolOrCharacter(part) then return end
            local ok,mag=pcall(function() return part.Size.Magnitude end)
            if not ok or not mag or mag>35 then return end
            local sp,srcPreset=selectedPristinePart(); if not sp then return end
            copyPartAppearance(part,sp,false)
            clearInjectedBulletVisuals(part)
            cloneBulletExtras(srcPreset,part)
        end
        workspace.DescendantAdded:Connect(function(d)
            if os.clock()>shotUntil then return end
            if d:IsA("BasePart") then task.defer(function() if d.Parent then applyLiveBulletPart(d) end end) end
        end)
        local function hookBulletFireTool(tool)
            if not tool or not tool:IsA("Tool") or tool:GetAttribute("KimqBulletFireHookV4") then return end
            tool:SetAttribute("KimqBulletFireHookV4",true)
            tool.Activated:Connect(function() shotUntil=os.clock()+.8 end)
        end
        local function hookBulletContainers()
            local function h(c)
                if not c then return end
                for _,ch in ipairs(c:GetChildren()) do hookBulletFireTool(ch) end
                if not c:GetAttribute("KimqBulletFireContainerV4") then
                    c:SetAttribute("KimqBulletFireContainerV4",true)
                    c.ChildAdded:Connect(hookBulletFireTool)
                end
            end
            h(lp:FindFirstChildOfClass("Backpack")); h(lp.Character)
        end
        hookBulletContainers()
        lp.CharacterAdded:Connect(function() task.delay(.3,hookBulletContainers) end)
        ...
        local function visibleKnifePart(part)
            ...
            if n:find("HITBOX",1,true) or n=="HUMANOIDROOTPART" or n:find("PARTICLE_PART",1,true) then return false end
            if part:IsA("MeshPart") or part:IsA("UnionOperation") or part:FindFirstChildWhichIsA("SpecialMesh") then return true end
            return part.Name=="Handle" or part.Transparency<.95
        end
