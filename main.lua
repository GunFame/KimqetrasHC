-- Kimqetras HC v2.23 • restored working macro from v2.10 + v2.22 stability fixes
-- Re-running this SAME build only reopens the current GUI instead of stacking another copy.
-- If another build is already active, v2.23 refuses to stack and asks for one clean rejoin.
do
    local BUILD = "v2.23-restored-v210-macro"
    ...
    -- Older Kimqetras runtimes are detected so v2.23 does not stack over them.
    if ACTIVE and ACTIVE ~= BUILD then
        pcall(function()
            game:GetService("StarterGui"):SetCore("SendNotification", {
                Title = "Kimqetras HC",
                Text = "Another Kimqetras HC runtime is active. Rejoin once, then run v2.23.",
                Duration = 7,
            })
        end)
        warn("[Kimqetras HC v2.23] Existing runtime detected; refusing to stack versions.")
        return
    end
...
-- v2.23 keeps the v2.22 bullet/glow/camera/credits/jump/respawn fixes and restores the exact proven v2.10 macro block.
...
-- Target Jump Zoom ---------------------------------------------------------
-- v2.23: keeps the v2.22 jump-target screen-coordinate fix,
