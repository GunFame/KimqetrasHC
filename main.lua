-- Kimqetras HC v2.22 • full stability audit: reliable jump zoom + macro + respawn safety
-- Re-running this SAME build only reopens the current GUI instead of stacking another copy.
-- If another build is already active, v2.22 refuses to stack and asks for one clean rejoin.
do
    local BUILD = "v2.22-full-stability-audit"
    local ACTIVE = rawget(_G, "KimqHC_CurrentBuild")

    if ACTIVE == BUILD then
        pcall(function()
    ...
    -- Use the stable CurrentBuild flag instead of a version-specific key. That means a
    -- v2.22/v2.20 runtime is also detected instead of accidentally stacking with v2.22.
    if ACTIVE and ACTIVE ~= BUILD then
        pcall(function()
            game:GetService("StarterGui"):SetCore("SendNotification", {
                Title = "Kimqetras HC",
                Text = "Another Kimqetras HC runtime is active. Rejoin once, then run v2.22.",
                Duration = 7,
            })
        end)
        warn("[Kimqetras HC v2.22] Existing runtime detected; refusing to stack versions.")
        return
    end

    _G.KimqShotCameraSwapEnabled = false
    _G.KimqJumpZoomEnabled = true
    _G.KimqHC_SingleRuntimeOptimized = true
    _G.KimqHC_v22_PerformanceLoaded = true
...
-- v2.22 keeps the working bullet/glow/camera/credits systems and audits the movement, macro, target-jump, and respawn paths.
...
_G.KimqJumpZoomEnabled = _G.KimqJumpZoomEnabled ~= false
...
local function currentEquippedGun()
    local char=lp.Character
    if not char then equippedCameraGun=nil; return nil end
    if equippedCameraGun and equippedCameraGun.Parent==char and isLikelyGun(equippedCameraGun) then
        return equippedCameraGun
    end
    for _,ch in ipairs(char:GetChildren()) do
        if ch:IsA("Tool") and isLikelyGun(ch) then
            equippedCameraGun=ch
            return ch
        end
    end
...
local function gunIsActuallyEquipped(tool)
    local current=currentEquippedGun()
    return tool~=nil and current==tool
end
...
-- Extra camera reaction: while Camera Swap is ON and a gun is actually equipped,
-- the opponent closest to your crosshair can trigger a smooth zoom-in when they jump.
-- This does NOT rotate/lock your camera. Manual mouse-wheel zoom still cancels it instantly.
addToggle("Target Jump Zoom", _G.KimqJumpZoomEnabled, function(v)
    _G.KimqJumpZoomEnabled = v == true
end)
...
local jumpStatusCard=createCard(38)
local jumpStatusLabel=Instance.new("TextLabel",jumpStatusCard)
jumpStatusLabel.Size=UDim2.new(1,-20,1,0)
jumpStatusLabel.Position=UDim2.fromOffset(10,0)
jumpStatusLabel.BackgroundTransparency=1
jumpStatusLabel.Text="Jump Zoom: waiting"
jumpStatusLabel.TextColor3=Color3.fromRGB(197,112,145)
jumpStatusLabel.Font=Enum.Font.GothamSemibold
jumpStatusLabel.TextSize=11
jumpStatusLabel.TextXAlignment=Enum.TextXAlignment.Left
jumpStatusLabel.TextWrapped=true
local lastJumpStatus=""
local function setJumpStatus(s)
    s=tostring(s or "")
    if s==lastJumpStatus then return end
    lastJumpStatus=s
    if jumpStatusLabel and jumpStatusLabel.Parent then jumpStatusLabel.Text=s end
end
...
-- Target Jump Zoom ---------------------------------------------------------
-- v2.22: uses the same screen-coordinate system as the working Silent Aim path,
-- automatically repairs stale gun tracking, and zooms to Camera Zoom Min instead
-- of forcing 0.5 first person. It never rotates/locks the camera.
local Players = C.Players
local cam = C.cam or workspace.CurrentCamera
local jumpZoomTrackedPlayer = nil
local jumpZoomWasAirborne = false
local jumpZoomLastTrigger = 0
local jumpZoomAccumulator = 0
local jumpZoomPartNames={"Head","UpperTorso","HumanoidRootPart","LowerTorso"}

local function jumpPlayerAlive(plr)
    if not plr or plr==lp then return false end
    local char=plr.Character
    local hum=char and char:FindFirstChildOfClass("Humanoid")
    local root=char and char:FindFirstChild("HumanoidRootPart")
    return char~=nil and hum~=nil and root~=nil and hum.Health>0
end

local function jumpSameTeam(plr)
    if not cfg.silentAimTeamCheck then return false end
    if not plr or not lp then return false end
    if lp.Team~=nil and plr.Team~=nil then return lp.Team==plr.Team end
    if lp.TeamColor~=nil and plr.TeamColor~=nil then return lp.TeamColor==plr.TeamColor end
    return false
end

local function jumpCursorPoint()
    cam=workspace.CurrentCamera or cam
    if UIS.MouseBehavior==Enum.MouseBehavior.LockCenter and cam then
...
local function screenDistanceFor(plr)
    if not jumpPlayerAlive(plr) then return nil end
    local char=plr.Character
    cam=workspace.CurrentCamera or cam
    if not cam then return nil end
    local root=char:FindFirstChild("HumanoidRootPart")
    local maxWorld=math.max(50,tonumber(cfg.silentAimMaxDist) or 1000)
    if not root or (root.Position-cam.CFrame.Position).Magnitude>maxWorld then return nil end
    local cursor=jumpCursorPoint()
    local best=math.huge
    for _,name in ipairs(jumpZoomPartNames) do
...
local function getJumpZoomTarget()
    local baseFov=tonumber(cfg.silentAimFOV) or 150
    local radius=math.clamp(baseFov*1.8,140,420)

    if jumpZoomTrackedPlayer and jumpPlayerAlive(jumpZoomTrackedPlayer) and not jumpSameTeam(jumpZoomTrackedPlayer) then
        local d=screenDistanceFor(jumpZoomTrackedPlayer)
        if d and d<=radius*1.25 then return jumpZoomTrackedPlayer end
    end

    local best,bestDist=nil,radius
    for _,plr in ipairs(Players:GetPlayers()) do
        if plr~=lp and not (_G.KHWhitelist and _G.KHWhitelist[plr.UserId]) and not jumpSameTeam(plr) then
            local d=screenDistanceFor(plr)
            if d and d<bestDist then best,bestDist=plr,d end
        end
    end
    return best
end

local function humanoidIsAirborne(hum,root)
    ...
    if ok and typeof(vel)=="Vector3" then
        return hum.FloorMaterial==Enum.Material.Air and math.abs(vel.Y)>1.35
    end
    return hum.FloorMaterial==Enum.Material.Air
end

local function triggerJumpZoom(targetPlayer)
    if not _G.KimqJumpZoomEnabled or not _G.KimqShotCameraSwapEnabled then return end
    local tool=currentEquippedGun()
    if not tool then setJumpStatus("Jump Zoom: equip a gun"); return end

    local now=os.clock()
    if now-jumpZoomLastTrigger<0.20 then return end
    jumpZoomLastTrigger=now
    ...
    if d>1.1 then lastThirdDistance=math.clamp(d,zmin,zmax) end
    if d<=zmin+0.20 then
        setJumpStatus("Jump Zoom: jump detected • already zoomed in")
        return
    end

    cameraSwapToken+=1
    local token=cameraSwapToken
    setJumpStatus("Jump Zoom: "..tostring(targetPlayer and targetPlayer.Name or "target").." jumped • zooming in")
    task.spawn(function()
        smoothZoomTo(zmin,token,tool)
    end)
end

RunService.RenderStepped:Connect(function(dt)
    jumpZoomAccumulator+=dt
    if jumpZoomAccumulator<0.04 then return end
    jumpZoomAccumulator=0

    if not _G.KimqJumpZoomEnabled then
        jumpZoomTrackedPlayer=nil; jumpZoomWasAirborne=false
        setJumpStatus("Jump Zoom: off")
        return
    end
    if not _G.KimqShotCameraSwapEnabled then
        jumpZoomTrackedPlayer=nil; jumpZoomWasAirborne=false
        setJumpStatus("Jump Zoom: turn Camera Swap on")
        return
    end
    local tool=currentEquippedGun()
    if not tool then
        jumpZoomTrackedPlayer=nil; jumpZoomWasAirborne=false
        setJumpStatus("Jump Zoom: equip a gun")
        return
    end

    local target=getJumpZoomTarget()
    if not target or not target.Character then
        jumpZoomTrackedPlayer=nil; jumpZoomWasAirborne=false
        setJumpStatus("Jump Zoom: aim near a player")
        return
    end
    ...
    local airborne=humanoidIsAirborne(hum,root)

    if jumpZoomTrackedPlayer~=target then
        jumpZoomTrackedPlayer=target
        jumpZoomWasAirborne=airborne
        setJumpStatus("Jump Zoom: watching "..target.Name)
        return
    end

    if airborne and not jumpZoomWasAirborne then
        triggerJumpZoom(target)
...
-- ========================================================
-- MACRO / SPEED - v2.22 reliability pass
-- ========================================================
...
local statusCard=createCard(46)
local statusButton=Instance.new("TextButton",statusCard)
statusButton.Size=UDim2.new(1,-20,0,30)
statusButton.Position=UDim2.fromOffset(10,8)
statusButton.BorderSizePixel=0
statusButton.AutoButtonColor=false
statusButton.Font=Enum.Font.GothamSemibold
statusButton.TextSize=11
Instance.new("UICorner",statusButton).CornerRadius=UDim.new(0,8)
local function paintMacroStatus()
    local p=_G.KimqThemeLivePalette
    local hot=(p and p.hot) or Color3.fromRGB(243,161,211)
    local soft=(p and (p.bg2 or p.bg)) or Color3.fromRGB(236,255,243)
    local text=(p and p.text) or Color3.fromRGB(82,116,94)
    statusButton.BackgroundColor3=MacroActive and hot or soft
    statusButton.TextColor3=MacroActive and Color3.new(1,1,1) or text
    if MacroActive then
        statusButton.Text="Macro ACTIVE • click or press "..MacroKey.Name.." to stop"
    elseif MacroMaster then
        statusButton.Text="Macro READY • click or press "..MacroKey.Name.." to start"
    else
        statusButton.Text="Macro OFF • turn Master on first"
    end
end
...
local function setMacroActive(v)
    v=(v==true) and MacroMaster
    if MacroActive==v then paintMacroStatus(); return end
    MacroActive=v
    ...
    if MacroActive then
        ...
        if lastHumanoid and lastHumanoid.Parent and baselineWalkSpeed then
            pcall(function() lastHumanoid.WalkSpeed=baselineWalkSpeed end)
        end
        lastHumanoid=nil
        baselineWalkSpeed=nil
...
addToggle("Macro / Speed Master", MacroMaster, function(v)
    MacroMaster=v
    if not MacroMaster then setMacroActive(false) else paintMacroStatus() end
end)

addKeybind("Macro Key", MacroKey, function(v)
    MacroKey=v
    MacroKeyChangedAt=os.clock()
    paintMacroStatus()
end)
...
addSlider("Macro Speed",16,1000,MacroSpeed,function(v)
    ...
    if MacroActive then
        local hum=currentHumanoid()
        if hum then pcall(function() hum.WalkSpeed=MacroSpeed end) end
    end
end)
...
-- Do not trust gameProcessed for the macro key: some games mark gameplay keyboard
-- input as processed. Only block the key while the user is actually typing.
UIS.InputBegan:Connect(function(input)
    if input.UserInputType~=Enum.UserInputType.Keyboard then return end
    if UIS:GetFocusedTextBox() then return end
    if input.KeyCode==MacroKey and os.clock()-MacroKeyChangedAt>0.15 then
        if MacroMaster then setMacroActive(not MacroActive) end
    end
end)
...
local function sendMacroIO(keyCode)
    ...
    -- Executor-native input is more reliable for injected scripts. VIM is fallback.
    if type(keypress)=="function" and type(keyrelease)=="function" then
        local ok=pcall(function()
            keypress(vk)
            task.delay(.006,function() pcall(function() keyrelease(vk) end) end)
        end)
...
RunService.Heartbeat:Connect(function(dt)
    if MacroMaster and MacroActive then
        MacroSpeedAccumulator+=dt
        if MacroSpeedAccumulator>=0.025 then
            MacroSpeedAccumulator=0
            local hum=currentHumanoid()
            if hum then
                ...
                if hum.WalkSpeed~=MacroSpeed then pcall(function() hum.WalkSpeed=MacroSpeed end) end
            end
        end
    else
        MacroSpeedAccumulator=0
    ...
    if MacroMaster and MacroActive and MacroIOSpam then
        ...
        local sends=0
        while MacroIOAccumulator>=MacroIOInterval and sends<3 do
...
lp.CharacterAdded:Connect(function(char)
    ...
    task.defer(function()
        ...
        if hum and MacroActive and MacroMaster then
            lastHumanoid=hum
            baselineWalkSpeed=hum.WalkSpeed
            pcall(function() hum.WalkSpeed=MacroSpeed end)
        end
...
_G.KimqMacroController={
    IsActive=function() return MacroActive end,
    SetActive=setMacroActive,
    GetSpeed=function() return MacroSpeed end,
}
...
local function hookExtraAntiFall(char)
    ...
    extraAntiFallStateConn=hum.StateChanged:Connect(function(_,newState)
        -- Fresh characters legitimately pass through unstable physics states. Forcing
        -- GettingUp during that window can launch the rig, so Anti Fall waits briefly.
        if os.clock()-spawnedAt < 2.0 then return end
...
task.spawn(function()
    ...
    do
        ...
        if spawnPage then
            ...
            local function moveCharacter(char)
                if not useSelectedSpawn or not char or char~=lp.Character then return false end
                ...
                local root=char:FindFirstChild("HumanoidRootPart") or char:WaitForChild("HumanoidRootPart",4)
                ...
                local extentsY=5
                pcall(function() extentsY=math.max(4,char:GetExtentsSize().Y) end)
                local lift=math.max(4.75,extentsY*0.5+1.75,(hum and hum.HipHeight or 2)+2.5)
                local dest=cf*CFrame.new(0,lift,0)

                -- A raw PivotTo on a newly spawned unanchored rig can collide with the
                -- game's own spawn placement and create a huge upward impulse. Freeze the
                -- root and collisions for a few physics frames, move once, then restore.
                ...
                local wasAutoRotate=hum and hum.AutoRotate
                ...
                if ok then
                    for _=1,3 do
                        ...
                        if not char.Parent or char~=lp.Character then break end
                        zeroCharacterVelocity(char)
                    end
                end

                if char.Parent and char==lp.Character then
                    zeroCharacterVelocity(char)
                    pcall(function() root.Anchored=wasAnchored end)
                    if hum and hum.Parent then pcall(function() hum.AutoRotate=wasAutoRotate end) end
                    for part,state in pairs(collisionState) do
                        if part and part.Parent then pcall(function() part.CanCollide=state end) end
                    end
                    task.defer(function() if char and char.Parent then zeroCharacterVelocity(char) end end)
                end
                return ok
            end
            ...
            nowBtn.MouseButton1Click:Connect(function()
                ...
                local ok = moveCharacter(lp.Character)
            ...
            lp.CharacterAdded:Connect(function(char)
                ...
                if obj and obj:IsA("SpawnLocation") then
                    -- Real SpawnLocations are handled natively and need no teleport at all.
                    setRespawnLocationProperty(obj)
                    return
                end

                -- Generic map markers are client-only. Wait until the server has finished
                -- creating/placing the new character, then perform one frozen safe move.
                task.spawn(function()
                    local root=char:WaitForChild("HumanoidRootPart",5)
                    local hum=char:WaitForChild("Humanoid",5)
                    if not root or not hum then return end
                    local settleUntil=os.clock()+1.35
                    while os.clock()<settleUntil and char.Parent and char==lp.Character do
                        RunService.Heartbeat:Wait()
                    end
                    if char and char.Parent and char==lp.Character and useSelectedSpawn then
                        moveCharacter(char)
                    end
                end)
            end)
            ...
            _G.KimqSpawnPointController = {
                ...
                SpawnNow=function() return moveCharacter(lp.Character) end,
            }
...
-- ========================================================
-- v2.20 owner / developer credit guard
