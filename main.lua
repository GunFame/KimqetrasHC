-- Kimqetras HC v2.79 • v2.78 base + cached avatar template respawn lock (no network re-fetch on death)
...
local KIMQ_BUILD = "v2.79-cached-avatar-respawn-lock"
...
do
    ...
    if running and runningBuild == KIMQ_BUILD then
        if runningGui then
            ...
            notifyBoot((runtimeState=="ready") and "v2.79 is already running ♡" or "v2.79 is already starting ♡",4)
            return
        end
        ...
        notifyBoot("Recovering a failed v2.79 boot ♡",4)
    end
    ...
    if oldRuntimeDetected then
        ...
        if staleBoot then
            ...
            notifyBoot("STOP: another Kimqetras HC build is already active. v2.79 DID NOT LOAD. Rejoin once, then run only v2.79.",9)
            warn("[Kimqetras HC v2.79] Another build is active. v2.79 DID NOT LOAD; rejoin once before testing.")
            return
        end
    end
...
Hello.Text = "v2.79  •  cached avatar respawn lock ♡"
...
local function restoreKeptAvatar(char)
    ...
    -- Apply body/package/scales from the frozen description, then force the exact
    -- cached head, face, accessories and classic visuals on top just like manual Apply.
...
task.spawn(function()
    ...
    pcall(function()
        game:GetService("StarterGui"):SetCore("SendNotification",{
            Title="Kimqetras HC",
            Text="v2.79 ACTIVE ♡ • cached avatar respawn lock armed",
            Duration=7,
        })
    end)
