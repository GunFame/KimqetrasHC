-- Kimqetras HC v2.63 • stable RAGE target camera + Silent Aim overhaul
...
local KIMQ_BUILD = "v2.63-rage-stablecam-silentaim-overhaul"
...
do
    ...
    if running and runningBuild == KIMQ_BUILD then
        ...
        pcall(function()
            StarterGui0:SetCore("SendNotification",{
                Title="Kimqetras HC",
                Text=(runtimeState=="ready") and "v2.63 is already running ♡" or "v2.63 is already starting ♡",
                Duration=4,
            })
        end)
        return
    ...
    if oldRuntimeDetected then
        pcall(function()
            StarterGui0:SetCore("SendNotification",{
                Title="Kimqetras HC",
                Text="An older Kimqetras HC runtime is still active. Rejoin once, then run v2.63. v2.63 will not stack on top of it.",
                Duration=9,
            })
        end)
        warn("[Kimqetras HC v2.63] Older runtime detected. Refusing to stack; rejoin once.")
        return
    end
...
Hello.Text = "v2.63  •  rage camera + silent aim ♡"
...
local function cursorDistance(part)
    if not part or not part:IsA("BasePart") then return math.huge,false end
...
local function fovAllows(part,extraScale)
    local d,on=cursorDistance(part)
...
local function getClosestBodyPart(char)
    ...
    for _, pName in ipairs(bodyPartsList) do
        ...
        if part and part:IsA("BasePart") then
            local dist,on=cursorDistance(part)
            if on and dist < shortestDist then
                shortestDist = dist
                closestPart = part
            end
        end
...
local function getTargetPart(char)
    ...
    return char:FindFirstChild(cfg.silentAimPart)
...
local function validCandidate(pl,part,fovScale)
    if not pl or pl==lp or not part or not part.Parent then return false,nil,nil end
    ...
    local fovOK,cursorDist=fovAllows(part,fovScale)
    if not fovOK then return false,nil,nil end
    if wallBetween(part.Position,pl.Character) then return false,nil,nil end
...
local function getClosestPlayerToCursor()
    ...
    -- Keep the current target stable, but NEVER let stickiness defeat Strict FOV.
    ...
    if silentAimCachedPart and silentAimCachedPart.Parent then
        ...
        local ok,cursorDist,dist3D=validCandidate(stickyPlayer,silentAimCachedPart,stickyScale)
        if ok then
    ...
    for _,p in ipairs(Players:GetPlayers()) do
        if p~=lp and not _G.KHWhitelist[p.UserId] and isAlive(p) then
            ...
            local ok,cursorDist,dist3D=validCandidate(p,part,1)
            if ok then
    ...
    if stickyPart then
        if not bestPart then return stickyPart end
        local stick=math.clamp(tonumber(cfg.silentAimStickiness) or 0,0,80)/100
        -- Higher stickiness means a new target must be meaningfully better.
        local stealThreshold=stickyScore*(1-stick)
        if bestScore>=stealThreshold then return stickyPart end
    end
    return bestPart
end

local function predictedHitPosition(part)
...
pcall(function()
    ...
    _grm.__index = function(self, key)
        if not checkcaller() and self == mouse and cfg.silentAim and not isHoldingRevolver() then
            ...
            then
                local camera=workspace.CurrentCamera
                local hitPos=predictedHitPosition(part)
...
-- The old cfg already had a Silent Aim key but never actually used it.
UIS.InputBegan:Connect(function(input,gpe)
    if gpe or not cfg.useKeybind then return end
    if input.UserInputType==Enum.UserInputType.Keyboard and input.KeyCode==cfg.silentAimKey then
        local wanted=not cfg.silentAim
        local registry=_G.KimqConfigControls
        local ctl=registry and registry["Silent Aim"]
        if ctl and type(ctl.set)=="function" then
            pcall(ctl.set,wanted)
        else
            cfg.silentAim=wanted
        end
    end
end)
...
addToggle("Use Silent Aim Keybind", cfg.useKeybind, function(v) cfg.useKeybind=v end)
addKeybind("Silent Aim Key", cfg.silentAimKey, function(v) cfg.silentAimKey=v end)
...
task.spawn(function()
    ...
    task.defer(function()
        local rageOK,rageERR=pcall(function()
            ...
            local function button(parent,text,pos,size,fn)
                ...
                b.MouseButton1Click:Connect(function()
                    ...
                    if not ok then warn("[Kimqetras HC v2.63 RAGE button] "..tostring(err)) end
                end)
                return b
            end
            ...
            local function startOrbitMotion()
                ...
                orbitMotionConn=RunServiceR.Heartbeat:Connect(function(dt)
                    ...
                    if not ok then
                        warn("[Kimqetras HC v2.63 RAGE orbit] "..tostring(err))
                    end
                end)
            end
            ...
            RunServiceR.RenderStepped:Connect(function(dt)
                ...
                if not ok then
                    warn("[Kimqetras HC v2.63 RAGE camera] "..tostring(err))
                end
            end)
            ...
            RunServiceR.Heartbeat:Connect(function(dt)
                ...
                if not ok then
                    warn("[Kimqetras HC v2.63 RAGE heartbeat] "..tostring(err))
                end
            end)
        ...
        if not rageOK then
            warn("[Kimqetras HC v2.63] RAGE module isolated error: "..tostring(rageERR))
        end
    end)
