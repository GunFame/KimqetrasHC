...
task.spawn(function()
    ...
    local function hasAnimatedVisuals(obj)
        ...
        for _,d in ipairs(obj:GetDescendants()) do
            if d:IsA("ParticleEmitter") or d:IsA("Trail") or d:IsA("Beam") or d:IsA("AnimationController") or d:IsA("Animator") or d:IsA("Animation") or d:IsA("Constraint") or d:IsA("BodyMover") or d:IsA("Motor6D") or d:IsA("Bone") or d:IsA("LocalScript") or d:IsA("Script") then return true end
        end
        return false
    end
    ...
    local function playEmbeddedAnimations(cloneObj)
        if not cloneObj then return 0 end
        local animations={}
        for _,d in ipairs(cloneObj:GetDescendants()) do
            if d:IsA("Animation") and d.AnimationId~="" then table.insert(animations,d) end
        end
        if #animations==0 then return 0 end
        local animator=getOrCreateAnimator(cloneObj)
        if not animator then return 0 end

        local preferred={}
        for _,anim in ipairs(animations) do if animationLooksCosmetic(anim.Name) then table.insert(preferred,anim) end end
        local toPlay=(#preferred>0 and preferred) or (#animations==1 and animations or {})
        local count=0
        for _,anim in ipairs(toPlay) do
            local ok,track=pcall(function() return animator:LoadAnimation(anim) end)
            if ok and track then
                -- These are cosmetic/non-combat tracks; loop them so an unnamed
                -- skin animation does not play once and immediately stop.
                pcall(function() track.Looped=true end)
                pcall(function() track:Play(.08,1,1) end)
                count+=1
            end
        end
        return count
    ...
    local function applySkin(w,s,tool,quiet)
        ...
        local embedded=playEmbeddedAnimations(visual)
        ...
        if not quiet then
            ...
            if clientScripts>0 then detail=" • client animation script"..(clientScripts==1 and "" or "s").." kept"
        ...
        for _,d in ipairs(main:GetDescendants()) do
            if not hasPreviewAncestor(d) then
                if d:IsA("TextButton") then
                    ...
                    if low:find("apply",1,true) or low:find("selected",1,true) then
