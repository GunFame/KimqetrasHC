-- Kimqetras HC v2.74 • v2.73 base + avatar respawn persistence + dedicated RAGE presets page
...
local KIMQ_BUILD = "v2.74-avatar-respawn-rage-presets"
...
do
    ...
    if running and runningBuild == KIMQ_BUILD then
        if runningGui then
            ...
            notifyBoot((runtimeState=="ready") and "v2.74 is already running ♡" or "v2.74 is already starting ♡",4)
            return
        end
        ...
        notifyBoot("Recovering a failed v2.74 boot ♡",4)
    end
    ...
    if oldRuntimeDetected then
        ...
        if staleBoot then
            ...
            notifyBoot("Another Kimqetras HC runtime is still active. Rejoin once, then run v2.74.",9)
            warn("[Kimqetras HC v2.74] Older/live runtime detected. Refusing to stack; rejoin once.")
            return
        end
    end
...
Hello.Text = "v2.74  •  avatar persistence + RAGE presets ♡"
...
local function applyAvatarSnapshot(state,char,quiet)
    ...
    -- Build the fallback dummy from the SAVED description itself, never the source
    -- user's current avatar. This keeps frozen avatar configs actually frozen.
    local dummy=AvatarVisual.createDummyFromDescription(desc,humanoid.RigType)
    local ok=AvatarVisual.applyDescription(humanoid,desc)
    ...
    if ok then
        ...
        task.defer(function()
            ...
            if char and char.Parent then
                AvatarVisual.finish(char,humanoid,dummy)
        ...
        if dummy then pcall(function() dummy:Destroy() end) end
        if not quiet then setAvatarStatus("Saved avatar snapshot could not be applied",false) end
    end
    return ok
end
...
local function restoreKeptAvatar(char)
    if not AvatarEnabled or not char or char~=lp.Character then return end
...
lp.CharacterAdded:Connect(function(char)
    ...
    task.spawn(function()
        ...
        -- First restore quickly, then repeat after the game's own appearance scripts
        -- have had time to finish. The repeats are bounded; there is no permanent loop.
        task.wait(.30)
...
-- Roblox/custom games can finish loading the default character appearance AFTER
-- CharacterAdded. Re-apply once at that exact point so it cannot wipe the copied avatar.
lp.CharacterAppearanceLoaded:Connect(function(char)
    if not AvatarEnabled or char~=lp.Character then return end
...
task.spawn(function()
    ...
    local pageDefs = {
        ...
        {"ragepresets", "rage presets", "one-tap setups for solo targets, queue sweeps, orbit, or calm mode", "RAGE"},
    ...
    task.defer(function()
        local rageOK,rageERR=pcall(function()
            ...
            do
            local ragePresetCard=card(pages.ragepresets,170)
            ...
            local ragePresetSub=role(label(
                ragePresetCard,"pick a preset here • only RAGE settings change • your other sections stay untouched",
