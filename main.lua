-- Kimqetras HC v2.24 • stable rebuild from the known-good v2.19 base
-- Keeps the working bullet/knife/macro/camera/config systems, rebuilds spawn handling,
-- adds locked Day/Night + Night Lights, and adds smooth double-jump Angel Wings flight.
-- Re-running this SAME build only reopens the current GUI instead of stacking another copy.
-- If another build is already active, v2.24 refuses to stack and asks for one clean rejoin.
do
    local SINGLE_KEY = "KimqHC_v21_SingleRuntimeOptimized"
    local BUILD = "v2.24-stable-day-night-wing-flight"
    ...
    -- Older builds did not expose a complete disconnect/cleanup handle. Starting v2.24
    ...
    if rawget(_G, SINGLE_KEY) and rawget(_G, "KimqHC_CurrentBuild") ~= BUILD then
        pcall(function()
            game:GetService("StarterGui"):SetCore("SendNotification", {
                Title = "Kimqetras HC",
                Text = "Older runtime detected. Rejoin once, then run v2.24 so only one version is active.",
                Duration = 7,
            })
        end)
        warn("[Kimqetras HC v2.24] Older runtime detected. Rejoin once before running v2.24; refusing to stack versions.")
        return
    end
...
-- v2.24 is intentionally rebuilt from v2.19, the last build reported as broadly working.
...
-- ========================================================
-- MACRO / SPEED - MERGED FROM KIM.MAIN MISC > SPEED
-- ========================================================
...
-- Optional Hood-style I/O key spam. It only runs while the macro itself is active.
...
MacroHint.Text = "Turn Master on, then press the Macro Key. Optional I/O zoom spam works like before and stops with the macro."
...
local function sendMacroIO(keyCode)
    -- VirtualInputManager is the cleanest path; executor keypress/keyrelease are fallbacks.
    local sent=false
...
local _, updateConfigButton = addButton("Update Selected Config", function()
    ...
    -- SaveConfig overwrites the existing file with the current live settings.
    configName = selectedConfig
    configBox.Text = selectedConfig
    if KIM.SaveConfig and KIM.SaveConfig(selectedConfig) then
