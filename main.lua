-- Kimqetras HC v2.44 • Potassium shared-env + direct weapon list repair
...
local KIMQ_BUILD = "v2.44-potassium-direct-extras"
...
do
    ...
    if running and runningBuild == KIMQ_BUILD then
        ...
        pcall(function()
            StarterGui0:SetCore("SendNotification",{
                Title="Kimqetras HC",
                Text=(runtimeState=="ready") and "v2.44 is already running ♡" or "v2.44 is already starting ♡",
                Duration=4,
            })
        end)
        return
    ...
    if oldRuntimeDetected then
        pcall(function()
            StarterGui0:SetCore("SendNotification",{
                Title="Kimqetras HC",
                Text="An older Kimqetras HC runtime is still active. Rejoin once, then run v2.44. v2.44 will not stack on top of it.",
                Duration=9,
            })
        end)
        warn("[Kimqetras HC v2.44] Older runtime detected. Refusing to stack; rejoin once.")
        return
    end
    ...
    local knownRenderBindings = {
        "KimqMacroSpeedV240","KimqMacroSpeedV241","KimqMacroSpeedV242","KimqMacroSpeedV244",
        ...
        "KimqPermanentTimeV241","KimqPermanentTimeV242","KimqPermanentTimeV244",
    }
...
pcall(function()
    RunService:BindToRenderStep("KimqMacroSpeedV244",Enum.RenderPriority.Last.Value,function()
        enforceMacroSpeed()
    end)
end)
...
local function addCleanSection(title, subtitle)
    ...
    -- v2.44: pageHead already displays the section name.
...
task.spawn(function()
    ...
    -- v2.44: pages are grouped by what they actually do so the sidebar is
...
-- v2.44 module: local catalog / limited accessory try-on.
task.spawn(function()
    _G.KimqAccessoryUIReady=false
    local Players = game:GetService("Players")
    local CoreGui = game:GetService("CoreGui")
...
-- v2.44 module: Environment + Weapon Skins (canonical sidebar pages).
task.spawn(function()
    local pageReadyStart=tick()
    while not _G.KimqBasePagesReady and tick()-pageReadyStart<8 do task.wait(.02) end
    ...
    -- v2.44: Environment and Weapon Skins are already real canonical pages/buttons.
    ...
    pcall(function()
        RunService:BindToRenderStep("KimqPermanentTimeV244",Enum.RenderPriority.Last.Value,function()
    ...
    if not envBuildOk then
        warn("[Kimqetras HC v2.44] Environment page fallback: "..tostring(envBuildErr))
    ...
    local function setupWeaponExtras()
        ...
        selectedBeamSource=function()
            ...
            local root=ReplicatedStorage:FindFirstChild("BulletBeams") or findFolder({"BulletBeams","Bullet Beams","BulletBeam"})
        ...
        -- v2.44: this refresh sits immediately before the old stable
        ...
        if not refreshOk then
            warn("[Kimqetras HC v2.44] bullet refresh skipped: "..tostring(refreshErr))
        end
        end
        ...
        local function scanKnives()
            ...
            local root=ReplicatedStorage:FindFirstChild("Knives") or findFolder({"Knives","KnifeSkins","Knife Skins"}); local names={"None"}
        ...
        local function scanEquipables()
            ...
            local root=ReplicatedStorage:FindFirstChild("EquipableItem") or ReplicatedStorage:FindFirstChild("EquippableItem") or findFolder({"EquipableItem","EquippableItem","EquipableItems","EquippableItems"}); local names={"None"}
        ...
        local function scanAllExtras()
            ...
            local okBeam,errBeam=pcall(bulletAPI.scanBeams)
            if not okBeam then
                table.insert(errors,"BulletBeams: "..tostring(errBeam))
                beamStatus.Text="Bullet Beams scan failed"
                beamStatus.TextColor3=pcolor("sub",subColor)
            end

            local okKnife,errKnife=pcall(scanKnives)
            if not okKnife then
                table.insert(errors,"Knives: "..tostring(errKnife))
                knifeStatus.Text="Knife scan failed"
                knifeStatus.TextColor3=pcolor("sub",subColor)
            end

            local okEquip,errEquip=pcall(scanEquipables)
            if not okEquip then
                table.insert(errors,"Equippables: "..tostring(errEquip))
                equipStatus.Text="Equippable scan failed"
                equipStatus.TextColor3=pcolor("sub",subColor)
            end
            ...
            if rootGui then
                ...
                rootGui:SetAttribute("KimqWeaponScanError",table.concat(errors," | "))
            end
    ...
    if not extrasOk then
        ...
        warn("[Kimqetras HC v2.44] Weapon extras: "..tostring(extrasErr))
        ...
        task.defer(function()
            ...
            if extras and type(extras.Refresh)=="function" then
                ...
                if not ok then
                    warn("[Kimqetras HC v2.44] extras refresh: "..tostring(err))
                end
            end
        end)
    end
...
task.spawn(function()
    ...
    -- v2.44: preserve the grouped sidebar order from pageDefs.
    ...
    local function applyTheme(name,settlePass)
        ...
        -- v2.44 hard guarantee: old controls created with literal pink/yellow
    ...
    -- v2.44: section navigation does not repaint/rescan the entire GUI.
    ...
    -- v2.44 reveal happens only after this initial theme pass is complete.
end)


-- v2.44: delayed post-show cleanup passes removed.
...
-- ========================================================
-- v2.44 branding is deterministic at construction time; no delayed scrub pass.

-- ========================================================
-- v2.44 DETERMINISTIC REVEAL
...
task.spawn(function()
    ...
    -- v2.44: remove the old one-page section divider labels from the
