-- Kimqetras HC V34 RECOVERY — based on the last user-confirmed working V22 build.
-- Feature/backend code is intentionally preserved; only boot timing/version and a safe right-click toggle are added.
...
do
    ...
    pcall(function()
        for _, root in ipairs({CoreGui, pg}) do
            for _, n in ipairs({"KimqV22CuteLoader", "KimqV21CuteLoader", "KimpetrasHC_Boot", "KimqV4Loader"}) do
    ...
    task.spawn(function()
        while gui.Parent and not _G.KimqV22Ready do
            pcall(function()
                for _, root in ipairs({CoreGui, pg}) do
                    for _, n in ipairs({"KimqV21CuteLoader","KimpetrasHC_Boot","KimqV4Loader","KimpetrasHC_Loading"}) do
                        local x = root:FindFirstChild(n)
    ...
    local steps = {
        {"loading your profile...",.16,.35},
        {"organizing the feature pages...",.31,.45},
        {"matching your theme...",.46,.45},
        {"setting up seasonal effects...",.61,.45},
        {"finding weapon wraps...",.76,.55},
        {"adding the finishing hearts...",.88,.40},
    }
    task.spawn(function()
        for _,s in ipairs(steps) do
            if not gui.Parent or _G.KimqV22Ready then break end
            status.Text = s[1]
    ...
    _G.KimqV22Loader = {Gui=gui,Status=status,Bar=bar,Tip=tip,Background=bg,Sticker=sticker}
...
task.spawn(function()
    ...
    if not pageHost then
        _G.KimqV22Ready=true; main.Visible=true; if loader and loader.Gui then loader.Gui:Destroy() end; return
    end
    ...
    if not inactiveTemplate then _G.KimqV22Ready=true; main.Visible=true; if loader and loader.Gui then loader.Gui:Destroy() end; return end
...
-- V34 recovery micro-patch: only adds Right Click hide/show and forces the visible version badge.
task.spawn(function()
    local Players = game:GetService("Players")
    local CoreGui = game:GetService("CoreGui")
    local UIS = game:GetService("UserInputService")
    ...
    local root, main
    repeat
        root = CoreGui:FindFirstChild("KimpetrasHC") or pg:FindFirstChild("KimpetrasHC")
    ...
    if not main:GetAttribute("KimqV34RightClick") then
        main:SetAttribute("KimqV34RightClick", true)
