-- V26 decal refresh: original V26 GUI/features, real uploaded decals, no stitching.

-- V26 decal asset replacements (Roblox Open Use assets)
local KIMQ_V26_ASSETS = {
    Pompompurin = "rbxassetid://98379642851874",
...
do
    ...
    pcall(function()
        for _, root in ipairs({CoreGui, pg}) do
            for _, name in ipairs({
                ...
                "KimqV22CuteLoader","KimqV21CuteLoader","KimpetrasHC_Boot","KimpetrasHC_Loading","KimqV4Loader"
            }) do
    ...
    gui.Name = "KimqV26PompomLoader"
    gui.IgnoreGuiInset = true
    gui.ResetOnSpawn = false
    gui.DisplayOrder = 4000000
    pcall(function() gui.Parent = CoreGui end)
    if not gui.Parent then gui.Parent = pg end

    local HOT = Color3.fromRGB(82, 145, 255)
    local HOT2 = Color3.fromRGB(127, 181, 255)
    local CREAM = Color3.fromRGB(255, 251, 237)
    local CREAM2 = Color3.fromRGB(250, 246, 232)
    local LINE = Color3.fromRGB(132, 181, 246)
    local TEXT = Color3.fromRGB(68, 89, 126)
    local SUB = Color3.fromRGB(119, 132, 157)
    local YELLOW = Color3.fromRGB(255, 225, 118)
    local BROWN = Color3.fromRGB(118, 72, 59)

    local bg = Instance.new("Frame", gui)
    bg.Name = "Background"
    bg.Size = UDim2.fromScale(1,1)
    bg.BackgroundColor3 = CREAM2
    bg.BorderSizePixel = 0
    local bgGrad = Instance.new("UIGradient", bg)
    bgGrad.Color = ColorSequence.new({
        ColorSequenceKeypoint.new(0, Color3.fromRGB(231, 242, 255)),
        ColorSequenceKeypoint.new(.44, Color3.fromRGB(247, 249, 240)),
        ColorSequenceKeypoint.new(1, Color3.fromRGB(225, 239, 255)),
    })
    bgGrad.Rotation = 18
    ...
    local function stitches(parent,inset,color)
        local frame=Instance.new("Frame",parent)
        frame.Name="StitchesRemoved"
    ...
    local function mascot(parent,pos,size)
        local img=Instance.new("ImageLabel",parent)
        img.Name="PompomMascot"
        img.AnchorPoint=Vector2.new(.5,.5)
        img.Position=pos
        img.Size=UDim2.fromOffset(size,size)
        img.BackgroundTransparency=1
        img.Image=KIMQ_V26_ASSETS.Pompompurin
        img.ScaleType=Enum.ScaleType.Fit
        img.ImageColor3=Color3.new(1,1,1)
        return img
    end

    -- soft floating blue shapes behind the card
    for _,d in ipairs({{.10,.18,160,.82},{.88,.18,130,.86},{.08,.78,190,.82},{.90,.76,175,.84},{.50,.08,230,.91}}) do
        local b=Instance.new("Frame",bg); b.AnchorPoint=Vector2.new(.5,.5); b.Position=UDim2.new(d[1],0,d[2],0); b.Size=UDim2.fromOffset(d[3],d[3]); b.BackgroundColor3=HOT2; b.BackgroundTransparency=d[4]; b.BorderSizePixel=0; corner(b,999)
    end

    local card=Instance.new("Frame",bg)
    card.Name="Card"
    card.AnchorPoint=Vector2.new(.5,.5); card.Position=UDim2.fromScale(.5,.5); card.Size=UDim2.fromOffset(720,430)
    card.BackgroundColor3=CREAM; card.BorderSizePixel=0; corner(card,28); stroke(card,HOT,.18,3)
    stitches(card,13,HOT)
    paw(card,.055,.90,.9,HOT,-18); paw(card,.945,.90,.9,HOT,18)

    mascot(card,UDim2.new(0,100,0,90),126)
    local title=Instance.new("ImageLabel",card)
    title.Name="KimqetrasTitleDecal"
    title.BackgroundTransparency=1
    title.Position=UDim2.fromOffset(164,42)
    title.Size=UDim2.fromOffset(300,74)
    title.Image=KIMQ_V26_ASSETS.Title
    title.ScaleType=Enum.ScaleType.Fit
    local subtitle=Instance.new("ImageLabel",card)
    subtitle.Name="KimqetrasSubtitleDecal"
    subtitle.BackgroundTransparency=1
    subtitle.Position=UDim2.fromOffset(218,102)
    subtitle.Size=UDim2.fromOffset(205,48)
    subtitle.Image=KIMQ_V26_ASSETS.Subtitle
    subtitle.ScaleType=Enum.ScaleType.Fit

    local profile=Instance.new("Frame",card); profile.Size=UDim2.fromOffset(190,54); profile.Position=UDim2.new(1,-215,0,35); profile.BackgroundColor3=Color3.fromRGB(255,253,245); profile.BorderSizePixel=0; corner(profile,15); stroke(profile,LINE,.35,1)
    local avatar=Instance.new("ImageLabel",profile); avatar.Size=UDim2.fromOffset(42,42); avatar.Position=UDim2.fromOffset(7,6); avatar.BackgroundColor3=Color3.fromRGB(236,244,255); avatar.BorderSizePixel=0; corner(avatar,999)
    ...
    label(profile,lp.DisplayName,UDim2.new(1,-62,0,22),UDim2.fromOffset(57,6),Enum.Font.GothamBold,12,TEXT)
    label(profile,"@"..lp.Name,UDim2.new(1,-62,0,19),UDim2.fromOffset(57,27),Enum.Font.Gotham,10,SUB)

    local center=Instance.new("Frame",card); center.Size=UDim2.new(1,-86,0,158); center.Position=UDim2.fromOffset(43,166); center.BackgroundColor3=Color3.fromRGB(255,253,246); center.BorderSizePixel=0; corner(center,18); stroke(center,LINE,.42,1)
    local cSt=stitches(center,8,LINE); cSt.ZIndex=center.ZIndex+1
    paw(center,.07,.19,.55,HOT,-8)
    local status=label(center,"getting everything ready...",UDim2.new(1,-40,0,28),UDim2.fromOffset(20,26),Enum.Font.FredokaOne,19,TEXT,Enum.TextXAlignment.Center)
    local detail=label(center,"pages  •  themes  •  environment  •  weapon skins",UDim2.new(1,-40,0,20),UDim2.fromOffset(20,58),Enum.Font.GothamSemibold,11,SUB,Enum.TextXAlignment.Center)
    local track=Instance.new("Frame",center); track.Size=UDim2.new(1,-90,0,15); track.Position=UDim2.new(0,45,0,95); track.BackgroundColor3=Color3.fromRGB(226,238,250); track.BorderSizePixel=0; corner(track,999); stroke(track,LINE,.55,1)
    local bar=Instance.new("Frame",track); bar.Name="Bar"; bar.Size=UDim2.new(.05,0,1,0); bar.BackgroundColor3=HOT; bar.BorderSizePixel=0; corner(bar,999)
    local barGrad=Instance.new("UIGradient",bar); barGrad.Color=ColorSequence.new(HOT2,HOT); barGrad.Rotation=0
    local percent=label(center,"5%",UDim2.fromOffset(54,24),UDim2.new(1,-88,0,119),Enum.Font.FredokaOne,16,HOT,Enum.TextXAlignment.Center)

    local footer=Instance.new("Frame",card); footer.AnchorPoint=Vector2.new(.5,.5); footer.Position=UDim2.new(.5,0,1,-48); footer.Size=UDim2.fromOffset(245,44); footer.BackgroundColor3=Color3.fromRGB(255,253,246); footer.BorderSizePixel=0; corner(footer,16); stroke(footer,LINE,.35,1)
    paw(footer,.12,.5,.42,HOT,-8); paw(footer,.88,.5,.42,HOT,8)
    label(footer,"made with love ♡",UDim2.new(1,-60,1,0),UDim2.fromOffset(30,0),Enum.Font.FredokaOne,15,HOT,Enum.TextXAlignment.Center)

    local mascotScale=Instance.new("UIScale",card:FindFirstChild("PompomMascot"))
    task.spawn(function()
        local up=false
        while gui.Parent and not _G.KimqV26Ready do
            up=not up
            TweenService:Create(mascotScale,TweenInfo.new(.9,Enum.EasingStyle.Sine,Enum.EasingDirection.InOut),{Scale=up and 1.035 or 1}):Play()
            TweenService:Create(card,TweenInfo.new(.9,Enum.EasingStyle.Sine,Enum.EasingDirection.InOut),{Position=UDim2.new(.5,0,.5,up and -2 or 2)}):Play()
            task.wait(.88)
        end
    end)
    ...
    task.spawn(function()
        while gui.Parent and not _G.KimqV26Ready do
            pcall(function()
                for _,root in ipairs({CoreGui,pg}) do
                    for _,name in ipairs({"KimqV25CanvaLoader","KimqV24CuteLoader","KimqV23DreamyLoader","KimqV22CuteLoader","KimqV21CuteLoader","KimpetrasHC_Boot","KimpetrasHC_Loading","KimqV4Loader"}) do
                        local x=root:FindFirstChild(name); if x then x:Destroy() end
                    end
                    local rg=root:FindFirstChild("KimpetrasHC")
                    local m=rg and rg:FindFirstChild("Main")
    ...
    local steps={
        {"loading your profile...",.16,1.0},{"sorting the feature pages...",.31,1.35},{"stitching the interface...",.47,1.35},
        {"adding the paw prints...",.62,1.35},{"matching the recolors...",.76,1.35},{"polishing the banner...",.89,1.25},{"almost ready ♡",.97,.9}
    }
    task.spawn(function()
        for _,s in ipairs(steps) do
            if not gui.Parent or _G.KimqV26Ready then break end
            status.Text=s[1]
    ...
    _G.KimqV26Loader={Gui=gui,Background=bg,Card=card,Status=status,Bar=bar,Percent=percent}
    task.delay(50,function() if gui and gui.Parent then pcall(function() gui:Destroy() end) end end)
end
...
-- V26 decal/no-stitch GUI polish. Keeps the original V26 feature/backend structure intact.
task.spawn(function()
    local Players=game:GetService("Players")
    local CoreGui=game:GetService("CoreGui")
    local TweenService=game:GetService("TweenService")
    ...
    local t0=tick()
    while not _G.KimqV24Ready and tick()-t0<45 do task.wait(.12) end
    local root=CoreGui:FindFirstChild("KimpetrasHC") or pg:FindFirstChild("KimpetrasHC")
    local main=root and root:FindFirstChild("Main")
    ...
    if main:FindFirstChild("KimqV26PompomMarker") then
        _G.KimqV26Ready=true; main.Visible=true
        if loader and loader.Gui then pcall(function() loader.Gui:Destroy() end) end
        return
    end
    local marker=Instance.new("BoolValue",main); marker.Name="KimqV26PompomMarker"
    ...
    local function mascot(parent,pos,size,P)
        local img=Instance.new("ImageLabel",parent)
        img.Name="V26Mascot"
        img.AnchorPoint=Vector2.new(.5,.5)
        img.Position=pos
    ...
    local function stitches(parent,inset,P,name)
        ...
        f.Name=name or "V26StitchesRemoved"
    ...
    local function forceBadge()
        for _,d in ipairs(main:GetDescendants()) do if d:IsA("TextLabel") and tostring(d.Text or ""):match("^V%d+") then d.Text="V26 ♥" end end
    end
    forceBadge()

    local function palette()
        local hot=badge and badge.BackgroundColor3 or Color3.fromRGB(82,145,255)
        local ivory=Color3.fromRGB(255,251,238)
        local ivory2=Color3.fromRGB(250,246,232)
        local ink=Color3.fromRGB(70,74,93)
        return {
            hot=hot,
            hot2=hot:Lerp(Color3.new(1,1,1),.24),
            light=hot:Lerp(Color3.new(1,1,1),.76),
            line=hot:Lerp(Color3.new(1,1,1),.48),
            cream=ivory:Lerp(hot,.025),
            cream2=ivory2:Lerp(hot,.035),
            panel=Color3.fromRGB(255,253,246):Lerp(hot,.018),
            text=hot:Lerp(ink,.69),
            sub=hot:Lerp(Color3.fromRGB(117,115,121),.72),
            white=Color3.fromRGB(255,255,252),
        }
    end
    local P=palette()
    ...
    paw(main,UDim2.new(0,26,1,-24),30,P.hot,-22,24)
    paw(main,UDim2.new(1,-26,1,-24),30,P.hot,22,24)
    ...
    -- Top branding: same V26 placement, now using the user's real Roblox decals.
    local oldMascot=shell:FindFirstChild("V26TopMascot"); if oldMascot then oldMascot:Destroy() end
    local mascotWrap=Instance.new("Frame",shell); mascotWrap.Name="V26TopMascot"; mascotWrap.Size=UDim2.fromOffset(126,126); mascotWrap.Position=UDim2.fromOffset(0,-23); mascotWrap.BackgroundTransparency=1; mascotWrap.ZIndex=24
    mascot(mascotWrap,UDim2.fromScale(.5,.5),122,P)

    for _,d in ipairs(shell:GetDescendants()) do
        if d:IsA("TextLabel") then
            local n=norm(d.Text)
            if n=="kimqetras hc" and d.AbsolutePosition.Y<main.AbsolutePosition.Y+105 then
                d.Visible=false
            elseif tostring(d.Text or ""):lower():find("cute controls, clean pages",1,true) then
                d.Visible=false
            end
        end
    end
    local titleDecal=Instance.new("ImageLabel",shell)
    titleDecal.Name="V26TitleDecal"
    titleDecal.BackgroundTransparency=1
    titleDecal.Position=UDim2.fromOffset(116,2)
    titleDecal.Size=UDim2.fromOffset(286,72)
    titleDecal.Image=KIMQ_V26_ASSETS.Title
    titleDecal.ScaleType=Enum.ScaleType.Fit
    titleDecal.ZIndex=24
    role(titleDecal,"brandImage")
    local subtitleDecal=Instance.new("ImageLabel",shell)
    subtitleDecal.Name="V26SubtitleDecal"
    subtitleDecal.BackgroundTransparency=1
    subtitleDecal.Position=UDim2.fromOffset(172,55)
    subtitleDecal.Size=UDim2.fromOffset(190,46)
    subtitleDecal.Image=KIMQ_V26_ASSETS.Subtitle
    subtitleDecal.ScaleType=Enum.ScaleType.Fit
    subtitleDecal.ZIndex=24
    role(subtitleDecal,"brandImage")
        ...
        for _,d in ipairs(shell:GetDescendants()) do
            if d:IsA("TextLabel") and norm(d.Text)=="features" then
                ...
                if not d.Parent:FindFirstChild("V26FeaturePawL") then
                    local p1=paw(d.Parent,UDim2.new(0,24,.5,0),23,P.hot,-12,d.ZIndex+1); p1.Name="V26FeaturePawL"
                    local p2=paw(d.Parent,UDim2.new(1,-24,.5,0),23,P.hot,12,d.ZIndex+1); p2.Name="V26FeaturePawR"
                end
            end
        end
    end
    ...
    local function styleNav()
        for _,b in ipairs(navButtons) do
            ...
            local p=b:FindFirstChild("V26NavPaw")
            if not p then p=paw(b,UDim2.new(0,15,.5,0),17,selected and P.white or P.hot,-12,b.ZIndex+2); p.Name="V26NavPaw" end
            if p:IsA("ImageLabel") then
                p.ImageColor3=Color3.new(1,1,1)
                p.ImageTransparency=selected and .08 or 0
            end
        end
    end
    styleNav()
    ...
    local function stylePage(page)
        ...
        for _,ch in ipairs(page:GetChildren()) do
            if ch:IsA("Frame") then
                ...
                -- tiny paw by obvious section-title cards, not every simple row.
                local hasBigTitle=false
                for _,q in ipairs(ch:GetChildren()) do if q:IsA("TextLabel") and q.TextSize>=18 then hasBigTitle=true break end end
                if hasBigTitle and not ch:FindFirstChild("V26CardPaw") then local cp=paw(ch,UDim2.new(0,18,0,18),18,P.hot,-8,ch.ZIndex+2); cp.Name="V26CardPaw" end
    ...
    if overview and overview:IsA("ScrollingFrame") then
        ...
        paw(banner,UDim2.new(.18,.28,0,0),25,P.white,-12,banner.ZIndex+2); paw(banner,UDim2.new(.82,.28,0,0),25,P.white,12,banner.ZIndex+2)
        ...
        mascot(welcome,UDim2.new(1,-68,.53,0),91,P)

        local about=card(126,"V26About")
        paw(about,UDim2.new(0,24,0,24),23,P.hot,-8,about.ZIndex+2)
        local at=label(about,"about",UDim2.new(1,-100,0,28),UDim2.fromOffset(44,8),Enum.Font.FredokaOne,21,P.hot); role(at,"hotText")
        local div=label(about,"",UDim2.new(1,-110,0,16),UDim2.fromOffset(44,34),Enum.Font.GothamBold,11,P.line); role(div,"lineText")
        ...
        paw(about,UDim2.new(1,-38,.60,0),34,P.hot,15,about.ZIndex+2)

        local controls=card(82,"V26Controls")
        paw(controls,UDim2.new(0,24,.5,0),23,P.hot,-8,controls.ZIndex+2)
        local ct=label(controls,"controls",UDim2.new(0,160,0,28),UDim2.fromOffset(44,9),Enum.Font.FredokaOne,20,P.hot); role(ct,"hotText")
        ...
        for _,d in ipairs(main:GetDescendants()) do
            ...
            if r then
                ...
                if d:IsA("TextLabel") or d:IsA("TextButton") or d:IsA("TextBox") then
                    if r=="hotText" then d.TextColor3=P.hot elseif r=="subText" then d.TextColor3=P.sub elseif r=="textText" then d.TextColor3=P.text elseif r=="lineText" then d.TextColor3=P.line elseif r=="whiteText" then d.TextColor3=P.white end
                end
                if r=="stitch" and d:IsA("Frame") then d.Visible=false end
                if (r=="pawImage" or r=="mascotImage" or r=="brandImage") and d:IsA("ImageLabel") then
