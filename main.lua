-- Kimqetras HC v2.78 • v2.77 base + direct respawn reapply through the proven manual avatar path
...
local KIMQ_BUILD = "v2.78-direct-respawn-reapply"
...
do
    ...
    if running and runningBuild == KIMQ_BUILD then
        if runningGui then
            ...
            notifyBoot((runtimeState=="ready") and "v2.78 is already running ♡" or "v2.78 is already starting ♡",4)
            return
        end
        ...
        notifyBoot("Recovering a failed v2.78 boot ♡",4)
    end
    ...
    if oldRuntimeDetected then
        ...
        if staleBoot then
            ...
            notifyBoot("STOP: another Kimqetras HC build is already active. v2.78 DID NOT LOAD. Rejoin once, then run only v2.78.",9)
            warn("[Kimqetras HC v2.78] Another build is active. v2.78 DID NOT LOAD; rejoin once before testing.")
            return
        end
    end
...
Hello.Text = "v2.78  •  direct respawn avatar reapply ♡"
...
local function avatarVisualLooksIntact(char)
    ...
    if not activeAvatarSnapshot then return false end

    -- Every copied avatar build creates this exact visual head.  Its absence is
    -- the clearest sign that reset/game appearance code replaced our visuals.
    local visualHead=char:FindFirstChild("KimqAvatarVisualHead")
    ...
    local props=activeAvatarSnapshot.Properties or {}
    if tonumber(props.Shirt or 0)>0 and not char:FindFirstChildOfClass("Shirt") then return false end
    if tonumber(props.Pants or 0)>0 and not char:FindFirstChildOfClass("Pants") then return false end
    return true
end

local function restoreKeptAvatar(char)
    if not AvatarEnabled or not char or char~=lp.Character then return end
    if avatarVisualRepairBusy then return end
    avatarVisualRepairBusy=true
    avatarSuppressRepairUntil=os.clock()+1.35

    local restored=false

    -- v2.78: Manual Apply User Avatar is the proven-good path.  If the current
    -- copied avatar came from that button, use the SAME function after respawn.
    -- Earlier builds restored through applyAvatarSnapshot(), which is why a
    -- manual click worked while death/reset did not in this game.
    if avatarPersistenceMode=="user" and tonumber(activeAvatarUserId) then
        pcall(applyAvatarUser,tonumber(activeAvatarUserId),char,true)
        restored=true
    elseif activeAvatarSnapshot then
        local ok,result=pcall(applyAvatarSnapshot,activeAvatarSnapshot,char,true)
        ...
        if tostring(AvatarTarget or "")~="" then
            ...
            if userId then
                activeAvatarUserId=tonumber(userId)
                avatarPersistenceMode="user"
                pcall(applyAvatarUser,userId,char,true)
                restored=true
            end
        end
    end

    task.delay(restored and .65 or .85,function()
...
local function startKeptAvatarRespawnGuard(char)
    ...
    task.spawn(function()
        -- CharacterAdded can arrive while the LocalPlayer.Character reference and
        -- appearance are still settling.  Wait for the actual live character.
        local t0=os.clock()
        while os.clock()-t0<5 and (lp.Character~=char or not char.Parent) do task.wait(.05) end
        ...
        -- Fallback if CharacterAppearanceLoaded is late/missing.  We intentionally
        -- wait long enough that it normally does NOT collide with the event-based
        -- reapply below.
        task.wait(4.0)
        if serial~=avatarRespawnGuardSerial or not AvatarEnabled or lp.Character~=char then return end
        if not avatarVisualLooksIntact(char) then restoreKeptAvatar(char) end

        -- Lifetime repair remains cheap: only repair if our copied markers vanish.
    ...
    char.ChildRemoved:Connect(function(obj)
        ...
        task.delay(.18,function()
            if serial==avatarRespawnGuardSerial and AvatarEnabled and lp.Character==char
                and not avatarVisualLooksIntact(char) then
...
lp.CharacterAdded:Connect(function(char)
    ...
    -- New character = invalidate the old monitor immediately.
...
lp.CharacterAppearanceLoaded:Connect(function(char)
    if not AvatarEnabled or not char then return end
    task.spawn(function()
        -- This is deliberately the primary respawn restore.  Once Roblox says its
        -- normal appearance finished loading, put the copied avatar back on top.
        local t0=os.clock()
        while os.clock()-t0<3 and lp.Character~=char do task.wait(.05) end
        if not AvatarEnabled or lp.Character~=char then return end
        char:WaitForChild("Humanoid",6)
        char:WaitForChild("Head",6)
        task.wait(.22)
        if AvatarEnabled and lp.Character==char then restoreKeptAvatar(char) end
        -- One settle check catches games that do one extra appearance pass.
        task.wait(1.35)
...
task.spawn(function()
    ...
    pcall(function()
        game:GetService("StarterGui"):SetCore("SendNotification",{
            Title="Kimqetras HC",
            Text="v2.78 ACTIVE ♡ • copied avatar will reapply after respawn",
            Duration=7,
        })
    end)
