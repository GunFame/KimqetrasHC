-- Kimqetras HC v2.99 • v2.98 base + far-distance Mouse Aim coordinate/alignment fix
...
local KIMQ_BUILD = "v2.99-mouse-aim-far-distance-alignment"
...
Hello.Text = "v2.99  •  Mouse Aim far-distance alignment ♡"
...
local function passesHitChance(part)
    -- Mouse Aim Silent Correction is an explicit Q-lock. While it is active,
    -- do not let the normal random Hit Chance roll make some locked shots miss.
    -- Outside Mouse Aim, the original Silent Aim Hit Chance behavior is unchanged.
    local mouseAimForce=rawget(_G,"KimqMouseAimForceCorrection")
    if type(mouseAimForce)=="function" then
        local ok,force=pcall(mouseAimForce)
        if ok and force then return true end
    end
...
RunService.RenderStepped:Connect(function()
    ...
    -- v2.91 Mouse Aim bridge.  This only overrides WHO Silent Aim resolves while
    -- Mouse Aim is actively Q-locked with Silent Correction enabled.  With the
    -- bridge inactive, the exact v2.89 resolver runs untouched.
    local overridePart,overridePoint=nil,nil
    local mouseAimOverride=rawget(_G,"KimqMouseAimOverride")
    if type(mouseAimOverride)=="function" then
        local ok,p,pt=pcall(mouseAimOverride)
        if ok and p and p:IsA("BasePart") and p.Parent and typeof(pt)=="Vector3" then
            overridePart,overridePoint=p,pt
        end
    end
    if overridePart then
        silentAimCachedPart,silentAimCachedPoint=overridePart,overridePoint
    else
        local suppressFallback=false
        local suppressFn=rawget(_G,"KimqMouseAimSuppressFallback")
        if type(suppressFn)=="function" then
            local ok,v=pcall(suppressFn)
            suppressFallback=ok and v==true
        end
        if suppressFallback then
            silentAimCachedPart,silentAimCachedPoint=nil,nil
        else
...
if not runChunk("mouse_aim", [=====[
local C=_G.KimpetrasCtx
if not C then error("Kimqetras core context missing") end
local UIS,Players,RunService,lp=C.UIS,C.Players,C.RunService,C.lp
local addToggle,addSlider,addDropdown,addKeybind=C.addToggle,C.addSlider,C.addDropdown,C.addKeybind
local cfg=C.cfg
_G.KimqBuildSection="mouseaim"

-- v2.99 Mouse Aim 2.1
-- Far-distance alignment fix. Tracking math now lives entirely in VIEWPORT
-- coordinates (Mouse.X/Y + WorldToViewportPoint), so the Roblox top-bar / GUI
-- inset cannot create a constant vertical offset that only becomes obvious when
-- a distant character is tiny on screen. Relative mouse movement remains the
-- same physical-pointer backend and the camera is never rotated.

if cfg.mouseAimEngineVersion~="v2.99" then
    -- Preserve the user's existing v2.98 tuning instead of resetting sliders.
    -- Defaults are only filled when a value does not exist yet.
    if cfg.mouseAimEnabled==nil then cfg.mouseAimEnabled=true end
    cfg.mouseAimKey=cfg.mouseAimKey or Enum.KeyCode.Q
    cfg.mouseAimPart=cfg.mouseAimPart or "UpperTorso"
    cfg.mouseAimFOV=cfg.mouseAimFOV or 220
    cfg.mouseAimAssistStrength=cfg.mouseAimAssistStrength or 72
    cfg.mouseAimSmoothness=cfg.mouseAimSmoothness or 76
    cfg.mouseAimCatchUp=cfg.mouseAimCatchUp or 82
    cfg.mouseAimOnBodyAssist=cfg.mouseAimOnBodyAssist or 14
    if cfg.mouseAimVisibleFollow==nil then cfg.mouseAimVisibleFollow=true end
    if cfg.mouseAimSilentCorrection==nil then cfg.mouseAimSilentCorrection=true end
    if cfg.mouseAimWallCheck==nil then cfg.mouseAimWallCheck=true end
    if cfg.mouseAimTeamCheck==nil then cfg.mouseAimTeamCheck=false end
    if cfg.mouseAimKnockCheck==nil then cfg.mouseAimKnockCheck=true end
    if cfg.mouseAimAutoUnlock==nil then cfg.mouseAimAutoUnlock=true end
    cfg.mouseAimEngineVersion="v2.99"
end

local function normalizeKey(v)
    if typeof(v)=="EnumItem" and v.EnumType==Enum.KeyCode then return v end
    local name=tostring(v or "Q"):gsub("^Enum%.KeyCode%.","")
    local key=nil
    pcall(function() key=Enum.KeyCode[name] end)
    if not key or key==Enum.KeyCode.Unknown then key=Enum.KeyCode.Q end
    return key
end

cfg.mouseAimEnabled=cfg.mouseAimEnabled~=false
cfg.mouseAimKey=normalizeKey(cfg.mouseAimKey)
cfg.mouseAimPart=tostring(cfg.mouseAimPart or "UpperTorso")
cfg.mouseAimFOV=math.clamp(tonumber(cfg.mouseAimFOV) or 220,20,800)
cfg.mouseAimAssistStrength=math.clamp(tonumber(cfg.mouseAimAssistStrength) or 72,1,100)
cfg.mouseAimSmoothness=math.clamp(tonumber(cfg.mouseAimSmoothness) or 76,1,100)
cfg.mouseAimCatchUp=math.clamp(tonumber(cfg.mouseAimCatchUp) or 82,1,100)
cfg.mouseAimOnBodyAssist=math.clamp(tonumber(cfg.mouseAimOnBodyAssist) or 14,0,55)
cfg.mouseAimVisibleFollow=cfg.mouseAimVisibleFollow~=false
cfg.mouseAimSilentCorrection=cfg.mouseAimSilentCorrection~=false
cfg.mouseAimWallCheck=cfg.mouseAimWallCheck~=false
cfg.mouseAimTeamCheck=cfg.mouseAimTeamCheck==true
cfg.mouseAimKnockCheck=cfg.mouseAimKnockCheck~=false
cfg.mouseAimAutoUnlock=cfg.mouseAimAutoUnlock~=false

local GuiService=game:GetService("GuiService")
local CAS=game:GetService("ContextActionService")
local StarterGui=game:GetService("StarterGui")
local mouseObj=lp:GetMouse()
local VIM=nil
...
local function getGlobalFunction(name)
    ...
    pcall(function()
        if type(getgenv)=="function" then
            local env=getgenv()
            if type(env)=="table" and type(env[name])=="function" then found=env[name] end
        end
    end)
    if type(found)=="function" then return found end
    pcall(function()
        local v=rawget(_G,name)
        if type(v)=="function" then found=v end
    end)
    if type(found)=="function" then return found end
    if name=="mousemoverel" then
        pcall(function() if type(mousemoverel)=="function" then found=mousemoverel end end)
    elseif name=="mousemoveabs" then
        pcall(function() if type(mousemoveabs)=="function" then found=mousemoveabs end end)
    end
    return type(found)=="function" and found or nil
end

local moveRel=getGlobalFunction("mousemoverel")
local moveAbs=getGlobalFunction("mousemoveabs")
local backendName=moveRel and "mousemoverel" or (moveAbs and "mousemoveabs" or (VIM and "virtual input" or "none"))
_G.KimqMouseAimBackend=backendName

-- v2.97 used a separately-drawn cursor. v2.98 deliberately turns that off:
-- Custom Cursor, if enabled in Settings, follows the REAL mouse position.
_G.KimqMouseAimCursorOverride=_G.KimqMouseAimCursorOverride or {active=false,position=nil}
_G.KimqMouseAimCursorOverride.active=false
_G.KimqMouseAimCursorOverride.position=nil

local state={
    active=false,target=nil,part=nil,point=nil,
    anchorX=0,anchorY=0,
    velocity=Vector2.zero,
    fractional=Vector2.zero,
    lastMouse=nil,lastIssued=Vector2.zero,lastClock=os.clock(),
    manualUntil=0,engagedAt=0,
    noBackendWarned=false,
}
_G.KimqMouseAimState=state
...
local function notify(text,duration)
    pcall(function()
        StarterGui:SetCore("SendNotification",{Title="Mouse Aim ♡",Text=text,Duration=duration or 2})
    end)
end

local function mouseFree()
    return UIS.MouseBehavior==Enum.MouseBehavior.Default
end

local function viewportCenter()
    local cam=workspace.CurrentCamera
    if not cam then return Vector2.zero end
    return cam.ViewportSize*.5
end

local function cursorViewportPosition()
    -- Player:GetMouse().X/Y and WorldToViewportPoint share the same viewport
    -- coordinate space. This avoids the top-bar inset ambiguity of GetMouseLocation.
    if mouseFree() and mouseObj then
        local ok,x,y=pcall(function() return mouseObj.X,mouseObj.Y end)
        if ok and type(x)=="number" and type(y)=="number" then
            return Vector2.new(x,y)
        end
    end
    if not mouseFree() then return viewportCenter() end

    -- Rare fallback for clients where Mouse.X/Y is unavailable: convert the
    -- absolute UIS position back into viewport coordinates explicitly.
    local mp=UIS:GetMouseLocation()
    local inset=Vector2.zero
    pcall(function()
        local tl=GuiService:GetGuiInset()
        if typeof(tl)=="Vector2" then inset=tl end
    end)
    return mp-inset
end

local function currentAimPosition()
    return cursorViewportPosition()
end
...
local function humanoid(pl)
    local c=pl and pl.Character
    return c and c:FindFirstChildOfClass("Humanoid")
end

local function isKnocked(pl)
    if not cfg.mouseAimKnockCheck then return false end
    local c=pl and pl.Character
    local h=c and c:FindFirstChildOfClass("Humanoid")
    if not c or not h or h.Health<=0 or h:GetState()==Enum.HumanoidStateType.Dead then return true end
    local containers={c,c:FindFirstChild("BodyEffects")}
        ...
        if container then
            for _,name in ipairs({"K.O","KO","Knocked","Downed","Dead","Unconscious"}) do
                ...
                if v then
                    ...
                    if (v:IsA("IntValue") or v:IsA("NumberValue")) and v.Value~=0 then return true end
                end
            end
        end
    end
...
local function validPlayer(pl)
    if not pl or pl==lp then return false end
    if _G.KHWhitelist and _G.KHWhitelist[pl.UserId] then return false end
    local h=humanoid(pl)
    if not h or h.Health<=0 or not pl.Character then return false end
    if isKnocked(pl) then return false end
    if cfg.mouseAimTeamCheck and lp.Team and pl.Team and lp.Team==pl.Team then return false end
    return true
end

local function wallBlocked(pl,pos)
    if not cfg.mouseAimWallCheck then return false end
    local cam=workspace.CurrentCamera
    ...
    return hit and pl.Character and not hit.Instance:IsDescendantOf(pl.Character) or false
end

local function usableParts(char)
    local list={}
...
local function choosePart(pl,mousePos)
    local char=pl and pl.Character
    if not char then return nil,nil end
    local wanted=tostring(cfg.mouseAimPart or "UpperTorso")
    if wanted~="Closest Point" then
        local p=char:FindFirstChild(wanted)
        if not p and (wanted=="UpperTorso" or wanted=="LowerTorso") then p=char:FindFirstChild("Torso") end
        if p and p:IsA("BasePart") then return p,p.Position end
    end
    mousePos=mousePos or currentAimPosition()
    local best,bestD=nil,math.huge
    for _,p in ipairs(usableParts(char)) do
        local sp,on=toScreen(p.Position)
        if on then
            local d=(sp-mousePos).Magnitude
            if d<bestD then bestD,best=d,p end
        end
    end
    best=best or char:FindFirstChild("UpperTorso") or char:FindFirstChild("Torso") or char:FindFirstChild("HumanoidRootPart") or char:FindFirstChild("Head")
    return best,best and best.Position or nil
end

local function partScreenBox(part)
    if not part then return nil,4,5,false end
    local center,on=toScreen(part.Position)
    if not center or not on then return center,4,5,false end
    local minX,maxX,minY,maxY=math.huge,-math.huge,math.huge,-math.huge
    local half=part.Size*.49
    for _,x in ipairs({-half.X,half.X}) do
        for _,y in ipairs({-half.Y,half.Y}) do
            for _,z in ipairs({-half.Z,half.Z}) do
                local sp,visible=toScreen(part.CFrame:PointToWorldSpace(Vector3.new(x,y,z)))
                if sp and visible then
    ...
    if minX==math.huge then return center,1,1,true end
    return center,math.clamp((maxX-minX)*.5,.65,55),math.clamp((maxY-minY)*.5,.75,65),true
end

local function insideEllipse(pos,center,rx,ry,scale,extra)
    if not pos or not center then return false end
    local sx=math.max(1,rx*(scale or 1)+(extra or 0))
    local sy=math.max(1,ry*(scale or 1)+(extra or 0))
    local dx=(pos.X-center.X)/sx
    local dy=(pos.Y-center.Y)/sy
    return dx*dx+dy*dy<=1
end

local function candidate(pl,limit,mousePos)
    if not validPlayer(pl) then return math.huge,nil,nil end
    local part,point=choosePart(pl,mousePos)
    if not part or not point or wallBlocked(pl,point) then return math.huge,nil,nil end
    local sp,on=toScreen(point)
    if not on then return math.huge,nil,nil end
    local d=(sp-mousePos).Magnitude
    if limit and d>limit then return math.huge,nil,nil end
    return d,part,point
end

local function playerUnderMouse()
    if not mouseFree() then return nil end
    local inst=mouseObj and mouseObj.Target
    if not inst then return nil end
    for _,pl in ipairs(Players:GetPlayers()) do
        if pl.Character and inst:IsDescendantOf(pl.Character) and validPlayer(pl) then return pl end
    end
    return nil
end
...
local function resetMotion()
    ...
    state.fractional=Vector2.zero
    state.lastMouse=nil
    state.lastIssued=Vector2.zero
    state.lastClock=os.clock()
    state.manualUntil=0
    state.anchorX=0;state.anchorY=0
end

local function unlock(silent)
    state.active=false;state.target=nil;state.part=nil;state.point=nil
    resetMotion()
    _G.KimqMouseAimTarget=nil
    _G.KimqMouseAimCursorOverride.active=false
    _G.KimqMouseAimCursorOverride.position=nil
    if not silent then notify("unlocked") end
end

local function startLock(pl,part,point,mousePos)
    state.active=true;state.target=pl;state.part=part;state.point=point
    state.engagedAt=os.clock()
    resetMotion()
    local center,rx,ry,on=partScreenBox(part)
    if center and on then
        local dx=(mousePos.X-center.X)/math.max(rx,1)
        local dy=(mousePos.Y-center.Y)/math.max(ry,1)
        if dx*dx+dy*dy<=1 then
            -- Preserve a little of where the user was already aiming, but keep it
            -- comfortably inside the body so it never floats above/beside them.
            state.anchorX=math.clamp(dx*.28,-.18,.18)
            state.anchorY=math.clamp(dy*.28,-.18,.18)
        end
    end
    _G.KimqMouseAimTarget=pl
end

local function acquire()
    local mousePos=currentAimPosition()
    local direct=playerUnderMouse()
    if direct then
        local p,pt=choosePart(direct,mousePos)
        if p and pt and not wallBlocked(direct,pt) then
            startLock(direct,p,pt,mousePos)
            return true
        end
    end
    local best,bestD,bestPart,bestPoint=nil,math.huge,nil,nil
    local fov=math.max(20,tonumber(cfg.mouseAimFOV) or 220)
    for _,pl in ipairs(Players:GetPlayers()) do
        local d,p,pt=candidate(pl,fov,mousePos)
        if d<bestD then best,bestD,bestPart,bestPoint=pl,d,p,pt end
    end
    if not best then
        local fallback=math.min(360,math.max(fov*1.35,260))
        for _,pl in ipairs(Players:GetPlayers()) do
            local d,p,pt=candidate(pl,fallback,mousePos)
            if d<bestD then best,bestD,bestPart,bestPoint=pl,d,p,pt end
        end
    end
    if not best then return false end
    startLock(best,bestPart,bestPoint,mousePos)
    return true
end

local function roundSigned(v)
    if v>=0 then return math.floor(v+.5) end
    return math.ceil(v-.5)
end

local function movePhysicalMouse(step)
    if not mouseFree() then return false end
    if step.Magnitude<.05 then return true end
    local total=step+state.fractional
    local ix,iy=roundSigned(total.X),roundSigned(total.Y)
    state.fractional=Vector2.new(total.X-ix,total.Y-iy)
    if ix==0 and iy==0 then return true end
    local ok=false
    if moveRel then
        ok=pcall(moveRel,ix,iy)
    elseif moveAbs then
        local mp=UIS:GetMouseLocation()
        ok=pcall(moveAbs,roundSigned(mp.X+ix),roundSigned(mp.Y+iy))
    elseif VIM then
        local mp=UIS:GetMouseLocation()
        ok=pcall(function() VIM:SendMouseMoveEvent(roundSigned(mp.X+ix),roundSigned(mp.Y+iy),game) end)
    end
    if ok then state.lastIssued=Vector2.new(ix,iy) end
    return ok
end

local function desiredPoint(part,mousePos)
    local center,rx,ry,on=partScreenBox(part)
    if not center or not on then return nil,center,rx,ry,false end

    -- At distance, even a 2-3 pixel offset can visually sit above a tiny torso.
    -- Fade the preserved hand-anchor out as the projected part becomes smaller,
    -- and use the exact projected center for genuinely tiny/far targets.
    local projected=math.min(rx,ry)
    local anchorScale=math.clamp((projected-3)/10,0,1)
    local desired=Vector2.new(
        center.X+rx*state.anchorX*anchorScale,
        center.Y+ry*state.anchorY*anchorScale
    )
    if projected<6 then desired=center end
    if not insideEllipse(desired,center,rx,ry,.70,0) then desired=center end
    return desired,center,rx,ry,true
end

local function worldPointFromCursor(part,cursor,center,rx,ry)
    if not part or not cursor or not center then return nil end
    -- For tiny distant parts, use the real part center. Mapping a one-pixel cursor
    -- displacement back into world space exaggerates the offset and can miss.
    if math.min(rx,ry)<6 then return part.Position end
    local nx=math.clamp((cursor.X-center.X)/math.max(rx,1),-.38,.38)
    local ny=math.clamp((cursor.Y-center.Y)/math.max(ry,1),-.38,.38)
    return part.Position + part.CFrame.RightVector*(part.Size.X*nx) - part.CFrame.UpVector*(part.Size.Y*ny)
end

local function alignedShotPoint(pl,part,point)
    if not pl or not part or not point or not validPlayer(pl) or wallBlocked(pl,point) then return nil end
    local aimPos=currentAimPosition()
    local center,rx,ry,on=partScreenBox(part)
    if not center or not on then return nil end
    -- Keep correction visually honest at long range too. The old +3px allowance
    -- was larger than some distant torsos, so the cursor could visibly hover above
    -- them while still being treated as "on body". Tolerance now scales with the
    -- actual projected part size.
    local tolerance=math.clamp(math.min(rx,ry)*.10,.20,1.10)
    if not insideEllipse(aimPos,center,rx,ry,.98,tolerance) then return nil end
    return worldPointFromCursor(part,aimPos,center,rx,ry) or point
end

_G.KimqMouseAimSuppressFallback=function()
    return cfg.mouseAimEnabled and state.active
end
_G.KimqMouseAimOverride=function()
    if not cfg.mouseAimEnabled or not cfg.mouseAimSilentCorrection or not state.active or not validPlayer(state.target) then return nil,nil end
    local part,point=choosePart(state.target,currentAimPosition())
    if not part or not point then return nil,nil end
    local hit=alignedShotPoint(state.target,part,point)
    if not hit then return nil,nil end
    state.part,state.point=part,point
    return part,hit
end
_G.KimqMouseAimForceCorrection=function()
    if not cfg.mouseAimEnabled or not cfg.mouseAimSilentCorrection or not state.active or not validPlayer(state.target) then return false end
    local part,point=choosePart(state.target,currentAimPosition())
    return alignedShotPoint(state.target,part,point)~=nil
end

local lastToggle=0
local actionName="KimqMouseAimToggle_v299"
local function requestToggle()
    if UIS:GetFocusedTextBox() then return end
    local now=os.clock()
    if now-lastToggle<.14 then return end
    lastToggle=now
    if not cfg.mouseAimEnabled then cfg.mouseAimEnabled=true end
    if state.active then
        unlock(false)
        return
    end
    if acquire() then
        local pl=state.target
        local name=tostring((pl and (pl.DisplayName or pl.Name)) or "target")
        if mouseFree() and backendName=="none" then
            notify("locked → "..name.." • executor has no mouse-move API",4)
            state.noBackendWarned=true
        else
            notify("locked → "..name)
        end
    else
        notify("no visible target near your mouse")
    end
end

local function bindKey()
    pcall(function() CAS:UnbindAction(actionName) end)
    cfg.mouseAimKey=normalizeKey(cfg.mouseAimKey)
    pcall(function()
        CAS:BindActionAtPriority(actionName,function(_,inputState)
            if inputState==Enum.UserInputState.Begin then requestToggle() end
            return Enum.ContextActionResult.Pass
        end,false,5000,cfg.mouseAimKey)
    end)
end
bindKey()
...
UIS.InputBegan:Connect(function(input)
    if UIS:GetFocusedTextBox() then return end
    if input.UserInputType==Enum.UserInputType.Keyboard and input.KeyCode==normalizeKey(cfg.mouseAimKey) then requestToggle() end
end)
Players.PlayerRemoving:Connect(function(pl) if state.target==pl then unlock(true) end end)
lp.CharacterAdded:Connect(function() unlock(true) end)

RunService.RenderStepped:Connect(function()
    _G.KimqMouseAimCursorOverride.active=false
    _G.KimqMouseAimCursorOverride.position=nil
    if not cfg.mouseAimEnabled then
        if state.active then unlock(true) end
        return
    end
    if not state.active then return end
    if not validPlayer(state.target) then
        if cfg.mouseAimAutoUnlock then unlock(true) end
        return
    end

    local aimPos=currentAimPosition()
    local part,point=choosePart(state.target,aimPos)
    if not part or not point then return end
    state.part,state.point=part,point
    if wallBlocked(state.target,point) then return end

    local desired,center,rx,ry,on=desiredPoint(part,aimPos)
    if not desired or not center or not on then return end

    -- Keep first-person / shift-lock camera completely untouched. In those modes
    -- Mouse Aim still keeps the Q target and Silent Correction only works when the
    -- center crosshair is genuinely on the selected body part.
    if not cfg.mouseAimVisibleFollow or not mouseFree() then
        state.velocity=Vector2.zero
        state.lastMouse=aimPos
        return
    end
    if backendName=="none" then
        if not state.noBackendWarned then
            state.noBackendWarned=true
            notify("your executor is not exposing mousemoverel / mousemoveabs",4)
        end
        return
    end

    local now=os.clock()
    local dt=math.clamp(now-(state.lastClock or now),1/240,1/20)
    state.lastClock=now
    local mp=cursorViewportPosition()

    -- Detect the player's OWN mouse movement and briefly loosen the assist instead
    -- of fighting it. This is the main reason the lock feels like assistance rather
    -- than a robotic magnet.
    if state.lastMouse then
        local actualDelta=mp-state.lastMouse
        local manualDelta=actualDelta-(state.lastIssued or Vector2.zero)
        if manualDelta.Magnitude>1.75 then state.manualUntil=now+.085 end
    end
    state.lastMouse=mp
    state.lastIssued=Vector2.zero

    local error=desired-mp
    local dist=error.Magnitude
    if dist<.35 then
        state.velocity=state.velocity*.72
        return
    end

    local strength=math.clamp(tonumber(cfg.mouseAimAssistStrength) or 72,1,100)/100
    local smooth=math.clamp(tonumber(cfg.mouseAimSmoothness) or 76,1,100)/100
    local catch=math.clamp(tonumber(cfg.mouseAimCatchUp) or 82,1,100)/100
    local onBody=math.clamp(tonumber(cfg.mouseAimOnBodyAssist) or 14,0,55)/100
    local projected=math.min(rx,ry)
    local tinyTarget=projected<7
    local insideCore=insideEllipse(mp,center,rx,ry,.58,0)
    local insideBody=insideEllipse(mp,center,rx,ry,.94,.25)

    local assistScale=1
    if insideCore then
        assistScale=onBody
    elseif insideBody then
        assistScale=.34+.24*strength
    end
    if now<state.manualUntil then assistScale=assistScale*.34 end
    -- A distant target occupies only a handful of pixels. Do not weaken the
    -- correction too early there; keep pulling until the pointer is truly centered.
    if tinyTarget then
        assistScale=math.max(assistScale,.82)
        if now<state.manualUntil then assistScale=math.max(assistScale,.62) end
    end

    local response=(10+20*strength)*assistScale
    if not insideBody then response=response*(1+.35*catch) end
    if dist>120 then response=response*(1+.28*catch) end

    local desiredVelocity=error*response
    local maxSpeed=(650+1850*catch)
    if dist>160 then maxSpeed=maxSpeed+650*catch end
    if insideCore then maxSpeed=maxSpeed*(.22+.20*onBody) end
    if desiredVelocity.Magnitude>maxSpeed and desiredVelocity.Magnitude>0 then
        desiredVelocity=desiredVelocity.Unit*maxSpeed
    end

    -- Critically-smoothed velocity: fast enough to catch strafes, but acceleration
    -- and deceleration are gradual so the pointer glides instead of snapping.
    local velocityResponse=9+(1-smooth)*13+strength*5
    local blend=1-math.exp(-velocityResponse*dt)
    state.velocity=state.velocity:Lerp(desiredVelocity,blend)
    local step=state.velocity*dt
    if step.Magnitude>dist and dist>0 then step=error end

    -- Stop only when the pointer is actually centered enough for the projected
    -- part size. A fixed 3px dead-zone was huge at distance and caused the visible
    -- cursor to park above tiny characters.
    local stopRadius=math.clamp(projected*.22,.35,1.50)
    if insideCore and dist<stopRadius then
        state.velocity=state.velocity*.45
        return
    end

    movePhysicalMouse(step)
end)

local parts={"Closest Point","Head","UpperTorso","HumanoidRootPart","LowerTorso"}
addToggle("Mouse Aim Enabled",cfg.mouseAimEnabled,function(v)
    cfg.mouseAimEnabled=v==true
    if not cfg.mouseAimEnabled then unlock(true) end
end)
addKeybind("Mouse Aim Key",cfg.mouseAimKey,function(v)
    cfg.mouseAimKey=normalizeKey(v);bindKey()
end)
addToggle("Mouse Follow",cfg.mouseAimVisibleFollow,function(v)
    cfg.mouseAimVisibleFollow=v==true
end)
addToggle("Silent Correction",cfg.mouseAimSilentCorrection,function(v)
    cfg.mouseAimSilentCorrection=v==true
end)
addDropdown("Mouse Aim Part",parts,cfg.mouseAimPart,function(v)
    cfg.mouseAimPart=tostring(v)
end)
addSlider("Assist Strength",1,100,cfg.mouseAimAssistStrength,function(v)
    cfg.mouseAimAssistStrength=v
end)
addSlider("Smoothness",1,100,cfg.mouseAimSmoothness,function(v)
    cfg.mouseAimSmoothness=v
end)
addSlider("Catch-Up",1,100,cfg.mouseAimCatchUp,function(v)
    cfg.mouseAimCatchUp=v
end)
addSlider("On-Body Assist",0,55,cfg.mouseAimOnBodyAssist,function(v)
    cfg.mouseAimOnBodyAssist=v
end)
addSlider("Mouse Aim FOV",20,800,cfg.mouseAimFOV,function(v)
    cfg.mouseAimFOV=v
end)
addToggle("Mouse Aim Wall Check",cfg.mouseAimWallCheck,function(v)
    cfg.mouseAimWallCheck=v==true
end)
addToggle("Mouse Aim Team Check",cfg.mouseAimTeamCheck,function(v)
    cfg.mouseAimTeamCheck=v==true
end)
addToggle("Mouse Aim Knock Check",cfg.mouseAimKnockCheck,function(v)
    cfg.mouseAimKnockCheck=v==true
end)
addToggle("Auto Unlock On K.O.",cfg.mouseAimAutoUnlock,function(v)
    cfg.mouseAimAutoUnlock=v==true
end)
]=====], false) then return end
...
-- CUSTOM CURSOR ------------------------------------------------------------
-- Completely visual and isolated from Mouse Aim/Silent Aim targeting logic.
do
    ...
    local function refreshCursorVisual()
        local override=_G.KimqMouseAimCursorOverride
        local forced=override and override.active==true and typeof(override.position)=="Vector2"
        local style=tostring(cfg.customCursorStyle or "Heart")
        -- If the user normally keeps Custom Cursor OFF, Mouse Aim uses a small Dot
        -- while Q-locked. Turning Custom Cursor ON makes Mouse Aim use that chosen style.
        if forced and not cfg.customCursorEnabled then style="Dot" end
        if forced and style=="Default" then style="Dot" end
        local size=math.clamp(tonumber(cfg.customCursorSize) or 24,12,52)
        if forced and not cfg.customCursorEnabled then size=math.clamp(math.floor(size*.72),12,26) end
        ...
        holder.Visible=(forced or cfg.customCursorEnabled) and style~="Default" and customReady
    ...
    RunService.RenderStepped:Connect(function()
        local forced,override=refreshCursorVisual()
        local mp=(forced and override.position) or UIS:GetMouseLocation()
...
task.spawn(function()
    ...
    local pageDefs = {
        ...
        {"mouseaim", "mouse aim", "soft Q target lock with free camera + silent correction", "COMBAT"},
    ...
    task.defer(function()
        local rageOK,rageERR=pcall(function()
            ...
            function R.isWeaponTool(tool)
                ...
                if table.find(ForceHitAllowedTools,tool.Name) then return true end
            ...
            function R.shoot(pl)
                ...
                task.spawn(function()
                    for i=1,count do
                        ...
                        pcall(ForceHit_Fire,hit)
                        if i<count then task.wait(.003) end
                    end
                    R.shotBusy=false
                end)
                return true
...
task.spawn(function()
    ...
    local exact = {
        ...
        ["Mouse Aim Enabled"] = "mouseaim",
        ["Mouse Aim Key"] = "mouseaim",
        ["Visible Mouse Follow"] = "mouseaim",
        ["Silent Correction"] = "mouseaim",
        ["Mouse Aim Part"] = "mouseaim",
        ["Mouse Follow Strength"] = "mouseaim",
        ["Aim Softness"] = "mouseaim",
        ["Mouse Aim FOV"] = "mouseaim",
        ["Mouse Aim Wall Check"] = "mouseaim",
        ["Mouse Aim Team Check"] = "mouseaim",
        ["Mouse Aim Knock Check"] = "mouseaim",
        ["Auto Unlock On K.O."] = "mouseaim",
    ...
    local sectionHeaderNames = {
        ["Silent Aim"] = true, ["Mouse Aim"] = true, ["Macro"] = true, ["Whitelist"] = true,
...
task.spawn(function()
    ...
    local pageNames={overview=true,["silent aim"]=true,["mouse aim"]=true,macro=true,whitelist=true,protection=true,["anti fall"]=true,["delay changer"]=true,esp=true,avatar=true,["fog / atmosphere"]=true,environment=true,["weapon skins"]=true,["hc silent aim"]=true,["force hit"]=true,["hitbox expander"]=true,flamelock=true,camlock=true,headless=true,["anti mod"]=true,["spawn point"]=true,["rage camlock"]=true,["target orbit"]=true,["rage combat"]=true,settings=true,theme=true,information=true}
    ...
    local function activePageName()
        local map={antifall="anti fall",antimod="anti mod",delay="delay changer",fog="fog / atmosphere",forcehit="hc silent aim",hcsilent="hc silent aim",hitbox="hitbox expander",info="information",weaponskins="weapon skins",silent="silent aim",mouseaim="mouse aim"}
        for _,d in ipairs(main:GetDescendants()) do
...
task.spawn(function()
    ...
    pcall(function()
        game:GetService("StarterGui"):SetCore("SendNotification",{
            Title="Kimqetras HC",
            Text="v2.99 ACTIVE ♡ • far-distance Mouse Aim alignment ready",
            Duration=7,
        })
    end)
...
-- ========================================================
-- v2.89 CLEAN SEMANTIC FEATURE PAGE ORGANIZER (VISUAL ONLY)
-- Only LayoutOrder + decorative labels are changed. Existing controls, callbacks,
-- state, config keys, and parents remain untouched.
-- ========================================================
task.spawn(function()
    local Players=game:GetService("Players")
    local CoreGui=game:GetService("CoreGui")
    ...
    local pretty={
        hcsilent="HC combat",silent="silent aim",mouseaim="mouse aim",camlock="camlock",flamelock="flamelock",hitbox="hitbox",
        delay="delay changer",fog="fog + atmosphere",environment="environment",esp="ESP",avatar="avatar",
        weaponskins="weapon skins",whitelist="whitelist",protection="protection",antifall="anti fall",
        antimod="anti mod",spawn="spawn point",macro="macro",ragecam="rage targets + camlock",
        rageorbit="target orbit",ragecombat="rage combat",ragepresets="rage presets"
    }
    local primary={
        ...
        esp={"esp","enable"},macro={"macro","enable"},fog={"fog","atmosphere"},
        ...
        camlock={"camlock"},flamelock={"flamelock"},hitbox={"hitbox"},silent={"silent aim"},mouseaim={"mouse aim enabled","mouse aim key","visible mouse follow","silent correction"},hcsilent={"silent aim","hc"}
    }
    local behavior={"auto ","wall check","knock check","stickiness","prediction","priority","skip ","whitelist","respawn","headless","accessor","anti lock","evasive","reload","stomp","advance","camera","view","orbit","queue order","persist","keep ","mode"}
    local tuning={"speed","radius","height","distance","smooth","strength","delay","interval","burst","chance","fov","size","density","color","power","attempt","repeat","min","max","frequency","scale","offset","rotation"}
    local extras={"note","tip","status","preset","debug","other","extra"}

    local function pal()
        local p=_G.KimqThemeLivePalette
        return type(p)=="table" and p or {panel=Color3.new(1,1,1),soft=Color3.fromRGB(246,255,250),hot=Color3.fromRGB(243,161,211),text=Color3.fromRGB(82,116,94),sub=Color3.fromRGB(122,153,133),line=Color3.fromRGB(255,212,243)}
    end
    local function corner(o,r)
        local c=o:FindFirstChildOfClass("UICorner") or Instance.new("UICorner",o); c.CornerRadius=UDim.new(0,r or 10)
    end
    local function stroke(o,c,t)
        local s=o:FindFirstChildOfClass("UIStroke") or Instance.new("UIStroke",o); s.Color=c;s.Transparency=t or .35;s.Thickness=1
    end
    local function mklabel(parent,text,size,pos,font,sz,color)
        local l=Instance.new("TextLabel",parent);l.BackgroundTransparency=1;l.Size=size;l.Position=pos;l.Text=text;l.Font=font;l.TextSize=sz;l.TextColor3=color;l.TextXAlignment=Enum.TextXAlignment.Left;return l
    end
    ...
    local function has(txt,arr)
        for _,v in ipairs(arr or {}) do if txt:find(v,1,true) then return true end end
        return false
    end
    local function groupFor(key,txt)
        if has(txt,primary[key]) or txt:find("master",1,true) then return 1 end
        if has(txt,behavior) then return 2 end
        if has(txt,tuning) then return 3 end
        if has(txt,extras) then return 4 end
        return 2
    end
    ...
    for _,page in ipairs(pageHost:GetChildren()) do
        if page:IsA("ScrollingFrame") and page.Name:match("Page$") then
            ...
            if not excluded[key] then
                ...
                if list then list.Padding=UDim.new(0,8);list.SortOrder=Enum.SortOrder.LayoutOrder end

                for _,ch in ipairs(page:GetChildren()) do if ch:GetAttribute("KimqOrganizerDecor") then ch:Destroy() end end

                local cards={}
                for _,ch in ipairs(page:GetChildren()) do
                    if ch:IsA("GuiObject") and not ch:IsA("UIListLayout") and not ch:IsA("UIPadding") then
                        local txt=allText(ch)
                        table.insert(cards,{obj=ch,old=ch.LayoutOrder,group=groupFor(key,txt)})
                    end
                end
                ...
                for i,e in ipairs(cards) do
                    ...
                    e.obj.LayoutOrder=e.group*1000+i*10
                    if e.obj:IsA("Frame") then
                        local p=pal();corner(e.obj,12);stroke(e.obj,p.line,.35)
                        local accent=Instance.new("Frame",e.obj)
                        accent.Name="KimqOrganizerAccent";accent:SetAttribute("KimqOrganizerDecor",true);accent:SetAttribute("KimqV26Role","hotBg")
                        accent.Size=UDim2.fromOffset(3,math.clamp(e.obj.Size.Y.Offset-16,18,38));accent.Position=UDim2.fromOffset(4,8);accent.BorderSizePixel=0;accent.BackgroundColor3=p.hot;accent.ZIndex=0;corner(accent,999)
                    end
                end

                local p=pal()
                local head=Instance.new("Frame",page);head.Name="KimqOrganizerHeader";head:SetAttribute("KimqOrganizerDecor",true);head:SetAttribute("KimqV26Role","lightBg")
                head.LayoutOrder=1;head.Size=UDim2.new(1,-6,0,34);head.BackgroundColor3=p.soft;head.BorderSizePixel=0;corner(head,12);stroke(head,p.line,.34)
                local h=mklabel(head,"♥",UDim2.fromOffset(28,28),UDim2.fromOffset(10,3),Enum.Font.FredokaOne,18,p.hot);h:SetAttribute("KimqV26Role","hotText")
                local t=mklabel(head,pretty[key] or key,UDim2.new(1,-52,1,0),UDim2.fromOffset(38,0),Enum.Font.GothamBold,11,p.text);t:SetAttribute("KimqV26Role","textText")

                local names={[1]="MAIN ♡",[2]="AUTOMATION ♡",[3]="TUNING ♡",[4]="EXTRAS ♡"}
                for g=1,4 do
                    if present[g] then
                        local d=Instance.new("Frame",page);d.Name="KimqOrganizerGroup"..g;d:SetAttribute("KimqOrganizerDecor",true);d.LayoutOrder=g*1000-10;d.Size=UDim2.new(1,-6,0,22);d.BackgroundTransparency=1
                        local l=mklabel(d,names[g],UDim2.new(1,-18,1,0),UDim2.fromOffset(9,0),Enum.Font.GothamBold,9,p.hot);l:SetAttribute("KimqV26Role","hotText")
                    end
                end
            end
        end
