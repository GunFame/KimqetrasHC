-- Kimqetras HC v2.51 • restored v2.19 weapon-extras core + isolated wing flight
...
local KIMQ_BUILD = "v2.51-restored-v219-extras"
...
do
    ...
    if running and runningBuild == KIMQ_BUILD then
        ...
        pcall(function()
            StarterGui0:SetCore("SendNotification",{
                Title="Kimqetras HC",
                Text=(runtimeState=="ready") and "v2.51 is already running ♡" or "v2.51 is already starting ♡",
                Duration=4,
            })
        end)
        return
    ...
    if oldRuntimeDetected then
        pcall(function()
            StarterGui0:SetCore("SendNotification",{
                Title="Kimqetras HC",
                Text="An older Kimqetras HC runtime is still active. Rejoin once, then run v2.51. v2.51 will not stack on top of it.",
                Duration=9,
            })
        end)
        warn("[Kimqetras HC v2.51] Older runtime detected. Refusing to stack; rejoin once.")
        return
    end
    ...
    local knownRenderBindings = {
        "KimqMacroSpeedV240","KimqMacroSpeedV241","KimqMacroSpeedV242","KimqMacroSpeedV251",
        ...
        "KimqPermanentTimeV241","KimqPermanentTimeV242","KimqPermanentTimeV251",
    }
...
pcall(function()
    RunService:BindToRenderStep("KimqMacroSpeedV251",Enum.RenderPriority.Last.Value,function()
        enforceMacroSpeed()
    end)
end)
...
local function addCleanSection(title, subtitle)
    ...
    -- v2.51: pageHead already displays the section name.
...
task.spawn(function()
    ...
    -- v2.51: pages are grouped by what they actually do so the sidebar is
    ...
    local pageDefs = {
        ...
        -- RAGE
        {"ragecam", "rage camlock", "multi-target queue with 3d avatar preview and persistent target lock", "RAGE"},
        {"rageorbit", "target orbit", "auto teleport and fast orbit the current queued target", "RAGE"},
        {"ragecombat", "rage combat", "auto hunt queued targets with force-hit bursts and auto finish", "RAGE"},
    ...
    local function refreshPageSearch()
        ...
        for _, entry in ipairs(navButtons) do
            ...
            if entry.key=="ragecam" then aliases=" rage target player lock spectate view" end
            if entry.key=="rageorbit" then aliases=" teleport orbit circle angel wings fly" end
            if entry.key=="ragecombat" then aliases=" auto shoot one shot force hit target" end
    ...
    -- ================================================================
    -- v2.51 RAGE TARGET SUITE
    -- Multi-target queue + 3D avatar preview + auto hunt / orbit / finish.
    -- Separate from the normal Camlock / Force Hit systems.
    -- ================================================================
    do
        ...
        if rageCamPage and rageOrbitPage and rageCombatPage then
            ...
            local cameraR=workspace.CurrentCamera

            -- ---------------- TARGET QUEUE ----------------
            local targetQueue={}
            local targetSet={}
            local currentTargetIndex=1
            local hoverUserId=nil

            local rageLockEnabled=false
            local rageViewEnabled=false
            local rageTargetPart="Head"
            local rageSmoothness=.24

            local rageOrbitEnabled=false
            local rageOrbitSpeed=8
            local rageOrbitRadius=8
            local rageOrbitHeight=4.5
            local rageOrbitAngle=0
            local rageWingAttempt=0

            local rageAutoTeleport=false
            local rageTeleportInterval=.22
            local rageTeleportAccumulator=0

            local rageAutoShoot=false
            local rageShootInterval=.08
            local rageShootAccumulator=0
            local rageShootPart="Head"

            local rageForceBurstCount=8
            local rageAutoHunt=false
            local rageAutoStomp=true
            local rageAutoAdvance=true
            local rageStompRepeats=4
            local rageHuntOrbit=true
            ...
            local orderCounters={}
            local function rageCard(page,height)
                orderCounters[page]=(orderCounters[page] or 0)+1
                local f=Instance.new("Frame")
                f.Parent=page
                f.LayoutOrder=orderCounters[page]
                f.Size=UDim2.new(1,-6,0,height)
                f.BackgroundColor3=T.panel
                f.BorderSizePixel=0
                corner(f,14)
                stroke(f,T.stroke,.22,1)
                return f
            end

            local function rageText(parent,text,size,pos,font,textSize,color,align)
                return textLabel(parent,text,size,pos,font or Enum.Font.Gotham,textSize or 11,color or T.text,align or Enum.TextXAlignment.Left)
            end

            local function queueIndexOf(userId)
                for i,id in ipairs(targetQueue) do
                    if id==userId then return i end
                end
                return nil
            end

            local function normalizeCurrentIndex()
                if #targetQueue==0 then
                    currentTargetIndex=1
                    return nil
                end
                currentTargetIndex=math.clamp(currentTargetIndex,1,#targetQueue)
                return targetQueue[currentTargetIndex]
            end

            local function selectedUserId()
                return normalizeCurrentIndex()
            end
            ...
            local function targetName()
                ...
                if not p then
                    return #targetQueue>0 and ("Target unavailable • "..#targetQueue.." queued") or "No target selected"
                end
                return p.DisplayName.."  (@"..p.Name..")"
            end

            local function queueStatus()
                if #targetQueue==0 then return "0 targets queued" end
                return tostring(currentTargetIndex).."/"..tostring(#targetQueue).." • "..targetName()
            end

            local previewBackdrop,previewGradient,previewHelp=nil,nil,nil

            local function ragePreviewPalette()
                local p=_G.KimqThemeLivePalette or T or {}
                return {
                    bg=p.bg or T.bg,
                    bg2=p.bg2 or p.soft or T.bg2,
                    panel=p.panel or T.panel,
                    card2=p.card2 or T.card2,
                    hot=p.hot or T.hot,
                    soft=p.soft or T.soft,
                    text=p.text or T.text,
                    sub=p.sub or T.sub,
                    stroke=p.stroke or T.stroke,
                    white=p.white or Color3.new(1,1,1),
                }
            end
            ...
            local function refreshRagePreviewTheme()
                local p=ragePreviewPalette()

                if previewPanel and previewPanel.Parent then
                    previewPanel.BackgroundColor3=p.bg2
                    local st=previewPanel:FindFirstChildOfClass("UIStroke")
                    if st then st.Color=p.stroke end
                end

                if previewBackdrop and previewBackdrop.Parent then
                    previewBackdrop.BackgroundColor3=p.panel
                    local st=previewBackdrop:FindFirstChildOfClass("UIStroke")
                    if st then st.Color=p.stroke end
                end

                if previewGradient then
                    previewGradient.Color=ColorSequence.new({
                        ColorSequenceKeypoint.new(0,p.soft),
                        ColorSequenceKeypoint.new(.55,p.bg2),
                ...
                if previewViewport then
                    previewViewport.BackgroundColor3=p.panel
                end

                if previewTitle then previewTitle.TextColor3=p.text end
                if previewHelp then previewHelp.TextColor3=p.sub end
            end

            local function showAvatarPreview(pl)
                if not previewViewport or not previewViewport.Parent then return end
                previewNonce+=1
                local nonce=previewNonce
                clearViewport()

                if not pl then
                    if previewTitle then previewTitle.Text="Hover a player for 3D preview" end
                    return
                end
                ...
                task.spawn(function()
                    ...
                    if not ok or not model or nonce~=previewNonce or not previewViewport or not previewViewport.Parent then
                        if model then pcall(function() model:Destroy() end) end
                        return
                    end

                    local world=Instance.new("WorldModel")
                    world.Name="KimqRageAvatarWorld"
                    ...
                    -- cute circle platform
                    local platform=Instance.new("Part")
                    platform.Name="KimqPreviewPlatform"
                    platform.Shape=Enum.PartType.Cylinder
                    platform.Size=Vector3.new(.6,5.2,5.2)
                    platform.Material=Enum.Material.SmoothPlastic
                    platform.Color=p.soft
                    platform.Transparency=.08
                    platform.Anchored=true
                    platform.CanCollide=false
                    platform.CastShadow=false
                    platform.CFrame=CFrame.Angles(0,0,math.rad(90)) * CFrame.new(0,-(size.Y*.5)-.35,0)
                    platform.Parent=world

                    local ring=Instance.new("Part")
                    ring.Name="KimqPreviewRing"
                    ring.Shape=Enum.PartType.Cylinder
                    ring.Size=Vector3.new(.16,5.9,5.9)
                    ...
                    ring.CFrame=CFrame.Angles(0,0,math.rad(90)) * CFrame.new(0,-(size.Y*.5)-.28,0)
                    ring.Parent=world

                    local spark=Instance.new("Part")
                    spark.Name="KimqPreviewSpark"
                    spark.Shape=Enum.PartType.Ball
                    spark.Size=Vector3.new(.22,.22,.22)
                    ...
                    spark.CFrame=CFrame.new(1.95,(size.Y*.22),1.2)
                    spark.Parent=world

                    local spark2=spark:Clone()
                    spark2.CFrame=CFrame.new(-1.7,(size.Y*.36),-1.35)
                    spark2.Parent=world

                    -- Center body above the platform.
                    pcall(function()
                        local pivot=model:GetPivot()
                        model:PivotTo(CFrame.new(-cf.Position.X,-cf.Position.Y+(size.Y*.5)-.02,-cf.Position.Z)*pivot)
                    end)

                    local cam=Instance.new("Camera")
                    cam.Name="KimqRageAvatarCamera"
                    cam.FieldOfView=26
                    cam.Parent=previewViewport
                    previewViewport.CurrentCamera=cam

                    local dist=math.max(size.Y*1.35,math.max(size.X,size.Z)*2.55,7.3)
                    local lookAt=Vector3.new(0,size.Y*.06,0)
                    cam.CFrame=CFrame.lookAt(
                        Vector3.new(0,size.Y*.18,-dist),
                        lookAt
                    )
                end)
            end

            local function updateSelectedLabels()
                local t=queueStatus()
                for _,lbl in ipairs(selectedLabels) do
                    if lbl and lbl.Parent then lbl.Text=t end
                end
                for _,lbl in ipairs(queueLabels) do
                    if lbl and lbl.Parent then
                        lbl.Text=#targetQueue==0
                            and "Click players to add them to your target queue."
                            or ("Queued targets: "..tostring(#targetQueue).." • current "..tostring(currentTargetIndex))
                    end
                end
                if not hoverUserId then
                    showAvatarPreview(selectedPlayer())
                end
            end

            local function addTarget(pl)
                if not pl or pl==lp or targetSet[pl.UserId] then return end
                targetSet[pl.UserId]=true
                table.insert(targetQueue,pl.UserId)
                if #targetQueue==1 then currentTargetIndex=1 end
                updateSelectedLabels()
                if refreshPlayersList then refreshPlayersList() end
            end

            local function removeTargetById(userId)
                local idx=queueIndexOf(userId)
                if not idx then return end
                targetSet[userId]=nil
                table.remove(targetQueue,idx)

                if #targetQueue==0 then
                    currentTargetIndex=1
                elseif idx<currentTargetIndex then
                    currentTargetIndex-=1
                elseif currentTargetIndex>#targetQueue then
                    currentTargetIndex=1
                end
            ...
            local function toggleTarget(pl)
                if not pl or pl==lp then return end
                if targetSet[pl.UserId] then
                    removeTargetById(pl.UserId)
                else
                    addTarget(pl)
                end
            end

            local function clearTargetQueue()
                table.clear(targetQueue)
                table.clear(targetSet)
                currentTargetIndex=1
                rageLockEnabled=false
                rageViewEnabled=false
                rageOrbitEnabled=false
                rageAutoShoot=false
                rageAutoTeleport=false
                rageAutoHunt=false
                updateSelectedLabels()
                if refreshPlayersList then refreshPlayersList() end
            end

            local function advanceTarget(removeCurrent)
                ...
                if #targetQueue>1 then
                    currentTargetIndex=(currentTargetIndex % #targetQueue)+1
                end
                updateSelectedLabels()
                if refreshPlayersList then refreshPlayersList() end
                return selectedPlayer()
            ...
            local function targetPartFor(pl,choice)
                ...
                local mousePos=UISR:GetMouseLocation()
                ...
                for _,name in ipairs({
                    ...
                    if part and part:IsA("BasePart") then
                        local v,on=cameraR:WorldToViewportPoint(part.Position)
                        if on then
            ...
            local function restoreOwnCamera()
                ...
                local char=lp.Character
                local hum=char and char:FindFirstChildOfClass("Humanoid")
                if hum then pcall(function() cameraR.CameraSubject=hum end) end
            end

            local function ensureViewTarget()
                if not rageViewEnabled then return end
                local pl=selectedPlayer()
                local hum=pl and pl.Character and pl.Character:FindFirstChildOfClass("Humanoid")
                if hum and cameraR.CameraSubject~=hum then
                    pcall(function() cameraR.CameraSubject=hum end)
                end
            end

            local function ensureAngelWings()
                local char=lp.Character
                if not char or char:FindFirstChild("KimqWornAngelWings") then return end
                if os.clock()-rageWingAttempt<1.2 then return end
                rageWingAttempt=os.clock()

                local controller=_G.KimqWeaponExtrasController
                if not controller and type(getgenv)=="function" then
                    controller=getgenv().KimqWeaponExtrasController
                end
                if controller and type(controller.EquipItem)=="function" then
                    pcall(controller.EquipItem,"Angel Wings")
                end
            end

            local function teleportToPlayer(pl,nearForStomp)
                local targetRoot=pl and pl.Character and pl.Character:FindFirstChild("HumanoidRootPart")
                local char=lp.Character
                local myRoot=char and char:FindFirstChild("HumanoidRootPart")
                if not targetRoot or not myRoot or not char then return false end

                ensureAngelWings()

                local offset
                if nearForStomp then
                    offset=targetRoot.CFrame.LookVector*2.25 + Vector3.new(0,1.3,0)
                else
                    offset=Vector3.new(0,math.max(rageOrbitHeight,3),math.max(rageOrbitRadius,4))
                end

                local pos=targetRoot.Position+offset
                pcall(function()
                    char:PivotTo(CFrame.lookAt(pos,targetRoot.Position))
            ...
            local function teleportToSelected()
                return teleportToPlayer(selectedPlayer(),false)
            end

            local function forceHitPlayer(pl,count)
                if not pl then return false end
                local backend=_G.KimpetrasKIMBackend
                if not backend and type(getgenv)=="function" then
                    backend=getgenv().KimpetrasKIMBackend
                end
                if not backend or type(backend.ForceHitFire)~="function" then
                    return false
                end

                local n=math.clamp(math.floor(tonumber(count) or 1),1,20)
                task.spawn(function()
                    for i=1,n do
                        local part=targetPartFor(pl,rageShootPart)
                        if not part then break end
                        pcall(backend.ForceHitFire,part)
                        if i<n then task.wait(.012) end
                    end
                end)
                return true
            end

            local function forceHitSelected(count)
                return forceHitPlayer(selectedPlayer(),count)
            end
            ...
            local function pressEOnce()
                ...
                if type(kp)=="function" then
                    pcall(kp,0x45)
                    task.wait(.025)
                    if type(kr)=="function" then pcall(kr,0x45) end
                    return true
                end
                return false
            end
            ...
            local downNames={
                ["k.o"]=true,["ko"]=true,["knocked"]=true,["knockedout"]=true,
                ["downed"]=true,["dead"]=true,["unconscious"]=true
            }

            local function boolishTrue(v)
                if v:IsA("BoolValue") then return v.Value==true end
                if v:IsA("IntValue") or v:IsA("NumberValue") then return v.Value~=0 end
                if v:IsA("StringValue") then
                    local s=tostring(v.Value):lower()
                    return s=="true" or s=="1" or s=="yes" or s=="knocked" or s=="downed"
                end
                return false
            end

            local function isTargetDowned(pl)
                local char=pl and pl.Character
                ...
                for _,d in ipairs(char:GetDescendants()) do
                    if downNames[d.Name:lower()] and boolishTrue(d) then
                        return true
                    end
                end

                local data=pl:FindFirstChild("DataFolder")
                if data then
            ...
            local function stompAndAdvance(pl)
                if stompBusy or not pl then return end
                stompBusy=true

                task.spawn(function()
                    if rageAutoStomp then
                        for _=1,math.clamp(math.floor(rageStompRepeats),1,10) do
                            if not pl.Parent then break end
                            teleportToPlayer(pl,true)
                            task.wait(.055)
                            pressEOnce()
                            task.wait(.075)
                        end
                    ...
                    if rageAutoAdvance then
                        local id=pl.UserId
                        if targetSet[id] then removeTargetById(id) end
                        rageOrbitAngle=0
                        rageTeleportAccumulator=0
                        rageShootAccumulator=0
                    end
                    stompBusy=false
                end)
            end
            ...
            local function makeButton(parent,text,pos,size,callback)
                ...
                corner(b,9)
                stroke(b,T.stroke,.3,1)
                b.MouseButton1Click:Connect(callback)
                return b
            end

            local function makeToggle(page,label,defaultValue,callback)
                local state=defaultValue
                local c=rageCard(page,48)
                rageText(c,label,UDim2.new(1,-82,1,0),UDim2.fromOffset(12,0),Enum.Font.GothamSemibold,12,T.text)
                local b
                b=makeButton(c,state and "ON" or "OFF",UDim2.new(1,-68,.5,-15),UDim2.fromOffset(56,30),function()
                    state=not state
                    b.Text=state and "ON" or "OFF"
                    b.BackgroundColor3=state and T.hot or T.card2
                    b.TextColor3=state and T.white or T.text
                    callback(state)
                end)
                if state then
                    b.BackgroundColor3=T.hot
            ...
            local function makeSlider(page,label,min,max,defaultValue,callback,suffix)
                local value=defaultValue
                local c=rageCard(page,64)
                rageText(c,label,UDim2.new(.65,-12,0,24),UDim2.fromOffset(12,5),Enum.Font.GothamSemibold,12,T.text)
                local val=rageText(c,"",UDim2.new(.35,-12,0,24),UDim2.new(.65,0,0,5),Enum.Font.Gotham,11,T.sub,Enum.TextXAlignment.Right)
                ...
                local function setValue(v)
                    value=math.clamp(tonumber(v) or defaultValue,min,max)
                    local a=(value-min)/(max-min)
                    fill.Size=UDim2.new(a,0,1,0)

                    if suffix=="s" then
                        val.Text=string.format("%.2fs",value)
                    elseif suffix=="x" then
                        val.Text=string.format("%.1fx",value)
                    elseif suffix=="#" then
                        val.Text=tostring(math.floor(value+.5))
                    else
                        val.Text=tostring(math.floor(value*10+.5)/10)..(suffix or "")
                    end
                    callback(value)
                end
            ...
            local function makeCycle(page,label,items,defaultValue,callback)
                local idx=table.find(items,defaultValue) or 1
                local c=rageCard(page,50)
                rageText(c,label,UDim2.new(.48,-12,1,0),UDim2.fromOffset(12,0),Enum.Font.GothamSemibold,12,T.text)
                local b
                b=makeButton(c,items[idx],UDim2.new(.48,0,.5,-15),UDim2.new(.52,-12,0,30),function()
                    idx=(idx % #items)+1
                    b.Text=items[idx]
                    callback(items[idx])
                end)
            ...
            -- ========================================================
            -- RAGE CAMLOCK / PLAYER QUEUE
            -- ========================================================
            local camTargetCard=rageCard(rageCamPage,350)

            local previewPanel=Instance.new("Frame",camTargetCard)
            previewPanel.Size=UDim2.new(.44,-14,1,-18)
            previewPanel.Position=UDim2.fromOffset(8,9)
            previewPanel.BackgroundColor3=T.bg2
            previewPanel.BorderSizePixel=0
            corner(previewPanel,12)
            stroke(previewPanel,T.stroke,.3,1)

            previewTitle=rageText(
                previewPanel,"Hover a player for 3D preview",
                UDim2.new(1,-16,0,34),UDim2.fromOffset(8,6),
                Enum.Font.GothamBold,11,T.text,Enum.TextXAlignment.Center
            )
            previewTitle.TextWrapped=true

            previewBackdrop=Instance.new("Frame")
            previewBackdrop.Name="KimqRagePreviewBackdrop"
            previewBackdrop.Parent=previewPanel
            previewBackdrop.Position=UDim2.fromOffset(8,44)
            previewBackdrop.Size=UDim2.new(1,-16,0,214)
            previewBackdrop.BackgroundColor3=T.panel
            previewBackdrop.BorderSizePixel=0
            corner(previewBackdrop,11)
            stroke(previewBackdrop,T.stroke,.36,1)

            previewGradient=Instance.new("UIGradient")
            previewGradient.Parent=previewBackdrop
            previewGradient.Color=ColorSequence.new({
                ColorSequenceKeypoint.new(0,T.soft),
                ColorSequenceKeypoint.new(.55,T.bg2),
                ColorSequenceKeypoint.new(1,T.hot),
            })
            previewGradient.Rotation=90

            local previewGlow=Instance.new("Frame")
            previewGlow.Name="Glow"
            previewGlow.Parent=previewBackdrop
            previewGlow.AnchorPoint=Vector2.new(.5,.5)
            previewGlow.Position=UDim2.fromScale(.5,.54)
            previewGlow.Size=UDim2.fromOffset(146,146)
            previewGlow.BackgroundColor3=T.white
            previewGlow.BackgroundTransparency=.86
            previewGlow.BorderSizePixel=0
            corner(previewGlow,999)

            previewViewport=Instance.new("ViewportFrame")
            previewViewport.Name="KimqRageAvatarPreview"
            previewViewport.Parent=previewBackdrop
            previewViewport.Position=UDim2.fromOffset(0,0)
            previewViewport.Size=UDim2.fromScale(1,1)
            previewViewport.BackgroundColor3=T.panel
            ...
            corner(previewViewport,11)

            local selectedPreviewLabel=rageText(
                previewPanel,queueStatus(),
                UDim2.new(1,-16,0,42),UDim2.fromOffset(8,267),
                Enum.Font.GothamSemibold,10,T.sub,Enum.TextXAlignment.Center
            )
            selectedPreviewLabel.TextWrapped=true
            table.insert(selectedLabels,selectedPreviewLabel)

            previewHelp=rageText(
                previewPanel,"click = add/remove target • hover = preview",
                UDim2.new(1,-16,0,20),UDim2.fromOffset(8,315),
                Enum.Font.Gotham,9,T.sub,Enum.TextXAlignment.Center
            )

            refreshRagePreviewTheme()
            _G.KimqRefreshRageTheme=refreshRagePreviewTheme

            local right=Instance.new("Frame",camTargetCard)
            right.Size=UDim2.new(.56,-18,1,-18)
            right.Position=UDim2.new(.44,8,0,9)
            right.BackgroundColor3=T.bg2
            right.BorderSizePixel=0
            corner(right,12)
            stroke(right,T.stroke,.3,1)

            rageText(right,"Target Queue",UDim2.new(1,-78,0,28),UDim2.fromOffset(10,6),Enum.Font.GothamBold,12,T.text)
            local refreshPlayers=makeButton(right,"refresh",UDim2.new(1,-70,0,6),UDim2.fromOffset(60,27),function() end)

            local queueInfo=rageText(
                right,"Click players to add them to your target queue.",
                UDim2.new(1,-20,0,28),UDim2.fromOffset(10,36),
                Enum.Font.Gotham,9,T.sub
            )
            queueInfo.TextWrapped=true
            table.insert(queueLabels,queueInfo)

            playerList=Instance.new("ScrollingFrame",right)
            playerList.Position=UDim2.fromOffset(8,68)
            playerList.Size=UDim2.new(1,-16,1,-76)
            playerList.BackgroundTransparency=1
            playerList.BorderSizePixel=0
            playerList.ScrollBarThickness=2
            playerList.ScrollBarImageColor3=T.hot
            ...
            plistLayout:GetPropertyChangedSignal("AbsoluteContentSize"):Connect(function()
                playerList.CanvasSize=UDim2.new(0,0,0,plistLayout.AbsoluteContentSize.Y+6)
            end)
            ...
            refreshPlayersList=function()
                ...
                local list={}
                for _,p in ipairs(PlayersR:GetPlayers()) do
                ...
                local currentId=selectedUserId()

                for i,p in ipairs(list) do
                    local queued=targetSet[p.UserId]==true
                    local active=p.UserId==currentId
                    local queueIdx=queueIndexOf(p.UserId)

                    local b=Instance.new("TextButton")
                    b.Parent=playerList
                    b.LayoutOrder=i
                    b.Size=UDim2.new(1,-2,0,48)
                    b.BackgroundColor3=active and T.hot or (queued and T.card2 or T.panel)
                    b.BorderSizePixel=0
                    b.Text=""
                    b.AutoButtonColor=false
                    corner(b,9)
                    stroke(b,queued and T.hot or T.stroke,queued and .08 or .3,1)

                    local badge=rageText(
                        b,queued and ("#"..tostring(queueIdx)) or "+",
                        UDim2.fromOffset(28,28),UDim2.fromOffset(6,10),
                        Enum.Font.GothamBold,11,active and T.white or (queued and T.hot or T.sub),
                        Enum.TextXAlignment.Center
                    )

                    local dn=rageText(
                        b,p.DisplayName,
                        UDim2.new(1,-46,0,20),UDim2.fromOffset(39,5),
                        Enum.Font.GothamBold,11,active and T.white or T.text
                    )
                    local un=rageText(
                        b,"@"..p.Name,
                        UDim2.new(1,-46,0,16),UDim2.fromOffset(39,25),
                        Enum.Font.Gotham,9,active and T.white or T.sub
                    )

                    b.MouseEnter:Connect(function()
                        hoverUserId=p.UserId
                        showAvatarPreview(p)
                    end)
                    b.MouseLeave:Connect(function()
                        if hoverUserId==p.UserId then
                            hoverUserId=nil
                            showAvatarPreview(selectedPlayer())
                        end
                    end)
            ...
            refreshPlayers.MouseButton1Click:Connect(refreshPlayersList)
            PlayersR.PlayerAdded:Connect(function()
                task.defer(refreshPlayersList)
            end)
            PlayersR.PlayerRemoving:Connect(function(p)
                if targetSet[p.UserId] then
                    removeTargetById(p.UserId)
                end
                if hoverUserId==p.UserId then hoverUserId=nil end
                task.defer(refreshPlayersList)
            end)
            refreshPlayersList()

            local actionCard=rageCard(rageCamPage,100)
            makeButton(actionCard,"Lock",UDim2.fromOffset(10,9),UDim2.new(.25,-13,0,35),function()
                if selectedPlayer() then rageLockEnabled=true end
            end)
            makeButton(actionCard,"Unlock",UDim2.new(.25,3,0,9),UDim2.new(.25,-13,0,35),function()
                rageLockEnabled=false
            end)
            makeButton(actionCard,"View",UDim2.new(.5,6,0,9),UDim2.new(.25,-13,0,35),function()
                if selectedPlayer() then
                    rageViewEnabled=true
                    ensureViewTarget()
                end
            end)
            makeButton(actionCard,"Unview",UDim2.new(.75,9,0,9),UDim2.new(.25,-19,0,35),function()
                restoreOwnCamera()
            end)
            makeButton(actionCard,"Teleport",UDim2.fromOffset(10,54),UDim2.new(.5,-15,0,35),function()
                teleportToSelected()
            end)
            makeButton(actionCard,"Clear Queue",UDim2.new(.5,5,0,54),UDim2.new(.5,-15,0,35),function()
                clearTargetQueue()
                restoreOwnCamera()
            end)

            makeCycle(
                rageCamPage,"Target Part",
                {"Head","UpperTorso","HumanoidRootPart","Closest Part"},
                rageTargetPart,
                function(v) rageTargetPart=v end
            )
            makeSlider(
                rageCamPage,"Camlock Smoothness",
                .03,1,rageSmoothness,
                function(v) rageSmoothness=v end,""
            )

            -- ========================================================
            -- TARGET ORBIT / AUTO TELEPORT
            -- ========================================================
            local orbitTargetCard=rageCard(rageOrbitPage,78)
            rageText(orbitTargetCard,"Current Queue Target",UDim2.new(1,-24,0,22),UDim2.fromOffset(12,8),Enum.Font.GothamBold,13,T.hot)
            local orbitSelected=rageText(orbitTargetCard,queueStatus(),UDim2.new(1,-24,0,38),UDim2.fromOffset(12,34),Enum.Font.Gotham,11,T.sub)
            orbitSelected.TextWrapped=true
            table.insert(selectedLabels,orbitSelected)

            local orbitAction=rageCard(rageOrbitPage,54)
            makeButton(
                orbitAction,"♥  Teleport To Current Target With Angel Wings",
                UDim2.fromOffset(10,10),UDim2.new(1,-20,0,34),
                function() teleportToSelected() end
            )

            makeToggle(rageOrbitPage,"Auto Teleport To Current Target",false,function(v)
                rageAutoTeleport=v
                rageTeleportAccumulator=0
                if v then ensureAngelWings() end
            end)
            makeSlider(rageOrbitPage,"Auto Teleport Delay",.05,2,rageTeleportInterval,function(v)
                rageTeleportInterval=v
            end,"s")

            makeToggle(rageOrbitPage,"Auto Orbit Current Target",false,function(v)
                rageOrbitEnabled=v
                rageOrbitAngle=0
                if v then ensureAngelWings() end
            end)
            makeSlider(rageOrbitPage,"Orbit Speed",.25,25,rageOrbitSpeed,function(v)
                rageOrbitSpeed=v
            end,"x")
            makeSlider(rageOrbitPage,"Orbit Radius",2.5,35,rageOrbitRadius,function(v)
                rageOrbitRadius=v
            end,"")
            makeSlider(rageOrbitPage,"Orbit Height",-2,20,rageOrbitHeight,function(v)
                rageOrbitHeight=v
            end,"")

            local orbitNote=rageCard(rageOrbitPage,62)
            local note=rageText(
                orbitNote,
                "Orbit follows whichever player is CURRENT in your queue. Auto Hunt can use this same fast orbit and automatically switch to the next target.",
                UDim2.new(1,-24,1,-12),UDim2.fromOffset(12,6),
                Enum.Font.Gotham,10,T.sub
            )
            note.TextWrapped=true

            -- ========================================================
            -- RAGE COMBAT / AUTO HUNT
            -- ========================================================
            local combatTargetCard=rageCard(rageCombatPage,78)
            rageText(combatTargetCard,"Current Queue Target",UDim2.new(1,-24,0,22),UDim2.fromOffset(12,8),Enum.Font.GothamBold,13,T.hot)
            local combatSelected=rageText(combatTargetCard,queueStatus(),UDim2.new(1,-24,0,38),UDim2.fromOffset(12,34),Enum.Font.Gotham,11,T.sub)
            combatSelected.TextWrapped=true
            table.insert(selectedLabels,combatSelected)

            makeCycle(
                rageCombatPage,"Hit Part",
                {"Head","UpperTorso","HumanoidRootPart","Closest Part"},
                rageShootPart,
                function(v) rageShootPart=v end
            )

            makeToggle(rageCombatPage,"Auto Hunt Target Queue",false,function(v)
                rageAutoHunt=v
                rageShootAccumulator=0
                rageTeleportAccumulator=0
                rageOrbitAngle=0
            ...
            makeToggle(rageCombatPage,"Hunt Uses Fast Orbit",true,function(v)
                rageHuntOrbit=v
            end)

            makeToggle(rageCombatPage,"Auto Shoot Current Target",false,function(v)
                rageAutoShoot=v
                rageShootAccumulator=0
            end)

            makeSlider(rageCombatPage,"Auto Shoot Delay",.02,1,rageShootInterval,function(v)
                rageShootInterval=v
            end,"s")

            makeSlider(rageCombatPage,"Force Hit Burst",1,15,rageForceBurstCount,function(v)
                rageForceBurstCount=math.floor(v+.5)
            end,"#")

            makeToggle(rageCombatPage,"Auto Stomp / Finish With E",true,function(v)
                rageAutoStomp=v
            end)

            makeSlider(rageCombatPage,"Stomp E Repeats",1,8,rageStompRepeats,function(v)
                rageStompRepeats=math.floor(v+.5)
            end,"#")

            makeToggle(rageCombatPage,"Auto Advance After Finish",true,function(v)
                rageAutoAdvance=v
            end)

            local oneShotCard=rageCard(rageCombatPage,78)
            makeButton(
                oneShotCard,
                "♥  ONE-CLICK FORCE HIT BURST",
                UDim2.fromOffset(10,8),UDim2.new(1,-20,0,36),
                function()
                    forceHitSelected(rageForceBurstCount)
                end
            )
            local oneShotNote=rageText(
                oneShotCard,
                "One click now sends your chosen Force Hit burst at the current target. Server-side damage rules can still limit the result.",
                UDim2.new(1,-20,0,25),UDim2.fromOffset(10,47),
                Enum.Font.Gotham,9,T.sub
            )
            oneShotNote.TextWrapped=true

            local huntNoteCard=rageCard(rageCombatPage,70)
            local huntNote=rageText(
                huntNoteCard,
                "Auto Hunt: goes to the CURRENT queued player → fast-orbits them → fires Force Hit bursts → detects KO/down state → teleports close and presses E → removes them from the queue → continues to the next target.",
                UDim2.new(1,-24,1,-12),UDim2.fromOffset(12,6),
                Enum.Font.Gotham,9,T.sub
            )
            huntNote.TextWrapped=true
            ...
            RunServiceR.RenderStepped:Connect(function(dt)
                ...
                if rageLockEnabled and pl then
                    local part=targetPartFor(pl,rageTargetPart)
                    if part and cameraR then
                        local wanted=CFrame.lookAt(cameraR.CFrame.Position,part.Position)
                        cameraR.CFrame=cameraR.CFrame:Lerp(
                            wanted,
                            math.clamp(rageSmoothness,0,1)
                        )
                    end
                end

                if rageViewEnabled then
                    ensureViewTarget()
                end

                local shouldOrbit=(rageOrbitEnabled or (rageAutoHunt and rageHuntOrbit))
                if shouldOrbit and pl then
                    local targetRoot=pl.Character and pl.Character:FindFirstChild("HumanoidRootPart")
                    local char=lp.Character
                    local myRoot=char and char:FindFirstChild("HumanoidRootPart")

                    if targetRoot and char and myRoot then
                        ensureAngelWings()
                        rageOrbitAngle += dt*rageOrbitSpeed*math.pi*2

                        local offset=Vector3.new(
                            math.cos(rageOrbitAngle)*rageOrbitRadius,
                            rageOrbitHeight,
                            math.sin(rageOrbitAngle)*rageOrbitRadius
                        )
                        local pos=targetRoot.Position+offset
            ...
            RunServiceR.Heartbeat:Connect(function(dt)
                ...
                -- Manual auto teleport or Auto Hunt without orbit.
                if pl and (rageAutoTeleport or (rageAutoHunt and not rageHuntOrbit)) then
                    rageTeleportAccumulator+=dt
                    if rageTeleportAccumulator>=rageTeleportInterval then
                        rageTeleportAccumulator=0
                        teleportToPlayer(pl,false)
                    end
                else
                    rageTeleportAccumulator=0
                end

                -- Auto Shoot is normal repeated fire.
                -- Auto Hunt uses the stronger configured Force Hit burst.
                local shouldShoot=pl and (rageAutoShoot or rageAutoHunt)
                if shouldShoot then
                    rageShootAccumulator+=dt
                    if rageShootAccumulator>=rageShootInterval then
                        rageShootAccumulator=0
                        forceHitPlayer(pl,rageAutoHunt and rageForceBurstCount or 1)
                    end
                else
                    rageShootAccumulator=0
                end

                -- Don't recursively scan the target's state every frame.
                local now=os.clock()
                if rageAutoHunt and pl and not stompBusy and now-lastDownCheck>=.08 then
                    lastDownCheck=now
                    if isTargetDowned(pl) then
                        stompAndAdvance(pl)
                    end
                end
            end)

            -- Your own death does not clear the queue/current target.
            lp.CharacterAdded:Connect(function()
                task.delay(.35,function()
                    if rageViewEnabled then ensureViewTarget() end
                    if rageOrbitEnabled or rageAutoHunt or rageAutoTeleport then
                        ensureAngelWings()
                    end
                end)
            end)
...
-- v2.51 module: local catalog / limited accessory try-on.
task.spawn(function()
    _G.KimqAccessoryUIReady=false
    local Players = game:GetService("Players")
    local CoreGui = game:GetService("CoreGui")
...
-- v2.51 module: Environment + Weapon Skins (canonical sidebar pages).
task.spawn(function()
    local pageReadyStart=tick()
    while not _G.KimqBasePagesReady and tick()-pageReadyStart<8 do task.wait(.02) end
    ...
    -- v2.51: Environment and Weapon Skins are already real canonical pages/buttons.
    ...
    pcall(function()
        RunService:BindToRenderStep("KimqPermanentTimeV251",Enum.RenderPriority.Last.Value,function()
    ...
    if not envBuildOk then
        warn("[Kimqetras HC v2.51] Environment page fallback: "..tostring(envBuildErr))
    ...
    if not extrasOk then
        ...
        warn("[Kimqetras HC v2.51] restored v2.19 Weapon Extras: "..tostring(extrasErr))
    ...
    -- ========================================================
    -- v2.51 ISOLATED ANGEL WINGS FLIGHT
    ...
    task.spawn(function()
        ...
        local function makeMini()
            ...
            f.Name="Frame"
            f.Size=UDim2.fromOffset(238,116)
            f.Position=UDim2.new(1,-254,.5,-58)
            f.BackgroundColor3=p.bg
            f.BorderSizePixel=0
            local c=Instance.new("UICorner",f); c.CornerRadius=UDim.new(0,14)
            local s=Instance.new("UIStroke",f); s.Color=p.line; s.Transparency=.25; s.Thickness=1

            local title=Instance.new("TextLabel",f)
            title.Name="Title"; title.Size=UDim2.new(1,-24,0,26); title.Position=UDim2.fromOffset(12,8)
            title.BackgroundTransparency=1; title.Text="♥  Angel Wings"; title.TextColor3=p.text
            ...
            track.Name="Track"; track.Size=UDim2.new(1,-24,0,10); track.Position=UDim2.fromOffset(12,61)
            ...
            status.Name="Status"; status.Size=UDim2.new(1,-24,0,25); status.Position=UDim2.fromOffset(12,80)
    ...
    skinsBtn.MouseButton1Click:Connect(function()
        task.defer(function()
            ...
            if extras and type(extras.Refresh)=="function" then
                ...
                if not ok then
                    warn("[Kimqetras HC v2.51] extras refresh: "..tostring(err))
...
task.spawn(function()
    ...
    -- v2.51: preserve the grouped sidebar order from pageDefs.
    ...
    local function applyTheme(name,settlePass)
        ...
        if type(_G.KimqRefreshRageTheme)=="function" then pcall(_G.KimqRefreshRageTheme) end
        ...
        -- v2.51 hard guarantee: old controls created with literal pink/yellow
    ...
    -- v2.51: section navigation does not repaint/rescan the entire GUI.
    ...
    -- v2.51 reveal happens only after this initial theme pass is complete.
end)


-- v2.51: delayed post-show cleanup passes removed.
...
-- ========================================================
-- v2.51 branding is deterministic at construction time; no delayed scrub pass.

-- ========================================================
-- v2.51 DETERMINISTIC REVEAL
...
task.spawn(function()
    ...
    -- v2.51: remove the old one-page section divider labels from the
