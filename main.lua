-- Kimqetras HC v2.21 • fixed sticky target jump zoom + safe spawn/respawn + softer local bullet glow + owner/credits cleanup
-- Re-running this SAME build only reopens the current GUI instead of stacking another copy.
-- If an older build is already active, v2.21 refuses to stack and asks for one clean rejoin.
do
    local SINGLE_KEY = "KimqHC_v21_SingleRuntimeOptimized"
    local BUILD = "v2.21-jump-zoom-respawn-fix"
    ...
    -- Older builds did not expose a complete disconnect/cleanup handle. Starting v2.21
    ...
    if rawget(_G, SINGLE_KEY) and rawget(_G, "KimqHC_CurrentBuild") ~= BUILD then
        pcall(function()
            game:GetService("StarterGui"):SetCore("SendNotification", {
                Title = "Kimqetras HC",
                Text = "Older runtime detected. Rejoin once, then run v2.21 so only one version is active.",
                Duration = 7,
            })
        end)
        warn("[Kimqetras HC v2.21] Older runtime detected. Rejoin once before running v2.21; refusing to stack versions.")
        return
    end
...
-- v2.21 keeps the v2.15 local bullet fix, camera settings, owner/credit cleanup, soft bullet glow, and fixes jump zoom + respawn stability.
...
-- Target Jump Zoom ---------------------------------------------------------
-- v2.21: self-contained + sticky.  v2.20 accidentally referenced core locals
-- (Players/cam/helper functions) from a separate loadstring chunk, so the watcher
-- could error instead of reacting.  This version only uses values exported in C.
-- It never rotates or locks the camera; it only requests the existing smooth zoom.
local Players = C.Players
local cam = C.cam or workspace.CurrentCamera
local mouse = C.mouse or lp:GetMouse()
...
local function cursorPoint()
    -- Mouse.X/Y lines up with WorldToViewportPoint better than GetMouseLocation
    -- on clients where the Roblox top-bar inset is present.
    local ok,x,y=pcall(function() return mouse.X,mouse.Y end)
    if ok and type(x)=="number" and type(y)=="number" then return Vector2.new(x,y) end
    local p=UIS:GetMouseLocation()
    return Vector2.new(p.X,p.Y)
end

local function screenInfoFor(plr)
    ...
    if (root.Position-cam.CFrame.Position).Magnitude>maxWorld then return nil end
    local sp,onScreen=cam:WorldToViewportPoint(root.Position)
    if not onScreen or sp.Z<=0 then return nil end
    local d=(Vector2.new(sp.X,sp.Y)-cursorPoint()).Magnitude
    return d,root
end

local function getJumpZoomTarget()
    local baseFov=tonumber(cfg.silentAimFOV) or 150
    -- Slightly more forgiving than silent aim because this is only a camera reaction.
    local radius=math.clamp(baseFov*1.55,120,340)

    -- Sticky target: while the person you were aiming at is still reasonably close
    -- to the cursor, keep them instead of bouncing between two people in a fight.
    if jumpZoomTrackedPlayer and jumpPlayerAlive(jumpZoomTrackedPlayer) and not jumpSameTeam(jumpZoomTrackedPlayer) then
        local d=screenInfoFor(jumpZoomTrackedPlayer)
        if d and d<=radius*1.35 then return jumpZoomTrackedPlayer end
    end

    local best,bestDist=nil,radius
    for _,plr in ipairs(Players:GetPlayers()) do
        if plr~=lp and not (_G.KHWhitelist and _G.KHWhitelist[plr.UserId]) and not jumpSameTeam(plr) then
            local d=screenInfoFor(plr)
...
local function humanoidIsAirborne(hum,root)
    ...
    -- Some games override Humanoid states. Upward velocity catches the actual jump.
    local ok,vy=pcall(function() return root.AssemblyLinearVelocity.Y end)
    return ok and type(vy)=="number" and vy>2.2 and hum.FloorMaterial==Enum.Material.Air
end

local function triggerJumpZoom()
    if not _G.KimqJumpZoomEnabled or not _G.KimqShotCameraSwapEnabled then return end
    if not gunIsActuallyEquipped(equippedCameraGun) then return end
    if inFirstPerson() then return end

    local now=os.clock()
    if now-jumpZoomLastTrigger<0.24 then return end
    ...
    cameraSwapToken+=1
    local token=cameraSwapToken
    local tool=equippedCameraGun
    task.spawn(function()
        smoothZoomTo(0.5,token,tool)
    end)
end

RunService.RenderStepped:Connect(function(dt)
    jumpZoomAccumulator+=dt
    if jumpZoomAccumulator<0.033 then return end -- ~30 Hz
    jumpZoomAccumulator=0

    if not _G.KimqJumpZoomEnabled or not _G.KimqShotCameraSwapEnabled or not gunIsActuallyEquipped(equippedCameraGun) then
        jumpZoomTrackedPlayer=nil
        jumpZoomWasAirborne=false
        return
    end
    ...
    if jumpZoomTrackedPlayer~=target then
        ...
        -- Treat a newly acquired target as grounded for edge detection. If you move
        -- onto them during the upward part of their jump, it still reacts once.
        jumpZoomWasAirborne=false
    end

    if airborne and not jumpZoomWasAirborne then triggerJumpZoom() end
    ...
    do
        ...
        if spawnPage then
            ...
            local function moveCharacter(char)
                ...
                local root = char and char:FindFirstChild("HumanoidRootPart")
                if not root then root = char and char:FindFirstChild("UpperTorso") end
                if not root then root = char and char:FindFirstChild("Torso") end
                local hum = char and char:FindFirstChildOfClass("Humanoid")
                if not root then return false end

                -- Put the rig clearly above the pad and clear any velocity before/after.
                -- v2.20 moved the fresh character twice (.16s and .48s), which could
                -- collide with the game's own spawn placement and launch the rig upward.
                local lift = math.max(4.25, (hum and hum.HipHeight or 2) + 2.25)
                local dest = cf * CFrame.new(0, lift, 0)
                zeroCharacterVelocity(char)
                local ok = pcall(function() char:PivotTo(dest) end)
                if ok then
                    task.defer(function()
                        if char and char.Parent then zeroCharacterVelocity(char) end
                    end)
                end
                return ok
            end
            ...
            lp.CharacterAdded:Connect(function(char)
                ...
                -- A real SpawnLocation already handles respawning through
                -- LocalPlayer.RespawnLocation; do not fight the game with a PivotTo.
                local obj = selectedSpawn
                if not (obj and obj.Parent) then obj = resolvePath(selectedPath); selectedSpawn = obj end
                ...
                -- Generic map spawn markers need one manual move, only after the fresh
                -- character has settled. One move + zero velocity prevents respawn fling.
                task.delay(.60,function()
                    if char and char.Parent and useSelectedSpawn then moveCharacter(char) end
                end)
            end)
