-- Kimqetras HC v2.71 • v2.69 base + isolated RAGE local-register startup fix
...
local KIMQ_BUILD = "v2.71-rage-local-limit-fix"
...
do
    ...
    if running and runningBuild == KIMQ_BUILD then
        if runningGui then
            ...
            notifyBoot((runtimeState=="ready") and "v2.71 is already running ♡" or "v2.71 is already starting ♡",4)
            return
        end
        ...
        notifyBoot("Recovering a failed v2.71 boot ♡",4)
    end
    ...
    if oldRuntimeDetected then
        ...
        if staleBoot then
            ...
            notifyBoot("Another Kimqetras HC runtime is still active. Rejoin once, then run v2.71.",9)
            warn("[Kimqetras HC v2.71] Older/live runtime detected. Refusing to stack; rejoin once.")
            return
        end
    end
...
Hello.Text = "v2.71  •  startup fix + avatar editor ♡"
...
local function safeApplyDescription(humanoid, desc)
    ...
    if not ok then
        ok = pcall(function() humanoid:ApplyDescription(desc) end)
    end
    return ok
end

local function copyBodyColorsFromDummy(char, dummy)
    ...
    if src then
        ...
        if dst then
            pcall(function()
                dst.HeadColor3 = src.HeadColor3
                dst.TorsoColor3 = src.TorsoColor3
                dst.LeftArmColor3 = src.LeftArmColor3
                dst.RightArmColor3 = src.RightArmColor3
                dst.LeftLegColor3 = src.LeftLegColor3
                dst.RightLegColor3 = src.RightLegColor3
            end)
        else
...
local function copyHeadAppearanceFromDummy(char, dummy)
    local targetHead=dummy and dummy:FindFirstChild("Head")
    ...
    for _,child in ipairs(myHead:GetChildren()) do
        if child:IsA("SpecialMesh") or child:IsA("CharacterMesh")
            ...
            or child:IsA("WrapLayer") or child:IsA("FaceControls") then
            pcall(function() child:Destroy() end)
        end
    end
    pcall(function() myHead.Color=targetHead.Color end)
    pcall(function() myHead.Transparency=targetHead.Transparency end)
    ...
    for _,child in ipairs(targetHead:GetChildren()) do
        if child:IsA("SpecialMesh") or child:IsA("Decal") or child:IsA("Texture")
            or child:IsA("SurfaceAppearance") or child:IsA("WrapLayer")
            or child:IsA("FaceControls") then
            pcall(function() child:Clone().Parent=myHead end)
        end
    end
end

local function accessoriesEquivalent(a,b)
    if not a or not b or not a:IsA("Accessory") or not b:IsA("Accessory") then return false end
    if a.Name ~= b.Name then return false end
    local ah, bh = a:FindFirstChild("Handle"), b:FindFirstChild("Handle")
    if ah and bh then
        local amesh = ah:IsA("MeshPart") and tostring(ah.MeshId or "") or ""
        local bmesh = bh:IsA("MeshPart") and tostring(bh.MeshId or "") or ""
        local atex = ah:IsA("MeshPart") and tostring(ah.TextureID or "") or ""
        local btex = bh:IsA("MeshPart") and tostring(bh.TextureID or "") or ""
        if amesh ~= bmesh then return false end
        if atex ~= btex then return false end
    end
    return true
end

local function hasMatchingAccessory(char, srcAcc)
    if not char or not srcAcc then return false end
    for _,obj in ipairs(char:GetChildren()) do
        if obj:IsA("Accessory") and accessoriesEquivalent(obj, srcAcc) then
            return true, obj
        end
    end
    return false, nil
...
local function addCopiedAccessory(char, humanoid, srcAcc)
    if not char or not srcAcc or not srcAcc:IsA("Accessory") then return nil end
    local exists, existing = hasMatchingAccessory(char, srcAcc)
    if exists then
        pcall(function() existing:SetAttribute("KimqCopiedAvatarAccessory", true) end)
        return existing
    end
    local acc = srcAcc:Clone()
    local handle=acc:FindFirstChild("Handle")
    ...
    local added=false
    if humanoid and humanoid.AddAccessory then
        added = pcall(function() humanoid:AddAccessory(acc) end)
    end
    if not added then
        acc.Parent=char
    end
...
local function copyAccessoriesFromDummy(char, humanoid, dummy)
    if not char or not dummy then return end
    for _,item in ipairs(dummy:GetChildren()) do
        if item:IsA("Accessory") then
            pcall(function() addCopiedAccessory(char, humanoid, item) end)
        end
    end
end

local function createAvatarDummyFromUserId(userId)
    if not userId then return nil end
    local ok,dummy=pcall(function()
        return Players:CreateHumanoidModelFromUserId(userId)
    end)
    if ok then return dummy end
    return nil
end

local function finalizeAvatarVisuals(char, humanoid, sourceDummy)
    if not char or not char.Parent then return end
    copyBodyColorsFromDummy(char, sourceDummy)
    copyHeadAppearanceFromDummy(char, sourceDummy)
    copyAccessoriesFromDummy(char, humanoid, sourceDummy)
    applyHeadless(char)
...
local function applyAvatarSnapshot(state,char,quiet)
    ...
    local ok=safeApplyDescription(humanoid, desc)
    pcall(function() desc:Destroy() end)
    if ok then
        activeAvatarSnapshot=state
        loadHiddenAccessoriesFromSnapshot(state)
        task.defer(function()
            local sourceDummy=createAvatarDummyFromUserId(state.SourceUserId)
            task.wait(.16)
            if char and char.Parent then
                finalizeAvatarVisuals(char, humanoid, sourceDummy)
                -- Apply once more after layered/classic accessories finish settling.
                task.delay(.35,function()
                    if char and char.Parent then
                        finalizeAvatarVisuals(char, humanoid, sourceDummy)
                    end
                    if sourceDummy then pcall(function() sourceDummy:Destroy() end) end
                end)
            elseif sourceDummy then
                pcall(function() sourceDummy:Destroy() end)
            end
        end)
...
local function applyAvatarUser(userId, char, quiet)
    ...
    task.spawn(function()
        ...
        local dummy=createAvatarDummyFromUserId(userId)
        ...
        if (not desc) and dummyHum then
            pcall(function() desc=dummyHum:GetAppliedDescription() end)
        end
        if desc then
            ...
            safeApplyDescription(humanoid, desc)
        end
        ...
        -- Head appearance + accessories from the chosen Roblox avatar.
        finalizeAvatarVisuals(char, humanoid, dummy)
        ...
        local rageOK,rageERR=pcall(function()
            ...
            local function applyRagePreset(name)
                ...
                if name=="Solo Full Send" then
                    ...
                    setRageToggle(rageControl.setAutoAdvanceToggle,false)
                ...
                elseif name=="Queue Sweep" then
                    ...
                    setRageToggle(rageControl.setAutoReloadToggle,true)
                    setRageToggle(rageControl.setAutoStompToggle,true)
                    setRageToggle(rageControl.setAutoAdvanceToggle,true)
                    setRageToggle(rageControl.setAntiLockToggle,true)
                    setRageToggle(rageControl.setAutoTeleportToggle,false)
                    setRageToggle(rageControl.setOrbitMasterToggle,false)
                    setRageToggle(rageControl.setAutoShootToggle,false)
                ...
                elseif name=="Orbit + Evasive" then
                    ...
                    setRageToggle(rageControl.setRageHudToggle,true)
                    setRageToggle(rageControl.setAntiLockToggle,true)
                    setRageToggle(rageControl.setOrbitMasterToggle,true)
