-- Kimqetras HC v2.80 • v2.79 base + stable avatar guard + random camera zoom + silent knock check + GUI visibility fix
...
local KIMQ_BUILD = "v2.80-stable-avatar-random-zoom-knock-gui-fix"
...
do
    ...
    if running and runningBuild == KIMQ_BUILD then
        if runningGui then
            ...
            notifyBoot((runtimeState=="ready") and "v2.80 is already running ♡" or "v2.80 is already starting ♡",4)
            return
        end
        ...
        notifyBoot("Recovering a failed v2.80 boot ♡",4)
    end
    ...
    if oldRuntimeDetected then
        ...
        if staleBoot then
            ...
            notifyBoot("STOP: another Kimqetras HC build is already active. v2.80 DID NOT LOAD. Rejoin once, then run only v2.80.",9)
            warn("[Kimqetras HC v2.80] Another build is active. v2.80 DID NOT LOAD; rejoin once before testing.")
            return
        end
    end
...
Hello.Text = "v2.80  •  stable avatar + camera + knock check ♡"
...
task.spawn(function()
    ...
    pcall(function()
        game:GetService("StarterGui"):SetCore("SendNotification",{
            Title="Kimqetras HC",
            Text="v2.80 ACTIVE ♡ • stable avatar guard + knock check + random zoom ready",
            Duration=7,
        })
    end)
