...
addCleanSection("Credits", "developer / credits")
]=====], false) then return end
...
task.spawn(function()
    ...
    local function setupWeaponExtras()
        ...
        -- Restyle only shot VFX. Gun parts are intentionally excluded so selecting
        -- a bullet style can never recolor the weapon itself.
        workspace.DescendantAdded:Connect(function(d)
            if extraState.BulletBeam=="None" then return end
            if not (d:IsA("Beam") or d:IsA("Trail") or d:IsA("ParticleEmitter") or d:IsA("BasePart")) then return end
            if insideToolOrCharacter(d) then return end
            local recent=(os.clock()-lastLocalShot)<=.85
            local looksBullet=bulletish(d)
            if not recent and not looksBullet then return end
            if d:IsA("BasePart") then
                local n=tostring(d.Name):lower()
                if not (n:find("bullet",1,true) or n:find("tracer",1,true) or n:find("projectile",1,true) or n:find("laser",1,true)) then return end
            end
            local src=selectedBeamSource(); if src then copyBulletStyle(d,src) end
        end)
        local function hookShotTool(tool)
            if not tool or not tool:IsA("Tool") or tool:GetAttribute("KimqBulletBeamHook") then return end
            tool:SetAttribute("KimqBulletBeamHook",true)
            styleToolShotVfx(tool)
            tool.DescendantAdded:Connect(function(d)
                if extraState.BulletBeam=="None" then return end
                if d:IsA("Beam") or d:IsA("Trail") or d:IsA("ParticleEmitter") then
                    task.defer(function() if d.Parent then styleToolShotVfx(tool) end end)
                end
            end)
        ...
        local function hookShotContainer(container)
            ...
            if container:GetAttribute("KimqBulletContainerHook") then return end
            container:SetAttribute("KimqBulletContainerHook",true)
            container.ChildAdded:Connect(hookShotTool)
        end
        hookShotContainer(lp:FindFirstChildOfClass("Backpack")); if lp.Character then hookShotContainer(lp.Character) end
        ...
        local function findKnifeTool()
            ...
            local function scan(container)
                ...
                for _,ch in ipairs(container:GetChildren()) do if ch:IsA("Tool") and ch.Name:lower():find("knife",1,true) then return ch end end
            end
        ...
        local function matchingGripAttachment(sourceRoot,targetRoot)
            ...
            local preferred={"RightGripAttachment","GripAttachment","HandleAttachment"}
        ...
        local function modelExtent(obj)
            if not obj then return nil end
            if obj:IsA("Model") then
                local ok,v=pcall(function() return obj:GetExtentsSize() end)
                if ok then return v end
            end
            local p=firstPart(obj); return p and p.Size or nil
        end
        local function buildKnifeVisual(source,tool)
            ...
            if source:IsA("Model") then
                ...
                if cloned:IsA("Tool") or cloned:IsA("Folder") then
        ...
        local function moveKnifeVisual(visual,delta)
            if visual:IsA("Model") then
                local ok,piv=pcall(function() return visual:GetPivot() end)
                if ok and piv then
                    local moved=pcall(function() visual:PivotTo(delta*piv) end)
        ...
        local function findGripPair(visual,tool)
            ...
            for _,n in ipairs(preferred) do
                local a=visual:FindFirstChild(n,true)
                local b=tool:FindFirstChild(n,true)
                if a and b and a:IsA("Attachment") and b:IsA("Attachment") then return a,b end
            end
            for _,a in ipairs(visual:GetDescendants()) do
                if a:IsA("Attachment") then
                    local b=tool:FindFirstChild(a.Name,true)
                    if b and b:IsA("Attachment") then return a,b end
                end
            end
            return nil,nil
        ...
        local function applyKnifeSkin(name,quiet)
            ...
            -- Always anchor the cosmetic to the REAL Tool Handle. Do not auto-scale:
            -- these knife assets are authored at different intentional proportions and
            -- ScaleTo was making some skins tiny/invisible.
            local target=tool:FindFirstChild("Handle")
            ...
            local visual=buildKnifeVisual(source,tool)
            ...
            if not cloneRoot then visual:Destroy(); return false,"Knife skin has no visual Handle" end

            local srcGrip,targetGrip=findGripPair(visual,tool)
            local delta
            if srcGrip and targetGrip then
                -- World-space attachment matching also works when the attachment is not
                -- directly inside the skin's root Handle.
            ...
            -- Preserve the skin's own joints/animation rig. Only weld loose geometry.
            local jointed={}
            for _,j in ipairs(visual:GetDescendants()) do
                if j:IsA("JointInstance") then
                    if j.Part0 then jointed[j.Part0]=true end; if j.Part1 then jointed[j.Part1]=true end
            ...
            -- Hide only the original knife geometry after the replacement exists.
            for _,d in ipairs(tool:GetDescendants()) do
                if d:IsA("BasePart") and not d:IsDescendantOf(visual) then
                    d:SetAttribute("KimqKnifeOriginalPart",true); d:SetAttribute("KimqKnifeOldLTM",d.LocalTransparencyModifier)
        ...
        txt(knifeCard,"Choose a knife skin. Size stays original and animated skins keep their rig.",UDim2.new(1,-24,0,20),UDim2.fromOffset(12,32),Enum.Font.Gotham,11,pcolor("sub",subColor))
...
task.spawn(function()
    ...
    local function classify(o)
        ...
        if o:IsA("UIGradient") then
            if o.Name=="V26BannerGradient" then addEntry(o,{gradient=true}) end
            return
        end
