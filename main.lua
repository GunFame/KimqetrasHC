-- Kimqetras HC v2.88 • v2.87 base + locked stomp finish + semantic page organizer + stronger coherent OP presets
...
local KIMQ_BUILD = "v2.88-stomp-lock-organized-op"
...
do
    ...
    if running and runningBuild == KIMQ_BUILD then
        if runningGui then
            ...
            notifyBoot((runtimeState=="ready") and "v2.88 is already running ♡" or "v2.88 is already starting ♡",4)
            return
        end
        ...
        notifyBoot("Recovering a failed v2.88 boot ♡",4)
    end
    ...
    if oldRuntimeDetected then
        ...
        if staleBoot then
            ...
            notifyBoot("STOP: another Kimqetras HC build is already active. v2.88 DID NOT LOAD. Rejoin once, then run only v2.88.",9)
            warn("[Kimqetras HC v2.88] Another build is active. v2.88 DID NOT LOAD; rejoin once before testing.")
            return
        end
    end
...
Hello.Text = "v2.88  •  stomp lock + organized OP rage ♡"
...
task.spawn(function()
    ...
    task.defer(function()
        local rageOK,rageERR=pcall(function()
            ...
            function R.button(parent,text,pos,size,fn)
                ...
                b.MouseButton1Click:Connect(function()
                    ...
                    if not ok then warn("[Kimqetras HC v2.84 RAGE button] "..tostring(err)) end
                end)
                return b
            end
            ...
            function R.findGun()
                ...
                if equipped and table.find(ForceHitAllowedTools,equipped.Name) then return equipped end
                local backpack=lp:FindFirstChildOfClass("Backpack")
                if backpack then
                    for _,name in ipairs(ForceHitAllowedTools) do
                        local tool=backpack:FindFirstChild(name)
                        if tool and tool:IsA("Tool") then pcall(function() hum:EquipTool(tool) end); return tool end
                    end
                end
                return nil
            end
            ...
            function R.effectiveDelay(tool)
                ...
                return math.max(.025,p and math.min(R.fireDelay,p.delay) or R.fireDelay)
            end
            ...
            function R.shoot(pl)
                ...
                local count=math.clamp(math.floor(R.burst),1,14)
                R.shotBusy=true
                task.spawn(function()
                    for i=1,count do
            ...
            function R.completeCurrent(id)
                ...
                if R.runScope=="solo" or not R.autoAdvance then
            ...
            function R.applyOpSettings(scope,style)
                -- One coherent preset owns the whole loop: acquire/equip -> orbit ->
                -- shoot/reload -> detect K.O. -> lock directly above -> stomp -> next.
                R.autoShoot=true
                R.autoReload=true
                R.autoStomp=true
                ...
                for name,setter in pairs(R.ui.controls) do
                    if type(setter)=="function" then
                        if name=="autoShoot" then pcall(setter,true,true)
                        ...
                        elseif name=="hud" then pcall(setter,true,true)
                        ...
                        elseif name=="orbitHeight" then pcall(setter,R.orbitHeight,true) end
                    end
                end
                R.startRun(scope)
            ...
            function R.buildHud()
                if R.ui.hud and R.ui.hud.Parent then return end
                for _,old in ipairs(gui:GetChildren()) do
                    if old.Name=="KimqRageMiniHUD" then
                        pcall(function() old:Destroy() end)
                    end
                end
                local p=R.pal()
                local f=Instance.new("Frame")
                f.Name="KimqRageMiniHUD"
                f.Parent=gui
                f.AnchorPoint=Vector2.new(1,0)
                f.Position=UDim2.new(1,-22,0,22)
                f.Size=UDim2.fromOffset(300,156)
                f.BorderSizePixel=0
                f.Visible=false
                f.ZIndex=80
                R.role(f,"panel")
                corner(f,12)
                stroke(f,p.line or T.stroke,.18,1)
                R.ui.hud=f
                ...
                R.role(head,"lightBg")
                corner(head,9)
                stroke(head,p.line or T.stroke,.45,1)
                R.ui.hudHeader=head

                local a=Instance.new("Frame")
                a.Parent=head
                a.Size=UDim2.fromOffset(7,26)
                a.Position=UDim2.fromOffset(10,9)
                a.BorderSizePixel=0
                R.role(a,"hotBg")
                corner(a,999)

                R.ui.hudTitle=R.role(R.label(head,"♥  RAGE V3",UDim2.new(1,-130,0,24),UDim2.fromOffset(24,3),Enum.Font.GothamBold,13,p.hot or T.hot),"hotText")
                R.ui.hudHint=R.role(R.label(head,"multi-target combat ♡",UDim2.new(1,-130,0,12),UDim2.fromOffset(24,25),Enum.Font.Gotham,8,p.sub or T.sub),"subText")
                local pill=Instance.new("Frame")
                pill.Parent=head
                pill.Size=UDim2.fromOffset(88,24)
                pill.Position=UDim2.new(1,-98,.5,-12)
                pill.BorderSizePixel=0
                R.role(pill,"hotBg")
                corner(pill,999)
                R.ui.hudPill=pill
                R.ui.hudStatus=R.role(R.label(pill,"READY",UDim2.fromScale(1,1),UDim2.new(),Enum.Font.GothamBold,9,p.white or Color3.new(1,1,1),Enum.TextXAlignment.Center),"whiteText")

                local body=Instance.new("Frame")
                body.Parent=f
                body.Position=UDim2.fromOffset(8,58)
                body.Size=UDim2.new(1,-16,1,-66)
                body.BorderSizePixel=0
                R.role(body,"lightBg")
                corner(body,12)
                stroke(body,p.line or T.stroke,.48,1)
                R.ui.hudBody=body
                R.ui.hudTarget=R.role(R.label(body,"TARGET  ♡  none",UDim2.new(1,-18,0,20),UDim2.fromOffset(12,9),Enum.Font.GothamBold,10,p.text or T.text),"textText")
                R.ui.hudQueue=R.role(R.label(body,"QUEUE  0/0",UDim2.new(1,-18,0,16),UDim2.fromOffset(12,33),Enum.Font.Gotham,9,p.sub or T.sub),"subText")
                R.ui.hudWeapon=R.role(R.label(body,"WEAPON  ♡  none",UDim2.new(1,-18,0,18),UDim2.fromOffset(12,54),Enum.Font.GothamSemibold,9,p.text or T.text),"textText")
                R.ui.hudNext=R.role(R.label(body,"NEXT  ♡  none",UDim2.new(1,-18,0,16),UDim2.fromOffset(12,76),Enum.Font.Gotham,9,p.sub or T.sub),"subText")

                local dragging=false
                local startMouse,startPos
                head.InputBegan:Connect(function(input)
                    if input.UserInputType==Enum.UserInputType.MouseButton1 or input.UserInputType==Enum.UserInputType.Touch then dragging=true; startMouse=input.Position; startPos=f.Position end
                end)
                R.UIS.InputEnded:Connect(function(input)
                    if input.UserInputType==Enum.UserInputType.MouseButton1 or input.UserInputType==Enum.UserInputType.Touch then dragging=false end
                end)
                R.UIS.InputChanged:Connect(function(input)
                    if dragging and startMouse and startPos and (input.UserInputType==Enum.UserInputType.MouseMovement or input.UserInputType==Enum.UserInputType.Touch) then
            ...
            function R.refreshHudTheme()
                local p=R.pal()
                if R.ui.hud then
                    R.ui.hud.BackgroundColor3=p.panel or T.panel
                    if R.ui.hudHeader then R.ui.hudHeader.BackgroundColor3=p.soft or T.bg2 end
                    if R.ui.hudBody then R.ui.hudBody.BackgroundColor3=p.soft or T.bg2 end
                    if R.ui.hudPill then R.ui.hudPill.BackgroundColor3=p.hot or T.hot end
                end
            ...
            function R.updateHud()
                R.buildHud()
                if not R.ui.hud then return end
                local visible=R.hudEnabled and (R.master or R.orbitMaster or R.camLock or R.freeView or R.autoShoot)
                R.ui.hud.Visible=visible
                if not visible then return end
                local pl=R.currentPlayer()
                R.ui.hudStatus.Text=string.upper(R.phase or "READY")
                R.ui.hudTarget.Text=pl and ("TARGET  ♡  "..pl.DisplayName.."  (@"..pl.Name..")") or "TARGET  ♡  none"
                local idx=R.master and R.runIndex or R.activeIndex
                local total=R.master and #R.runIds or #R.queue
                R.ui.hudQueue.Text=("QUEUE  %d/%d  ♡  %s"):format(total==0 and 0 or idx,total,string.upper(R.queueOrder))
                local tool=lp.Character and lp.Character:FindFirstChildOfClass("Tool")
                if tool then
                    local ammo=R.ammo(tool)
                    R.ui.hudWeapon.Text="WEAPON  ♡  "..tostring(tool.Name):gsub("%[",""):gsub("%]","").."  •  AMMO "..(ammo~=nil and tostring(math.floor(ammo+.5)) or "?")
                else
                    R.ui.hudWeapon.Text="WEAPON  ♡  none"
                end
                local nextId=nil
                if R.master then
                    local ni=R.findNextRunIndex(R.runIndex+1)
                    nextId=ni and R.runIds[ni] or nil
                elseif #R.queue>1 then
                    nextId=R.queue[(R.activeIndex%#R.queue)+1]
                end
                local np=nextId and R.Players:GetPlayerByUserId(nextId)
                R.ui.hudNext.Text=np and ("NEXT  ♡  "..np.DisplayName.."  (@"..np.Name..")") or "NEXT  ♡  none"
            end
            ...
            do
                ...
                local _,setHud=R.makeToggle(R.pages.combat,"Rage Mini HUD",R.hudEnabled,function(v) R.hudEnabled=v; R.updateHud() end)
                R.ui.controls.hud=setHud
                ...
                local _,setDelay=R.makeSlider(R.pages.combat,"Fire Interval",.025,.5,R.fireDelay,function(v) R.fireDelay=v end,function(v) return string.format("%.3fs",v) end)
                R.ui.controls.fireDelay=setDelay
                local _,setBurst=R.makeSlider(R.pages.combat,"One-Shot Burst",1,14,R.burst,function(v) R.burst=math.floor(v+.5) end,function(v) return tostring(math.floor(v+.5)) end)
            ...
            -- ---------------- OP PRESETS ----------------
            do
                local c=R.card(R.pages.presets,242)
                R.role(R.label(c,"♥  OP RAGE Presets",UDim2.new(1,-24,0,24),UDim2.fromOffset(12,8),Enum.Font.GothamBold,13,R.pal().hot or T.hot),"hotText")
                local sub=R.role(R.label(c,"full auto: acquire → orbit → shoot/reload → K.O → LOCK ON TOP → E stomp → next ♡",UDim2.new(1,-34,0,38),UDim2.fromOffset(12,34),Enum.Font.Gotham,9,R.pal().sub or T.sub),"subText")
                sub.TextWrapped=true

                R.button(c,"OP ALL TARGETS",UDim2.fromOffset(12,80),UDim2.new(.5,-23,0,40),function()
                    if #R.queue==0 then R.selectAll() end
                    R.applyOpSettings("all","health")
                    R.notify("OP ALL TARGETS armed • "..#R.queue.." selected ♡",3)
                end)
                R.button(c,"OP SOLO",UDim2.new(.5,5,0,80),UDim2.new(.5,-17,0,40),function()
                    R.applyOpSettings("solo","health")
                    R.notify("OP SOLO armed ♡",3)
                end)
                R.button(c,"OP CLOSE HUNT",UDim2.fromOffset(12,130),UDim2.new(.5,-23,0,40),function()
                    if #R.queue==0 then R.selectAll() end
                    R.applyOpSettings("all","close")
                    R.notify("OP CLOSE HUNT armed • "..#R.queue.." selected ♡",3)
                end)
                R.button(c,"STOP / CALM",UDim2.new(.5,5,0,130),UDim2.new(.5,-17,0,40),function()
                    R.stopRun()
                    R.autoShoot=false
                    R.orbitMaster=false
                    R.camLock=false
                ...
                local hint=R.role(R.label(c,"Tip: OP ALL TARGETS keeps your whole checked queue. It auto-equips a supported gun, shoots/reloads, locks over each K.O. for the stomp, then moves on.",UDim2.new(1,-36,0,44),UDim2.fromOffset(12,182),Enum.Font.Gotham,9,R.pal().sub or T.sub),"subText")
            ...
            R.RunService.RenderStepped:Connect(function(dt)
                ...
                if not ok then warn("[Kimqetras HC v2.84 RAGE camera] "..tostring(err)) end
            end)
            ...
            R.RunService.Heartbeat:Connect(function(dt)
                local ok,err=pcall(function()
                    ...
                    if R.master then
                        if not pl then
                            local ni=R.findNextRunIndex(R.runIndex+1)
                            if ni then R.runIndex=ni; R.phase="ACQUIRE"; pl=R.currentPlayer() else R.stopRun("Queue complete ♡"); return end
                        end

                        if R.phase=="ACQUIRE" then
                            local down=R.downState(pl)
                    ...
                    local shouldOrbit=(R.orbitMaster or R.master) and R.phase~="STOMP" and R.phase~="CONFIRM" and not R.finishBusy
                    ...
                    if pl and (R.autoShoot or R.master) and R.phase=="ATTACK" and not R.finishBusy then
                        local tool=R.findGun()
                    ...
                    R.hudClock+=dt
                    if R.hudClock>=.10 then R.hudClock=0; R.updateHud() end
                end)
                if not ok then warn("[Kimqetras HC v2.84 RAGE heartbeat] "..tostring(err)) end
            end)
        ...
        if not rageOK then
            ...
            warn("[Kimqetras HC v2.84] RAGE V3 isolated error: "..tostring(rageERR))
        end
    end)
    ...
    task.spawn(function()
        ...
        local function makeMini()
            ...
            for _,parent in ipairs({CoreGui2,playerGui2}) do
                if parent then
                    for _,old in ipairs(parent:GetChildren()) do
                        if old.Name=="KimqAngelWingsMini" then
                            pcall(function() old:Destroy() end)
                        end
                    end
                end
            ...
            task.delay(.35,function()
                if mini==gui and gui.Parent then
                    local frame=gui:FindFirstChild("Frame")
                    local valid=frame and frame:FindFirstChild("Title") and frame:FindFirstChild("Status") and frame:FindFirstChild("Track")
                    if not valid then
                        pcall(function() gui:Destroy() end)
                        if mini==gui then mini=nil end
                    end
                end
            end)
        end
...
task.spawn(function()
    ...
    pcall(function()
        game:GetService("StarterGui"):SetCore("SendNotification",{
            Title="Kimqetras HC",
            Text="v2.88 ACTIVE ♡ • stomp lock + organized OP rage ready",
            Duration=7,
        })
    end)
...
-- ========================================================
-- v2.88 SEMANTIC FEATURE PAGE ORGANIZER (VISUAL ONLY)
...
task.spawn(function()
    ...
    local pretty={
        hcsilent="HC combat",silent="silent aim",camlock="camlock",flamelock="flamelock",hitbox="hitbox",
    ...
    for _,page in ipairs(pageHost:GetChildren()) do
        if page:IsA("ScrollingFrame") and page.Name:match("Page$") then
            ...
            if not excluded[key] then
                ...
                head.LayoutOrder=1;head.Size=UDim2.new(1,-6,0,44);head.BackgroundColor3=p.soft;head.BorderSizePixel=0;corner(head,12);stroke(head,p.line,.34)
                local h=mklabel(head,"♥",UDim2.fromOffset(28,28),UDim2.fromOffset(10,8),Enum.Font.FredokaOne,18,p.hot);h:SetAttribute("KimqV26Role","hotText")
                local t=mklabel(head,(pretty[key] or key).."  •  organized",UDim2.new(1,-52,0,20),UDim2.fromOffset(38,5),Enum.Font.GothamBold,11,p.text);t:SetAttribute("KimqV26Role","textText")
                local s=mklabel(head,"main feature first ♡  •  related controls underneath",UDim2.new(1,-52,0,16),UDim2.fromOffset(38,23),Enum.Font.Gotham,9,p.sub);s:SetAttribute("KimqV26Role","subText")

                local names={[1]="MAIN FEATURE ♡",[2]="AUTOMATION + BEHAVIOR ♡",[3]="TUNING ♡",[4]="EXTRAS ♡"}
                for g=1,4 do
                    if present[g] then
