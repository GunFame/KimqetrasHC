-- Kimqetras HC v2.1 • universal skin animations + owner notification + saved configs
-- KIMQETRAS HC v2.1 - SINGLE THEME AUTHORITY + LIVE SKIN ANIMATION BRIDGE
...
local function snapshotControls()
    ...
    for name, entry in pairs(_G.KimqConfigControls or {}) do
        if type(entry) == "table" and type(entry.get) == "function" then
...
addButton("Save Current Config", function()
...
addButton("Load Selected Config", function()
...
addButton("Delete Selected Config", function()
    local name = selectedConfig or configName
...
addButton("Refresh Config List", function() refreshConfigList() end)
addSmallNote("Configs save the full setup: every registered toggle/slider/dropdown/keybind, theme, whitelist, avatar target + avatar options, local accessories, fog color/amount, environment preset, weapon-skin selections, and GUI size/position. Loading a preset restores the setup automatically.")

addCleanSection("Credits", "Original document (3) backend")
addSmallNote("KIM feature backend • merged into Kimqetras HC")
]=====], false) then return end
...
task.spawn(function()
    ...
    local tiny = label(banner, "♡  cute tools • clean pages • themed hearts  ♡", UDim2.new(0, 420, 0, 18), UDim2.new(0.5,-210,0.5,20), Enum.Font.GothamSemibold, 11, T.white, Enum.TextXAlignment.Center)
    ...
    local devText = label(dev, "Interface: Kimqetras HC v2.1\nOwner: kimqetras • https://www.roblox.com/users/11150537473/profile\nOriginal developer / scripter: famesgun\nDeveloper profile: https://www.roblox.com/users/4246488996/profile", UDim2.new(1,-24,0,68), UDim2.fromOffset(12,56), Enum.Font.Gotham, 12, T.sub)
