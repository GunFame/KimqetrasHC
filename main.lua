-- Kimqetras HC v2.11 • Angel Wings wearable equippable • keeps v2.10 bullet + macro fixes
-- Re-running this SAME build only reopens the current GUI instead of stacking another copy.
-- If an older build is already active, v2.11 refuses to stack and asks for one clean rejoin.
do
    local SINGLE_KEY = "KimqHC_v21_SingleRuntimeOptimized"
    local BUILD = "v2.11-angel-wings-wearable"
    ...
    -- Older builds did not expose a complete disconnect/cleanup handle. Starting v2.11
    ...
    if rawget(_G, SINGLE_KEY) and rawget(_G, "KimqHC_CurrentBuild") ~= BUILD then
        pcall(function()
            game:GetService("StarterGui"):SetCore("SendNotification", {
                Title = "Kimqetras HC",
                Text = "Older runtime detected. Rejoin once, then run v2.11 so only one version is active.",
                Duration = 7,
            })
        end)
        warn("[Kimqetras HC v2.11] Older runtime detected. Rejoin once before running v2.11; refusing to stack versions.")
        return
    end
...
-- v2.11 keeps the low-lag v2.10 path, but claims the first local shot immediately so its selected color is present before the first visible frame.
    ...
    local function setupWeaponExtras()
        ...
        local function styleLocalToolTemplates(tool)
            ...
            for _,d in ipairs(getToolTemplateEffects(tool)) do
                if d and d.Parent then applyColorOnly(d,c) end
            end
        end
        ...
        local function hookBulletContainer(container)
            ...
            if serial~=0 and (os.clock()-activeShotAt)<=.38 then
                -- Never style the shared bullet_rays container itself; only its newest
                -- shot child can be claimed as ours.
        ...
        -- Catch an already-existing container cheaply, then hook future ones as they appear.
        ...
        local function makeAngelWingsWearable(source,tool)
            ...
            -- The source has its own Humanoid only to make the stored rig usable.
            -- Once worn, the player's Humanoid drives the character, so remove the
            -- nested one to avoid extra character/physics work.
            for _,d in ipairs(visual:GetDescendants()) do
                if d:IsA("Humanoid") then
                    pcall(function() d:Destroy() end)
                elseif d:IsA("BasePart") then
                    -- Keep the clone frozen while it is aligned to the back. It is
                    -- released only after the body weld exists, avoiding a one-frame fling.
                    d.Anchored=true
                    d.CanCollide=false
                    d.CanTouch=false
                    d.CanQuery=false
            ...
            -- Angel Wings' Handle is a carrier block, not part of the visible wings.
            -- Keep it completely invisible so no gray rectangle appears on the back.
            pcall(function()
            ...
            local sourceAttachment,targetAttachment=findMatchingBodyAttachment(handle,char)
            local desiredHandle
            if sourceAttachment and targetAttachment then
                desiredHandle=targetAttachment.Parent.CFrame * targetAttachment.CFrame * sourceAttachment.CFrame:Inverse()
            else
                -- R15/R6 fallback: sit the carrier slightly behind the torso.
                -- The wing pieces retain their authored offsets through their welds.
                desiredHandle=torso.CFrame * CFrame.new(0,0.05,0.52)
            end
            ...
            bodyWeld.Name="KimqAngelWingsBodyWeld"
            bodyWeld.Part0=torso
            bodyWeld.Part1=handle
            bodyWeld.Parent=handle
            ...
            -- If the stored wings include authored animation objects, play them locally.
            task.defer(function()
                if visual.Parent then pcall(function() playSkinAnimationsFromIds(source,visual) end) end
            end)
