-- Kimqetras HC v2.87 • v2.86 base + isolated fast stomp/advance flow
...
local KIMQ_BUILD = "v2.87-fast-stomp-advance"
...
Hello.Text = "v2.87  •  fast stomp + next target ♡"
    ...
    task.defer(function()
        local rageOK,rageERR=pcall(function()
            ...
            function R.finishTarget(pl)
                ...
                -- v2.87: quick finish flow. Stomp once, check whether the target is
                -- still knocked, retry ONE time only if the first stomp appears to
                -- have missed, then immediately advance to the next queued target.
                task.spawn(function()
                    ...
                    while R.master and token==R.runToken and pl.Parent and attempts<maxAttempts do
                        ...
                        -- Give the game a short moment to register the E stomp. If
                        -- the target is still downed afterwards, that counts as a
                        -- miss and we allow exactly one retry.
                        task.wait(.16)
                        local char=pl.Character
                        local hum=char and char:FindFirstChildOfClass("Humanoid")
                        if not char or not hum or hum.Health<=0 or not R.stompable(pl) then
                            break
                        end
                    end
...
task.spawn(function()
    ...
    pcall(function()
        game:GetService("StarterGui"):SetCore("SendNotification",{
            Title="Kimqetras HC",
            Text="v2.87 ACTIVE ♡ • fast stomp + next-target flow ready",
            Duration=7,
        })
    end)
