-- Kimqetras HC v2.4 • instant local bullets • clean knife visuals • persistent full themes
...
-- v2.6 config polish: selected configs can be updated and exact Fog / Atmosphere state is restored.
...
-- Optional camera swap on each local gun shot.
-- Third person -> briefly first person -> back to the same zoom.
-- First person -> briefly third person -> back to first person.
...
local function inFirstPerson()
    ...
    return cameraDistance() <= 1.15
end

local function doShotCameraSwap(tool)
    if not _G.KimqShotCameraSwapEnabled or cameraSwapBusy then return end
    if not tool or tool.Parent ~= lp.Character or not isLikelyGun(tool) then return end

    cameraSwapBusy = true
    local wasFirst = inFirstPerson()
    local oldMode = lp.CameraMode
    local oldMin = lp.CameraMinZoomDistance
    local oldMax = lp.CameraMaxZoomDistance
    local oldDistance = cameraDistance()
    local swapDistance = math.clamp(math.max(oldDistance, 8), 7, 14)

    -- Apply the opposite camera state immediately for the shot.
    pcall(function()
        if wasFirst then
            lp.CameraMode = Enum.CameraMode.Classic
            lp.CameraMinZoomDistance = swapDistance
            lp.CameraMaxZoomDistance = swapDistance
        else
    ...
    task.delay(0.095, function()
        if not lp or not lp.Parent then cameraSwapBusy=false return end

        -- Force the camera back to the exact side it started on before restoring
        -- the user's normal zoom limits. This prevents it getting stuck zoomed.
        pcall(function()
            lp.CameraMode = Enum.CameraMode.Classic
            if wasFirst then
                ...
                local restoreDistance = math.clamp(oldDistance, 0.5, 128)
                lp.CameraMinZoomDistance = restoreDistance
                lp.CameraMaxZoomDistance = restoreDistance
            end
        end)
        ...
        pcall(function()
            ...
            lp.CameraMode = oldMode
        end)
        cameraSwapBusy = false
    end)
end
...
if lp.Character then hookCameraContainer(lp.Character) end
...
local function LoadConfig(configName)
    ...
    if savedFogState and _G.KimqFogController and type(_G.KimqFogController.SetState)=="function" then
        ...
        restoreSavedFog()
        task.delay(0.12, restoreSavedFog)
        task.delay(0.40, restoreSavedFog)
        task.delay(0.90, restoreSavedFog)
    end
...
task.spawn(function()
    ...
    local function setupWeaponExtras()
        ...
        -- v2.3 strict-local path:
        --   * never edits ReplicatedStorage bullet templates
        --   * ONLY opens a capture window from THIS player's Tool.Activated event
        --   * rejects rays with owner metadata for another player
        --   * when owner metadata is missing, requires the new bullet_rays object to
        --     physically start right next to this player's gun muzzle/Handle
        -- This is intentionally stricter than the old MouseButton1 timing fallback.
        local rainbowObjects=setmetatable({}, {__mode="k"})
        local shotTool=nil
        ...
        local claimedShotAt=setmetatable({}, {__mode="k"})
        ...
        local function applyBulletModeTo(d)
            ...
            if extraState.BulletColorMode=="Custom" then
                applyColorOnly(d,bulletCustomColor)
            elseif extraState.BulletColorMode=="Rainbow" then
                rainbowObjects[d]=true
            end
        end

        local function toolMuzzleOrigin(tool)
            if not tool or not tool.Parent then return nil end
            -- Prefer attachments whose names usually describe the actual muzzle.
        ...
        local function ownerVerdict(obj)
            -- true = definitely local, false = definitely someone else, nil = no owner metadata.
            ...
            for _=1,6 do
                ...
                for _,k in ipairs(keys) do
                    ...
                    if ok and v~=nil then
                        ...
                        if s~="" then return s==lp.Name:lower() or s==lp.DisplayName:lower() end
                    end
                end
        ...
        local function bulletRayShotRoot(obj)
            -- Shared bullet_rays folder -> immediate shot child.
            -- Ephemeral object itself named bullet_rays -> that object.
            local x=obj
            local child=obj
            for _=1,12 do
        ...
        local function rayStartsAtOrigin(root,origin)
            ...
            -- Tight distance on purpose: this is what prevents nearby players' rays
            -- from being recolored during the same fraction of a second.
            local MAX_START_DISTANCE=6.5
            local function closePos(pos)
                return pos and (pos-origin).Magnitude<=MAX_START_DISTANCE
            end
            ...
            if root:IsA("BasePart") then
                if closePos(root.Position) then return true end
                local ok,localPoint=pcall(function() return root.CFrame:PointToObjectSpace(origin) end)
                if ok then
                    local half=root.Size*.5+Vector3.new(3,3,3)
            ...
            for _,d in ipairs(root:GetDescendants()) do
                ...
                if d:IsA("BasePart") and closePos(d.Position) then return true end
            end
            return false
        end

        local function styleLocalBulletRay(rootObj)
            if not bulletEffectEnabled() or not rootObj or not rootObj.Parent then return end
            local now=os.clock()
            -- Most bullet_rays objects appear immediately after Tool.Activated. Keep
            -- this window tight so another player's later shot cannot be captured.
            if activeShotSerial==0 or (now-activeShotAt)>.28 then return end

            local shotRoot=bulletRayShotRoot(rootObj)
            if not shotRoot or claimedShotRoots[shotRoot] then return end

            local verdict=ownerVerdict(shotRoot)
            if verdict==false then return end
            if verdict~=true and not rayStartsAtOrigin(shotRoot,activeShotOrigin) then return end

            claimedShotRoots[shotRoot]=activeShotSerial
            claimedShotAt[shotRoot]=now
            applyBulletModeTo(shotRoot)
            for _,d in ipairs(shotRoot:GetDescendants()) do applyBulletModeTo(d) end
            pcall(function()
                shotRoot:SetAttribute("KimqLocalBulletStyled",true)
                shotRoot:SetAttribute("KimqLocalShotSerial",activeShotSerial)
            end)
        end
        ...
        local function hookShotTool(tool)
            ...
            tool.Activated:Connect(function()
                -- Tool.Activated is local to this player's equipped tool, unlike a
                -- global mouse click. That makes ownership matching much more reliable.
                markLocalShot(tool)
            end)
        end
        local function hookShotContainer(container)
            ...
            if container:GetAttribute("KimqBulletContainerHook3") then return end
            container:SetAttribute("KimqBulletContainerHook3",true)
        ...
        lp.CharacterAdded:Connect(function()
            activeShotSerial=0; activeShotOrigin=nil; shotTool=nil
            task.delay(.25,hookAllShotContainers)
        end)

        workspace.DescendantAdded:Connect(function(d)
            if activeShotSerial==0 or (os.clock()-activeShotAt)>.28 then return end
            local shotRoot=bulletRayShotRoot(d)
            if not shotRoot and tostring(d.Name):lower()~="bullet_rays" then return end

            -- If this shot was already verified as ours, recolor each new effect the
            -- instant it is parented so the original color never gets a visible frame.
            if shotRoot and claimedShotRoots[shotRoot]==activeShotSerial then
                applyBulletModeTo(d)
                return
            end

            -- Do ownership/origin verification synchronously instead of task.defer.
            -- This removes the brief flash of the game's original bullet color.
            if d.Parent then styleLocalBulletRay(d) end
        end)

        RunService.Heartbeat:Connect(function()
            if extraState.BulletColorMode~="Rainbow" then return end
            local c=Color3.fromHSV((os.clock()*.45)%1,1,1)
            for d,_ in pairs(rainbowObjects) do
                if d and d.Parent then applyColorOnly(d,c) else rainbowObjects[d]=nil end
            end
        end)

        -- For the first few rendered frames of a verified local shot, force the chosen
        -- color again just before rendering. Some games assign their default beam color
        -- after parenting the effect; this prevents that default color from flashing first.
        RunService.RenderStepped:Connect(function()
            if extraState.BulletColorMode~="Custom" and extraState.BulletColorMode~="Rainbow" then return end
            local now=os.clock()
            local rainbowColor=(extraState.BulletColorMode=="Rainbow") and Color3.fromHSV((now*.45)%1,1,1) or nil
            for root,t0 in pairs(claimedShotAt) do
                if not root or not root.Parent or now-t0>.22 then
                    claimedShotAt[root]=nil
                else
                    local c=(extraState.BulletColorMode=="Custom") and bulletCustomColor or rainbowColor
                    if c then
                        applyColorOnly(root,c)
                        for _,d in ipairs(root:GetDescendants()) do applyColorOnly(d,c) end
                    end
                end
            end
        end)
        ...
        local function setExtraState(state)
            ...
            task.defer(function()
                scanAllExtras(); refreshBeamStyle(); refreshKnifeStyle(); refreshEquipStyle(); refreshBulletColorUI()
                applyBulletBeamOverride(extraState.BulletBeam or "None")
...
task.spawn(function()
    ...
    -- Some feature callbacks still write their original Matcha/Pink colors when a
    -- button is clicked. Re-apply the currently selected theme right after each user
    -- interaction so every normal GUI surface stays in the chosen palette.
    UIS.InputEnded:Connect(function(input)
        if input.UserInputType==Enum.UserInputType.MouseButton1 or input.UserInputType==Enum.UserInputType.Touch then
            task.delay(.035,function()
                if main.Parent then applyTheme(currentName) end
            end)
        end
    end)
