-- Kimqetras HC v2.28 • execute recovery + source macro isolated
-- Keeps every v2.28 feature, fixes the startup guard so a failed boot cannot permanently block re-execution,
-- and runs the supplied Speed/Macro behavior inside its own chunk instead of depending on the legacy backend.

local KIMQ_SINGLE_KEY = "KimqHC_v21_SingleRuntimeOptimized"
local KIMQ_BUILD = "v2.28-execute-recovery-source-macro"

local function kimqFindMainGui()
    local Players = game:GetService("Players")
    local CoreGui = game:GetService("CoreGui")
    local lp = Players.LocalPlayer
    local pg = lp and lp:FindFirstChildOfClass("PlayerGui")
    return CoreGui:FindFirstChild("KimpetrasHC") or (pg and pg:FindFirstChild("KimpetrasHC"))
end

local function kimqShowBootMessage(message)
    pcall(function()
...
do
    local oldRunning = rawget(_G, KIMQ_SINGLE_KEY)
    local oldBuild = rawget(_G, "KimqHC_CurrentBuild")
    local oldGui = kimqFindMainGui()

    if oldRunning and oldBuild == KIMQ_BUILD and oldGui then
        pcall(function()
            oldGui.Enabled = true
            local main = oldGui:FindFirstChild("Main")
            if main then main.Visible = true end
        end)
        return
    end

    -- A previous build could set the runtime flag before its GUI actually finished.
    -- If there is no GUI, that flag is stale: clear it and allow a clean retry in the SAME server.
    if oldRunning and not oldGui then
        _G[KIMQ_SINGLE_KEY] = nil
        _G.KimqHC_CurrentBuild = nil
        _G.KimqHC_v21_PerformanceLoaded = nil
        oldRunning = false
        oldBuild = nil
    end

    -- If a real older GUI is still alive, don't stack all of its old input/render connections.
    if oldRunning and oldGui and oldBuild ~= KIMQ_BUILD then
        kimqShowBootMessage("Another Kimqetras HC version is still running. Rejoin once, then run v2.28.")
        warn("[Kimqetras HC v2.28] Active older runtime detected; rejoin once before switching builds.")
        return
    end

    _G.KimqHC_BootingBuild = KIMQ_BUILD
...
-- v2.28 keeps the v2.27 feature set, repairs startup recovery, and isolates the source Macro/Speed implementation.
...
_G.KimpetrasCtx = {
    ...
    _G.KimqHC_BootingBuild = nil
    return
end

-- Core started successfully and created KimpetrasHC. Only now mark this build active.
do
    local root = kimqFindMainGui()
    if root then
...
-- ========================================================
-- MACRO / SPEED - isolated source behavior
-- ========================================================
-- This is the supplied script's Speed system kept self-contained so it still works
-- even if an unrelated legacy backend feature errors later.
_G.SpeedMaster = _G.SpeedMaster == true
_G.SpeedActive = _G.SpeedActive == true
_G.SpeedValue = tonumber(_G.SpeedValue) or 50
if typeof(_G.SpeedKey) ~= "EnumItem" then _G.SpeedKey = Enum.KeyCode.X end
...
addToggle("Macro / Speed Master", _G.SpeedMaster, function(v)
    _G.SpeedMaster = not not v
    if not _G.SpeedMaster then _G.SpeedActive = false end
end)

addKeybind("Macro Key", _G.SpeedKey, function(k)
    _G.SpeedKey = k
end)

addSlider("Macro Speed", 16, 1000, _G.SpeedValue, function(v)
    _G.SpeedValue = tonumber(v) or 50
end)
...
MacroHint.Text = "Turn Master on, then press the Macro Key to toggle your selected speed."
...
-- Source-style key toggle. We only ignore the key while typing in a textbox;
-- some games mark X as gameProcessed even though the player still expects the macro key.
UIS.InputBegan:Connect(function(input)
    if UIS:GetFocusedTextBox() then return end
    if input.UserInputType == Enum.UserInputType.Keyboard and input.KeyCode == _G.SpeedKey then
        if _G.SpeedMaster then _G.SpeedActive = not _G.SpeedActive end
    end
end)

-- Exact movement method from the supplied source: keep Humanoid.WalkSpeed at SpeedValue.
RunService.Heartbeat:Connect(function()
    if _G.SpeedMaster and _G.SpeedActive and lp.Character then
