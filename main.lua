-- Kimqetras HC v2.37 • weapon/avatar/environment/theme repair + UI polish
...
local KIMQ_BUILD = "v2.37-full-feature-repair"
    ...
    if running and runningGui and runningBuild ~= KIMQ_BUILD then
        pcall(function()
            game:GetService("StarterGui"):SetCore("SendNotification", {
                Title = "Kimqetras HC",
                Text = "Another Kimqetras HC version is still active. Rejoin once, then run v2.37.",
                Duration = 8,
            })
        end)
        warn("[Kimqetras HC v2.37] Another runtime is active; rejoin once before switching builds.")
        return
    end
...
_G.KimqAccessoryUIReady = false
    ...
    pcall(function() game:GetService("RunService"):UnbindFromRenderStep("KimqPermanentTimeV237") end)
end
...
local function addCleanSection(title, subtitle)
    ...
    -- v2.37: pageHead already displays the section name.
...
task.spawn(function()
    ...
    -- v2.37: pages are grouped by what they actually do so the sidebar is
...
-- v2.37 module: local catalog / limited accessory try-on.
task.spawn(function()
    _G.KimqAccessoryUIReady=false
    local Players = game:GetService("Players")
    local CoreGui = game:GetService("CoreGui")
...
-- v2.37 module: Environment + Weapon Skins (canonical sidebar pages).
task.spawn(function()
    local pageReadyStart=tick()
    while not _G.KimqBasePagesReady and tick()-pageReadyStart<8 do task.wait(.02) end
    ...
    -- v2.37: Environment and Weapon Skins are already real canonical pages/buttons.
    ...
    pcall(function()
        RunService:BindToRenderStep("KimqPermanentTimeV237",Enum.RenderPriority.Last.Value,function()
    ...
    if not envBuildOk then
        warn("[Kimqetras HC v2.37] Environment page fallback: "..tostring(envBuildErr))
    ...
    if not extrasOk then
        _G.KimqWeaponExtrasInstalled=false
        warn("[Kimqetras HC v2.37] Weapon extras: "..tostring(extrasErr))
    ...
    skinsBtn.MouseButton1Click:Connect(function()
        task.defer(function()
            scanWeapons()
            if _G.KimqWeaponExtrasController and type(_G.KimqWeaponExtrasController.Refresh)=="function" then
                pcall(_G.KimqWeaponExtrasController.Refresh)
            end
        end)
    end)
...
task.spawn(function()
    ...
    -- v2.37: preserve the grouped sidebar order from pageDefs.
    ...
    local function applyTheme(name,settlePass)
        ...
        -- v2.37 hard guarantee: old controls created with literal pink/yellow
    ...
    -- v2.37: section navigation does not repaint/rescan the entire GUI.
    ...
    -- v2.37 reveal happens only after this initial theme pass is complete.
end)


-- v2.37: delayed post-show cleanup passes removed.
...
-- ========================================================
-- v2.37 branding is deterministic at construction time; no delayed scrub pass.

-- ========================================================
-- v2.37 DETERMINISTIC REVEAL
