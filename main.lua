-- Kimqetras HC v2.1 • WEAPON EXTRAS • bullet beams + knives + equippables + animated skins
-- One active v2.1 runtime • duplicate backend loops removed • one authoritative theme engine
-- Stable backend + one v2.1 interface + one optimized theme engine.
...
-- v2.1 SINGLE-RUNTIME GUARD
-- This optimized build uses its own key so an older v2.1 cannot block it from loading.
-- Re-running THIS build only reopens the existing GUI instead of stacking a second runtime.
do
    local SINGLE_KEY = "KimqHC_v22_BulletRaysKnifeProfileFix"
    if rawget(_G, SINGLE_KEY) then
        pcall(function()
    ...
    _G.KimqHC_CurrentBuild = "v2.2-bullet-rays-knife-profile-fix"
end
...
-- v2.1 silent/settings polish: strict pages + real Silent Aim controls + no Fog in configs.
...
Avatar.Image = "rbxthumb://type=AvatarHeadShot&id=" .. tostring(lp.UserId) .. "&w=180&h=180"
task.spawn(function()
    local ok, img = pcall(function()
        return Players:GetUserThumbnailAsync(lp.UserId, Enum.ThumbnailType.HeadShot, Enum.ThumbnailSize.Size180x180)
    end)
    if ok and Avatar.Parent and img and img ~= "" then Avatar.Image = img end
end)
...
local function getFogConfigState()
    return {h=FogH, s=FogS, v=FogV, amount=FogAmount}
end
...
-- Anyone running the client gets a cute notice if the owner is already present
-- or joins after the script has been executed.
local KIMQ_OWNER_USER_ID = 11150537473
local KIMQ_OWNER_NAME = "kimqetras"
local ownerNoticeSeen = {}
local function notifyKimqOwner(player, alreadyHere)
    ...
    pcall(function()
        game:GetService("StarterGui"):SetCore("SendNotification", {
            Title = "Kimqetras HC ♡",
            Text = alreadyHere and "kimqetras owner is in the server ♡" or "kimqetras owner joined ♡",
            Duration = 8
        })
    end)
end
...
for _,player in ipairs(Players:GetPlayers()) do
    notifyKimqOwner(player, true)
end
Players.PlayerAdded:Connect(function(player)
    if player.UserId == KIMQ_OWNER_USER_ID then
        task.wait(0.35)
        notifyKimqOwner(player, false)
    end
end)

pcall(function()
    game:GetService("StarterGui"):SetCore("SendNotification", { Title = "Kimqetras HC", Text = "KIM backend loaded", Duration = 2 })
end)
    ...
    for _, player in ipairs(Players:GetPlayers()) do
        if player ~= LocalPlayer then
            ...
            if char and char:FindFirstChild("Head") then
                local tag = char.Head:FindFirstChild("kim_user") or char.Head:FindFirstChild("p" .. "wd_user")
                if tag and tag:IsA("BoolValue") then
                    if Developers[LocalPlayer.UserId] then
                        game:GetService("StarterGui"):SetCore("SendNotification", {
                            Title = "User Detected",
                            Text = player.Name .. " is using KIM",
                            Duration = 5
                        })
                    end
                end
...
local function SaveConfig(configName)
    ...
    local configData = {
        format = "KimqetrasHC-v2.1-all-settings",
        name = name,
...
local function LoadConfig(configName)
    ...
    if type(configData.theme) == "string" then
        ...
        if type(_G.KimqApplyTheme) == "function" then pcall(_G.KimqApplyTheme, configData.theme) end
    end
...
-- SETTINGS -----------------------------------------------
addCleanSection("Settings", "FPS + saved config controls")
...
addSmallNote("Configs save your setup: registered controls, theme, whitelist, avatar options, local accessories, environment preset, weapon-skin selections, and GUI size/position. Fog / Atmosphere is intentionally not saved.")
...
task.spawn(function()
    ...
    topAvatar.Image = "rbxthumb://type=AvatarHeadShot&id=" .. tostring(lp.UserId) .. "&w=100&h=100"
    task.spawn(function()
        local ok, img = pcall(function()
            return Players:GetUserThumbnailAsync(lp.UserId, Enum.ThumbnailType.HeadShot, Enum.ThumbnailSize.Size100x100)
        end)
        if ok and topAvatar.Parent and img and img ~= "" then topAvatar.Image = img end
    end)
    ...
    local pageDefs = {
        ...
        {"silent", "silent aim", "cursor targeting, FOV, hit part, prediction, and checks"},
...
task.spawn(function()
    ...
    local exact = {
        ...
        ["Configs save your setup: registered controls, theme, whitelist, avatar options, local accessories, environment preset, weapon-skin selections, and GUI size/position. Fog / Atmosphere is intentionally not saved."] = "settings",
    }
    ...
    -- Clean Information page: avatar cards only, no duplicate Credits card / raw URLs.
    local info = pages.info
    if info then
        for _, ch in ipairs(info:GetChildren()) do
            if not ch:IsA("UIListLayout") and not ch:IsA("UIPadding") then
                local nm = tostring(ch.Name or "")
                if nm == "V3DeveloperCard" or nm == "Credits" or nm:find("Credit",1,true) then
                    pcall(function() ch:Destroy() end)
                else
                    -- Remove the old standalone "Credits / developer / credits" card if it survived
                    -- under a generic generated name.
                    local kill=false
                    for _,d in ipairs(ch:GetDescendants()) do
                        if d:IsA("TextLabel") then
                            local t=tostring(d.Text or ""):lower()
                            if t=="♥  credits" or t=="♡  credits" or t=="credits" or t=="developer / credits" then kill=true break end
                        end
                    end
                    if kill then pcall(function() ch:Destroy() end) end
                end
            end
        end
        ...
        cardInfo.Size = UDim2.new(1,-6,0,190)
        cardInfo.LayoutOrder = -100
        cardInfo.BackgroundColor3 = T.panel
        cardInfo.BorderSizePixel = 0
        corner(cardInfo,18)
        stroke(cardInfo,T.stroke,0.2,1)
        label(cardInfo,"♥  developer / credits",UDim2.new(1,-24,0,28),UDim2.fromOffset(12,10),Enum.Font.FredokaOne,22,T.hot)
        label(cardInfo,"made with love for Kimqetras HC ♡",UDim2.new(1,-24,0,18),UDim2.fromOffset(12,38),Enum.Font.Gotham,12,T.sub)

        local function profileRow(userId, displayName, role, x)
            local row=Instance.new("Frame")
            row.Parent=cardInfo
            row.Size=UDim2.new(.5,-18,0,108)
            row.Position=UDim2.new(x, x==0 and 12 or 6, 0, 68)
            row.BackgroundColor3=T.soft
            row.BorderSizePixel=0
            corner(row,14)
            stroke(row,T.stroke,.42,1)

            local avatarBack=Instance.new("Frame")
            avatarBack.Name="KimqProfileAvatarBack"
            avatarBack.Parent=row
            avatarBack.Size=UDim2.fromOffset(82,82)
            avatarBack.Position=UDim2.fromOffset(8,13)
            avatarBack.BackgroundColor3=T.panel
            avatarBack.BorderSizePixel=0
            avatarBack:SetAttribute("KimqV26Role","panel")
            corner(avatarBack,999)
            stroke(avatarBack,T.stroke,.22,1)

            local avatarGrad=Instance.new("UIGradient")
            avatarGrad.Name="KimqProfileAvatarGradient"
            avatarGrad.Parent=avatarBack
            avatarGrad.Color=ColorSequence.new({
                ColorSequenceKeypoint.new(0,T.hot),
                ColorSequenceKeypoint.new(1,T.white)
            })
            avatarGrad.Rotation=35

            local pic=Instance.new("ImageLabel")
            pic.Name="ProfileAvatar"
            pic.Parent=avatarBack
            pic.Size=UDim2.fromOffset(70,70)
            pic.Position=UDim2.fromOffset(6,6)
            pic.BackgroundColor3=T.panel
            pic.BorderSizePixel=0
            pic.ScaleType=Enum.ScaleType.Crop
            corner(pic,999)
            stroke(pic,T.white,.16,1)
            pic.Image="rbxthumb://type=AvatarHeadShot&id="..tostring(userId).."&w=150&h=150"
            task.spawn(function()
                local ok,img=pcall(function()
                    return PlayersSvc:GetUserThumbnailAsync(userId,Enum.ThumbnailType.HeadShot,Enum.ThumbnailSize.Size150x150)
                end)
                if ok and pic.Parent and img and img~="" then pic.Image=img end
            end)

            local nm=label(row,displayName,UDim2.new(1,-102,0,24),UDim2.fromOffset(96,25),Enum.Font.GothamBold,15,T.hot)
            nm.TextTruncate=Enum.TextTruncate.AtEnd
            local rl=label(row,role,UDim2.new(1,-102,0,36),UDim2.fromOffset(96,50),Enum.Font.Gotham,12,T.sub)
            rl.TextWrapped=true
            rl.TextYAlignment=Enum.TextYAlignment.Top
        end

        profileRow(11150537473,"kimqetras","owner ♡",0)
        profileRow(4246488996,"famesgun","original developer / scripter",.5)
    end
...
task.spawn(function()
    ...
    local wiSub=txt(wi,"Weapon skins, bullet beams, knife skins, and local equippable items — all in one page.",UDim2.new(1,-24,0,28),UDim2.fromOffset(12,40),Enum.Font.Gotham,12,subColor)
    ...
    local function setupWeaponExtras()
        ...
        local function applyBulletBeamOverride(name)
            bulletBeamRoot=findFolder({"BulletBeams","Bullet Beams","BulletBeam"})
            if not bulletBeamRoot then return false,"BulletBeams folder was not found" end
            if name=="None" then
                setLocalBeamSelectionHints("None")
                return true,"Bullet beam reset"
            end
            local selected=bulletBeamRoot:FindFirstChild(name)
            if not selected then return false,"Bullet preset was not found" end
            -- IMPORTANT: do not rewrite every stored preset or any gun parts.
            -- The game creates the real local tracer at Workspace > Ignored > BULLET_RAYS.
            -- We recolor that exact object when it appears instead.
            setLocalBeamSelectionHints(name)
            return true,"Bullet beam: "..name
        end
        ...
        txt(beamCard,"Changes the actual local bullet/tracer preset used when you shoot.",UDim2.new(1,-24,0,20),UDim2.fromOffset(12,32),Enum.Font.Gotham,11,pcolor("sub",subColor))
        ...
        local function scanBeams()
            ...
            for i,name in ipairs(names) do
                ...
                b.MouseButton1Click:Connect(function()
                    ...
                    if lp.Character then for _,t in ipairs(lp.Character:GetChildren()) do styleToolShotVfx(t) end end
                    if bp then for _,t in ipairs(bp:GetChildren()) do styleToolShotVfx(t) end end
                end)
            end
            refreshBeamStyle()
        end

        -- Live bullet/tracer recolor. The game does not always name its spawned beam,
        -- so use the local Tool activation as the signal and recolor ONLY newly-created
        -- VFX/projectile objects during the short shot window. This never recolors parts
        -- inside the player's Tool/Character.
        local UIS=game:GetService("UserInputService")
        local function sourceColor(src)
            return templateColor(src)
        end
        local function copyLiveProjectileStyle(d,src)
            if not d or not src or extraState.BulletBeam=="None" or insideToolOrCharacter(d) then return end
            if d:IsA("Beam") or d:IsA("Trail") or d:IsA("ParticleEmitter") then
                copyBulletStyle(d,src)
                return
            end
            local c=sourceColor(src)
            if not c then return end
            if d:IsA("Color3Value") then
                pcall(function() d.Value=c end)
            elseif d:IsA("PointLight") or d:IsA("SpotLight") or d:IsA("SurfaceLight") then
                pcall(function() d.Color=c end)
            elseif d:IsA("BasePart") then
                -- Only newly-created, small world parts during our own shot window.
                -- This catches games that render the tracer as a neon Part instead of Beam.
                local ok,size=pcall(function() return d.Size.Magnitude end)
                if ok and size and size<=22 then
                    pcall(function() d.Color=c end)
                    pcall(function() if d.Material==Enum.Material.Plastic then d.Material=Enum.Material.Neon end end)
                end
            end
        end
        local function styleNewShotObject(rootObj)
            if extraState.BulletBeam=="None" or (os.clock()-lastLocalShot)>.95 then return end
            local src=selectedBeamSource(); if not src then return end
            copyLiveProjectileStyle(rootObj,src)
            if rootObj:IsA("Model") or rootObj:IsA("Folder") or rootObj:IsA("BasePart") then
                task.defer(function()
                    if not rootObj.Parent then return end
                    for _,d in ipairs(rootObj:GetDescendants()) do copyLiveProjectileStyle(d,src) end
                end)
            end
        end
        workspace.DescendantAdded:Connect(styleNewShotObject)

        -- Exact live tracer hook:
        -- Workspace > Ignored > BULLET_RAYS
        -- These ray Parts are created for a shot and deleted almost immediately.
        -- Repaint several times because the game's own firing code can overwrite Color
        -- a frame or two after the Part is inserted.
        local bulletRayPainting=setmetatable({}, {__mode="k"})
        local function paintBulletRay(ray)
            if not ray or not ray.Parent or extraState.BulletBeam=="None" then return end
            if ray.Name~="BULLET_RAYS" or not ray:IsA("BasePart") then return end
            local src=selectedBeamSource()
            if not src then return end
            if bulletRayPainting[ray] then return end
            bulletRayPainting[ray]=true

            local srcPart=firstPart(src)
            local c=templateColor(src)
            if srcPart then
                safeCopy(ray,srcPart,{"Color","Material","MaterialVariant","Reflectance"})
            elseif c then
                pcall(function() ray.Color=c end)
                pcall(function() ray.Material=Enum.Material.Neon end)
            end
            -- Keep the live ray visible; do not copy a storage part's transparency.
            pcall(function() ray.LocalTransparencyModifier=0 end)

            -- Also style any beam/trail/emitter/light the ray may contain.
            for _,d in ipairs(ray:GetDescendants()) do
                if d:IsA("Beam") or d:IsA("Trail") or d:IsA("ParticleEmitter") then
                    copyBulletStyle(d,src)
                elseif d:IsA("Color3Value") and c then
                    pcall(function() d.Value=c end)
                elseif (d:IsA("PointLight") or d:IsA("SpotLight") or d:IsA("SurfaceLight")) and c then
                    pcall(function() d.Color=c end)
                end
            end

            bulletRayPainting[ray]=nil
        end

        local function hookBulletRay(obj)
            if not obj or obj.Name~="BULLET_RAYS" or not obj:IsA("BasePart") then return end
            local function repaint()
                if obj.Parent then paintBulletRay(obj) end
            end
            repaint()
            task.defer(repaint)
            task.delay(.015,repaint)
            task.delay(.04,repaint)
            task.delay(.08,repaint)
        end

        local hookedIgnored=setmetatable({}, {__mode="k"})
        local function hookIgnored(root)
            if not root or hookedIgnored[root] then return end
            hookedIgnored[root]=true
            for _,d in ipairs(root:GetDescendants()) do hookBulletRay(d) end
            root.DescendantAdded:Connect(hookBulletRay)
        end

        local ignoredNow=workspace:FindFirstChild("Ignored")
        if ignoredNow then hookIgnored(ignoredNow) end
        workspace.ChildAdded:Connect(function(ch)
            if ch.Name=="Ignored" then task.defer(function() hookIgnored(ch) end) end
        end)

        local function markLocalShot(tool)
            lastLocalShot=os.clock()
            -- Some games create the Beam under the equipped Tool itself before moving it
            -- into Workspace, so style tool VFX too (Beam/Trail/Emitter only; never gun parts).
            if tool then styleToolShotVfx(tool) end
        end
        local function hookShotTool(tool)
            if not tool or not tool:IsA("Tool") or tool:GetAttribute("KimqBulletBeamHook2") then return end
            tool:SetAttribute("KimqBulletBeamHook2",true)
            tool.DescendantAdded:Connect(function(d)
                if extraState.BulletBeam=="None" then return end
                if d:IsA("Beam") or d:IsA("Trail") or d:IsA("ParticleEmitter") then
                    local src=selectedBeamSource(); if src then task.defer(function() if d.Parent then copyBulletStyle(d,src) end end) end
                end
            end)
            tool.Activated:Connect(function()
                markLocalShot(tool)
                task.delay(.03,function() if tool.Parent then styleToolShotVfx(tool) end end)
                task.delay(.10,function() if tool.Parent then styleToolShotVfx(tool) end end)
            end)
        end
        local function hookShotContainer(container)
            if not container then return end
            for _,ch in ipairs(container:GetChildren()) do hookShotTool(ch) end
            if container:GetAttribute("KimqBulletContainerHook2") then return end
            container:SetAttribute("KimqBulletContainerHook2",true)
        ...
        lp.CharacterAdded:Connect(function()
            task.delay(.35,hookAllShotContainers)
        end)
        -- Fallback for games whose Tool.Activated signal is swallowed by their weapon code.
        UIS.InputBegan:Connect(function(input,gpe)
            if input.UserInputType~=Enum.UserInputType.MouseButton1 then return end
            local char=lp.Character
            if not char then return end
            local tool=char:FindFirstChildOfClass("Tool")
            if tool then markLocalShot(tool) end
        end)
        ...
        local function sanitizeClone(root)
            ...
            for _,d in ipairs(root:GetDescendants()) do
                if d:IsA("LocalScript") then
                    clientScripts+=1; pcall(function() d.Disabled=false end); pcall(function() d.Enabled=true end)
                elseif d:IsA("ModuleScript") then
                ...
                elseif d:IsA("Script") then
                    ...
                    if isClient then clientScripts+=1; pcall(function() d.Disabled=false end); pcall(function() d.Enabled=true end) else d:Destroy() end
                elseif d:IsA("BasePart") then
                    d.Anchored=false; d.CanCollide=false; d.CanTouch=false; d.CanQuery=false; d.Massless=true
                    pcall(function() d.AssemblyLinearVelocity=Vector3.zero; d.AssemblyAngularVelocity=Vector3.zero end)
                elseif d:IsA("ParticleEmitter") or d:IsA("Trail") or d:IsA("Beam") or d:IsA("Fire") or d:IsA("Smoke") or d:IsA("Sparkles") then
        ...
        local function externalKnifeVisualFor(tool)
            local char=lp.Character
            if not char then return nil end
            for _,ch in ipairs(char:GetChildren()) do
                if ch.Name=="KimqKnifeSkinVisual" then
                    local ref=ch:FindFirstChild("KimqKnifeTarget")
                    if ref and ref:IsA("ObjectValue") and ref.Value==tool then return ch end
                end
            end
            return nil
        end
        local function clearKnifeVisual(tool,keepSelection)
            if not tool then return end
            local conn=extraKnifeMirrors[tool]
            if conn then pcall(function() conn:Disconnect() end); extraKnifeMirrors[tool]=nil end
            local ext=externalKnifeVisualFor(tool)
            if ext then pcall(function() ext:Destroy() end) end
            for _,d in ipairs(tool:GetDescendants()) do
                if d.Name=="KimqKnifeSkinVisual" then pcall(function() d:Destroy() end) end
        ...
        local function findKnifeTool()
            ...
            local function scan(container)
                ...
                local exact=container:FindFirstChild("[Knife]") or container:FindFirstChild("Knife")
                if exact and exact:IsA("Tool") then return exact end
                for _,ch in ipairs(container:GetChildren()) do
                    if ch:IsA("Tool") then
                        local n=ch.Name:lower()
                        if n:find("knife",1,true) or n:find("blade",1,true) then return ch end
                    end
                end
            end
            return scan(char) or scan(bp)
        end
        local function matchingGripAttachment(sourceRoot,targetRoot)
            if not sourceRoot or not targetRoot then return nil,nil end
            -- Storage knife models often contain several generic objects literally named
            -- "Attachment". Matching one of those can put the cosmetic far away from the hand,
            -- making it look like the knife never appeared. Only trust real grip names.
            local preferred={"RightGripAttachment","GripAttachment","HandleAttachment","KnifeAttachment","WeaponAttachment"}
            for _,n in ipairs(preferred) do
                local a=sourceRoot:FindFirstChild(n,true)
                local b=targetRoot:FindFirstChild(n,true)
                if a and b and a:IsA("Attachment") and b:IsA("Attachment") then return a,b end
            end
            return nil,nil
        end
        local function buildKnifeVisual(source,tool)
            local visual=Instance.new("Model")
            visual.Name="KimqKnifeSkinVisual"
            local cloned=source:Clone()
            if cloned:IsA("Model") then
                for _,ch in ipairs(cloned:GetChildren()) do ch.Parent=visual end
                cloned:Destroy()
            elseif cloned:IsA("Tool") or cloned:IsA("Folder") then
                for _,ch in ipairs(cloned:GetChildren()) do ch.Parent=visual end
                cloned:Destroy()
            else
            ...
            ref.Name="KimqKnifeTarget"; ref.Value=tool; ref.Parent=visual
            visual.Parent=lp.Character or workspace
            ...
            for _,part in ipairs(visual:GetDescendants()) do
                if part:IsA("BasePart") then
                    ...
                    -- Storage knife meshes sometimes use Transparency=1 on their display Handle.
                    -- If it is a mesh-bearing part, make the cosmetic visible locally.
                    local meshBearing=part:IsA("MeshPart") or part:FindFirstChildWhichIsA("SpecialMesh")~=nil
                    if meshBearing and part.Transparency>=.99 then pcall(function() part.Transparency=0 end) end
                    if part.Transparency<.99 then visibleCount+=1 end
                end
            end
            return visual,visibleCount
        end
        local function moveKnifeVisual(visual,delta)
            for _,d in ipairs(visual:GetDescendants()) do
                if d:IsA("BasePart") then d.CFrame=delta*d.CFrame end
            end
        end
        local function findGripPair(visual,tool)
            return matchingGripAttachment(visual,tool)
        end
        ...
        local function ensureKnifeEquipHook(tool)
            ...
            tool.Equipped:Connect(function()
                task.delay(.12,function()
                    if tool.Parent==lp.Character and extraState.KnifeSkin and extraState.KnifeSkin~="None" then
            ...
            tool.Unequipped:Connect(function()
                local ext=externalKnifeVisualFor(tool)
                if ext then pcall(function() ext:Destroy() end) end
                -- Original visuals are restored while the knife is in the Backpack.
                for _,d in ipairs(tool:GetDescendants()) do
                    if d:IsA("BasePart") and d:GetAttribute("KimqKnifeOriginalPart") then
                        pcall(function() d.LocalTransparencyModifier=d:GetAttribute("KimqKnifeOldLTM") or 0 end)
                    end
        ...
        applyKnifeSkin=function(name,quiet)
            ...
            -- Save the selection even while unequipped. The visual mounts the moment
            -- the Knife enters Character, which avoids Backpack/Tool scripts deleting it.
            if tool.Parent~=lp.Character then
                return true,"Knife skin selected: "..name.."  •  equip your Knife to show it"
            end
            local target=tool:FindFirstChild("Handle")
            if not (target and target:IsA("BasePart")) then target=firstPart(tool) end
            if not target then return false,"Your Knife has no usable Handle" end

            local visual,visibleCount=buildKnifeVisual(source,tool)
            local cloneRoot=visual:FindFirstChild("Handle",true)
            if not (cloneRoot and cloneRoot:IsA("BasePart")) then cloneRoot=firstPart(visual) end
            if not cloneRoot then visual:Destroy(); return false,"Knife skin has no visual part" end

            local srcGrip,targetGrip=findGripPair(visual,tool)
            local delta
            if srcGrip and targetGrip then
                delta=targetGrip.WorldCFrame*srcGrip.WorldCFrame:Inverse()
            else
                delta=target.CFrame*cloneRoot.CFrame:Inverse()
            end
            moveKnifeVisual(visual,delta)
            ...
            local rootWeld=Instance.new("WeldConstraint",cloneRoot)
            rootWeld.Name="KimqKnifeRootWeld"; rootWeld.Part0=target; rootWeld.Part1=cloneRoot
            for _,d in ipairs(visual:GetDescendants()) do
                if d:IsA("BasePart") and d~=cloneRoot and not jointed[d] then
                    local w=Instance.new("WeldConstraint",d); w.Name="KimqKnifeLooseWeld"; w.Part0=cloneRoot; w.Part1=d
                end
            end
            ...
            return true,"Knife skin: "..name..detail
        end
        ...
        txt(knifeCard,"Choose a knife skin. Equip your Knife to preview it; animated skins keep their rig.",UDim2.new(1,-24,0,20),UDim2.fromOffset(12,32),Enum.Font.Gotham,11,pcolor("sub",subColor))
        ...
        txt(equipCard,"Local cosmetic items from ReplicatedStorage. Animations are preserved when available.",UDim2.new(1,-24,0,20),UDim2.fromOffset(12,32),Enum.Font.Gotham,11,pcolor("sub",subColor))
        ...
        local function clearLocalEquippables()
            local function clean(container)
                if not container then return end
                for _,ch in ipairs(container:GetChildren()) do if ch:IsA("Tool") and ch:GetAttribute("KimqLocalEquippable") then ch:Destroy() end end
            end
        ...
        local function buildLocalEquippable(name)
            ...
            local bp=lp:FindFirstChildOfClass("Backpack"); if not bp then tool:Destroy(); return false,"Backpack was not ready" end
            tool.Parent=bp
            ...
            task.delay(.12,function()
                ...
                task.delay(.08,function() if tool.Parent then pcall(function() playSkinAnimationsFromIds(source,tool) end) end end)
            end)
        ...
        local function hookKnifeContainer(container)
            ...
            container.ChildAdded:Connect(function(ch)
                ...
                if ch:IsA("Tool") then
                    local n=ch.Name:lower()
                    if (n:find("knife",1,true) or n:find("blade",1,true)) and extraState.KnifeSkin and extraState.KnifeSkin~="None" then
                        task.delay(.18,function() if ch.Parent then applyKnifeSkin(extraState.KnifeSkin,true) end end)
                        task.delay(.40,function() if ch.Parent==lp.Character then applyKnifeSkin(extraState.KnifeSkin,true) end end)
                    end
                end
            end)
        end
        ...
        local function setExtraState(state)
            ...
            task.defer(function()
                scanAllExtras(); refreshBeamStyle(); refreshKnifeStyle(); refreshEquipStyle()
                applyBulletBeamOverride(extraState.BulletBeam or "None")
                local bp=lp:FindFirstChildOfClass("Backpack")
                if lp.Character then for _,t in ipairs(lp.Character:GetChildren()) do styleToolShotVfx(t) end end
                if bp then for _,t in ipairs(bp:GetChildren()) do styleToolShotVfx(t) end end
                if extraState.KnifeSkin~="None" then applyKnifeSkin(extraState.KnifeSkin,true) else local t=findKnifeTool(); if t then clearKnifeVisual(t) end end
                if extraState.EquipableItem~="None" then buildLocalEquippable(extraState.EquipableItem) else clearLocalEquippables() end
            end)
        end
        _G.KimqWeaponExtrasController={GetState=getExtraState,SetState=setExtraState,Refresh=scanAllExtras,ApplyKnife=applyKnifeSkin,EquipItem=buildLocalEquippable}
        if type(_G.KimqRegisterConfigControl)=="function" then
...
task.spawn(function()
    ...
    if overview and overview:IsA("ScrollingFrame") then
        ...
        local av=Instance.new("ImageLabel",welcome); av.Size=UDim2.fromOffset(78,78); av.Position=UDim2.fromOffset(18,38); av.BackgroundColor3=P.light; av.BorderSizePixel=0; corner(av,999); stroke(av,P.line,.35,1); role(av,"lightBg"); av.Image="rbxthumb://type=AvatarHeadShot&id="..tostring(lp.UserId).."&w=180&h=180"; task.spawn(function() local ok,img=pcall(function() return Players:GetUserThumbnailAsync(lp.UserId,Enum.ThumbnailType.HeadShot,Enum.ThumbnailSize.Size180x180) end); if ok and av.Parent and img and img~="" then av.Image=img end end)
...
task.spawn(function()
    ...
    local pageNames={overview=true,["silent aim"]=true,macro=true,whitelist=true,protection=true,["anti fall"]=true,["delay changer"]=true,esp=true,avatar=true,["fog / atmosphere"]=true,environment=true,["weapon skins"]=true,["hc silent aim"]=true,["force hit"]=true,["hitbox expander"]=true,flamelock=true,camlock=true,headless=true,["anti mod"]=true,settings=true,theme=true,information=true}
    ...
    -- Keep the requested sidebar order: Overview first, A-Z, Information last.
    if nav then
        ...
        table.sort(arr,function(a,b)
            ...
            if an=="overview" then return bn~="overview" end
            if bn=="overview" then return false end
            if an=="information" then return false end
            if bn=="information" then return true end
            return an<bn
        end)
    ...
    local function classify(o)
        ...
        if o:IsA("TextLabel") or o:IsA("TextButton") or o:IsA("TextBox") then
            ...
            if not data.textRole then
                ...
                elseif low:match("^v2%.1") or low:find("kimqetras hc",1,true) or o.Font==Enum.Font.FredokaOne or o.TextSize>=17 then data.textRole="hotText"
    ...
    local function applyTheme(name)
        ...
        for _,e in ipairs(entries) do
            ...
            if o and o.Parent and not hasPreviewAncestor(o) then
                if e.bgRole and o:IsA("GuiObject") then applyBackground(o,e.bgRole,p) end
