-- Kimqetras HC v2.2 • LOCAL BULLET COLORS • knife visibility fix • optional I/O macro
...
task.spawn(function()
    ...
    local function setupWeaponExtras()
        ...
        -- The user identified the actual shot objects as "bullet_rays". v2.2 watches only
        -- those objects and only during a very short local Tool-activation window. We never
        -- rewrite the shared ReplicatedStorage templates, so other players keep normal bullets.
        local UIS=game:GetService("UserInputService")
        ...
        local function applyBulletModeTo(d)
            ...
            local src=selectedBeamSource()
            -- Preset still controls the beam/trail shape/texture when selected.
            if src and extraState.BulletBeam~="None" then
                if d:IsA("Beam") or d:IsA("Trail") or d:IsA("ParticleEmitter") then copyBulletStyle(d,src)
                elseif d:IsA("BasePart") then
        ...
        local function nearestShotOrigin()
            local tool=shotTool
        ...
        local function bulletRayShotRoot(obj)
            -- If bullet_rays is a shared container, return the immediate shot object under it.
            -- If the ephemeral shot itself is named bullet_rays, return that object.
            local x=obj
            local child=obj
            for _=1,10 do
        ...
        local function hasOriginNear(root)
            local origin=nearestShotOrigin()
            if not origin then return false end
            local function closePos(pos) return (pos-origin).Magnitude<=32 end
            local function partTouchesOrigin(part)
                if closePos(part.Position) then return true end
                local ok,lp0=pcall(function() return part.CFrame:PointToObjectSpace(origin) end)
                if not ok then return false end
                local h=part.Size*.5 + Vector3.new(8,8,8)
                return math.abs(lp0.X)<=h.X and math.abs(lp0.Y)<=h.Y and math.abs(lp0.Z)<=h.Z
            end
            ...
            if root:IsA("Beam") then
                local a0,a1=root.Attachment0,root.Attachment1
                if (a0 and closePos(a0.WorldPosition)) or (a1 and closePos(a1.WorldPosition)) then return true end
            end
            for _,d in ipairs(root:GetDescendants()) do
                ...
                if d:IsA("Beam") then
                    ...
                    if (a0 and closePos(a0.WorldPosition)) or (a1 and closePos(a1.WorldPosition)) then return true end
                elseif d:IsA("BasePart") and partTouchesOrigin(d) then return true end
            end
            return false
        end
        local function styleLocalBulletRay(rootObj)
            ...
            if (os.clock()-lastLocalShot)>.42 then return end
            ...
            -- Without explicit owner metadata, require the ray to originate at/near our gun.
            if verdict~=true and not hasOriginNear(shotRoot) then return end
        ...
        local function markLocalShot(tool)
            lastLocalShot=os.clock(); shotTool=tool
        end
        ...
        lp.CharacterAdded:Connect(function() task.delay(.25,hookAllShotContainers) end)
        UIS.InputBegan:Connect(function(input,gpe)
        ...
        workspace.DescendantAdded:Connect(function(d)
            ...
            if tostring(d.Name):lower()=="bullet_rays" or bulletRayShotRoot(d) then
                task.defer(function() if d.Parent then styleLocalBulletRay(d) end end)
            end
        end)
        RunService.Heartbeat:Connect(function()
...
task.spawn(function()
    ...
    -- v2.2 font cleanup: one family across the whole GUI so controls do not jump
    -- between SourceSans / Gotham / Fredoka. Text sizes still preserve hierarchy.
    local function unifyFont(o)
        if o:IsA("TextLabel") or o:IsA("TextButton") or o:IsA("TextBox") then
            o.Font=Enum.Font.Gotham
        end
    end
    for _,o in ipairs(main:GetDescendants()) do unifyFont(o) end
    main.DescendantAdded:Connect(function(o) task.defer(function() if o.Parent then unifyFont(o) end end) end)
end)
