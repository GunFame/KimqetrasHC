-- Kimqetras HC v2.69 • v2.68 base + isolated startup/reveal recovery fix
...
local KIMQ_BUILD = "v2.69-boot-recovery"
...
do
    ...
    if running and runningBuild == KIMQ_BUILD then
        if runningGui then
            ...
            notifyBoot((runtimeState=="ready") and "v2.69 is already running ♡" or "v2.69 is already starting ♡",4)
            return
        end
        ...
        notifyBoot("Recovering a failed v2.69 boot ♡",4)
    end
    ...
    if oldRuntimeDetected then
        ...
        if staleBoot then
            ...
            notifyBoot("Another Kimqetras HC runtime is still active. Rejoin once, then run v2.69.",9)
            warn("[Kimqetras HC v2.69] Older/live runtime detected. Refusing to stack; rejoin once.")
            return
        end
    end
...
Hello.Text = "v2.69  •  startup recovery + avatar editor ♡"
...
task.spawn(function()
    ...
    task.defer(function()
        ...
        if not rageOK then
            warn("[Kimqetras HC v2.69] RAGE module isolated error: "..tostring(rageERR))
        end
    end)
