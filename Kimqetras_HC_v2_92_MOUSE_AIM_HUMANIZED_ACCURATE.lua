-- Kimqetras HC v2.92 • v2.91 base + isolated humanized Mouse Aim tracking + accurate silent correction
...
local KIMQ_BUILD = "v2.92-mouse-aim-humanized-accurate"
...
Hello.Text = "v2.92  •  humanized mouse aim ♡"
...
-- v2.92 Mouse Aim: free camera + humanized cursor tracking + exact silent correction.
-- The visible cursor aims INSIDE the selected body part instead of orbiting beside it.
-- It uses adaptive catch-up: faster when the target creates distance, calmer when close.
-- No Camera.CFrame writes are used here, so this is still not Camlock.

-- One-time migration from the v2.90/v2.91 stock defaults to the new natural profile.
-- Custom values that no longer match the old defaults are preserved.
if cfg.mouseAimNaturalProfileVersion~="v2.92" then
    if cfg.mouseAimPart==nil or tostring(cfg.mouseAimPart)=="Closest Point" then cfg.mouseAimPart="UpperTorso" end
    if cfg.mouseAimFOV==nil or tonumber(cfg.mouseAimFOV)==260 then cfg.mouseAimFOV=205 end
    if cfg.mouseAimFollowStrength==nil or tonumber(cfg.mouseAimFollowStrength)==22 then cfg.mouseAimFollowStrength=36 end
    if cfg.mouseAimSoftness==nil or tonumber(cfg.mouseAimSoftness)==3 then cfg.mouseAimSoftness=2 end
    cfg.mouseAimNaturalProfileVersion="v2.92"
end
...
cfg.mouseAimFOV = tonumber(cfg.mouseAimFOV) or 205
cfg.mouseAimFollowStrength = tonumber(cfg.mouseAimFollowStrength) or 36
cfg.mouseAimSoftness = tonumber(cfg.mouseAimSoftness) or 2
...
local state={
    active=false,target=nil,part=nil,point=nil,lastScreen=nil,lastRenderClock=os.clock(),
    localAim=Vector2.zero,localAimTarget=Vector2.zero,nextAimShiftAt=0,
    visibleVelocity=Vector2.zero,lastRawScreen=nil,shotFocusUntil=0
}
...
local function candidateDistance(pl)
    ...
    if d>math.max(10,tonumber(cfg.mouseAimFOV) or 205) then return math.huge,nil,nil end
    return d,part,point
end
local function acquire()
    ...
    if best then
        ...
        state.localAim=Vector2.zero;state.localAimTarget=Vector2.zero;state.nextAimShiftAt=0
        state.visibleVelocity=Vector2.zero;state.lastRawScreen=nil;state.lastRenderClock=os.clock();state.shotFocusUntil=0
...
local function unlock()
    state.active=false;state.target=nil;state.part=nil;state.point=nil;state.lastScreen=nil
    state.localAim=Vector2.zero;state.localAimTarget=Vector2.zero;state.nextAimShiftAt=0
    state.visibleVelocity=Vector2.zero;state.lastRawScreen=nil;state.lastRenderClock=os.clock();state.shotFocusUntil=0
    _G.KimqMouseAimTarget=nil
end
...
-- Estimate a conservative on-screen footprint for the selected part. Humanized
-- offsets are clamped INSIDE this footprint so the cursor stays on the avatar.
local function partScreenRadius(part,centerScreen)
    if not part or not centerScreen then return 5,7 end
    local rx,ry=5,7
    local ok=pcall(function()
        local sx,onX=toScreen(part.Position + part.CFrame.RightVector*(part.Size.X*.42))
        local sy,onY=toScreen(part.Position + part.CFrame.UpVector*(part.Size.Y*.42))
        if onX and sx then rx=math.max(3,(sx-centerScreen).Magnitude) end
        if onY and sy then ry=math.max(4,(sy-centerScreen).Magnitude) end
    end)
    return math.clamp(rx,3,28),math.clamp(ry,4,34)
end

-- Silent Aim asks this bridge for the Q-locked target. It returns the actual
-- selected body part center for reliable correction; visible mouse humanization
-- is intentionally separate and never changes the shot target.
...
_G.KimqMouseAimForceCorrection=function()
    return cfg.mouseAimEnabled and cfg.mouseAimSilentCorrection and state.active and valid(state.target)
end
...
UIS.InputBegan:Connect(function(input,gpe)
    ...
    elseif state.active and input.UserInputType==Enum.UserInputType.MouseButton1 then
        -- Briefly settle the visible cursor closer to the selected part when the
        -- user actually shoots. This is a short natural correction, not a snap.
        state.shotFocusUntil=os.clock()+.13
    end
end)
...
RunService.RenderStepped:Connect(function()
    ...
    -- Never move the physical cursor in center-lock mode; doing so would rotate
    -- the camera and turn this feature into Camlock. Silent Correction stays live.
    if not cfg.mouseAimVisibleFollow or UIS.MouseBehavior==Enum.MouseBehavior.LockCenter then return end
    if not moveRel and not VIM then return end

    local center,on=toScreen(point)
    if not on then return end
    local now=os.clock()
    local dt=math.clamp(now-(state.lastRenderClock or now),1/240,1/20)
    state.lastRenderClock=now

    -- Track screen velocity so fast strafes get a natural trailing catch-up rather
    -- than a perfect glued cursor. The trail is tiny and always stays on the part.
    ...
    state.visibleVelocity=state.visibleVelocity:Lerp(rawVel,1-math.exp(-dt*7.5))

    local rx,ry=partScreenRadius(part,center)
    local softness=math.clamp(tonumber(cfg.mouseAimSoftness) or 2,0,18)

    -- Pick a stable point INSIDE the body part and move it only occasionally.
    -- This looks like a hand settling on torso/head rather than a scripted orbit.
    if now>=(state.nextAimShiftAt or 0) then
        local scale=math.clamp(softness/6,0,.8)
        local nx=(math.random()*2-1)*rx*.20*scale
        local ny=(math.random()*2-1)*ry*.15*scale
        state.localAimTarget=Vector2.new(nx,ny)
        state.nextAimShiftAt=now+.55+math.random()*.75
    end
    local localAlpha=1-math.exp(-dt*2.8)
    state.localAim=state.localAim:Lerp(state.localAimTarget,localAlpha)

    -- Small opposite-velocity trail. Clamp it so the desired cursor remains on
    -- the selected body part even during fast movement.
    local shotFocus=now<(state.shotFocusUntil or 0)
    local trailScale=shotFocus and .24 or 1
    local trail=Vector2.new(
        math.clamp(-state.visibleVelocity.X*.0055,-rx*.24,rx*.24),
        math.clamp(-state.visibleVelocity.Y*.0045,-ry*.20,ry*.20)
    )*trailScale
    local humanAim=shotFocus and state.localAim*.35 or state.localAim
    local desired=center+humanAim+trail
    desired=Vector2.new(
        math.clamp(desired.X,center.X-rx*.42,center.X+rx*.42),
        math.clamp(desired.Y,center.Y-ry*.40,center.Y+ry*.40)
    )
    state.lastScreen=desired

    local mp=UIS:GetMouseLocation()
    local delta=desired-mp
    local dist=delta.Magnitude
    local strength=math.clamp(tonumber(cfg.mouseAimFollowStrength) or 36,1,100)/100

    -- Adaptive response: large errors catch up quickly; close-range corrections
    -- slow down automatically. This avoids both the sluggish v2.91 feel and a
    -- robotic one-frame snap.
    local distanceBoost=math.clamp(dist/130,0,1)
    local response=3.8+(strength*9.2)+(distanceBoost*5.0)+(shotFocus and 3.2 or 0)
    local followAlpha=1-math.exp(-response*dt)
    local maxStep=6+(strength*38)+(distanceBoost*18)+(shotFocus and 6 or 0)
    local dx=math.clamp(delta.X*followAlpha,-maxStep,maxStep)
    local dy=math.clamp(delta.Y*followAlpha,-maxStep,maxStep)

    -- Human-scale dead zone when already on the selected part. Silent Correction
    -- still uses the exact body part center underneath this visible movement.
    local deadZone=math.max(.65,1.35-(strength*.55))
    if dist<deadZone then return end

    -- Slightly reduce microscopic corrections so the cursor does not vibrate.
    ...
    if dx~=0 or dy~=0 then
        if moveRel then
            pcall(moveRel,dx,dy)
        elseif VIM then
            pcall(function() VIM:SendMouseMoveEvent(mp.X+dx,mp.Y+dy,game) end)
        end
    end
end)
...
task.spawn(function()
    ...
    pcall(function()
        game:GetService("StarterGui"):SetCore("SendNotification",{
            Title="Kimqetras HC",
            Text="v2.92 ACTIVE ♡ • humanized mouse aim + existing systems ready",
            Duration=7,
        })
    end)
