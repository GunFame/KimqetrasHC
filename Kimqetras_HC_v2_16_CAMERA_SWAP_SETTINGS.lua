-- Kimqetras HC v2.16 • Camera swap tuning + v2.15 bullet fix + v2.14 features
-- Re-running this SAME build only reopens the current GUI instead of stacking another copy.
-- If an older build is already active, v2.16 refuses to stack and asks for one clean rejoin.
do
    local SINGLE_KEY = "KimqHC_v21_SingleRuntimeOptimized"
    local BUILD = "v2.16-camera-swap-settings"
    ...
    -- Older builds did not expose a complete disconnect/cleanup handle. Starting v2.16
    ...
    if rawget(_G, SINGLE_KEY) and rawget(_G, "KimqHC_CurrentBuild") ~= BUILD then
        pcall(function()
            game:GetService("StarterGui"):SetCore("SendNotification", {
                Title = "Kimqetras HC",
                Text = "Older runtime detected. Rejoin once, then run v2.16 so only one version is active.",
                Duration = 7,
            })
        end)
        warn("[Kimqetras HC v2.16] Older runtime detected. Rejoin once before running v2.16; refusing to stack versions.")
        return
    end
...
-- v2.16 keeps the v2.15 local bullet_rays fix and adds tunable smooth camera-swap timing.
...
-- Each accepted shot changes the view and leaves it there until the next accepted shot.
-- The tuning below matches the requested camera settings:
-- Zoom Speed 8 / Zoom Min 5 / Zoom Max 25 / Stay Min 0.500 /
-- Stay Max 0.500 / Frequency 2.000.
...
local function smoothZoomTo(target,token)
    ...
    -- Fast but smooth at the requested defaults. Higher Zoom Speed/Frequency
    -- makes the transition quicker without turning it into an instant snap.
...
local function doShotCameraSwap(tool)
    ...
    local stay=stayMin
    if stayMax>stayMin then
        stay=stayMin+(stayMax-stayMin)*math.random()
    end
    -- Stay and Frequency both act as a clean debounce. With the requested
    -- .500 stay and 2.000 frequency this permits one camera change per .5s.
    ...
    if not first and dist>1.1 then
        lastThirdDistance=math.clamp(dist,zmin,zmax)
    end
    ...
    local target=cameraTargetFirst and 0.5 or math.clamp(lastThirdDistance or zmin,zmin,zmax)
...
-- Camera Swap tuning controls. These are normal registered controls,
-- so configs remember them automatically.
