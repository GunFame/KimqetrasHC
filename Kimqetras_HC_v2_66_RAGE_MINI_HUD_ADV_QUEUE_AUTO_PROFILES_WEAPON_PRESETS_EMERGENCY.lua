-- Kimqetras HC v2.66 • themed RAGE mini HUD + advanced queue + auto weapon profiles + weapon presets + emergency restore
...
local KIMQ_BUILD = "v2.66-rage-hud-queue-profiles-presets-restore"
...
do
    ...
    if running and runningBuild == KIMQ_BUILD then
        ...
        pcall(function()
            StarterGui0:SetCore("SendNotification",{
                Title="Kimqetras HC",
                Text=(runtimeState=="ready") and "v2.66 is already running ♡" or "v2.66 is already starting ♡",
                Duration=4,
            })
        end)
        return
    ...
    if oldRuntimeDetected then
        pcall(function()
            StarterGui0:SetCore("SendNotification",{
                Title="Kimqetras HC",
                Text="An older Kimqetras HC runtime is still active. Rejoin once, then run v2.66. v2.66 will not stack on top of it.",
                Duration=9,
            })
        end)
        warn("[Kimqetras HC v2.66] Older runtime detected. Refusing to stack; rejoin once.")
        return
    end
...
Hello.Text = "v2.66  •  rage + presets upgrade ♡"
...
addSmallNote("Emergency Restore instantly gives normal camera + movement back without deleting your configs or target queue.")
local _, emergencyRestoreButton = addButton("♥  Emergency Restore", function()
    pcall(function()
        if type(_G.KimqRageEmergencyRestore)=="function" then
            _G.KimqRageEmergencyRestore()
        end
    end)
    ...
    pcall(function()
        game:GetService("StarterGui"):SetCore("SendNotification",{
            Title="KIM ♡",
            Text="Emergency Restore complete ♡",
            Duration=3
        })
    end)
end)
...
emergencyRestoreButton.TextSize=13
...
task.spawn(function()
    ...
    task.defer(function()
        local rageOK,rageERR=pcall(function()
            ...
            -- v2.66: RAGE-only helpers. These do not modify normal COMBAT/Silent Aim.
            ...
            local function button(parent,text,pos,size,fn)
                ...
                b.MouseButton1Click:Connect(function()
                    ...
                    if not ok then warn("[Kimqetras HC v2.66 RAGE button] "..tostring(err)) end
                end)
                return b
            end
            ...
            local function startOrbitMotion()
                ...
                orbitMotionConn=RunServiceR.Heartbeat:Connect(function(dt)
                    ...
                    if not ok then
                        warn("[Kimqetras HC v2.66 RAGE orbit] "..tostring(err))
                    end
                end)
            end
            ...
            local function refreshRageHudTheme()
                ...
                if rageHudHeader then rageHudHeader.BackgroundColor3=p.soft or p.bg2 or T.bg2 end
                ...
                if rageHudTitle then rageHudTitle.TextColor3=p.white or Color3.new(1,1,1) end
            ...
            local function buildRageHud()
                ...
                rageHudFrame.Size=UDim2.fromOffset(282,154)
                rageHudFrame.BorderSizePixel=0
                rageHudFrame.Visible=false
                rageHudFrame.ZIndex=80
                corner(rageHudFrame,18)
                rageHudStroke=stroke(rageHudFrame,pal().line or T.stroke,.12,1.5)

                rageHudHeader=Instance.new("Frame")
                rageHudHeader.Name="Header"
                rageHudHeader.Parent=rageHudFrame
                rageHudHeader.Size=UDim2.new(1,-12,0,38)
                rageHudHeader.Position=UDim2.fromOffset(6,6)
                rageHudHeader.BorderSizePixel=0
                rageHudHeader.Active=true
                rageHudHeader.ZIndex=81
                corner(rageHudHeader,13)

                rageHudGradient=Instance.new("UIGradient")
                rageHudGradient.Parent=rageHudHeader
                rageHudGradient.Rotation=8

                rageHudTitle=label(
                    rageHudHeader,"♥  RAGE MINI",
                    UDim2.new(1,-112,1,0),UDim2.fromOffset(12,0),
                    Enum.Font.FredokaOne,14,pal().white or Color3.new(1,1,1)
                )
                rageHudTitle.TextColor3=pal().white or Color3.new(1,1,1)
                rageHudTitle.ZIndex=82

                rageHudStatusPill=Instance.new("Frame")
                rageHudStatusPill.Parent=rageHudHeader
                rageHudStatusPill.Size=UDim2.fromOffset(88,24)
                ...
                rageHudTarget=label(
                    rageHudFrame,"TARGET  •  none",
                    UDim2.new(1,-24,0,22),UDim2.fromOffset(12,49),
                    Enum.Font.GothamBold,11,pal().text or T.text
                )
                rageHudTarget.ZIndex=82

                rageHudQueue=label(
                    rageHudFrame,"QUEUE  0/0  •  MANUAL",
                    UDim2.new(1,-24,0,19),UDim2.fromOffset(12,72),
                    Enum.Font.Gotham,9,pal().sub or T.sub
                )
                rageHudQueue.ZIndex=82

                rageHudWeapon=label(
                    rageHudFrame,"WEAPON  •  none",
                    UDim2.new(1,-24,0,21),UDim2.fromOffset(12,94),
                    Enum.Font.GothamSemibold,10,pal().text or T.text
                )
                rageHudWeapon.ZIndex=82

                rageHudNext=label(
                    rageHudFrame,"NEXT  →  none",
                    UDim2.new(1,-24,0,19),UDim2.fromOffset(12,118),
                    Enum.Font.Gotham,9,pal().sub or T.sub
                )
                rageHudNext.ZIndex=82

                rageHudHint=label(
                    rageHudFrame,"drag the pretty header to move me ♡",
                    UDim2.new(1,-24,0,14),UDim2.fromOffset(12,136),
            ...
            local function updateRageHud()
                ...
                rageHudFrame.Visible=rageHudEnabled and autoHunt
                ...
                elseif camEnabled then status="LOCKED" end
            ...
            makeCycle(rageCamPage,"Queue Order",{"Manual","Closest","Lowest Health","Random"},queueOrderMode,function(v)
                queueOrderMode=v
                applyQueueOrdering(v=="Random")
            end)
            makeToggle(rageCamPage,"Skip Downed / Dead On Advance",true,function(v)
                queueSkipDowned=v
            end)
            makeToggle(rageCamPage,"Skip Whitelisted On Advance",true,function(v)
            ...
            makeToggle(rageCombatPage,"Auto Weapon Profiles",true,function(v)
                autoWeaponProfiles=v
            end)
            makeToggle(rageCombatPage,"Rage Mini HUD",true,function(v)
                rageHudEnabled=v
                updateRageHud()
            end)
            ...
            RunServiceR.RenderStepped:Connect(function(dt)
                ...
                if not ok then
                    warn("[Kimqetras HC v2.66 RAGE camera] "..tostring(err))
                end
            end)
            ...
            RunServiceR.Heartbeat:Connect(function(dt)
                ...
                if not ok then
                    warn("[Kimqetras HC v2.66 RAGE heartbeat] "..tostring(err))
                end
            end)
        ...
        if not rageOK then
            warn("[Kimqetras HC v2.66] RAGE module isolated error: "..tostring(rageERR))
        end
    end)
...
task.spawn(function()
    ...
    if not weaponPresetOK then warn("[Kimqetras HC v2.66 weapon presets] "..tostring(weaponPresetERR)) end
