...
-- v2.1 SINGLE-RUNTIME GUARD (stale-lock safe)
-- Only block a re-run when a fully loaded v2.1 GUI actually exists.
-- If a previous run errored halfway through, its stale guard is cleared automatically.
do
    ...
    if rawget(_G, SINGLE_KEY) and root and root:IsA("ScreenGui") and root:GetAttribute("KimqReady") == true then
        pcall(function()
            root.Enabled = true
    ...
    -- Stale/failed previous run: let this execution start cleanly.
    _G[SINGLE_KEY] = nil
    if root and root:IsA("ScreenGui") and root:GetAttribute("KimqReady") ~= true then
    ...
    if info then
        ...
        local function profileRow(userId, displayName, role, x)
            ...
            task.spawn(function()
                ...
                if ok and pic.Parent then pic.Image=img end
            end)
...
task.spawn(function()
    ...
    local function setupWeaponExtras()
        ...
        local function applyBulletBeamOverride(name)
            ...
            -- Keep the game's storage templates untouched. The game creates the real
            -- tracer as Workspace.Ignored.BULLET_RAYS, so we recolor that exact object
            -- when it spawns instead of changing gun or storage parts.
        ...
        -- Exact live tracer hook discovered in-game:
        -- Workspace > Ignored > BULLET_RAYS. These Parts appear briefly for every shot.
        -- Paint only those Parts, then repaint a few milliseconds later because the
        -- game's own shot code may set their default color immediately after creation.
        ...
        local function paintBulletRay(ray)
            ...
            local src=selectedBeamSource(); if not src then return end
            local srcPart=firstPart(src); if not srcPart then return end
            if bulletRayPainting[ray] then return end
            bulletRayPainting[ray]=true
            safeCopy(ray,srcPart,{"Color","Material","MaterialVariant","Transparency","Reflectance"})
            pcall(function() ray.CastShadow=srcPart.CastShadow end)
        ...
        workspace.ChildAdded:Connect(function(ch)
            if ch.Name=="Ignored" then hookIgnored(ch) end
        end)
        ...
        local function matchingGripAttachment(sourceRoot,targetRoot)
            ...
            -- Knife storage models contain several attachments literally named
            -- "Attachment". Matching those arbitrarily can throw the cosmetic far
            -- away from the real Knife. Only trust explicit grip attachment names;
            -- otherwise align Handle-to-Handle below.
        ...
        local function buildKnifeVisual(source,tool)
            ...
            for _,part in ipairs(visual:GetDescendants()) do
                if part:IsA("BasePart") then
                    ...
                    local helper=lname:find("hitbox",1,true) or lname=="humanoidrootpart" or lname:find("particle_part",1,true)
                    local meshBearing=part:IsA("MeshPart") or part:IsA("UnionOperation") or part:FindFirstChildWhichIsA("SpecialMesh")~=nil
                    if not helper and meshBearing and part.Transparency>=.99 then pcall(function() part.Transparency=0 end) end
                    if not helper and part.Transparency<.99 then visibleCount+=1 end
                end
            end
...
-- Mark this runtime ready only after the full chunk reached the end successfully.
pcall(function()
    local Players = game:GetService("Players")
    local CoreGui = game:GetService("CoreGui")
    local lp = Players.LocalPlayer
    ...
    if root and root:IsA("ScreenGui") then
        root:SetAttribute("KimqReady", true)
    end
end)
