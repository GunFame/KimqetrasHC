-- Kimqetras HC v2.13 • Angel Wings original-rig root mount + live fit calibration • keeps v2.12 behavior
-- Re-running this SAME build only reopens the current GUI instead of stacking another copy.
-- If an older build is already active, v2.13 refuses to stack and asks for one clean rejoin.
do
    local SINGLE_KEY = "KimqHC_v21_SingleRuntimeOptimized"
    local BUILD = "v2.13-angel-wings-exact-fit"
    ...
    -- Older builds did not expose a complete disconnect/cleanup handle. Starting v2.13
    ...
    if rawget(_G, SINGLE_KEY) and rawget(_G, "KimqHC_CurrentBuild") ~= BUILD then
        pcall(function()
            game:GetService("StarterGui"):SetCore("SendNotification", {
                Title = "Kimqetras HC",
                Text = "Older runtime detected. Rejoin once, then run v2.13 so only one version is active.",
                Duration = 7,
            })
        end)
        warn("[Kimqetras HC v2.13] Older runtime detected. Rejoin once before running v2.13; refusing to stack versions.")
        return
    end
