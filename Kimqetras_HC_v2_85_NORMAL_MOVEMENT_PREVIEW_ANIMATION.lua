-- Kimqetras HC v2.85 • v2.84 base + normal avatar movement preservation + animated RAGE preview
...
local KIMQ_BUILD = "v2.85-normal-movement-preview-animation"
...
do
    ...
    if running and runningBuild == KIMQ_BUILD then
        if runningGui then
            ...
            notifyBoot((runtimeState=="ready") and "v2.84 is already running ♡" or "v2.84 is already starting ♡",4)
            return
        end
        ...
        notifyBoot("Recovering a failed v2.84 boot ♡",4)
    end
    ...
    if oldRuntimeDetected then
        ...
        if staleBoot then
            ...
            notifyBoot("STOP: another Kimqetras HC build is already active. v2.84 DID NOT LOAD. Rejoin once, then run only v2.84.",9)
            warn("[Kimqetras HC v2.84] Another build is active. v2.84 DID NOT LOAD; rejoin once before testing.")
            return
        end
    end
...
Hello.Text = "v2.85  •  normal movement + animated RAGE preview ♡"
    ...
    task.defer(function()
        local rageOK,rageERR=pcall(function()
            ...
            do
                ...
                local sub=R.role(R.label(c,"real multi-target run: acquire → orbit → fire → K.O → stop orbit → E stomp → next ♡",UDim2.new(1,-34,0,38),UDim2.fromOffset(12,34),Enum.Font.Gotham,9,R.pal().sub or T.sub),"subText")
                ...
                local hint=R.role(R.label(c,"Tip: check multiple names in RAGE Camlock, then press OP ALL TARGETS. Your checked queue is never collapsed into one target.",UDim2.new(1,-36,0,44),UDim2.fromOffset(12,182),Enum.Font.Gotham,9,R.pal().sub or T.sub),"subText")
...
task.spawn(function()
    ...
    pcall(function()
        game:GetService("StarterGui"):SetCore("SendNotification",{
            Title="Kimqetras HC",
            Text="v2.85 ACTIVE ♡ • normal avatar movement + animated preview ready",
            Duration=7,
        })
    end)
