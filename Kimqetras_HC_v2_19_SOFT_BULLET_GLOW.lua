-- Kimqetras HC v2.19 • softer local bullet glow + Kimqetras owner branding + famesgun developer credit + gun-only free camera swap
-- Re-running this SAME build only reopens the current GUI instead of stacking another copy.
-- If an older build is already active, v2.19 refuses to stack and asks for one clean rejoin.
do
    local SINGLE_KEY = "KimqHC_v21_SingleRuntimeOptimized"
    local BUILD = "v2.19-soft-bullet-glow"
    ...
    -- Older builds did not expose a complete disconnect/cleanup handle. Starting v2.19
    ...
    if rawget(_G, SINGLE_KEY) and rawget(_G, "KimqHC_CurrentBuild") ~= BUILD then
        pcall(function()
            game:GetService("StarterGui"):SetCore("SendNotification", {
                Title = "Kimqetras HC",
                Text = "Older runtime detected. Rejoin once, then run v2.19 so only one version is active.",
                Duration = 7,
            })
        end)
        warn("[Kimqetras HC v2.19] Older runtime detected. Rejoin once before running v2.19; refusing to stack versions.")
        return
    end
...
-- v2.19 keeps the v2.15 local bullet fix, tunable camera swap, owner/credit cleanup, and softens only bullet/tracer glow.
...
-- ========================================================
-- v2.19 owner / developer credit guard
