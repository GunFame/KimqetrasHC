-- Kimqetras HC v2.97 • v2.96 base + executor-independent visible Mouse Aim lock
...
local KIMQ_BUILD = "v2.97-mouse-aim-visible-lock"
...
do
    ...
    if running and runningBuild == KIMQ_BUILD then
        if runningGui then
            ...
            notifyBoot((runtimeState=="ready") and "v2.97 is already running ♡" or "v2.97 is already starting ♡",4)
            return
        end
        ...
        notifyBoot("Recovering a failed v2.97 boot ♡",4)
    end
    ...
    if oldRuntimeDetected then
        ...
        if staleBoot then
            ...
            notifyBoot("STOP: another Kimqetras HC build is already active. v2.97 DID NOT LOAD. Rejoin once, then run only v2.97.",9)
            warn("[Kimqetras HC v2.97] Another build is active. v2.97 DID NOT LOAD; rejoin once before testing.")
            return
        end
    end
...
Hello.Text = "v2.97  •  visible Q lock + custom cursors ♡"
...
-- v2.97 Mouse Aim: executor-independent visible lock.
-- The visible aim cursor is drawn by Kimqetras itself instead of depending on
-- mousemoverel/VirtualInputManager. Q therefore still produces an obvious lock
-- even on executors that cannot move the Windows/Roblox pointer.

if cfg.mouseAimNaturalProfileVersion~="v2.97" then
    cfg.mouseAimPart="UpperTorso"
    cfg.mouseAimFOV=190
    cfg.mouseAimFollowStrength=58
    cfg.mouseAimSoftness=7
    cfg.mouseAimNaturalProfileVersion="v2.97"
end

if cfg.mouseAimEnabled == nil then cfg.mouseAimEnabled=true else cfg.mouseAimEnabled=cfg.mouseAimEnabled==true end
...
cfg.mouseAimFollowStrength=tonumber(cfg.mouseAimFollowStrength) or 58
cfg.mouseAimSoftness=tonumber(cfg.mouseAimSoftness) or 7
...
local screenInset=Vector2.zero
pcall(function()
    local tl=GuiService:GetGuiInset()
    if typeof(tl)=="Vector2" then screenInset=tl end
end)

local state={
    active=false,target=nil,part=nil,point=nil,
    cursor=nil,lastCursor=nil,lastClock=os.clock(),
    anchorX=0,anchorY=0,engagedAt=0,
...
local cursorOverride=_G.KimqMouseAimCursorOverride
...
-- Everything in this section uses one coordinate system: absolute screen pixels.
-- UIS:GetMouseLocation() is screen-space; WorldToViewportPoint() is converted into
-- the same space by adding the current GUI inset. This removes the old vertical
-- offset that could make the cursor appear above the selected body part.
local function physicalCursorPos()
...
local function toScreen(pos)
    ...
    return Vector2.new(v.X+screenInset.X,v.Y+screenInset.Y),on and v.Z>0
end
local function aimCursorPos()
    if state.active and state.cursor then return state.cursor end
...
local function preferredPart(pl,mousePos)
    local c=pl and pl.Character
    ...
    mousePos=mousePos or physicalCursorPos()
...
local function partScreenBox(part)
    ...
    return center,math.clamp((maxX-minX)*.5,3,44),math.clamp((maxY-minY)*.5,4,52),true
end

local function candidateDistance(pl,limit,mousePos)
    if not valid(pl) then return math.huge,nil,nil end
    local part,point=preferredPart(pl,mousePos)
...
local function setOverride(active,pos)
    cursorOverride.active=active==true
    cursorOverride.position=pos
end
local function resetMotion()
    state.lastClock=os.clock();state.cursor=nil;state.lastCursor=nil
    state.anchorX=0;state.anchorY=0;state.engagedAt=0
    state.worldAimPoint=nil;state.worldAimPart=nil
    setOverride(false,nil)
end
local function startLock(pl,part,point,mousePos)
    state.active=true;state.target=pl;state.part=part;state.point=point
    state.cursor=mousePos or physicalCursorPos();state.lastCursor=state.cursor
    state.lastClock=os.clock();state.engagedAt=state.lastClock
    local center,rx,ry,on=partScreenBox(part)
    if center and on then
        local dx=(state.cursor.X-center.X)/math.max(rx,1)
        local dy=(state.cursor.Y-center.Y)/math.max(ry,1)
        -- Keep a small amount of the user's incoming aim direction so the lock
        -- does not always converge to the exact same center pixel.
        state.anchorX=math.clamp(dx*.18,-.24,.24)
        state.anchorY=math.clamp(dy*.18,-.22,.22)
    else
        state.anchorX,state.anchorY=0,0
    end
    _G.KimqMouseAimTarget=pl
    setOverride(cfg.mouseAimVisibleFollow,state.cursor)
end
local function acquire()
    local mousePos=physicalCursorPos()
    local direct=playerFromMouseTarget()
    if direct then
        local p,pt=preferredPart(direct,mousePos)
        local sp,on=pt and toScreen(pt) or nil,false
        if p and pt and sp and on and not wallBlocked(direct,pt) then
            startLock(direct,p,pt,mousePos);return true
        end
    end
    local best,bestD,bestPart,bestPoint=nil,math.huge,nil,nil
    local fov=math.max(20,tonumber(cfg.mouseAimFOV) or 190)
    for _,pl in ipairs(Players:GetPlayers()) do
        local d,p,pt=candidateDistance(pl,fov,mousePos)
        if d<bestD then best,bestD,bestPart,bestPoint=pl,d,p,pt end
    end
    if not best then
        local fallback=math.max(520,math.min(920,fov*2.9))
        for _,pl in ipairs(Players:GetPlayers()) do
            local d,p,pt=candidateDistance(pl,fallback,mousePos)
            if d<bestD then best,bestD,bestPart,bestPoint=pl,d,p,pt end
        end
    end
    if best then startLock(best,bestPart,bestPoint,mousePos);return true end
...
local function insideVisualPart(part,scale,extra)
    local center,rx,ry,on=partScreenBox(part)
    if not center or not on or not state.cursor then return false,center,rx,ry end
    local sx=rx*(scale or .9)+(extra or 0)
    local sy=ry*(scale or .9)+(extra or 0)
    local dx=(state.cursor.X-center.X)/math.max(sx,1)
    local dy=(state.cursor.Y-center.Y)/math.max(sy,1)
    return dx*dx+dy*dy<=1,center,rx,ry
end
local function worldPointFromVisual(part,center,rx,ry)
    if not part or not center or not state.cursor then return nil end
    local nx=math.clamp((state.cursor.X-center.X)/math.max(rx,1),-.46,.46)
    local ny=math.clamp((state.cursor.Y-center.Y)/math.max(ry,1),-.46,.46)
...
local function alignedShotPoint(pl,part,point)
    ...
    local inside,center,rx,ry=insideVisualPart(part,.98,3)
    if not inside or not center then return nil end
    return worldPointFromVisual(part,center,rx,ry) or point
end
...
_G.KimqMouseAimOverride=function()
    ...
    local part,point=preferredPart(state.target,state.cursor or physicalCursorPos())
...
local actionName="KimqMouseAimToggle_v297"
local function mouseAimNotify(text)
    pcall(function() StarterGui:SetCore("SendNotification",{Title="Mouse Aim ♡",Text=text,Duration=2}) end)
end
...
UIS.InputBegan:Connect(function(input)
    if UIS:GetFocusedTextBox() then return end
    if input.UserInputType==Enum.UserInputType.Keyboard and input.KeyCode==normalizeMouseAimKey(cfg.mouseAimKey) then requestMouseAimToggle() end
end)
...
RunService.RenderStepped:Connect(function()
    ...
    if not cfg.mouseAimVisibleFollow then
        setOverride(false,nil)
        state.cursor=physicalCursorPos()
        return
    end
    ...
    state.lastClock=now
    state.cursor=state.cursor or physicalCursorPos()

    -- Follow the same relative place on the selected body part. This keeps the
    -- cursor ON the body without pinning it to an identical center pixel.
    local desired=Vector2.new(
        center.X + rx*state.anchorX*.72,
        center.Y + ry*state.anchorY*.72
    )
    local delta=desired-state.cursor
    local dist=delta.Magnitude
    local strength=math.clamp(tonumber(cfg.mouseAimFollowStrength) or 58,1,100)/100
    local softness=math.clamp(tonumber(cfg.mouseAimSoftness) or 7,0,18)/18
    local response=11.5 + strength*12 - softness*2.4
    if now-(state.engagedAt or now)<.18 then response=response+8 end

    -- If lag has pushed the visual cursor outside the body, catch up more firmly;
    -- once it is on-body, ease off so the motion looks fluid instead of magnetic.
    local inside=insideVisualPart(part,.78,1)
    if not inside then response=response+math.clamp(dist/20,0,9) end
    local alpha=1-math.exp(-dt*math.max(8,response))
    local step=delta*alpha
    local maxSpeed=(900+strength*1500+(inside and 0 or 900))*dt
    if step.Magnitude>maxSpeed and step.Magnitude>0 then step=step.Unit*maxSpeed end
    if step.Magnitude>dist and dist>0 then step=delta end
    state.cursor=state.cursor+step
    state.lastCursor=state.cursor
    setOverride(true,state.cursor)
...
addKeybind("Mouse Aim Key",cfg.mouseAimKey,function(v)
    cfg.mouseAimKey=normalizeMouseAimKey(v);bindMouseAimAction()
end)
addToggle("Visible Mouse Follow",cfg.mouseAimVisibleFollow,function(v)
    cfg.mouseAimVisibleFollow=v==true
    if not cfg.mouseAimVisibleFollow then setOverride(false,nil) end
end)
...
task.spawn(function()
    ...
    pcall(function()
        game:GetService("StarterGui"):SetCore("SendNotification",{
            Title="Kimqetras HC",
            Text="v2.97 ACTIVE ♡ • visible Mouse Aim lock + custom cursors ready",
            Duration=7,
        })
    end)
