-- Kimqetras HC v2.33 • organized pages + avatar revamp + stable special pages
...
local KIMQ_BUILD = "v2.33-organized-avatar-special-pages"
    ...
    if running and runningGui and runningBuild ~= KIMQ_BUILD then
        pcall(function()
            game:GetService("StarterGui"):SetCore("SendNotification", {
                Title = "Kimqetras HC",
                Text = "Another Kimqetras HC version is still active. Rejoin once, then run v2.33.",
                Duration = 8,
            })
        end)
        warn("[Kimqetras HC v2.33] Another runtime is active; rejoin once before switching builds.")
        return
    end
...
_G.KimqAccessoryUIReady = false
    ...
    pcall(function() game:GetService("RunService"):UnbindFromRenderStep("KimqPermanentTimeV233") end)
end
...
local function setAvatarStatus(text, positive)
    ...
    AvatarStatus.TextColor3 = positive and Color3.fromRGB(230,40,135) or Color3.fromRGB(197,112,145)
end
...
local function getDescription(userId)
    ...
    -- Compatibility fallback for games/executors where the description API is restricted.
    ...
    if okDummy and dummy then
        ...
        if dh then
            local okDesc,d = pcall(function() return dh:GetAppliedDescription() end)
            pcall(function() dummy:Destroy() end)
            if okDesc and d then return d end
        end
        pcall(function() dummy:Destroy() end)
    end
    return nil
...
local function applyAvatarUser(userId, char, quiet)
    ...
    task.spawn(function()
        ...
        if not hum or serial~=avatarApplySerial then
            if not quiet then setAvatarStatus("Humanoid not found", false) end
            return
        end

        local desc = getDescription(userId)
        if not desc or serial~=avatarApplySerial then
        ...
        if not ok then
            if not quiet then setAvatarStatus("Avatar could not be applied", false) end
            return
        end

        task.wait(.08)
        if serial~=avatarApplySerial then return end
        applyHeadless(char)
...
task.spawn(function()
    ...
    -- v2.33: pages are grouped by what they actually do so the sidebar is
    ...
    local pageDefs = {
        ...
        {"hcsilent", "HC silent aim", "extra targeting settings", "COMBAT"},
        {"forcehit", "force hit", "adjust force-hit settings", "COMBAT"},
        {"macro", "macro", "control how your macro behaves", "COMBAT"},
        ...
        {"avatar", "avatar", "copy an avatar or wear local catalog items", "VISUALS"},
        ...
        {"weaponskins", "weapon skins", "wraps, bullets, knives, and equippable items", "VISUALS"},
        {"headless", "headless", "toggle the headless look", "VISUALS"},
        ...
        {"settings", "settings", "performance and saved setup options", "SETUP"},
        {"theme", "theme", "change the colors of the whole interface", "SETUP"},
        {"info", "information", "credits and roles for this build", "SETUP"},
    }
    ...
    local function refreshPageSearch()
        ...
        for _, entry in ipairs(navButtons) do
            local hay = (tostring(entry.label) .. " " .. tostring(entry.key)):lower()
...
-- v2.33 module: local catalog / limited accessory try-on.
task.spawn(function()
    _G.KimqAccessoryUIReady=false
    local Players = game:GetService("Players")
    local CoreGui = game:GetService("CoreGui")
...
-- v2.33 module: Environment + Weapon Skins (canonical sidebar pages).
task.spawn(function()
    local pageReadyStart=tick()
    while not _G.KimqBasePagesReady and tick()-pageReadyStart<8 do task.wait(.02) end
    ...
    -- v2.33: Environment and Weapon Skins are already real canonical pages/buttons.
    ...
    pcall(function()
        RunService:BindToRenderStep("KimqPermanentTimeV233",Enum.RenderPriority.Last.Value,function()
    ...
    local function syncTheme()
        ...
        refreshPreset(); refreshWeaponStyle(); refreshSkinStyle(); if _G.KimqWeaponExtrasController and _G.KimqWeaponExtrasController.RefreshColor then _G.KimqWeaponExtrasController.RefreshColor() end
    end
...
task.spawn(function()
    ...
    -- v2.33: section navigation does not repaint/rescan the entire GUI.
    ...
    -- v2.33 reveal happens only after this initial theme pass is complete.
end)


-- v2.33: delayed post-show cleanup passes removed.
...
-- ========================================================
-- v2.33 branding is deterministic at construction time; no delayed scrub pass.

-- ========================================================
-- v2.33 DETERMINISTIC REVEAL
