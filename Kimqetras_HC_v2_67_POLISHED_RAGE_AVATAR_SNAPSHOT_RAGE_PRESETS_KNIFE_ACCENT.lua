-- Kimqetras HC v2.67 • polished RAGE UI + frozen avatar configs + RAGE presets + knife accent recolor
...
local KIMQ_BUILD = "v2.67-polished-rage-avatar-snapshot-presets-knife-accent"
...
do
    ...
    if running and runningBuild == KIMQ_BUILD then
        ...
        pcall(function()
            StarterGui0:SetCore("SendNotification",{
                Title="Kimqetras HC",
                Text=(runtimeState=="ready") and "v2.67 is already running ♡" or "v2.67 is already starting ♡",
                Duration=4,
            })
        end)
        return
    ...
    if oldRuntimeDetected then
        pcall(function()
            StarterGui0:SetCore("SendNotification",{
                Title="Kimqetras HC",
                Text="An older Kimqetras HC runtime is still active. Rejoin once, then run v2.67. v2.67 will not stack on top of it.",
                Duration=9,
            })
        end)
        warn("[Kimqetras HC v2.67] Older runtime detected. Refusing to stack; rejoin once.")
        return
    end
...
Hello.Text = "v2.67  •  polished rage + avatar snapshots ♡"
...
local function SaveConfig(configName)
    ...
    local configData = {
        format = "KimqetrasHC-v2.67-all-settings",
        name = name,
...
task.spawn(function()
    ...
    task.defer(function()
        local rageOK,rageERR=pcall(function()
            ...
            -- Setters exposed only inside the isolated RAGE module so one-tap presets
            -- can update both the live feature state AND the matching GUI toggle.
            ...
            -- v2.67 RAGE Presets: one-tap combinations that only touch RAGE state.
        ...
        if not rageOK then
            warn("[Kimqetras HC v2.67] RAGE module isolated error: "..tostring(rageERR))
        end
    end)
