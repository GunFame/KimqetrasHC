-- Kimqetras HC v2.3 • STRICT LOCAL BULLETS • transparent knife carriers • full theme repaint
...
for _,b in ipairs({saveConfigButton,loadConfigButton,deleteConfigButton,refreshConfigButton}) do
...
task.spawn(function()
    ...
    -- Clean Information page: simple text credits only (no thumbnails / profile links).
    ...
    if info then
        ...
        cardInfo.Size = UDim2.new(1,-6,0,166)
        cardInfo.LayoutOrder = -100
        cardInfo.BackgroundColor3 = T.panel
        cardInfo.BorderSizePixel = 0
        corner(cardInfo,18)
        stroke(cardInfo,T.stroke,0.2,1)
        label(cardInfo,"♥  information",UDim2.new(1,-24,0,28),UDim2.fromOffset(12,10),Enum.Font.GothamSemibold,20,T.hot)
        label(cardInfo,"Kimqetras HC ♡",UDim2.new(1,-24,0,18),UDim2.fromOffset(12,38),Enum.Font.Gotham,12,T.sub)

        local function creditLine(y,name,role)
            local row=Instance.new("Frame")
            row.Parent=cardInfo; row.Size=UDim2.new(1,-24,0,42); row.Position=UDim2.fromOffset(12,y)
            row.BackgroundColor3=T.soft; row.BorderSizePixel=0; corner(row,12); stroke(row,T.stroke,.42,1)
            local nm=label(row,name,UDim2.new(0,150,1,0),UDim2.fromOffset(12,0),Enum.Font.GothamSemibold,14,T.hot)
            nm.TextYAlignment=Enum.TextYAlignment.Center
            local rl=label(row,role,UDim2.new(1,-174,1,0),UDim2.fromOffset(162,0),Enum.Font.Gotham,12,T.sub,Enum.TextXAlignment.Right)
            rl.TextYAlignment=Enum.TextYAlignment.Center
        end
        creditLine(68,"kimqetras","owner ♡")
        creditLine(116,"famesgun","original developer / scripter")
    end
...
task.spawn(function()
    ...
    local function setupWeaponExtras()
        ...
        local bulletColorSub=txt(bulletColorCard,"Custom + rainbow colors apply only to bullet_rays created by your shots.",UDim2.new(1,-24,0,32),UDim2.fromOffset(12,30),Enum.Font.Gotham,11,pcolor("sub",subColor))
        ...
        workspace.DescendantAdded:Connect(function(d)
            ...
            if tostring(d.Name):lower()=="bullet_rays" or bulletRayShotRoot(d) then
                task.defer(function()
        ...
        local function visibleKnifePart(part)
            ...
            -- A lot of the knife templates use a plain transparent Handle only as a
            -- weld/animation carrier. Revealing that Handle creates the grey block the
            -- user was seeing. MeshParts / unions / SpecialMesh geometry are real visuals.
            if part:IsA("MeshPart") or part:IsA("UnionOperation") or part:FindFirstChildWhichIsA("SpecialMesh") then
                return true
            end

            -- Plain BaseParts are only visible if the template itself authored them visible.
            -- In particular, a Transparency=1 Handle remains transparent.
