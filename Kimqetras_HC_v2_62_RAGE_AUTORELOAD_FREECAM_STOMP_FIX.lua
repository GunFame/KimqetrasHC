-- Kimqetras HC v2.62 • restored v2.19 weapon-extras core + isolated wing flight
...
local KIMQ_BUILD = "v2.62-restored-v219-extras"
...
do
    ...
    if running and runningBuild == KIMQ_BUILD then
        ...
        pcall(function()
            StarterGui0:SetCore("SendNotification",{
                Title="Kimqetras HC",
                Text=(runtimeState=="ready") and "v2.62 is already running ♡" or "v2.62 is already starting ♡",
                Duration=4,
            })
        end)
        return
    ...
    if oldRuntimeDetected then
        pcall(function()
            StarterGui0:SetCore("SendNotification",{
                Title="Kimqetras HC",
                Text="An older Kimqetras HC runtime is still active. Rejoin once, then run v2.62. v2.62 will not stack on top of it.",
                Duration=9,
            })
        end)
        warn("[Kimqetras HC v2.62] Older runtime detected. Refusing to stack; rejoin once.")
        return
    end
...
task.spawn(function()
    ...
    task.defer(function()
        local rageOK,rageERR=pcall(function()
            ...
            -- v2.62: optional reload helper for RAGE combat only.
            ...
            local function button(parent,text,pos,size,fn)
                ...
                b.MouseButton1Click:Connect(function()
                    ...
                    if not ok then warn("[Kimqetras HC v2.62 RAGE button] "..tostring(err)) end
                end)
                return b
            end
            ...
            local function startOrbitMotion()
                ...
                orbitMotionConn=RunServiceR.Heartbeat:Connect(function(dt)
                    ...
                    if not ok then
                        warn("[Kimqetras HC v2.62 RAGE orbit] "..tostring(err))
                    end
                end)
            end
            ...
            local function finish(pl)
                ...
                task.spawn(function()
                    if autoStomp then
                        -- v2.62 stomp fix:
                        -- 1) orbit is already paused by finishedWaiting[id]
                        -- 2) stand upright DIRECTLY on top of the knocked body
                        -- 3) give that position time to replicate before E
                        -- 4) keep re-snapping while several stomp attempts are sent
                        local presses=0
                        local wantedPresses=math.max(stompRepeats,8)
                        local deadline=os.clock()+2.45
            ...
            makeToggle(rageCombatPage,"Auto Reload When Empty",false,function(v)
                autoReload=v
                reloadBusy=false
                lastReloadAt=0
            end)
            ...
            makeToggle(rageCombatPage,"Rage Combat Master",false,function(v)
                ...
                if v then
                    ...
                    -- Start from a normal FREE local camera. Explicit RAGE Lock/View
                    -- buttons still work if the user chooses them afterward.
                    camEnabled=false
                    if viewEnabled then restoreCamera() end
                    local rageCam=workspace.CurrentCamera
                    local localHum=lp.Character and lp.Character:FindFirstChildOfClass("Humanoid")
                    if rageCam and localHum then
                        pcall(function()
                            rageCam.CameraType=Enum.CameraType.Custom
                            rageCam.CameraSubject=localHum
                        end)
                    end
            ...
            local noteText=role(label(
                note,
                "Rage Combat Master runs by itself: orbit → auto-fire → detect K.O → stand directly on the body + stomp → continue. Auto Reload can reload an empty gun. Combat Master starts with your normal free camera; Lock/View still stay optional.",
            ...
            RunServiceR.RenderStepped:Connect(function(dt)
                local ok,err=pcall(function()
                    ...
                    if viewEnabled then
                        ...
                        if hum then
                            ...
                            -- Do NOT force MouseBehavior every RenderStepped.
                            -- Roblox's CameraModule must be allowed to switch it while
                            -- you drag/rotate the camera with the mouse.
                        end
                ...
                if not ok then
                    warn("[Kimqetras HC v2.62 RAGE camera] "..tostring(err))
                end
            end)
            ...
            RunServiceR.Heartbeat:Connect(function(dt)
                ...
                if not ok then
                    warn("[Kimqetras HC v2.62 RAGE heartbeat] "..tostring(err))
                end
            end)
        ...
        if not rageOK then
            warn("[Kimqetras HC v2.62] RAGE module isolated error: "..tostring(rageERR))
        end
    end)
...
task.spawn(function()
    ...
    if not envBuildOk then
        warn("[Kimqetras HC v2.62] Environment page fallback: "..tostring(envBuildErr))
    ...
    if not extrasOk then
        ...
        warn("[Kimqetras HC v2.62] restored v2.19 Weapon Extras: "..tostring(extrasErr))
    ...
    skinsBtn.MouseButton1Click:Connect(function()
        task.defer(function()
            ...
            if extras and type(extras.Refresh)=="function" then
                ...
                if not ok then
                    warn("[Kimqetras HC v2.62] extras refresh: "..tostring(err))
