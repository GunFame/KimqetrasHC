-- Kimqetras HC v2.36 • requested layout + Macro in Player + Delay Changer in Combat
...
local KIMQ_BUILD = "v2.36-macro-player-delay-combat"
    ...
    if running and runningGui and runningBuild ~= KIMQ_BUILD then
        pcall(function()
            game:GetService("StarterGui"):SetCore("SendNotification", {
                Title = "Kimqetras HC",
                Text = "Another Kimqetras HC version is still active. Rejoin once, then run v2.36.",
                Duration = 8,
            })
        end)
        warn("[Kimqetras HC v2.36] Another runtime is active; rejoin once before switching builds.")
        return
    end
...
_G.KimqAccessoryUIReady = false
    ...
    pcall(function() game:GetService("RunService"):UnbindFromRenderStep("KimqPermanentTimeV236") end)
end
...
task.spawn(function()
    ...
    -- v2.36: pages are grouped by what they actually do so the sidebar is
...
-- v2.36 module: local catalog / limited accessory try-on.
task.spawn(function()
    _G.KimqAccessoryUIReady=false
    local Players = game:GetService("Players")
    local CoreGui = game:GetService("CoreGui")
...
-- v2.36 module: Environment + Weapon Skins (canonical sidebar pages).
task.spawn(function()
    local pageReadyStart=tick()
    while not _G.KimqBasePagesReady and tick()-pageReadyStart<8 do task.wait(.02) end
    ...
    -- v2.36: Environment and Weapon Skins are already real canonical pages/buttons.
    ...
    pcall(function()
        RunService:BindToRenderStep("KimqPermanentTimeV236",Enum.RenderPriority.Last.Value,function()
    ...
    if not envBuildOk then
        warn("[Kimqetras HC v2.36] Environment page fallback: "..tostring(envBuildErr))
...
task.spawn(function()
    ...
    -- v2.36: preserve the grouped sidebar order from pageDefs.
    ...
    -- v2.36: section navigation does not repaint/rescan the entire GUI.
    ...
    -- v2.36 reveal happens only after this initial theme pass is complete.
end)


-- v2.36: delayed post-show cleanup passes removed.
...
-- ========================================================
-- v2.36 branding is deterministic at construction time; no delayed scrub pass.

-- ========================================================
-- v2.36 DETERMINISTIC REVEAL
