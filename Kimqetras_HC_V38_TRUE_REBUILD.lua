-- V38 loader cover. Uses Roblox-hosted decals and stays separate from the working V6 backend.
do
    _G.KimqV38Ready = false
    ...
    pcall(function()
        for _,root in ipairs({CoreGui,pg}) do
            local old=root:FindFirstChild("KimqV38Loader")
            if old then old:Destroy() end
        end
    end)

    local gui=Instance.new("ScreenGui")
    gui.Name="KimqV38Loader"
    ...
    bg.BackgroundColor3=Color3.fromRGB(178,218,255)
    bg.BorderSizePixel=0
    local bgG=Instance.new("UIGradient",bg)
    bgG.Color=ColorSequence.new({
        ColorSequenceKeypoint.new(0,Color3.fromRGB(229,243,255)),
        ColorSequenceKeypoint.new(.5,Color3.fromRGB(190,222,255)),
        ColorSequenceKeypoint.new(1,Color3.fromRGB(118,177,255)),
    })
    bgG.Rotation=24

    for _,s in ipairs({{.07,.17,180,.55},{.91,.18,220,.58},{.14,.85,270,.64},{.88,.87,300,.65},{.49,.96,330,.68}}) do
        local f=Instance.new("Frame",bg)
        f.AnchorPoint=Vector2.new(.5,.5); f.Position=UDim2.fromScale(s[1],s[2]); f.Size=UDim2.fromOffset(s[3],s[3])
        f.BackgroundColor3=Color3.fromRGB(245,251,255); f.BackgroundTransparency=s[4]; f.BorderSizePixel=0
        Instance.new("UICorner",f).CornerRadius=UDim.new(1,0)
    end

    local card=Instance.new("Frame",bg)
    card.AnchorPoint=Vector2.new(.5,.5); card.Position=UDim2.fromScale(.5,.5); card.Size=UDim2.fromOffset(720,430)
    card.BackgroundColor3=Color3.fromRGB(235,246,255); card.BorderSizePixel=0
    Instance.new("UICorner",card).CornerRadius=UDim.new(0,30)
    local st=Instance.new("UIStroke",card); st.Color=Color3.fromRGB(40,101,236); st.Thickness=2; st.Transparency=.12

    local stitch=Instance.new("Frame",card)
    stitch.BackgroundTransparency=1; stitch.Position=UDim2.fromOffset(18,18); stitch.Size=UDim2.new(1,-36,1,-36); stitch.ClipsDescendants=true
    local function dash(sz,pos)
        local d=Instance.new("Frame",stitch); d.Size=sz; d.Position=pos; d.BackgroundColor3=Color3.fromRGB(103,163,238); d.BackgroundTransparency=.25; d.BorderSizePixel=0
        Instance.new("UICorner",d).CornerRadius=UDim.new(1,0)
    end
    for i=0,22 do local x=i/22; dash(UDim2.fromOffset(12,2),UDim2.new(x,-6,0,0)); dash(UDim2.fromOffset(12,2),UDim2.new(x,-6,1,-2)) end
    for i=1,12 do local y=i/13; dash(UDim2.fromOffset(2,12),UDim2.new(0,0,y,-6)); dash(UDim2.fromOffset(2,12),UDim2.new(1,-2,y,-6)) end

    local pom=Instance.new("ImageLabel",card)
    pom.BackgroundTransparency=1; pom.Size=UDim2.fromOffset(152,142); pom.Position=UDim2.fromOffset(54,36); pom.Image=A.Pompom; pom.ScaleType=Enum.ScaleType.Fit
    local title=Instance.new("ImageLabel",card)
    title.BackgroundTransparency=1; title.Size=UDim2.fromOffset(390,88); title.Position=UDim2.fromOffset(198,45); title.Image=A.Title; title.ScaleType=Enum.ScaleType.Fit
    local sub=Instance.new("ImageLabel",card)
    sub.BackgroundTransparency=1; sub.Size=UDim2.fromOffset(245,50); sub.Position=UDim2.fromOffset(264,116); sub.Image=A.Subtitle; sub.ScaleType=Enum.ScaleType.Fit

    local profile=Instance.new("Frame",card)
    profile.Size=UDim2.fromOffset(202,48); profile.Position=UDim2.new(1,-232,0,24); profile.BackgroundColor3=Color3.fromRGB(246,251,255); profile.BorderSizePixel=0
    Instance.new("UICorner",profile).CornerRadius=UDim.new(0,15)
    local ps=Instance.new("UIStroke",profile); ps.Color=Color3.fromRGB(145,188,239); ps.Transparency=.3
    local av=Instance.new("ImageLabel",profile); av.Size=UDim2.fromOffset(34,34); av.Position=UDim2.fromOffset(8,7); av.BackgroundTransparency=1
    ...
    local nm=Instance.new("TextLabel",profile); nm.BackgroundTransparency=1; nm.Position=UDim2.fromOffset(48,5); nm.Size=UDim2.new(1,-54,0,18); nm.Text=lp.DisplayName; nm.TextColor3=Color3.fromRGB(36,75,145); nm.Font=Enum.Font.GothamBold; nm.TextSize=12; nm.TextXAlignment=Enum.TextXAlignment.Left
    local un=Instance.new("TextLabel",profile); un.BackgroundTransparency=1; un.Position=UDim2.fromOffset(48,23); un.Size=UDim2.new(1,-54,0,16); un.Text="@"..lp.Name; un.TextColor3=Color3.fromRGB(93,122,171); un.Font=Enum.Font.Gotham; un.TextSize=10; un.TextXAlignment=Enum.TextXAlignment.Left

    local status=Instance.new("TextLabel",card)
    status.BackgroundTransparency=1; status.Size=UDim2.new(1,-80,0,28); status.Position=UDim2.fromOffset(40,206); status.Text="loading, please wait..."; status.TextColor3=Color3.fromRGB(38,79,157); status.Font=Enum.Font.FredokaOne; status.TextSize=19; status.TextXAlignment=Enum.TextXAlignment.Center

    local track=Instance.new("Frame",card)
    track.Size=UDim2.new(1,-150,0,18); track.Position=UDim2.fromOffset(55,252); track.BackgroundColor3=Color3.fromRGB(207,229,252); track.BorderSizePixel=0
    Instance.new("UICorner",track).CornerRadius=UDim.new(1,0)
    local ts=Instance.new("UIStroke",track); ts.Color=Color3.fromRGB(117,171,237); ts.Transparency=.35
    local fill=Instance.new("Frame",track); fill.Size=UDim2.new(.05,0,1,0); fill.BackgroundColor3=Color3.fromRGB(35,91,245); fill.BorderSizePixel=0; Instance.new("UICorner",fill).CornerRadius=UDim.new(1,0)
    local fg=Instance.new("UIGradient",fill); fg.Color=ColorSequence.new(Color3.fromRGB(89,165,255),Color3.fromRGB(26,82,238))
    local pct=Instance.new("TextLabel",card); pct.BackgroundTransparency=1; pct.Size=UDim2.fromOffset(70,28); pct.Position=UDim2.new(1,-105,0,247); pct.Text="5%"; pct.TextColor3=Color3.fromRGB(35,91,245); pct.Font=Enum.Font.FredokaOne; pct.TextSize=18

    local footer=Instance.new("Frame",card); footer.Size=UDim2.new(1,-110,0,66); footer.Position=UDim2.fromOffset(55,305); footer.BackgroundColor3=Color3.fromRGB(246,251,255); footer.BorderSizePixel=0; Instance.new("UICorner",footer).CornerRadius=UDim.new(0,17)
    local fs=Instance.new("UIStroke",footer); fs.Color=Color3.fromRGB(157,197,242); fs.Transparency=.4
    local fi=Instance.new("ImageLabel",footer); fi.BackgroundTransparency=1; fi.Size=UDim2.fromOffset(48,48); fi.Position=UDim2.fromOffset(12,9); fi.Image=A.PompomIcon; fi.ScaleType=Enum.ScaleType.Fit
    local ft=Instance.new("TextLabel",footer); ft.BackgroundTransparency=1; ft.Position=UDim2.fromOffset(68,9); ft.Size=UDim2.new(1,-80,0,23); ft.Text="Kimqetras HC"; ft.TextColor3=Color3.fromRGB(36,75,145); ft.Font=Enum.Font.FredokaOne; ft.TextSize=17; ft.TextXAlignment=Enum.TextXAlignment.Left
    local fb=Instance.new("TextLabel",footer); fb.BackgroundTransparency=1; fb.Position=UDim2.fromOffset(68,32); fb.Size=UDim2.new(1,-80,0,19); fb.Text="clean pages  •  hot blue details  •  made with love ♡"; fb.TextColor3=Color3.fromRGB(91,125,177); fb.Font=Enum.Font.Gotham; fb.TextSize=10; fb.TextXAlignment=Enum.TextXAlignment.Left

    pcall(function() ContentProvider:PreloadAsync({pom,title,sub,fi}) end)

    local steps={{"loading the working feature backend...",.23},{"building your sections...",.43},{"setting up fog and visuals...",.61},{"adding environment + weapon skins...",.78},{"finishing your cute GUI...",.95}}
    task.spawn(function()
        for _,s in ipairs(steps) do
            if not gui.Parent or _G.KimqV38Ready then break end
            status.Text=s[1]
            TweenService:Create(fill,TweenInfo.new(.55,Enum.EasingStyle.Quart,Enum.EasingDirection.Out),{Size=UDim2.new(s[2],0,1,0)}):Play()
    ...
    task.spawn(function()
        while gui.Parent and not _G.KimqV38Ready do
            pcall(function()
                for _,root in ipairs({CoreGui,pg}) do
                    for _,n in ipairs({"KimpetrasHC_Boot","KimqV22CuteLoader","KimqV21CuteLoader","KimqV37Loader","KimqV36Loader","KimqV35Loader"}) do
                        local x=root:FindFirstChild(n); if x and x~=gui then x:Destroy() end
                    end
                    local old=root:FindFirstChild("KimpetrasHC"); local m=old and old:FindFirstChild("Main"); if m then m.Visible=false end
                end
            end)
            task.wait(.06)
        end
    ...
    _G.KimqV38Loader={Gui=gui,Card=card,Background=bg,Status=status,Fill=fill,Percent=pct}
    task.delay(22,function() if gui and gui.Parent then pcall(function() gui:Destroy() end) end end)
end
...
task.spawn(function()
    ...
    if pages.overview then
        ...
        local about=card(overview,138,"V6About")
        label(about,"♥  about Kimqetras HC",UDim2.new(1,-24,0,30),UDim2.fromOffset(12,12),Enum.Font.FredokaOne,23,current.hot)
...
-- V38 actual GUI rebuild. Reuses V6 live page controls/callbacks instead of recreating them.
task.spawn(function()
    local Players=game:GetService("Players")
    local CoreGui=game:GetService("CoreGui")
    local UIS=game:GetService("UserInputService")
    ...
    local lp=Players.LocalPlayer
    local pg=lp:WaitForChild("PlayerGui")
    local A=_G.KimqV38Assets or {}

    local function waitBase()
        local t=tick()
        while tick()-t<12 do
            local old=CoreGui:FindFirstChild("KimpetrasHC") or pg:FindFirstChild("KimpetrasHC")
            local main=old and old:FindFirstChild("Main")
            if main and main:FindFirstChild("KimqV6Applied") then return old,main end
            task.wait(.08)
        end
    end
    local oldGui,oldMain=waitBase()
    if oldMain then task.wait(0.85) end
    if not oldMain then
        if _G.KimqV38Loader and _G.KimqV38Loader.Status then _G.KimqV38Loader.Status.Text="the working V6 backend did not finish loading" end
        return
    end

    pcall(function()
        for _,root in ipairs({CoreGui,pg}) do local x=root:FindFirstChild("KimqetrasHC_V38"); if x then x:Destroy() end end
    end)

    local gui=Instance.new("ScreenGui")
    gui.Name="KimqetrasHC_V38"; gui.ResetOnSpawn=false; gui.IgnoreGuiInset=true; gui.DisplayOrder=500000
    pcall(function() gui.Parent=CoreGui end); if not gui.Parent then gui.Parent=pg end

    local themes={
        Blue={bg=Color3.fromRGB(207,232,255),panel=Color3.fromRGB(237,247,255),card=Color3.fromRGB(248,252,255),card2=Color3.fromRGB(224,240,255),hot=Color3.fromRGB(31,82,240),hot2=Color3.fromRGB(83,157,255),line=Color3.fromRGB(126,178,239),text=Color3.fromRGB(35,70,137),sub=Color3.fromRGB(91,119,168),stitch=Color3.fromRGB(91,151,224),decal=Color3.new(1,1,1)},
        Purple={bg=Color3.fromRGB(232,217,255),panel=Color3.fromRGB(247,240,255),card=Color3.fromRGB(252,248,255),card2=Color3.fromRGB(239,227,255),hot=Color3.fromRGB(128,61,238),hot2=Color3.fromRGB(183,125,255),line=Color3.fromRGB(188,153,237),text=Color3.fromRGB(87,53,140),sub=Color3.fromRGB(129,102,166),stitch=Color3.fromRGB(157,113,221),decal=Color3.fromRGB(201,165,255)},
        Pink={bg=Color3.fromRGB(255,221,239),panel=Color3.fromRGB(255,243,249),card=Color3.fromRGB(255,249,252),card2=Color3.fromRGB(255,229,242),hot=Color3.fromRGB(238,61,143),hot2=Color3.fromRGB(255,132,189),line=Color3.fromRGB(240,158,197),text=Color3.fromRGB(151,58,102),sub=Color3.fromRGB(177,104,138),stitch=Color3.fromRGB(225,112,166),decal=Color3.fromRGB(255,177,216)},
        Red={bg=Color3.fromRGB(255,222,222),panel=Color3.fromRGB(255,243,243),card=Color3.fromRGB(255,250,250),card2=Color3.fromRGB(255,230,230),hot=Color3.fromRGB(220,52,62),hot2=Color3.fromRGB(255,113,119),line=Color3.fromRGB(234,157,162),text=Color3.fromRGB(137,55,60),sub=Color3.fromRGB(167,104,109),stitch=Color3.fromRGB(211,99,106),decal=Color3.fromRGB(255,170,175)},
        Aqua={bg=Color3.fromRGB(207,246,249),panel=Color3.fromRGB(237,252,253),card=Color3.fromRGB(248,254,255),card2=Color3.fromRGB(220,247,250),hot=Color3.fromRGB(21,164,190),hot2=Color3.fromRGB(84,215,228),line=Color3.fromRGB(124,207,217),text=Color3.fromRGB(37,103,118),sub=Color3.fromRGB(87,139,149),stitch=Color3.fromRGB(75,181,199),decal=Color3.fromRGB(154,245,250)},
        Green={bg=Color3.fromRGB(220,246,226),panel=Color3.fromRGB(242,253,245),card=Color3.fromRGB(249,255,250),card2=Color3.fromRGB(229,248,233),hot=Color3.fromRGB(46,166,84),hot2=Color3.fromRGB(105,214,137),line=Color3.fromRGB(145,209,163),text=Color3.fromRGB(53,112,70),sub=Color3.fromRGB(99,145,111),stitch=Color3.fromRGB(91,181,115),decal=Color3.fromRGB(176,244,194)},
    }
    local themeName="Blue"; local T=themes.Blue
    local reg={bg={},panel={},card={},card2={},hot={},hot2={},line={},text={},sub={},stitch={},decals={}}
    local function add(role,obj,prop) table.insert(reg[role],{o=obj,p=prop or (obj:IsA("UIStroke") and "Color" or "BackgroundColor3")}); return obj end
    local function corner(o,r) local c=o:FindFirstChildOfClass("UICorner") or Instance.new("UICorner",o); c.CornerRadius=UDim.new(0,r); return c end
    local function stroke(o,role,trans,w) local s=o:FindFirstChildOfClass("UIStroke") or Instance.new("UIStroke",o); s.Transparency=trans or .25; s.Thickness=w or 1; add(role or "line",s,"Color"); return s end
    local function txt(p,text,size,pos,font,ts,role,align)
        local l=Instance.new("TextLabel",p); l.BackgroundTransparency=1; l.Size=size; l.Position=pos; l.Text=text; l.Font=font; l.TextSize=ts; l.TextXAlignment=align or Enum.TextXAlignment.Left; l.TextYAlignment=Enum.TextYAlignment.Center; add(role or "text",l,"TextColor3"); return l
    end
    local function img(p,id,size,pos)
        local i=Instance.new("ImageLabel",p); i.BackgroundTransparency=1; i.Size=size; i.Position=pos; i.Image=id or ""; i.ScaleType=Enum.ScaleType.Fit; table.insert(reg.decals,{o=i,p="ImageColor3"}); return i
    end

    local main=Instance.new("Frame",gui)
    main.Name="Main"; main.AnchorPoint=Vector2.new(.5,.5); main.Position=UDim2.fromScale(.5,.5); main.Size=UDim2.fromOffset(1180,760); main.BorderSizePixel=0; add("bg",main); corner(main,28); stroke(main,"hot",.06,2)
    local scale=Instance.new("UIScale",main)
    local function fit() local v=workspace.CurrentCamera and workspace.CurrentCamera.ViewportSize or Vector2.new(1280,720); scale.Scale=math.min(1,(v.X-28)/1180,(v.Y-28)/760) end
    fit(); if workspace.CurrentCamera then workspace.CurrentCamera:GetPropertyChangedSignal("ViewportSize"):Connect(fit) end

    local gloss=Instance.new("Frame",main); gloss.Size=UDim2.new(1,-24,0,62); gloss.Position=UDim2.fromOffset(12,10); gloss.BorderSizePixel=0; add("card2",gloss); corner(gloss,21); stroke(gloss,"line",.35,1)
    local gg=Instance.new("UIGradient",gloss); gg.Color=ColorSequence.new(Color3.fromRGB(181,218,255),Color3.fromRGB(80,157,255)); gg.Rotation=90
    local handle=Instance.new("Frame",gloss); handle.AnchorPoint=Vector2.new(.5,.5); handle.Position=UDim2.fromScale(.5,.5); handle.Size=UDim2.fromOffset(124,10); handle.BorderSizePixel=0; add("hot2",handle); corner(handle,99); stroke(handle,"line",.25,1)
    local minus=img(gloss,A.Minus,UDim2.fromOffset(35,35),UDim2.new(1,-86,.5,-18)); local xx=img(gloss,A.X,UDim2.fromOffset(35,35),UDim2.new(1,-45,.5,-18))
    minus.Active=false; xx.Active=false

    local side=Instance.new("Frame",main); side.Position=UDim2.fromOffset(18,84); side.Size=UDim2.new(0,242,1,-102); side.BorderSizePixel=0; add("panel",side); corner(side,20); stroke(side,"line",.18,1)
    local sideTitle=txt(side,"features",UDim2.new(1,-24,0,32),UDim2.fromOffset(12,10),Enum.Font.FredokaOne,19,"text",Enum.TextXAlignment.Center)
    local sideDash=txt(side,"-  -  -  -  -  -  -",UDim2.new(1,-30,0,14),UDim2.fromOffset(15,38),Enum.Font.GothamBold,9,"stitch",Enum.TextXAlignment.Center)
    local nav=Instance.new("ScrollingFrame",side); nav.Position=UDim2.fromOffset(8,58); nav.Size=UDim2.new(1,-16,1,-106); nav.BackgroundTransparency=1; nav.BorderSizePixel=0; nav.ScrollBarThickness=3; nav.ScrollBarImageColor3=T.hot
    local nl=Instance.new("UIListLayout",nav); nl.Padding=UDim.new(0,6); nl.SortOrder=Enum.SortOrder.LayoutOrder; nl:GetPropertyChangedSignal("AbsoluteContentSize"):Connect(function() nav.CanvasSize=UDim2.new(0,0,0,nl.AbsoluteContentSize.Y+8) end)
    local hint=Instance.new("Frame",side); hint.Position=UDim2.new(0,8,1,-42); hint.Size=UDim2.new(1,-16,0,34); hint.BorderSizePixel=0; add("card",hint); corner(hint,12); stroke(hint,"line",.3,1)
    txt(hint,"Right Shift = hide / show",UDim2.fromScale(1,1),UDim2.fromOffset(0,0),Enum.Font.GothamSemibold,10,"hot",Enum.TextXAlignment.Center)

    local content=Instance.new("Frame",main); content.Position=UDim2.fromOffset(274,84); content.Size=UDim2.new(1,-292,1,-102); content.BorderSizePixel=0; add("panel",content); corner(content,20); stroke(content,"line",.18,1); content.ClipsDescendants=true
    local stitch=Instance.new("Frame",content); stitch.Name="CuteStitch"; stitch.Position=UDim2.fromOffset(10,10); stitch.Size=UDim2.new(1,-20,1,-20); stitch.BackgroundTransparency=1; stitch.ClipsDescendants=true; stitch.ZIndex=1
    local function sdash(size,pos)
        local d=Instance.new("Frame",stitch); d.Size=size; d.Position=pos; d.BorderSizePixel=0; add("stitch",d); corner(d,99); d.ZIndex=1
    end
    for i=0,30 do local x=i/30; sdash(UDim2.fromOffset(11,2),UDim2.new(x,-5,0,0)); sdash(UDim2.fromOffset(11,2),UDim2.new(x,-5,1,-2)) end
    for i=1,20 do local y=i/21; sdash(UDim2.fromOffset(2,11),UDim2.new(0,0,y,-5)); sdash(UDim2.fromOffset(2,11),UDim2.new(1,-2,y,-5)) end

    local head=Instance.new("Frame",content); head.Position=UDim2.fromOffset(18,18); head.Size=UDim2.new(1,-36,0,118); head.BackgroundTransparency=1; head.ZIndex=3
    local pom=img(head,A.Pompom,UDim2.fromOffset(126,112),UDim2.fromOffset(4,0)); pom.ZIndex=4
    local logo=img(head,A.Title,UDim2.fromOffset(352,70),UDim2.fromOffset(126,2)); logo.ZIndex=4
    local subtitle=img(head,A.Subtitle,UDim2.fromOffset(232,43),UDim2.fromOffset(180,68)); subtitle.ZIndex=4
    local profile=Instance.new("Frame",head); profile.Size=UDim2.fromOffset(238,60); profile.Position=UDim2.new(1,-250,0,8); profile.BorderSizePixel=0; add("card",profile); corner(profile,17); stroke(profile,"line",.25,1); profile.ZIndex=4
    local av=Instance.new("ImageLabel",profile); av.Size=UDim2.fromOffset(42,42); av.Position=UDim2.fromOffset(9,9); av.BackgroundTransparency=1; corner(av,99); av.ZIndex=5; pcall(function() av.Image=Players:GetUserThumbnailAsync(lp.UserId,Enum.ThumbnailType.HeadShot,Enum.ThumbnailSize.Size100x100) end)
    local dn=txt(profile,lp.DisplayName,UDim2.new(1,-108,0,20),UDim2.fromOffset(58,10),Enum.Font.GothamBold,13,"text"); dn.ZIndex=5
    local du=txt(profile,"@"..lp.Name,UDim2.new(1,-108,0,18),UDim2.fromOffset(58,30),Enum.Font.Gotham,10,"sub"); du.ZIndex=5
    local pp=img(profile,A.Paw,UDim2.fromOffset(43,43),UDim2.new(1,-51,.5,-21)); pp.ZIndex=5
    local badge=Instance.new("TextLabel",head); badge.Size=UDim2.fromOffset(66,28); badge.Position=UDim2.new(1,-78,0,78); badge.BorderSizePixel=0; badge.Text="V38 ♥"; badge.TextColor3=Color3.new(1,1,1); badge.Font=Enum.Font.FredokaOne; badge.TextSize=12; badge.ZIndex=4; add("hot",badge); corner(badge,99)

    local pageHead=Instance.new("Frame",content); pageHead.Position=UDim2.fromOffset(18,140); pageHead.Size=UDim2.new(1,-36,0,66); pageHead.BorderSizePixel=0; add("card",pageHead); corner(pageHead,16); stroke(pageHead,"line",.25,1); pageHead.ZIndex=3
    local phPaw=img(pageHead,A.Paw,UDim2.fromOffset(27,27),UDim2.fromOffset(13,9)); phPaw.ZIndex=4
    local pageTitle=txt(pageHead,"overview",UDim2.new(1,-62,0,28),UDim2.fromOffset(48,8),Enum.Font.FredokaOne,21,"text"); pageTitle.ZIndex=4
    local pageDesc=txt(pageHead,"your account, quick notes, and a little welcome page",UDim2.new(1,-32,0,20),UDim2.fromOffset(16,38),Enum.Font.Gotham,10,"sub"); pageDesc.ZIndex=4
    local dashes=txt(pageHead,"-  -  -  -  -  -",UDim2.fromOffset(160,16),UDim2.new(1,-175,0,37),Enum.Font.GothamBold,8,"stitch",Enum.TextXAlignment.Right); dashes.ZIndex=4

    local pageHost=Instance.new("Frame",content); pageHost.Position=UDim2.fromOffset(18,216); pageHost.Size=UDim2.new(1,-36,1,-234); pageHost.BackgroundTransparency=1; pageHost.ZIndex=3; pageHost.ClipsDescendants=true

    local defs={
        {"overview","overview","your account, quick notes, and a little welcome page"},{"silent","silent aim","cursor targeting, FOV, hit part, and checks"},{"macro","macro","speed controls and macro key"},{"whitelist","whitelist","players targeting and ESP should ignore"},{"protection","protection","anti-aim-view and protection controls"},{"antifall","anti fall","fall-state protection"},{"delay","delay changer","weapon cooldown values"},{"esp","esp","boxes, names, health, distance, tracers, and skeletons"},{"avatar","avatar","avatar and character visual tools"},{"fog","fog / atmosphere","fog color, amount, atmosphere, and saturation"},{"environment","environment","cute seasonal map styles that keep fog editable"},{"weaponskins","weapon skins","choose your weapon, skin, and apply it locally"},{"hcsilent","HC silent aim","Hood Customs targeting and prediction"},{"forcehit","force hit","force-hit controls"},{"hitbox","hitbox expander","hitbox size and visibility"},{"flamelock","flamelock","lock, smoothing, prediction, and offsets"},{"camlock","camlock","camera lock behavior and prediction"},{"headless","headless","local headless visual"},{"antimod","anti mod","anti-mod settings"},{"settings","settings","config and performance settings"},{"theme","theme","recolor the whole interface and decals"},{"info","information","credits and build information"},
    }
    local pages={}; local meta={}
    for _,d in ipairs(defs) do meta[d[1]]={title=d[2],desc=d[3]} end
    local nameMap={overview="overviewPage",silent="silentPage",macro="macroPage",whitelist="whitelistPage",protection="protectionPage",antifall="antifallPage",delay="delayPage",esp="espPage",avatar="avatarPage",fog="fogPage",hcsilent="hcsilentPage",forcehit="forcehitPage",hitbox="hitboxPage",flamelock="flamelockPage",camlock="camlockPage",headless="headlessPage",antimod="antimodPage",settings="settingsPage",theme="themePage",info="infoPage"}
    for key,n in pairs(nameMap) do
        local p=oldMain:FindFirstChild(n,true)
        if p and p:IsA("ScrollingFrame") then p.Parent=pageHost; p.Size=UDim2.fromScale(1,1); p.Position=UDim2.fromOffset(0,0); p.BackgroundTransparency=1; p.Visible=false; p.ScrollBarThickness=3; pages[key]=p end
    end
    for _,key in ipairs({"environment","weaponskins"}) do
        local p=Instance.new("ScrollingFrame",pageHost); p.Name=key.."PageV38"; p.Size=UDim2.fromScale(1,1); p.BackgroundTransparency=1; p.BorderSizePixel=0; p.Visible=false; p.ScrollBarThickness=3; pages[key]=p
        local pd=Instance.new("UIPadding",p); pd.PaddingRight=UDim.new(0,4)
        local ll=Instance.new("UIListLayout",p); ll.Padding=UDim.new(0,9); ll.SortOrder=Enum.SortOrder.LayoutOrder; ll:GetPropertyChangedSignal("AbsoluteContentSize"):Connect(function() p.CanvasSize=UDim2.new(0,0,0,ll.AbsoluteContentSize.Y+12) end)
    end
    if not pages.overview then
        local p=Instance.new("ScrollingFrame",pageHost); p.Name="overviewPageV38"; p.Size=UDim2.fromScale(1,1); p.BackgroundTransparency=1; p.BorderSizePixel=0; p.Visible=false; pages.overview=p
        local ll=Instance.new("UIListLayout",p); ll.Padding=UDim.new(0,10); ll:GetPropertyChangedSignal("AbsoluteContentSize"):Connect(function() p.CanvasSize=UDim2.new(0,0,0,ll.AbsoluteContentSize.Y+12) end)
    end

    local function clearOverview()
        ...
        if not p:FindFirstChildOfClass("UIListLayout") then local l=Instance.new("UIListLayout",p); l.Padding=UDim.new(0,10); l.SortOrder=Enum.SortOrder.LayoutOrder; l:GetPropertyChangedSignal("AbsoluteContentSize"):Connect(function() p.CanvasSize=UDim2.new(0,0,0,l.AbsoluteContentSize.Y+12) end) end
        local function ccard(h) local f=Instance.new("Frame",p); f.Size=UDim2.new(1,-5,0,h); f.BorderSizePixel=0; add("card",f); corner(f,17); stroke(f,"line",.24,1); return f end
        local banner=ccard(104); banner.Name="V38Banner"; banner.BackgroundColor3=T.hot2
        local bgrad=Instance.new("UIGradient",banner); bgrad.Color=ColorSequence.new(T.hot2,T.hot); bgrad.Rotation=0
        for i,x in ipairs({.09,.31,.68,.88}) do local h=txt(banner,i%2==0 and "♡" or "♥",UDim2.fromOffset(34,34),UDim2.new(x,-17,.5,-17),Enum.Font.FredokaOne,23,"text",Enum.TextXAlignment.Center); h.TextColor3=Color3.new(1,1,1); h:SetAttribute("V38White",true) end
        local bt=txt(banner,"Kimqetras HC",UDim2.new(1,0,1,0),UDim2.fromOffset(0,0),Enum.Font.FredokaOne,31,"text",Enum.TextXAlignment.Center); bt.TextColor3=Color3.new(1,1,1); bt:SetAttribute("V38White",true)
        local welcome=ccard(132)
        local wav=Instance.new("ImageLabel",welcome); wav.Size=UDim2.fromOffset(76,76); wav.Position=UDim2.fromOffset(18,27); wav.BackgroundTransparency=1; corner(wav,99); pcall(function() wav.Image=Players:GetUserThumbnailAsync(lp.UserId,Enum.ThumbnailType.HeadShot,Enum.ThumbnailSize.Size180x180) end)
        txt(welcome,"welcome, "..lp.DisplayName:lower(),UDim2.new(1,-116,0,34),UDim2.fromOffset(105,18),Enum.Font.FredokaOne,24,"text")
        txt(welcome,"@"..lp.Name.."  •  Kimqetras HC",UDim2.new(1,-116,0,20),UDim2.fromOffset(105,49),Enum.Font.GothamBold,11,"sub")
        local body=txt(welcome,"Everything is separated into its own feature page. Pick a tool on the left, or press Right Shift to hide / reopen the GUI.",UDim2.new(1,-126,0,48),UDim2.fromOffset(105,72),Enum.Font.GothamBold,12,"text"); body.TextWrapped=true; body.TextYAlignment=Enum.TextYAlignment.Top
        local about=ccard(115); txt(about,"about",UDim2.new(1,-40,0,28),UDim2.fromOffset(14,9),Enum.Font.FredokaOne,20,"hot"); txt(about,"-  -  -  -  -",UDim2.fromOffset(150,15),UDim2.fromOffset(14,36),Enum.Font.GothamBold,9,"stitch")
        local ab=txt(about,"Every feature has its own clean page.\nSwitch between aiming, movement, visuals, avatar tools, and utilities.\nPick a theme whenever you want the interface to match your style.",UDim2.new(1,-28,0,62),UDim2.fromOffset(14,48),Enum.Font.Gotham,11,"text"); ab.TextWrapped=true; ab.TextYAlignment=Enum.TextYAlignment.Top
        local controls=ccard(82); txt(controls,"controls",UDim2.new(1,-40,0,28),UDim2.fromOffset(14,8),Enum.Font.FredokaOne,20,"hot"); txt(controls,"Right Shift = hide / show the GUI",UDim2.new(1,-28,0,30),UDim2.fromOffset(14,39),Enum.Font.GothamSemibold,11,"text")
        local cp=img(controls,A.Paw,UDim2.fromOffset(32,32),UDim2.new(1,-45,.5,-16)); cp.ZIndex=5
    end
    clearOverview()

    local toggleOverlays={}
    local function skipVisual(obj)
        local a=obj
        while a and a~=pageHost do local n=(a.Name or ""):lower(); if n:find("fogsquare",1,true) or n:find("hue",1,true) or n:find("preview",1,true) or n:find("colorpicker",1,true) then return true end; a=a.Parent end
        return false
    end
    local function stylePage(p)
        ...
        for _,o in ipairs(p:GetDescendants()) do
            if o:IsA("TextLabel") or o:IsA("TextBox") then
                if not o:GetAttribute("V38White") then o.TextColor3=T.text end; o.TextTransparency=0
                if o.TextSize<11 then o.TextSize=11 end
            elseif o:IsA("TextButton") then
                if not skipVisual(o) then
                    local blank=(tostring(o.Text or "")=="")
                    if not blank then o.BackgroundColor3=T.card2; o.TextColor3=T.text; o.BackgroundTransparency=0; corner(o,10); stroke(o,"line",.34,1) end
                end
            elseif o:IsA("UIStroke") and not skipVisual(o) then o.Color=T.line
            elseif o:IsA("Frame") and o~=p and not skipVisual(o) and o.BackgroundTransparency<1 then
                local s=o.AbsoluteSize
                if s.Y>=28 then o.BackgroundColor3=T.card; corner(o,12) elseif s.Y<=12 and s.X>25 then o.BackgroundColor3=T.hot2 end
            end
        end
        for _,o in ipairs(p:GetDescendants()) do
            if o:IsA("TextButton") and o.Text=="" and o.AbsoluteSize.X>=34 and o.AbsoluteSize.X<=70 and o.AbsoluteSize.Y>=16 and o.AbsoluteSize.Y<=32 and not skipVisual(o) then
                local ov=o:FindFirstChild("V38ToggleImage")
                if not ov then ov=Instance.new("ImageLabel",o); ov.Name="V38ToggleImage"; ov.BackgroundTransparency=1; ov.Size=UDim2.fromScale(1,1); ov.Position=UDim2.fromOffset(0,0); ov.ScaleType=Enum.ScaleType.Fit; ov.ZIndex=o.ZIndex+3; table.insert(toggleOverlays,{btn=o,img=ov}); table.insert(reg.decals,{o=ov,p="ImageColor3"})
                    for _,ch in ipairs(o:GetChildren()) do if ch:IsA("Frame") then ch.Visible=false end end
                    o.BackgroundTransparency=1
                    o.MouseButton1Click:Connect(function() task.defer(function()
                        local on=false
                        for _,ch in ipairs(o:GetChildren()) do if ch:IsA("Frame") then on=ch.Position.X.Scale>.4 or ch.Position.X.Offset>5 end end
                        ov.Image=on and A.ToggleOn or A.ToggleOff
                    end) end)
                end
                local on=false; for _,ch in ipairs(o:GetChildren()) do if ch:IsA("Frame") then on=ch.Position.X.Scale>.4 or ch.Position.X.Offset>5 end end; ov.Image=on and A.ToggleOn or A.ToggleOff
            end
        end
    end

    local navButtons={}; local current="overview"; local onShow={}
    local function showPage(key)
        if not pages[key] then return end
        current=key
        for k,p in pairs(pages) do p.Visible=(k==key) end
        pageTitle.Text=meta[key] and meta[key].title or key; pageDesc.Text=meta[key] and meta[key].desc or ""
        for k,b in pairs(navButtons) do b.BackgroundColor3=(k==key) and T.hot or T.card; b.TextColor3=(k==key) and Color3.new(1,1,1) or T.text; local st=b:FindFirstChildOfClass("UIStroke"); if st then st.Color=(k==key) and T.hot or T.line end end
        stylePage(pages[key]); if onShow[key] then pcall(onShow[key]) end
    end
    for i,d in ipairs(defs) do
        local b=Instance.new("TextButton",nav); b.LayoutOrder=i; b.Size=UDim2.new(1,-4,0,36); b.BorderSizePixel=0; b.BackgroundColor3=T.card; b.Text=d[2]; b.TextColor3=T.text; b.Font=Enum.Font.GothamBold; b.TextSize=11; b.AutoButtonColor=false; corner(b,11); stroke(b,"line",.33,1); navButtons[d[1]]=b; b.MouseButton1Click:Connect(function() showPage(d[1]) end)
    end

    local function applyTheme(name)
        local p=themes[name]; if not p then return end; themeName=name; T=p
        for role,items in pairs(reg) do
            local val=role=="decals" and p.decal or p[role]
            if val then for _,e in ipairs(items) do if e.o and e.o.Parent then pcall(function() e.o[e.p]=val end) end end end
        end
        nav.ScrollBarImageColor3=p.hot
        for _,pgx in pairs(pages) do stylePage(pgx) end
        for k,b in pairs(navButtons) do b.BackgroundColor3=(k==current) and p.hot or p.card; b.TextColor3=(k==current) and Color3.new(1,1,1) or p.text end
        local banner=pages.overview and pages.overview:FindFirstChild("V38Banner")
        local grad=banner and banner:FindFirstChildOfClass("UIGradient"); if grad then grad.Color=ColorSequence.new(p.hot2,p.hot) end
        for _,o in ipairs(gui:GetDescendants()) do if (o:IsA("TextLabel") or o:IsA("TextButton")) and o:GetAttribute("V38White") then o.TextColor3=Color3.new(1,1,1) end end
    end

    _G.KimqV38Ctx={Gui=gui,Main=main,OldMain=oldMain,Pages=pages,PageHost=pageHost,Themes=themes,ApplyTheme=applyTheme,ShowPage=showPage,OnShow=onShow,Assets=A,Register=reg,StylePage=stylePage,Add=add,Corner=corner,Stroke=stroke,Text=txt,Image=img,GetTheme=function() return T end}

    for _,o in ipairs(pages.theme and pages.theme:GetDescendants() or {}) do
        if o:IsA("TextButton") then
            local nm=o:GetAttribute("ThemeName") or tostring(o.Text or ""):match("Blue") or tostring(o.Text or ""):match("Purple") or tostring(o.Text or ""):match("Pink") or tostring(o.Text or ""):match("Red") or tostring(o.Text or ""):match("Aqua") or tostring(o.Text or ""):match("Green")
            if nm and themes[nm] then o.MouseButton1Click:Connect(function() task.defer(function() applyTheme(nm) end) end) end
        end
    end

    local shown=true
    UIS.InputBegan:Connect(function(input,processed)
        if input.KeyCode==Enum.KeyCode.RightShift then shown=not shown; main.Visible=shown; task.defer(function() oldMain.Visible=false end) end
    end)
    RunService.RenderStepped:Connect(function() if oldMain.Parent and oldMain.Visible then oldMain.Visible=false end end)

    -- Only the blue top strip is draggable; no right-click hide and no clickable close icons.
    local dragging=false; local ds,sp,di
    gloss.Active=true
    gloss.InputBegan:Connect(function(input) if input.UserInputType==Enum.UserInputType.MouseButton1 then dragging=true; ds=input.Position; sp=main.Position; input.Changed:Connect(function() if input.UserInputState==Enum.UserInputState.End then dragging=false end end) end end)
    gloss.InputChanged:Connect(function(input) if input.UserInputType==Enum.UserInputType.MouseMovement then di=input end end)
    UIS.InputChanged:Connect(function(input) if dragging and input==di then local d=input.Position-ds; main.Position=UDim2.new(sp.X.Scale,sp.X.Offset+d.X,sp.Y.Scale,sp.Y.Offset+d.Y) end end)

    oldMain.Visible=false
    applyTheme("Blue")
    showPage("overview")
    pcall(function() ContentProvider:PreloadAsync({pom,logo,subtitle,pp,phPaw,minus,xx}) end)
end)

-- V38 Environment page. Separate scope so it cannot push the GUI over Luau's local-register limit.
task.spawn(function()
    local Lighting=game:GetService("Lighting")
    local RunService=game:GetService("RunService")
    local Players=game:GetService("Players")
    local lp=Players.LocalPlayer
    local ctx
    local t=tick(); repeat ctx=_G.KimqV38Ctx; task.wait(.05) until ctx or tick()-t>10
    if not ctx or not ctx.Pages.environment then return end
    local page=ctx.Pages.environment
    for _,c in ipairs(page:GetChildren()) do if not c:IsA("UIListLayout") and not c:IsA("UIPadding") then c:Destroy() end end
    local terrain=workspace:FindFirstChildOfClass("Terrain")
    local T=ctx.GetTheme()
    local function card(h)
        local f=Instance.new("Frame",page); f.Size=UDim2.new(1,-5,0,h); f.BorderSizePixel=0; ctx.Add("card",f); ctx.Corner(f,14); ctx.Stroke(f,"line",.27,1); return f
    end
    local intro=card(74)
    ctx.Text(intro,"environment",UDim2.new(1,-24,0,28),UDim2.fromOffset(12,8),Enum.Font.FredokaOne,21,"hot")
    ctx.Text(intro,"Cute seasonal map styles. Fog / Atmosphere stays editable while a season is active.",UDim2.new(1,-24,0,28),UDim2.fromOffset(12,39),Enum.Font.Gotham,11,"sub")
    ...
    local changed={}; local folder=workspace:FindFirstChild("KimqV38Environment") or Instance.new("Folder",workspace); folder.Name="KimqV38Environment"; local follow; local active="Normal"
    local function isGrass(p)
        if not p:IsA("BasePart") or p:IsDescendantOf(folder) then return false end
        ...
        local c=p.Color; return p.Size.Y<=5 and (p.Size.X>=6 or p.Size.Z>=6) and c.G>c.R*1.12 and c.G>c.B*1.08 and c.G>.22
    end
    local function save(p)
        if changed[p] then return end
        local r={Color=p.Color,Material=p.Material,Children={}}
        for _,d in ipairs(p:GetDescendants()) do if d:IsA("Texture") or d:IsA("Decal") then table.insert(r.Children,{Obj=d,T=d.Transparency}) end end
        changed[p]=r
    end
    local function recolor(color,material,hideTextures)
        local count=0
        for _,p in ipairs(workspace:GetDescendants()) do
            if isGrass(p) then save(p); p.Color=color; if material then p.Material=material end; if hideTextures then for _,r in ipairs(changed[p].Children) do if r.Obj and r.Obj.Parent then r.Obj.Transparency=1 end end end; count+=1; if count>4500 then break end end
        end
    end
    local function clearFX() if follow then pcall(function() follow:Disconnect() end); follow=nil end; folder:ClearAllChildren() end
    local function restore()
        clearFX()
        for p,r in pairs(changed) do if p and p.Parent then pcall(function() p.Color=r.Color; p.Material=r.Material end); for _,x in ipairs(r.Children or {}) do if x.Obj and x.Obj.Parent then pcall(function() x.Obj.Transparency=x.T end) end end end end
        table.clear(changed)
    ...
    local function snow()
        local holder=Instance.new("Part",folder); holder.Name="SnowCloud"; holder.Size=Vector3.new(160,1,160); holder.Transparency=1; holder.Anchored=true; holder.CanCollide=false; holder.CanTouch=false; holder.CanQuery=false
        local function layer(rate,a,b,s1,s2,spread,alpha)
            local e=Instance.new("ParticleEmitter",holder); e.Texture="rbxasset://textures/particles/sparkles_main.dds"; e.Rate=rate; e.Lifetime=NumberRange.new(7,10); e.Speed=NumberRange.new(s1,s2); e.Acceleration=Vector3.new(.25,-1.2,.1); e.Drag=.4; e.LightInfluence=0; e.EmissionDirection=Enum.NormalId.Bottom; e.SpreadAngle=Vector2.new(spread,spread); e.Rotation=NumberRange.new(0,360); e.RotSpeed=NumberRange.new(-10,10); e.Color=ColorSequence.new(Color3.new(1,1,1),Color3.fromRGB(218,238,255)); e.Size=NumberSequence.new({NumberSequenceKeypoint.new(0,a),NumberSequenceKeypoint.new(.55,b),NumberSequenceKeypoint.new(1,a*.5)}); e.Transparency=NumberSequence.new({NumberSequenceKeypoint.new(0,alpha),NumberSequenceKeypoint.new(.88,alpha+.1),NumberSequenceKeypoint.new(1,1)})
        end
        layer(170,.07,.12,1.5,2.8,24,.05); layer(68,.13,.21,1.0,2.1,32,.15)
        local function move() local hrp=lp.Character and lp.Character:FindFirstChild("HumanoidRootPart"); if hrp and holder.Parent then holder.CFrame=CFrame.new(hrp.Position+Vector3.new(0,46,0)) end end
        move(); follow=RunService.RenderStepped:Connect(move)
    end
    local rows={}; local status
    local function refresh()
        local th=ctx.GetTheme()
        for n,r in pairs(rows) do local on=n==active; r.B.BackgroundColor3=on and th.hot or th.card2; r.B.TextColor3=on and Color3.new(1,1,1) or th.text end
        if status then status.Text=active end
    end
    local function apply(name)
        restore(); active=name
        if name=="Christmas" then
            if terrain then pcall(function() terrain:SetMaterialColor(Enum.Material.Grass,Color3.fromRGB(236,245,252)) end); pcall(function() terrain:SetMaterialColor(Enum.Material.Ground,Color3.fromRGB(228,239,248)) end) end
            recolor(Color3.fromRGB(241,248,253),Enum.Material.Snow,true); Lighting.Ambient=original.Ambient:Lerp(Color3.fromRGB(218,233,248),.17); Lighting.OutdoorAmbient=original.OutdoorAmbient:Lerp(Color3.fromRGB(235,244,253),.20); Lighting.Brightness=math.max(original.Brightness,1.85); snow()
        elseif name=="Halloween" then
            if terrain then pcall(function() terrain:SetMaterialColor(Enum.Material.Grass,Color3.fromRGB(188,132,80)) end) end
            recolor(Color3.fromRGB(194,132,75),nil,false); Lighting.Ambient=original.Ambient:Lerp(Color3.fromRGB(205,150,135),.10); Lighting.OutdoorAmbient=original.OutdoorAmbient:Lerp(Color3.fromRGB(226,171,132),.11); Lighting.Brightness=math.max(original.Brightness*.99,1.8); Lighting.ClockTime=16.3
        end
        refresh()
    end
    local desc={Normal="restore the exact original map look",Christmas="soft snowy ground + layered local falling snow",Halloween="warm cute autumn colors, not super dark"}
    for _,name in ipairs({"Normal","Christmas","Halloween"}) do
        local r=card(62); ctx.Text(r,name,UDim2.fromOffset(145,22),UDim2.fromOffset(12,7),Enum.Font.GothamBold,14,"text"); ctx.Text(r,desc[name],UDim2.new(1,-285,0,22),UDim2.fromOffset(12,31),Enum.Font.Gotham,11,"sub")
        local b=Instance.new("TextButton",r); b.Size=UDim2.fromOffset(118,34); b.Position=UDim2.new(1,-130,.5,-17); b.BorderSizePixel=0; b.Text="choose"; b.Font=Enum.Font.GothamBold; b.TextSize=11; b.AutoButtonColor=false; ctx.Add("card2",b); ctx.Add("text",b,"TextColor3"); ctx.Corner(b,10); ctx.Stroke(b,"line",.3,1); b.MouseButton1Click:Connect(function() apply(name) end); rows[name]={B=b}
    end
    local s=card(50); ctx.Text(s,"Environment",UDim2.fromOffset(120,50),UDim2.fromOffset(12,0),Enum.Font.GothamBold,13,"text"); status=ctx.Text(s,"Normal",UDim2.new(1,-145,1,0),UDim2.fromOffset(135,0),Enum.Font.Gotham,12,"sub")
    refresh(); _G.KimqEnvironmentController={Apply=apply,Restore=restore,GetPreset=function() return active end}; ctx.StylePage(page)
end)

-- V38 Weapon Skins page. Whole-model local visual alignment with attachment-aware placement.
task.spawn(function()
    local Players=game:GetService("Players")
    local ReplicatedStorage=game:GetService("ReplicatedStorage")
    local lp=Players.LocalPlayer
    local ctx
    local t=tick(); repeat ctx=_G.KimqV38Ctx; task.wait(.05) until ctx or tick()-t>10
    if not ctx or not ctx.Pages.weaponskins then return end
    local page=ctx.Pages.weaponskins
    for _,c in ipairs(page:GetChildren()) do if not c:IsA("UIListLayout") and not c:IsA("UIPadding") then c:Destroy() end end
    local function card(h) local f=Instance.new("Frame",page); f.Size=UDim2.new(1,-5,0,h); f.BorderSizePixel=0; ctx.Add("card",f); ctx.Corner(f,14); ctx.Stroke(f,"line",.27,1); return f end
    local intro=card(70); ctx.Text(intro,"weapon skins",UDim2.new(1,-24,0,28),UDim2.fromOffset(12,7),Enum.Font.FredokaOne,21,"hot"); ctx.Text(intro,"Choose one of your weapons, pick its matching skin, then apply it locally.",UDim2.new(1,-24,0,24),UDim2.fromOffset(12,38),Enum.Font.Gotham,11,"sub")
    local weaponCard=card(148); ctx.Text(weaponCard,"Your weapons",UDim2.new(0,160,0,22),UDim2.fromOffset(12,8),Enum.Font.GothamBold,14,"text")
    local refresh=Instance.new("TextButton",weaponCard); refresh.Size=UDim2.fromOffset(88,28); refresh.Position=UDim2.new(1,-100,0,6); refresh.BorderSizePixel=0; refresh.Text="refresh"; refresh.Font=Enum.Font.GothamBold; refresh.TextSize=11; ctx.Add("card2",refresh); ctx.Add("text",refresh,"TextColor3"); ctx.Corner(refresh,9); ctx.Stroke(refresh,"line",.32,1)
    local weaponList=Instance.new("ScrollingFrame",weaponCard); weaponList.Size=UDim2.new(1,-20,0,99); weaponList.Position=UDim2.fromOffset(10,40); weaponList.BackgroundTransparency=1; weaponList.BorderSizePixel=0; weaponList.ScrollBarThickness=3
    local wg=Instance.new("UIGridLayout",weaponList); wg.CellSize=UDim2.new(.32,-6,0,32); wg.CellPadding=UDim2.fromOffset(8,7); wg:GetPropertyChangedSignal("AbsoluteContentSize"):Connect(function() weaponList.CanvasSize=UDim2.new(0,0,0,wg.AbsoluteContentSize.Y+5) end)
    local skinCard=card(286); local skinHead=ctx.Text(skinCard,"Skins",UDim2.new(1,-24,0,22),UDim2.fromOffset(12,8),Enum.Font.GothamBold,14,"text")
    local skinList=Instance.new("ScrollingFrame",skinCard); skinList.Size=UDim2.new(1,-20,1,-42); skinList.Position=UDim2.fromOffset(10,36); skinList.BackgroundTransparency=1; skinList.BorderSizePixel=0; skinList.ScrollBarThickness=3
    local sg=Instance.new("UIGridLayout",skinList); sg.CellSize=UDim2.new(.32,-6,0,34); sg.CellPadding=UDim2.fromOffset(8,7); sg:GetPropertyChangedSignal("AbsoluteContentSize"):Connect(function() skinList.CanvasSize=UDim2.new(0,0,0,sg.AbsoluteContentSize.Y+7) end)
    local actions=card(54)
    local apply=Instance.new("TextButton",actions); apply.Size=UDim2.new(.68,-14,0,34); apply.Position=UDim2.new(0,10,.5,-17); apply.BorderSizePixel=0; apply.Text="Apply Skin"; apply.Font=Enum.Font.GothamBold; apply.TextSize=12; apply.TextColor3=Color3.new(1,1,1); ctx.Add("hot",apply); ctx.Corner(apply,10)
    local reset=Instance.new("TextButton",actions); reset.Size=UDim2.new(.32,-14,0,34); reset.Position=UDim2.new(.68,4,.5,-17); reset.BorderSizePixel=0; reset.Text=""; reset.AutoButtonColor=false; ctx.Add("card2",reset); ctx.Corner(reset,10); ctx.Stroke(reset,"line",.3,1)
    local resetImg=ctx.Image(reset,ctx.Assets.Reset,UDim2.new(.65,0,.75,0),UDim2.new(.175,0,.125,0)); resetImg.ZIndex=reset.ZIndex+2
    local stat=card(46); local status=ctx.Text(stat,"Searching for your weapons...",UDim2.new(1,-24,1,0),UDim2.fromOffset(12,0),Enum.Font.Gotham,11,"sub")

    _G.KimqV38WeaponSkins=_G.KimqV38WeaponSkins or {Selected={}}
    local selectedByWeapon=_G.KimqV38WeaponSkins.Selected
    ...
    local function findTool(n) local char=lp.Character; local bp=lp:FindFirstChildOfClass("Backpack"); return (char and char:FindFirstChild(n)) or (bp and bp:FindFirstChild(n)) end
    local function locateWraps()
        local direct=workspace:FindFirstChild("Wraps"); if direct then return direct end
        local rec=workspace:FindFirstChild("Wraps",true); if rec then return rec end
    ...
    local function saveOriginal(tool)
        if originals[tool] then return end
        local r={}
        for _,d in ipairs(tool:GetDescendants()) do if d:IsA("BasePart") then r[d]=d.LocalTransparencyModifier end end
        originals[tool]=r
    end
    local function clear(tool)
        if not tool then return end
        for _,d in ipairs(tool:GetDescendants()) do if d.Name=="KimqV38SkinVisual" then pcall(function() d:Destroy() end) end end
        local r=originals[tool]; if r then for p,v in pairs(r) do if p and p.Parent then pcall(function() p.LocalTransparencyModifier=v end) end end end
    end
    local function matchingRootCF(sourceRoot,targetRoot)
        for _,sa in ipairs(sourceRoot:GetChildren()) do
            if sa:IsA("Attachment") then local ta=targetRoot:FindFirstChild(sa.Name); if ta and ta:IsA("Attachment") then return ta.WorldCFrame * sa.CFrame:Inverse() end end
        end
        return targetRoot.CFrame
    end
    local function applySkin(w,s,tool,quiet)
        local gun=tool or findTool(w); local folder=folderByName[w]; local source=folder and folder:FindFirstChild(s)
        if not gun then selectedByWeapon[w]=s; if not quiet then status.Text=display(w).." saved — equip it and the skin will apply" end; return true end
        local target=gun:FindFirstChild("Handle",true); if not target or not target:IsA("BasePart") then target=rootPart(gun) end
        local sr=rootPart(source); if not target or not sr then if not quiet then status.Text="That skin does not have usable visual parts" end; return false end
        saveOriginal(gun); clear(gun)
        local desiredRoot=matchingRootCF(sr,target); local parts={}
        if source:IsA("BasePart") then table.insert(parts,source) else for _,d in ipairs(source:GetDescendants()) do if d:IsA("BasePart") then table.insert(parts,d) end end end
        if #parts==0 then if not quiet then status.Text="No visual parts found in that skin" end; return false end
        for _,sp in ipairs(parts) do
            local cp=sp:Clone(); cp.Name="KimqV38SkinVisual"; cp.Anchored=false; cp.CanCollide=false; cp.CanTouch=false; cp.CanQuery=false; cp.Massless=true
            for _,d in ipairs(cp:GetDescendants()) do if d:IsA("Script") or d:IsA("LocalScript") or d:IsA("ModuleScript") or d:IsA("Weld") or d:IsA("WeldConstraint") or d:IsA("Motor6D") then pcall(function() d:Destroy() end) end end
            local rel=sr.CFrame:ToObjectSpace(sp.CFrame); cp.CFrame=desiredRoot*rel; cp.Parent=gun
            local weld=Instance.new("WeldConstraint",cp); weld.Part0=cp; weld.Part1=target
        end
        for p in pairs(originals[gun] or {}) do if p and p.Parent then p.LocalTransparencyModifier=1 end end
        selectedByWeapon[w]=s; if not quiet then status.Text=display(w).." • "..s.." applied locally" end; return true
    end
    local function mk(parent,text)
        local b=Instance.new("TextButton",parent); b.BorderSizePixel=0; b.BackgroundColor3=ctx.GetTheme().card2; b.Text=text; b.TextColor3=ctx.GetTheme().text; b.Font=Enum.Font.GothamBold; b.TextSize=10; b.AutoButtonColor=false; ctx.Corner(b,10); ctx.Stroke(b,"line",.35,1); return b
    end
    local function styleWeapons()
        local th=ctx.GetTheme(); weaponList.ScrollBarImageColor3=th.hot; skinList.ScrollBarImageColor3=th.hot
        for n,b in pairs(weaponButtons) do local on=n==currentWeapon; b.BackgroundColor3=on and th.hot or th.card2; b.TextColor3=on and Color3.new(1,1,1) or th.text end
        for n,b in pairs(skinButtons) do local on=n==selectedSkin; b.BackgroundColor3=on and th.hot or th.card2; b.TextColor3=on and Color3.new(1,1,1) or th.text end
    end
    local function buildSkins()
        ...
        local f=currentWeapon and folderByName[currentWeapon]; if not f then skinHead.Text="Skins"; status.Text="Choose a weapon first"; return end
        skinHead.Text="Skins • "..display(currentWeapon); local skins={}
        for _,s in ipairs(f:GetChildren()) do if rootPart(s) then table.insert(skins,s) end end; table.sort(skins,function(a,b) return a.Name:lower()<b.Name:lower() end)
        selectedSkin=selectedByWeapon[currentWeapon]
        for _,s in ipairs(skins) do local b=mk(skinList,s.Name); skinButtons[s.Name]=b; b.MouseButton1Click:Connect(function() selectedSkin=s.Name; status.Text="Selected "..s.Name.." — press Apply Skin"; styleWeapons() end) end
        status.Text=#skins>0 and ("Found "..#skins.." skins") or "No skins found for this weapon"; styleWeapons()
    end
    local function scan()
        for _,c in ipairs(weaponList:GetChildren()) do if c:IsA("TextButton") then c:Destroy() end end; table.clear(weaponButtons); table.clear(folderByName)
        wrapRoot=locateWraps(); if not wrapRoot then status.Text="Could not find Workspace.Wraps"; return end
        local folders={}; local owned={}; local bp=lp:FindFirstChildOfClass("Backpack"); if bp then for _,t in ipairs(bp:GetChildren()) do owned[t.Name]=true end end; if lp.Character then for _,t in ipairs(lp.Character:GetChildren()) do if t:IsA("Tool") then owned[t.Name]=true end end end
        for _,f in ipairs(wrapRoot:GetChildren()) do if (f:IsA("Folder") or f:IsA("Model")) and #f:GetChildren()>0 then folderByName[f.Name]=f; if owned[f.Name] then table.insert(folders,f) end end end
        if #folders==0 then for _,f in pairs(folderByName) do table.insert(folders,f) end end; table.sort(folders,function(a,b) return a.Name:lower()<b.Name:lower() end)
        for _,f in ipairs(folders) do local b=mk(weaponList,display(f.Name)); weaponButtons[f.Name]=b; b.MouseButton1Click:Connect(function() currentWeapon=f.Name; selectedSkin=selectedByWeapon[currentWeapon]; buildSkins(); styleWeapons() end) end
        if #folders>0 and (not currentWeapon or not folderByName[currentWeapon]) then currentWeapon=folders[1].Name end; buildSkins(); styleWeapons()
    end
    refresh.MouseButton1Click:Connect(scan); apply.MouseButton1Click:Connect(function() if currentWeapon and selectedSkin then applySkin(currentWeapon,selectedSkin,nil,false) else status.Text="Choose a weapon and skin first" end end); reset.MouseButton1Click:Connect(function() if currentWeapon then selectedByWeapon[currentWeapon]=nil; clear(findTool(currentWeapon)); selectedSkin=nil; status.Text=display(currentWeapon).." reset"; styleWeapons() end end)
    local function hook(container) if not container or container:GetAttribute("KimqV38SkinHook") then return end; container:SetAttribute("KimqV38SkinHook",true); container.ChildAdded:Connect(function(ch) local s=selectedByWeapon[ch.Name]; if s then task.delay(.15,function() applySkin(ch.Name,s,ch,true) end) end end) end
    hook(lp:FindFirstChildOfClass("Backpack")); if lp.Character then hook(lp.Character) end
    lp.CharacterAdded:Connect(function(char) hook(char); task.delay(1,function() hook(lp:FindFirstChildOfClass("Backpack")); for w,s in pairs(selectedByWeapon) do local tool=findTool(w); if tool then applySkin(w,s,tool,true) end end end) end)
    ctx.OnShow.weaponskins=scan; scan(); ctx.StylePage(page)
end)

-- V38 finishing pass: wait for the real GUI + special pages, then reveal it and close the loader.
task.spawn(function()
    local TweenService=game:GetService("TweenService")
    local t=tick()
    while not _G.KimqV38Ctx and tick()-t<12 do task.wait(.05) end
    local ctx=_G.KimqV38Ctx
    if not ctx then return end
    task.wait(.75)
    pcall(function() ctx.ApplyTheme("Blue"); ctx.ShowPage("overview") end)
    if ctx.OldMain then ctx.OldMain.Visible=false end
    ctx.Main.Visible=true
    _G.KimqV38Ready=true
    local loader=_G.KimqV38Loader
    if loader and loader.Gui and loader.Gui.Parent then
        if loader.Fill then TweenService:Create(loader.Fill,TweenInfo.new(.25,Enum.EasingStyle.Quart,Enum.EasingDirection.Out),{Size=UDim2.new(1,0,1,0)}):Play() end
        if loader.Percent then loader.Percent.Text="100%" end
        if loader.Status then loader.Status.Text="ready ♡" end
        task.wait(.28)
        if loader.Card then TweenService:Create(loader.Card,TweenInfo.new(.25,Enum.EasingStyle.Quad,Enum.EasingDirection.In),{BackgroundTransparency=1}):Play() end
        if loader.Background then TweenService:Create(loader.Background,TweenInfo.new(.28,Enum.EasingStyle.Quad,Enum.EasingDirection.Out),{BackgroundTransparency=1}):Play() end
        task.wait(.3)
