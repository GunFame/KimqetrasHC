-- Kimqetras HC v2.39 • persistent spawn correction + clean page content
...
local KIMQ_BUILD = "v2.39-spawn-page-cleanup"
    ...
    if running and runningGui and runningBuild ~= KIMQ_BUILD then
        pcall(function()
            game:GetService("StarterGui"):SetCore("SendNotification", {
                Title = "Kimqetras HC",
                Text = "Another Kimqetras HC version is still active. Rejoin once, then run v2.39.",
                Duration = 8,
            })
        end)
        warn("[Kimqetras HC v2.39] Another runtime is active; rejoin once before switching builds.")
        return
    end
...
_G.KimqAccessoryUIReady = false
    ...
    pcall(function() game:GetService("RunService"):UnbindFromRenderStep("KimqPermanentTimeV239") end)
end
...
local function addCleanSection(title, subtitle)
    ...
    -- v2.39: pageHead already displays the section name.
...
task.spawn(function()
    ...
    -- v2.39: pages are grouped by what they actually do so the sidebar is
    ...
    do
        ...
        if spawnPage then
            ...
            lp.CharacterAdded:Connect(function(char)
                ...
                -- Ask Roblox to use the selected real SpawnLocation first.
                setRespawnLocationProperty(obj)

                -- Always do one final correction after the new rig has settled.
                -- Some games ignore/reset LocalPlayer.RespawnLocation, which is
                -- why a real SpawnLocation could still randomly spawn elsewhere.
                task.spawn(function()
...
-- v2.39 module: local catalog / limited accessory try-on.
task.spawn(function()
    _G.KimqAccessoryUIReady=false
    local Players = game:GetService("Players")
    local CoreGui = game:GetService("CoreGui")
...
-- v2.39 module: Environment + Weapon Skins (canonical sidebar pages).
task.spawn(function()
    local pageReadyStart=tick()
    while not _G.KimqBasePagesReady and tick()-pageReadyStart<8 do task.wait(.02) end
    ...
    -- v2.39: Environment and Weapon Skins are already real canonical pages/buttons.
    ...
    pcall(function()
        RunService:BindToRenderStep("KimqPermanentTimeV239",Enum.RenderPriority.Last.Value,function()
    ...
    if not envBuildOk then
        warn("[Kimqetras HC v2.39] Environment page fallback: "..tostring(envBuildErr))
    ...
    if not extrasOk then
        _G.KimqWeaponExtrasInstalled=false
        warn("[Kimqetras HC v2.39] Weapon extras: "..tostring(extrasErr))
    ...
    skinsBtn.MouseButton1Click:Connect(function()
        task.defer(function()
            ...
            if extras and type(extras.Refresh)=="function" then
                ...
                if not ok then
                    warn("[Kimqetras HC v2.39] extras refresh: "..tostring(err))
...
task.spawn(function()
    ...
    -- v2.39: preserve the grouped sidebar order from pageDefs.
    ...
    local function applyTheme(name,settlePass)
        ...
        -- v2.39 hard guarantee: old controls created with literal pink/yellow
    ...
    -- v2.39: section navigation does not repaint/rescan the entire GUI.
    ...
    -- v2.39 reveal happens only after this initial theme pass is complete.
end)


-- v2.39: delayed post-show cleanup passes removed.
...
-- ========================================================
-- v2.39 branding is deterministic at construction time; no delayed scrub pass.

-- ========================================================
-- v2.39 DETERMINISTIC REVEAL
...
task.spawn(function()
    ...
    -- v2.39: remove the old one-page section divider labels from the
