...
local KIMQ_BUILD = "v2.29-stable-source-macro-wing-flight"
...
do
    ...
    if running and runningGui and runningBuild ~= KIMQ_BUILD then
        pcall(function()
            game:GetService("StarterGui"):SetCore("SendNotification", {
                Title = "Kimqetras HC",
                Text = "Another Kimqetras HC version is still active. Rejoin once, then run v2.29.",
                Duration = 8,
            })
        end)
        warn("[Kimqetras HC v2.29] Another runtime is active; rejoin once before switching builds.")
        return
    end
    ...
    pcall(function() game:GetService("RunService"):UnbindFromRenderStep("KimqPermanentTimeV229") end)
end
...
Hello.Text = "v2.29  •  stable rebuild ♡"
...
task.spawn(function()
    ...
    pcall(function()
        RunService:BindToRenderStep("KimqPermanentTimeV229",Enum.RenderPriority.Last.Value,function()
    ...
    local function setupWeaponExtras()
        ...
        local function startFlightAnimation(hum)
            ...
            local track=nil
            local okEmote,emoteTrack=pcall(function() return hum:PlayEmoteAndGetAnimTrackById(wingFlightAnimAssetId) end)
            if okEmote and typeof(emoteTrack)=="Instance" and emoteTrack:IsA("AnimationTrack") then track=emoteTrack end
            if not track then
                local anim=Instance.new("Animation"); anim.Name="KimqAngelWingFlightAnimation"; anim.AnimationId=wingFlightAnimId; wingFlightAnimObject=anim
                local okLoad,loaded=pcall(function() return animator:LoadAnimation(anim) end)
                if okLoad then track=loaded end
                if not track then local okHum,loadedHum=pcall(function() return hum:LoadAnimation(anim) end); if okHum then track=loadedHum end end
            end
            ...
            updateWingMiniStatus("Flying ♡  •  animation 108343889270403")
    ...
    pcall(function()
        game:GetService("StarterGui"):SetCore("SendNotification", {Title="Kimqetras HC",Text="v2.29 loaded ♡",Duration=4})
    end)
    main.Visible=true
