...
task.spawn(function()
    ...
    local function setupWeaponExtras()
        ...
        local function applyBulletBeamOverride(name)
            ...
            if name=="None" then
                if bulletBeamBackup then
                    for _,target in ipairs(bulletBeamRoot:GetChildren()) do
                        local original=bulletBeamBackup:FindFirstChild(target.Name)
                        if original then copyTemplateVisuals(target,original) end
                    end
                end
            ...
            local stableSource=selected
            pcall(function() stableSource=selected:Clone() end)
            -- Patch every preset so the game's currently equipped preset does not matter.
            for _,target in ipairs(bulletBeamRoot:GetChildren()) do
                if not target:IsA("Sound") then copyTemplateVisuals(target,stableSource) end
            end
            if stableSource~=selected then pcall(function() stableSource:Destroy() end) end
