-- Kimqetras HC v2.8 • smooth alternating camera • persistent local bullet color • low-lag single runtime
-- Re-running this SAME build only reopens the current GUI instead of stacking another copy.
-- If an older build is already active, v2.8 refuses to stack and asks for one clean rejoin.
do
    local SINGLE_KEY = "KimqHC_v21_SingleRuntimeOptimized"
    local BUILD = "v2.8-smooth-camera-persistent-bullets-low-lag"
    ...
    -- Older builds did not expose a complete disconnect/cleanup handle. Starting v2.8
    ...
    if rawget(_G, SINGLE_KEY) and rawget(_G, "KimqHC_CurrentBuild") ~= BUILD then
        pcall(function()
            game:GetService("StarterGui"):SetCore("SendNotification", {
                Title = "Kimqetras HC",
                Text = "Older runtime detected. Rejoin once, then run v2.8 so only one version is active.",
                Duration = 7,
            })
        end)
        warn("[Kimqetras HC v2.8] Older runtime detected. Rejoin once before running v2.8; refusing to stack versions.")
        return
    end
...
-- v2.8 keeps v2.6 config/fog persistence and adds smooth alternating camera + persistent low-lag local bullet colors.
...
-- Each shot changes the view and leaves it there until the next shot.
-- Third person -> smooth first person; next shot -> smooth third person.
...
local function smoothZoomTo(target,token)
    local start=cameraDistance()
    local duration=0.16
    local t0=os.clock()
    while token==cameraSwapToken and _G.KimqShotCameraSwapEnabled do
    ...
    if token~=cameraSwapToken or not _G.KimqShotCameraSwapEnabled then return end
    setExactZoom(target)
    RunService.RenderStepped:Wait()
...
local function doShotCameraSwap(tool)
    if not _G.KimqShotCameraSwapEnabled then return end
    ...
    if not first and dist>1.1 then lastThirdDistance=math.clamp(dist,4,40) end
    if cameraTargetFirst==nil or (first~=cameraTargetFirst and math.abs(dist-(cameraTargetFirst and 0.5 or lastThirdDistance))>1.5) then
        cameraTargetFirst=first
    end
    cameraTargetFirst=not cameraTargetFirst
    local target=cameraTargetFirst and 0.5 or math.clamp(lastThirdDistance or 10,5,40)
    cameraSwapToken+=1
    local token=cameraSwapToken
    task.spawn(function() smoothZoomTo(target,token) end)
end

addToggle("Shot Camera Swap", _G.KimqShotCameraSwapEnabled, function(v)
    _G.KimqShotCameraSwapEnabled=v==true
    cameraSwapToken+=1
    cameraTargetFirst=nil
    if not _G.KimqShotCameraSwapEnabled then
        ...
        if d>1.1 then lastThirdDistance=math.clamp(d,4,40) end
    end
end)
...
local function hookCameraTool(tool)
    ...
    tool.Activated:Connect(function() doShotCameraSwap(tool) end)
end
...
task.spawn(function()
    ...
    local function setupWeaponExtras()
        ...
        local function styleVerifiedRoot(root,serial)
            ...
            for _,delayTime in ipairs({0.012,0.035,0.075}) do
        ...
        local function captureWithShortRetry(obj,serial)
            ...
            for _,delayTime in ipairs({0.008,0.022,0.050,0.090}) do
                task.delay(delayTime,function()
        ...
        local function hasBulletAncestorName(d)
            ...
            for _=1,6 do
                ...
                if n:find("bullet",1,true) or n:find("tracer",1,true) or n:find("projectile",1,true) or n:find("ray",1,true) then return true end
                x=x.Parent
            end
            return false
        end
        ...
        local function refreshAllLocalGunTemplates()
            ...
            task.delay(0.035,function()
                ...
                if bp then for _,tool in ipairs(bp:GetChildren()) do if tool:IsA("Tool") then styleLocalToolTemplates(tool) end end end
                if lp.Character then for _,tool in ipairs(lp.Character:GetChildren()) do if tool:IsA("Tool") then styleLocalToolTemplates(tool) end end end
            end)
        end
        ...
        local function hookBulletContainer(container)
            ...
            container.ChildAdded:Connect(function(ch)
                ...
                if serial==0 or (os.clock()-activeShotAt)>.38 then return end
                captureWithShortRetry(ch,serial)
            end)
            local serial=activeShotSerial
            if serial~=0 and (os.clock()-activeShotAt)<=.38 then captureWithShortRetry(container,serial) end
        end

        for _,d in ipairs(workspace:GetDescendants()) do
            if tostring(d.Name):lower()=="bullet_rays" then hookBulletContainer(d) end
        end
            ...
            task.defer(function()
                for container,_ in pairs(bulletContainers) do
                    if container and container.Parent then
                        ...
                        local first=math.max(1,#children-5)
                        for i=first,#children do captureWithShortRetry(children[i],serial) end
                    end
                end
            end)
        end
        ...
        local function hookShotTool(tool)
            ...
            styleLocalToolTemplates(tool)
            tool.Equipped:Connect(function() task.defer(function() styleLocalToolTemplates(tool) end) end)
            tool.ChildAdded:Connect(function() if bulletEffectEnabled() then refreshAllLocalGunTemplates() end end)
