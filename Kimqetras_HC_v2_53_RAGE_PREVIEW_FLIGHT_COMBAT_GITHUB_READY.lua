-- Kimqetras HC v2.53 • restored v2.19 weapon-extras core + isolated wing flight
...
local KIMQ_BUILD = "v2.53-restored-v219-extras"
...
do
    ...
    if running and runningBuild == KIMQ_BUILD then
        ...
        pcall(function()
            StarterGui0:SetCore("SendNotification",{
                Title="Kimqetras HC",
                Text=(runtimeState=="ready") and "v2.53 is already running ♡" or "v2.53 is already starting ♡",
                Duration=4,
            })
        end)
        return
    ...
    if oldRuntimeDetected then
        pcall(function()
            StarterGui0:SetCore("SendNotification",{
                Title="Kimqetras HC",
                Text="An older Kimqetras HC runtime is still active. Rejoin once, then run v2.53. v2.53 will not stack on top of it.",
                Duration=9,
            })
        end)
        warn("[Kimqetras HC v2.53] Older runtime detected. Refusing to stack; rejoin once.")
        return
    end
    ...
    local knownRenderBindings = {
        "KimqMacroSpeedV240","KimqMacroSpeedV241","KimqMacroSpeedV242","KimqMacroSpeedV253",
        ...
        "KimqPermanentTimeV241","KimqPermanentTimeV242","KimqPermanentTimeV253",
    }
...
pcall(function()
    RunService:BindToRenderStep("KimqMacroSpeedV253",Enum.RenderPriority.Last.Value,function()
        enforceMacroSpeed()
    end)
end)
...
local function addCleanSection(title, subtitle)
    ...
    -- v2.53: pageHead already displays the section name.
...
task.spawn(function()
    ...
    -- v2.53: pages are grouped by what they actually do so the sidebar is
    ...
    task.defer(function()
        local rageOK,rageERR=pcall(function()
            ...
            local function button(parent,text,pos,size,fn)
                ...
                b.MouseButton1Click:Connect(function()
                    ...
                    if not ok then warn("[Kimqetras HC v2.53 RAGE button] "..tostring(err)) end
                end)
                return b
            end
            ...
            local function showPreview(pl)
                ...
                local animationId=getSourceAnimationId(pl)
                ...
                task.spawn(function()
                    ...
                    -- Put the avatar's FEET at y=0 instead of centering its body at y=0.
                    local bottomY=cf.Position.Y-(size.Y*.5)
                    pcall(function()
                        local pivot=model:GetPivot()
                        local delta=Vector3.new(-cf.Position.X,-bottomY+.05,-cf.Position.Z)
                    ...
                    -- More distance than v2.52 so tall/wide avatars stay fully visible.
                    local dist=math.max(size.Y*1.78,math.max(size.X,size.Z)*3.15,10.5)
            ...
            local function finish(pl)
                ...
                task.spawn(function()
                    ...
                    if autoAdvance and queued[pl.UserId] then
                        removeId(pl.UserId)
                    end
                    stompBusy=false
                end)
            ...
            makeToggle(rageOrbitPage,"Auto Orbit + Fly",false,function(v)
                orbitEnabled=v
                orbitAngle=0
                if v then
                    tryEquipRageWings()
            ...
            makeToggle(rageCombatPage,"Auto Hunt Queue",false,function(v)
                autoHunt=v
                orbitAngle=0
                shootClock=0
                if v then
            ...
            local noteText=role(label(
                note,
                "Auto Hunt only affects RAGE: fly/orbit current target → auto-fire your selected Combat Mode → detect KO/down → move close and press E → continue.",
            ...
            RunServiceR.RenderStepped:Connect(function(dt)
                local ok,err=pcall(function()
                    ...
                    if (orbitEnabled or autoHunt) and pl then
                        tryEquipRageWings()
                ...
                if not ok then
                    ...
                    warn("[Kimqetras HC v2.53 RAGE frame] "..tostring(err))
                end
            end)
            ...
            RunServiceR.Heartbeat:Connect(function(dt)
                local ok,err=pcall(function()
                    ...
                    if pl and (autoShoot or autoHunt) then
                        shootClock+=dt
                    ...
                    local now=os.clock()
                    if autoHunt and pl and not stompBusy and now-lastDownCheck>.1 then
                        lastDownCheck=now
                        if downed(pl) then finish(pl) end
                    end
                end)
                if not ok then
                    warn("[Kimqetras HC v2.53 RAGE heartbeat] "..tostring(err))
                end
            end)
        ...
        if not rageOK then
            warn("[Kimqetras HC v2.53] RAGE module isolated error: "..tostring(rageERR))
        end
    end)
...
-- v2.53 module: local catalog / limited accessory try-on.
task.spawn(function()
    _G.KimqAccessoryUIReady=false
    local Players = game:GetService("Players")
    local CoreGui = game:GetService("CoreGui")
...
-- v2.53 module: Environment + Weapon Skins (canonical sidebar pages).
task.spawn(function()
    local pageReadyStart=tick()
    while not _G.KimqBasePagesReady and tick()-pageReadyStart<8 do task.wait(.02) end
    ...
    -- v2.53: Environment and Weapon Skins are already real canonical pages/buttons.
    ...
    pcall(function()
        RunService:BindToRenderStep("KimqPermanentTimeV253",Enum.RenderPriority.Last.Value,function()
    ...
    if not envBuildOk then
        warn("[Kimqetras HC v2.53] Environment page fallback: "..tostring(envBuildErr))
    ...
    if not extrasOk then
        ...
        warn("[Kimqetras HC v2.53] restored v2.19 Weapon Extras: "..tostring(extrasErr))
    ...
    -- ========================================================
    -- v2.53 ISOLATED ANGEL WINGS FLIGHT
    ...
    skinsBtn.MouseButton1Click:Connect(function()
        task.defer(function()
            ...
            if extras and type(extras.Refresh)=="function" then
                ...
                if not ok then
                    warn("[Kimqetras HC v2.53] extras refresh: "..tostring(err))
...
task.spawn(function()
    ...
    -- v2.53: preserve the grouped sidebar order from pageDefs.
    ...
    local function applyTheme(name,settlePass)
        ...
        -- v2.53 hard guarantee: old controls created with literal pink/yellow
    ...
    -- v2.53: section navigation does not repaint/rescan the entire GUI.
    ...
    -- v2.53 reveal happens only after this initial theme pass is complete.
end)


-- v2.53: delayed post-show cleanup passes removed.
...
-- ========================================================
-- v2.53 branding is deterministic at construction time; no delayed scrub pass.

-- ========================================================
-- v2.53 DETERMINISTIC REVEAL
...
task.spawn(function()
    ...
    -- v2.53: remove the old one-page section divider labels from the
