-- Kimqetras HC v2.82 • v2.81 base + reliable stomp input/KO finish + immediate config avatar restore
...
local KIMQ_BUILD = "v2.82-stomp-config-avatar-fix"
...
do
    ...
    if running and runningBuild == KIMQ_BUILD then
        if runningGui then
            ...
            notifyBoot((runtimeState=="ready") and "v2.82 is already running ♡" or "v2.82 is already starting ♡",4)
            return
        end
        ...
        notifyBoot("Recovering a failed v2.82 boot ♡",4)
    end
    ...
    if oldRuntimeDetected then
        ...
        if staleBoot then
            ...
            notifyBoot("STOP: another Kimqetras HC build is already active. v2.82 DID NOT LOAD. Rejoin once, then run only v2.82.",9)
            warn("[Kimqetras HC v2.82] Another build is active. v2.82 DID NOT LOAD; rejoin once before testing.")
            return
        end
    end
...
Hello.Text = "v2.82  •  reliable stomp + instant saved avatar ♡"
    ...
    task.defer(function()
        local rageOK,rageERR=pcall(function()
            ...
            local function pressStomp()
                -- v2.82: some executors expose VirtualInputManager AND keypress, but
                -- only one of those paths actually reaches the game's E listener.
                -- Send a clean E tap through every available local-input path instead
                -- of returning after the first one. This mirrors the manual E stomp.
                if VIMR then
                    pcall(function()
                ...
                if type(keytap)=="function" then
                    ...
                    if type(keypress)=="function" then pcall(keypress,0x45) end
                    task.wait(.04)
                    if type(keyrelease)=="function" then pcall(keyrelease,0x45) end
                end

                -- Same-game fallback. If the game accepts its normal stomp remote,
                -- this lands in addition to the local E tap; failures stay protected.
                local mainEvent=ReplicatedStorageR:FindFirstChild("MainEvent")
                if mainEvent and mainEvent:IsA("RemoteEvent") then
            ...
            local function placeOnTopForStomp(pl)
                ...
                -- Keep our root close enough for the game's E stomp proximity check.
                -- 3 studs was visually "above" the body but could sit outside the
                -- interaction window on some knocked rigs. 2.05 keeps us on top while
                -- remaining upright instead of clipping underneath the body.
                local pos=targetRoot.Position+Vector3.new(0,2.05,0)
            ...
            local function stompableKnock(pl)
                local char=pl and pl.Character
                if not char then return false end
                local bodyEffects=char:FindFirstChild("BodyEffects")
                ...
                for _,d in ipairs(char:GetDescendants()) do
                    ...
                    if n=="knocked" or n=="downed" or n=="unconscious" then
                        if d:IsA("BoolValue") and d.Value then return true end
                        if (d:IsA("IntValue") or d:IsA("NumberValue")) and d.Value~=0 then return true end
                    end
                end
            ...
            local function finish(pl)
                ...
                -- The finish phase owns movement completely. Stop orbit first so it
                -- cannot re-enable PlatformStand or pull us away from the knocked body.
                rageControl.releaseMovementForStomp()

                task.spawn(function()
                    if autoStomp then
                        local presses=0
                        local wantedPresses=math.max(stompRepeats,12)
                        local deadline=os.clock()+5.0
                        local koWaitDeadline=os.clock()+1.35

                        -- Health can reach zero a fraction before BodyEffects.K.O flips.
                        -- Wait briefly for the actual stompable state instead of burning
                        -- all E presses before the game marks the target as knocked.
                        while pl.Parent and pl.Character and os.clock()<koWaitDeadline
                            and not stompableKnock(pl) do
                            placeOnTopForStomp(pl)
                            task.wait(.05)
                        end

                        while pl.Parent and pl.Character and os.clock()<deadline and presses<wantedPresses do
                            ...
                            -- Once the stomped character disappears / leaves the KO
                            -- state after several attempts, the finish succeeded and we
                            -- can move on without wasting the whole timeout.
                            if presses>=3 and not stompableKnock(pl) then
                                local th=pl.Character and pl.Character:FindFirstChildOfClass("Humanoid")
            ...
            local ragePresetSub=role(label(
                ragePresetCard,"aggressive targeting + K.O → stop orbit → stand above → E stomp ♡",
            ...
            RunServiceR.Heartbeat:Connect(function(dt)
                local ok,err=pcall(function()
                    ...
                    -- v2.82: KO detection happens BEFORE the reload/shoot block. This
                    -- makes the phase switch immediate and prevents One Shot from firing
                    -- another burst while the target is already waiting to be stomped.
                    local now=os.clock()
                    if autoHunt and pl and not waiting and not stompBusy and now-lastDownCheck>.06 then
                        lastDownCheck=now
                        if downed(pl) then
                            finish(pl)
                            waiting=true
...
task.spawn(function()
    ...
    pcall(function()
        game:GetService("StarterGui"):SetCore("SendNotification",{
            Title="Kimqetras HC",
            Text="v2.82 ACTIVE ♡ • stomp + config avatar restore ready",
            Duration=7,
        })
    end)
