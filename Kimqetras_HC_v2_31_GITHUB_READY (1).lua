-- Kimqetras HC v2.31 GITHUB / EXECUTOR BUILD
local StarterGui=game:GetService("StarterGui")
pcall(function()
    StarterGui:SetCore("SendNotification",{Title="Kimqetras HC",Text="v2.31 starting...",Duration=3})
end)

local SOURCE=[==========[
-- Kimqetras HC v2.31 • register-limit execute fix + exact source macro
...
local KIMQ_BUILD = "v2.31-register-scope-fix"
...
do
    local running = rawget(_G, KIMQ_SINGLE_KEY)
    local runningBuild = rawget(_G, "KimqHC_CurrentBuild")
    local runningGui = kimqFindExistingGui()

    if running and runningBuild == KIMQ_BUILD and runningGui then
        pcall(function()
            runningGui.Enabled = true
            local main0 = runningGui:FindFirstChild("Main")
            if main0 then main0.Visible = true end
        end)
        return
    end

    -- A failed older boot can leave only the global flag behind. That is safe to clear.
    if running and not runningGui then
    ...
    -- Never stack two real runtimes because their old input/render connections cannot
    -- be reliably disconnected. A clean rejoin is required when a real older GUI exists.
    if running and runningGui and runningBuild ~= KIMQ_BUILD then
        pcall(function()
            game:GetService("StarterGui"):SetCore("SendNotification", {
                Title = "Kimqetras HC",
                Text = "Another Kimqetras HC version is still active. Rejoin once, then run v2.31.",
                Duration = 8,
            })
        end)
        warn("[Kimqetras HC v2.31] Another runtime is active; rejoin once before switching builds.")
        return
    end
    ...
    pcall(function() game:GetService("RunService"):UnbindFromRenderStep("KimqPermanentTimeV231") end)
end
...
-- Macro / Speed behavior from the user's supplied source only.
...
addToggle("Macro / Speed Master", _G.SpeedMaster, function(v)
    ...
    if not _G.SpeedMaster then
        ...
        pcall(function()
            game:GetService("StarterGui"):SetCore("SendNotification", {
                Title = "Macro",
                Text = "Macro ready — press your Macro Key to toggle speed.",
                Duration = 3
            })
        end)
    end
...
MacroHint.Text = "Turn Master on, then press the Macro Key to toggle the selected speed."
...
local _, updateConfigButton = addButton("Update Selected Config", function()
    ...
    -- Update the selected config with live settings. If Config Name was edited,
    -- save under the new name first, then remove the old file only after save succeeds.
    ...
    do
        ...
        if spawnPage then
            ...
            lp.CharacterAdded:Connect(function(char)
                ...
                -- Generic markers get exactly one move after the new rig is fully created.
                task.spawn(function()
                    local root=char:WaitForChild("HumanoidRootPart",5)
                    local hum=char:FindFirstChildOfClass("Humanoid") or char:WaitForChild("Humanoid",5)
                    if not root or not hum then return end
                    task.wait(.70)
                    if char==lp.Character and char.Parent and useSelectedSpawn then
...
task.spawn(function()
    ...
    pcall(function()
        RunService:BindToRenderStep("KimqPermanentTimeV231",Enum.RenderPriority.Last.Value,function()
    ...
    local function setupWeaponExtras()
        ...
        bulletAPI.scanBeams=scanBeams
        bulletAPI.beamList=beamList
        bulletAPI.applyBulletBeamOverride=applyBulletBeamOverride
        bulletAPI.normalizeHex=normalizeHex
        bulletAPI.colorFromHex=colorFromHex
        bulletAPI.refreshBeamStyle=refreshBeamStyle
        bulletAPI.refreshBulletColorUI=refreshBulletColorUI
        bulletAPI.hookShotContainer=hookShotContainer
        bulletAPI.setSavedColor=function(savedHex)
            local c=colorFromHex(savedHex)
            if c then
                bulletCustomColor=c
                bulletHue,bulletSat,bulletVal=c:ToHSV()
                refreshBulletColorUI()
                return true
            end
            return false
        ...
        -- Angel Wings flight state. The wearable itself stays the original v2.19 rig.
        ...
        local wingFlightConnection=nil
        local wingFlightHum=nil
        local wingFlightOldAutoRotate=true
        local wingFlightLastJump=0
        local wingFlightTrack=nil
        local wingFlightAnimObject=nil
        local wingFlightAnimId="rbxassetid://108343889270403"
        local wingFlightAnimAssetId=108343889270403
        local wingFlightAnimMode="none"
        local wingFlightAnimateScript=nil
        local wingFlightAnimateOldDisabled=nil
        ...
        local wingMiniTrack=nil
        local wingHeadHalo=nil
        local stopWingFlight
        local destroyWingMiniGui
        local clearWingHeadHalo

        local function clearWornEquippable()
            if stopWingFlight then pcall(function() stopWingFlight() end) end
            if destroyWingMiniGui then pcall(destroyWingMiniGui) end
            if clearWingHeadHalo then pcall(clearWingHeadHalo) end
        ...
        clearWingHeadHalo=function()
            if wingHeadHalo and wingHeadHalo.Parent then pcall(function() wingHeadHalo:Destroy() end) end
            wingHeadHalo=nil
            local char=lp.Character
            if char then
        ...
        local function haloScore(obj)
            if not obj then return -1 end
            local n=tostring(obj.Name):lower()
            local score=0
            if n:find("halo",1,true) then score=score+100 end
            if n:find("ring",1,true) then score=score+30 end
            if n:find("angel",1,true) then score=score+10 end
            if score==0 then return -1 end
            if obj:IsA("Accessory") then return score+30 end
            if obj:IsA("Model") then return score+20 end
            if obj:IsA("MeshPart") then return score+18 end
            if obj:IsA("BasePart") then return score+12 end
            return -1
        end
        local function bestHalo(root)
            if not root then return nil end
            local best,bestScore=nil,-1
            local function consider(obj)
                local score=haloScore(obj)
        ...
        local function findWeaponHaloSource()
            -- First prefer the skin visual the player is actually using.
            for _,container in ipairs({lp.Character,lp:FindFirstChildOfClass("Backpack")}) do
                if container then
                    for _,tool in ipairs(container:GetChildren()) do
                        if tool:IsA("Tool") then
                            for _,d in ipairs(tool:GetDescendants()) do
                                if d.Name=="KimqV21AnimatedSkinVisual" or d.Name=="KimqV26SkinVisual" then
                                    local h=bestHalo(d); if h then return h end
                                end
                            end
                        end
                    end
            ...
            -- Otherwise use the selected weapon skin source.
            ...
            if root then
                for weaponName,skinName in pairs(selectedByWeapon or {}) do
                    ...
                    local h=bestHalo(sf); if h then return h end
                end
            end
            return nil
        end
        local function stripHaloJoints(obj)
            for _,d in ipairs(obj:GetDescendants()) do
                if d:IsA("WeldConstraint") or d:IsA("Weld") or d:IsA("ManualWeld") or d:IsA("Motor6D") then
                    pcall(function() d:Destroy() end)
                end
            end
        end
        local function scaleHalo(obj,scale)
            scale=scale or 1.45
            if obj:IsA("Model") then
                pcall(function() obj:ScaleTo(obj:GetScale()*scale) end)
            else
                local rootPart=obj:IsA("Accessory") and obj:FindFirstChild("Handle") or (obj:IsA("BasePart") and obj or nil)
                if rootPart and rootPart:IsA("BasePart") then
                    local sm=rootPart:FindFirstChildOfClass("SpecialMesh")
                    if sm then pcall(function() sm.Scale=sm.Scale*scale; sm.Offset=sm.Offset*scale end)
                    else pcall(function() rootPart.Size=rootPart.Size*scale end) end
                end
            end
        end
        local function attachWeaponHaloToHead()
            ...
            stripHaloJoints(clone)
            scaleHalo(clone,1.45)
            clone.Parent=char
            local parts={}
            if clone:IsA("BasePart") then table.insert(parts,clone) end
            for _,d in ipairs(clone:GetDescendants()) do
                if d:IsA("BasePart") then table.insert(parts,d)
                ...
                elseif d:IsA("LocalScript") or d:IsA("Script") then pcall(function() d.Disabled=true end) end
            end
            if #parts==0 then clone:Destroy(); return false end
            local pivot
            if clone:IsA("Model") then pivot=clone:GetPivot()
            elseif clone:IsA("Accessory") then local h=clone:FindFirstChild("Handle"); pivot=h and h.CFrame or nil
            elseif clone:IsA("BasePart") then pivot=clone.CFrame end
            if not pivot then clone:Destroy(); return false end
            local desired=head.CFrame*CFrame.new(0,1.35,0)
            local delta=desired*pivot:Inverse()
            for _,part in ipairs(parts) do
                part.CFrame=delta*part.CFrame
                part.Anchored=false; part.CanCollide=false; part.CanTouch=false; part.CanQuery=false; part.Massless=true
                part.LocalTransparencyModifier=0
                if part.Transparency>=.98 then part.Transparency=0 end
                local weld=Instance.new("WeldConstraint"); weld.Name="KimqAngelHaloHeadWeld"; weld.Part0=head; weld.Part1=part; weld.Parent=part
            end
            wingHeadHalo=clone
            return true
        end
        ...
        local function refreshWingMiniTheme()
            if not (wingMiniGui and wingMiniGui.Parent and wingMiniFrame) then return end
            local p=_G.KimqThemeLivePalette or {}
            local bg=p.bg or Color3.fromRGB(255,245,250)
            local soft=p.soft or p.bg2 or Color3.fromRGB(255,228,243)
            local hot=p.hot or Color3.fromRGB(243,161,211)
            local text=p.text or Color3.fromRGB(82,116,94)
            local sub=p.sub or Color3.fromRGB(122,153,133)
            local line=p.line or Color3.fromRGB(255,212,243)
            wingMiniFrame.BackgroundColor3=bg
            local st=wingMiniFrame:FindFirstChildOfClass("UIStroke"); if st then st.Color=line end
            if wingMiniTitle then wingMiniTitle.TextColor3=hot end
            ...
            if wingMiniTrack then wingMiniTrack.BackgroundColor3=soft end
            if wingMiniFill then wingMiniFill.BackgroundColor3=hot end
            if wingMiniKnob then wingMiniKnob.BackgroundColor3=Color3.new(1,1,1) end
            if wingMiniSpeedLabel then wingMiniSpeedLabel.TextColor3=text end
        end
        _G.KimqRefreshWingMiniTheme=refreshWingMiniTheme

        local function updateWingMiniSpeedVisual()
            if wingMiniSpeedLabel then wingMiniSpeedLabel.Text="Flight Speed  "..tostring(math.floor(wingFlightSpeed+.5)) end
            if wingMiniKnob and wingMiniTrack then
                local pct=math.clamp((wingFlightSpeed-16)/(140-16),0,1)
                wingMiniFill.Size=UDim2.new(pct,0,1,0)
                wingMiniKnob.Position=UDim2.new(pct,-7,.5,-7)
            end
        end
        local function updateWingMiniStatus(textOverride)
            if wingMiniStatus then
                if textOverride then wingMiniStatus.Text=textOverride
                elseif wingFlightActive then wingMiniStatus.Text="Flying ♡  •  double jump to stop"
                else wingMiniStatus.Text="Double jump to fly ♡" end
            end
            refreshWingMiniTheme()
        end
        destroyWingMiniGui=function()
            if wingMiniGui then pcall(function() wingMiniGui:Destroy() end) end
            wingMiniGui=nil; wingMiniFrame=nil; wingMiniTitle=nil; wingMiniClose=nil; wingMiniStatus=nil; wingMiniFill=nil; wingMiniKnob=nil; wingMiniSpeedLabel=nil; wingMiniTrack=nil
        end
        local function createWingMiniGui()
            destroyWingMiniGui()
            if not wingsAreWorn() then return end
            local gui=Instance.new("ScreenGui"); gui.Name="KimqAngelWingsMini"; gui.ResetOnSpawn=false; gui.IgnoreGuiInset=true; gui.DisplayOrder=999998
            pcall(function() gui.Parent=CoreGui end); if not gui.Parent then gui.Parent=playerGui end
            local frame=Instance.new("Frame",gui); frame.Name="Main"; frame.Size=UDim2.fromOffset(250,128); frame.Position=UDim2.new(1,-274,.5,-64); frame.BorderSizePixel=0; frame.Active=true
            Instance.new("UICorner",frame).CornerRadius=UDim.new(0,16)
            local stroke0=Instance.new("UIStroke",frame); stroke0.Thickness=1.4; stroke0.Transparency=.2
            local title=Instance.new("TextLabel",frame); title.Size=UDim2.new(1,-52,0,30); title.Position=UDim2.fromOffset(14,8); title.BackgroundTransparency=1; title.Text="Angel Wings ♡"; title.Font=Enum.Font.GothamBold; title.TextSize=15; title.TextXAlignment=Enum.TextXAlignment.Left
            local close=Instance.new("TextButton",frame); close.Size=UDim2.fromOffset(28,28); close.Position=UDim2.new(1,-38,0,7); close.BackgroundTransparency=1; close.Text="×"; close.Font=Enum.Font.GothamBold; close.TextSize=20
            local speedLabel=Instance.new("TextLabel",frame); speedLabel.Size=UDim2.new(1,-28,0,20); speedLabel.Position=UDim2.fromOffset(14,42); speedLabel.BackgroundTransparency=1; speedLabel.Font=Enum.Font.GothamSemibold; speedLabel.TextSize=11; speedLabel.TextXAlignment=Enum.TextXAlignment.Left
            local track=Instance.new("Frame",frame); track.Size=UDim2.new(1,-28,0,8); track.Position=UDim2.fromOffset(14,67); track.BorderSizePixel=0; Instance.new("UICorner",track).CornerRadius=UDim.new(1,0)
            local fill=Instance.new("Frame",track); fill.Size=UDim2.new(.25,0,1,0); fill.BorderSizePixel=0; Instance.new("UICorner",fill).CornerRadius=UDim.new(1,0)
            local knob=Instance.new("TextButton",track); knob.Size=UDim2.fromOffset(14,14); knob.AnchorPoint=Vector2.new(0,.5); knob.Position=UDim2.new(.25,-7,.5,0); knob.BorderSizePixel=0; knob.Text=""; knob.AutoButtonColor=false; Instance.new("UICorner",knob).CornerRadius=UDim.new(1,0)
            local status=Instance.new("TextLabel",frame); status.Size=UDim2.new(1,-28,0,30); status.Position=UDim2.fromOffset(14,86); status.BackgroundTransparency=1; status.Font=Enum.Font.Gotham; status.TextSize=10; status.TextWrapped=true; status.TextXAlignment=Enum.TextXAlignment.Left
            wingMiniGui=gui; wingMiniFrame=frame; wingMiniTitle=title; wingMiniClose=close; wingMiniStatus=status; wingMiniFill=fill; wingMiniKnob=knob; wingMiniSpeedLabel=speedLabel; wingMiniTrack=track
            close.MouseButton1Click:Connect(function() gui.Enabled=false end)
            local draggingSlider=false
            local function setFromX(x)
                local pct=math.clamp((x-track.AbsolutePosition.X)/math.max(track.AbsoluteSize.X,1),0,1)
                wingFlightSpeed=math.floor(16+(140-16)*pct+.5)
                wingFlightVerticalSpeed=math.clamp(wingFlightSpeed*.66,18,80)
                updateWingMiniSpeedVisual()
            end
            track.InputBegan:Connect(function(i) if i.UserInputType==Enum.UserInputType.MouseButton1 then draggingSlider=true; setFromX(i.Position.X) end end)
            knob.MouseButton1Down:Connect(function() draggingSlider=true end)
            UIS.InputChanged:Connect(function(i) if draggingSlider and i.UserInputType==Enum.UserInputType.MouseMovement then setFromX(i.Position.X) end end)
            ...
            UIS.InputChanged:Connect(function(i)
                if dragging and dragStart and i.UserInputType==Enum.UserInputType.MouseMovement then
            ...
            updateWingMiniSpeedVisual(); updateWingMiniStatus(); refreshWingMiniTheme()
        end

        local function restoreFlightPose()
            for motor,old in pairs(wingFlightPoseMotors) do if motor and motor.Parent then pcall(function() motor.Transform=old end) end end
            table.clear(wingFlightPoseMotors)
        end
        local function beginFallbackFlightPose(hum)
            restoreFlightPose(); wingFlightAnimMode="pose"
            local char=hum and hum.Parent; if not char then return end
            for _,m in ipairs(char:GetDescendants()) do
                if m:IsA("Motor6D") then
                    local n=tostring(m.Name):lower():gsub("%s+","")
                    if n=="rightshoulder" or n=="leftshoulder" or n=="righthip" or n=="lefthip" or n=="waist" or n=="rootjoint" or n=="neck" then wingFlightPoseMotors[m]=m.Transform end
                end
            end
            updateWingMiniStatus("Flying ♡  •  cute fallback animation")
        end
        local function applyFallbackFlightPose()
            if wingFlightAnimMode~="pose" then return end
            local bob=math.sin(os.clock()*2.4)
            for m,base in pairs(wingFlightPoseMotors) do
                if m and m.Parent then
                    local n=tostring(m.Name):lower():gsub("%s+","")
                    local t=base
                    if n=="rightshoulder" then t=base*CFrame.Angles(math.rad(-34+bob*2),0,math.rad(42))
                    elseif n=="leftshoulder" then t=base*CFrame.Angles(math.rad(-34+bob*2),0,math.rad(-42))
                    elseif n=="righthip" then t=base*CFrame.Angles(math.rad(18),0,math.rad(8))
                    elseif n=="lefthip" then t=base*CFrame.Angles(math.rad(18),0,math.rad(-8))
                    elseif n=="waist" or n=="rootjoint" then t=base*CFrame.Angles(math.rad(-8+bob*1.5),0,0)
                    elseif n=="neck" then t=base*CFrame.Angles(math.rad(4-bob),0,0) end
                    pcall(function() m.Transform=t end)
                end
            end
        end
        local function stopFlightAnimation()
            if wingFlightTrack then pcall(function() wingFlightTrack:Stop(.12) end); wingFlightTrack=nil end
            if wingFlightAnimObject then pcall(function() wingFlightAnimObject:Destroy() end); wingFlightAnimObject=nil end
            restoreFlightPose()
            if wingFlightAnimateScript and wingFlightAnimateScript.Parent and wingFlightAnimateOldDisabled~=nil then pcall(function() wingFlightAnimateScript.Disabled=wingFlightAnimateOldDisabled end) end
            wingFlightAnimateScript=nil; wingFlightAnimateOldDisabled=nil; wingFlightAnimMode="none"
        end
        local function startFlightAnimation(hum)
            stopFlightAnimation(); if not hum then return end
            local char=hum.Parent
            local animate=char and char:FindFirstChild("Animate")
            if animate and animate:IsA("LocalScript") then
                wingFlightAnimateScript=animate; wingFlightAnimateOldDisabled=animate.Disabled; pcall(function() animate.Disabled=true end)
            end
            local animator=hum:FindFirstChildOfClass("Animator")
            if not animator then animator=Instance.new("Animator"); animator.Parent=hum end
            pcall(function() for _,t in ipairs(animator:GetPlayingAnimationTracks()) do t:Stop(.08) end end)
            local track=nil

            -- The supplied Roblox Shop/catalog ID can differ from the underlying
            -- Animation.AnimationId. Resolve a nested Animation when possible.
            local resolvedAnimationId=nil
            local okObjects,objects=pcall(function()
                return game:GetObjects("rbxassetid://"..tostring(wingFlightAnimAssetId))
            end)
            if okObjects and type(objects)=="table" then
                for _,obj in ipairs(objects) do
                    ...
                    if found and tostring(found.AnimationId or "")~="" then
                        resolvedAnimationId=tostring(found.AnimationId)
                        break
                    end
                end
            ...
            local okEmote,emoteTrack=pcall(function()
                return hum:PlayEmoteAndGetAnimTrackById(wingFlightAnimAssetId)
            end)
            if okEmote and typeof(emoteTrack)=="Instance" and emoteTrack:IsA("AnimationTrack") then
                track=emoteTrack
            end
            ...
            if not track then
                ...
                anim.AnimationId=resolvedAnimationId or wingFlightAnimId
                wingFlightAnimObject=anim
                local okLoad,loaded=pcall(function() return animator:LoadAnimation(anim) end)
                if okLoad then track=loaded end
                if not track then
                    local okHum,loadedHum=pcall(function() return hum:LoadAnimation(anim) end)
                    if okHum then track=loadedHum end
                end
            end

            if not track then beginFallbackFlightPose(hum); return end
            wingFlightTrack=track; wingFlightAnimMode="track"
            pcall(function() track.Priority=Enum.AnimationPriority.Action4; track.Looped=true; track:Play(.05,1,1) end)
            updateWingMiniStatus("Flying ♡  •  cute flight animation")
            task.delay(.55,function()
                if not wingFlightActive or wingFlightTrack~=track then return end
                local playing=false; local length=0
                pcall(function() playing=track.IsPlaying; length=track.Length end)
                if not playing or length<=.01 then
                    pcall(function() track:Stop(0) end); wingFlightTrack=nil; beginFallbackFlightPose(hum)
                end
            end)
        end

        stopWingFlight=function(message)
            wingFlightActive=false
            stopFlightAnimation()
            if wingFlightConnection then pcall(function() wingFlightConnection:Disconnect() end); wingFlightConnection=nil end
            ...
            if wingFlightHum and wingFlightHum.Parent then pcall(function() wingFlightHum.AutoRotate=wingFlightOldAutoRotate end) end
            ...
            wingFlightHum=nil
            updateWingMiniStatus(message)
        end
        local function startWingFlight()
            if not wingFlightEnabled or not wingsAreWorn() then return false end
            ...
            stopWingFlight(); wingFlightActive=true; wingFlightHum=hum; wingFlightOldAutoRotate=hum.AutoRotate; hum.AutoRotate=false
            local bv=Instance.new("BodyVelocity"); bv.Name="KimqAngelWingFlightVelocity"; bv.MaxForce=Vector3.new(1e6,1e6,1e6); bv.P=9000; bv.Velocity=Vector3.zero; bv.Parent=root
            local bg=Instance.new("BodyGyro"); bg.Name="KimqAngelWingFlightGyro"; bg.MaxTorque=Vector3.new(1e6,1e6,1e6); bg.P=9500; bg.D=650; bg.CFrame=root.CFrame; bg.Parent=root
            wingFlightVelocity=bv; wingFlightGyro=bg; pcall(function() hum:ChangeState(Enum.HumanoidStateType.Freefall) end)
            startFlightAnimation(hum); updateWingMiniStatus()
            wingFlightConnection=RunService.RenderStepped:Connect(function(dt)
                if not wingFlightActive or not wingsAreWorn() or lp.Character~=char or hum.Health<=0 or not root.Parent then stopWingFlight(); return end
                local camera=workspace.CurrentCamera; if not camera then return end
                local look=camera.CFrame.LookVector; local flat=Vector3.new(look.X,0,look.Z)
                if flat.Magnitude<.05 then flat=Vector3.new(root.CFrame.LookVector.X,0,root.CFrame.LookVector.Z) end
                if flat.Magnitude<.05 then flat=Vector3.new(0,0,-1) end; flat=flat.Unit
                local right=Vector3.new(camera.CFrame.RightVector.X,0,camera.CFrame.RightVector.Z); if right.Magnitude<.05 then right=Vector3.new(1,0,0) else right=right.Unit end
                ...
                local horizontal=flat*f+right*side; if horizontal.Magnitude>1 then horizontal=horizontal.Unit end
                local idle=horizontal.Magnitude<.02 and up==0 and down==0
                local y=(up-down)*wingFlightVerticalSpeed; if idle then y=math.sin(os.clock()*2.35)*1.35 end
                local target=horizontal*wingFlightSpeed+Vector3.new(0,y,0)
                bv.Velocity=bv.Velocity:Lerp(target,math.clamp(dt*7.5,0,1))
                if wingFlightAnimMode=="pose" then applyFallbackFlightPose() end
                local pitch=math.rad(-f*7); local roll=math.rad(-side*9)
                local desired=CFrame.lookAt(root.Position,root.Position+flat)*CFrame.Angles(pitch,0,roll)
        ...
        UIS.InputBegan:Connect(function(input)
            ...
            local now=os.clock()
            if now-wingFlightLastJump<=.34 then
                wingFlightLastJump=0
            ...
            else wingFlightLastJump=now end
        end)
        lp.CharacterAdded:Connect(function() if stopWingFlight then stopWingFlight() end; destroyWingMiniGui(); clearWingHeadHalo() end)
        if type(_G.KimqRegisterConfigControl)=="function" then
            _G.KimqRegisterConfigControl("Angel Wings Flight","toggle",function() return wingFlightEnabled end,function(v) wingFlightEnabled=not not v; if not wingFlightEnabled then stopWingFlight() end end)
            _G.KimqRegisterConfigControl("Angel Wings Flight Speed","slider",function() return wingFlightSpeed end,function(v) wingFlightSpeed=math.clamp(tonumber(v) or 46,16,140); wingFlightVerticalSpeed=math.clamp(wingFlightSpeed*.66,18,80); updateWingMiniSpeedVisual() end)
        end
        ...
        local function makeAngelWingsWearable(source,tool)
            ...
            task.delay(.08,function()
                if visual.Parent and activeWornEquippable==visual then
                    createWingMiniGui()
        ...
        lp.CharacterAdded:Connect(function(char)
            bulletAPI.hookShotContainer(char); hookKnifeContainer(char)
            task.delay(1,function()
                local bp=lp:FindFirstChildOfClass("Backpack"); bulletAPI.hookShotContainer(bp); hookKnifeContainer(bp)
        ...
        local function scanAllExtras()
            bulletAPI.scanBeams(); scanKnives(); scanEquipables()
            if bulletAPI.beamList then bulletAPI.beamList.ScrollBarImageColor3=pcolor("hot",hotColor) end; knifeList.ScrollBarImageColor3=pcolor("hot",hotColor); equipList.ScrollBarImageColor3=pcolor("hot",hotColor)
        end
        ...
        task.defer(function()
            ...
            bulletAPI.applyBulletBeamOverride(extraState.BulletBeam or "None")
        end)
        ...
        local function setExtraState(state)
            ...
            if extraState.BulletColorMode~="Preset" and extraState.BulletColorMode~="Custom" and extraState.BulletColorMode~="Rainbow" then extraState.BulletColorMode="Preset" end
            local savedHex=type(state.BulletColorHex)=="string" and bulletAPI.normalizeHex(state.BulletColorHex) or nil
            if savedHex then
                extraState.BulletColorHex=savedHex
                bulletAPI.setSavedColor(savedHex)
            end
            ...
            task.defer(function()
                ...
                bulletAPI.refreshBeamStyle(); refreshKnifeStyle(); refreshEquipStyle(); bulletAPI.refreshBulletColorUI()
                bulletAPI.applyBulletBeamOverride(extraState.BulletBeam or "None")
        ...
        _G.KimqWeaponExtrasController={GetState=getExtraState,SetState=setExtraState,Refresh=scanAllExtras,RefreshColor=bulletAPI.refreshBulletColorUI,ApplyKnife=applyKnifeSkin,EquipItem=buildLocalEquippable}
        if type(_G.KimqRegisterConfigControl)=="function" then
    ...
    skinsBtn.MouseButton1Click:Connect(function() showSpecial(skinsPage,skinsBtn,"weapon skins","wraps, bullet beams, knife skins, and local equippable items") end)
    for _,b in ipairs(normalButtons) do
        if b and b.Parent then
            b.MouseButton1Click:Connect(function()
    ...
    pcall(function()
        game:GetService("StarterGui"):SetCore("SendNotification", {Title="Kimqetras HC",Text="v2.31 loaded ♡",Duration=4})
    end)
    main.Visible=true
...
-- ========================================================
-- v2.31 owner / developer credit guard
...
local fn,compileErr=loadstring(SOURCE)
if not fn then
    warn("[Kimqetras HC v2.31] COMPILE ERROR: "..tostring(compileErr))
    pcall(function()
        StarterGui:SetCore("SendNotification",{Title="Kimqetras HC compile error",Text=tostring(compileErr):sub(1,180),Duration=12})
    end)
    return
end
...
local ok,runtimeErr=xpcall(fn,function(err)
    local tb=""
    pcall(function() if debug and debug.traceback then tb=debug.traceback() end end)
    return tostring(err).."\n"..tostring(tb)
end)

if not ok then
    warn("[Kimqetras HC v2.31] RUNTIME ERROR: "..tostring(runtimeErr))
    pcall(function()
        StarterGui:SetCore("SendNotification",{Title="Kimqetras HC runtime error",Text=tostring(runtimeErr):sub(1,180),Duration=12})
    end)
end
