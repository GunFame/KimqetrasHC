...
local KIMQ_BUILD = "v2.104-corner-contact-shadows"
...
Hello.Text = "v2.104  •  corner/contact shadows ♡"
...
task.spawn(function()
    ...
    local function applyShadowLook()
        if shadowEnabled then
            ...
            -- Lower only environment diffuse fill. This makes creases, wall/floor
            -- junctions, stairs, and surfaces facing away from light read darker
            -- while leaving the visible sky/atmosphere colors alone.
            Lighting.EnvironmentDiffuseScale=math.clamp(original.EnvironmentDiffuseScale*(1-d*.90),0,1)
    ...
    do
        ...
        local refreshDark=makeShadowSlider(70,"Corner Darkness",0,100,function() return shadowDarkness end,function(v) shadowDarkness=math.clamp(v,0,100); applyShadowLook() end)
        ...
        local function setShadowDarkness(v) shadowDarkness=math.clamp(tonumber(v) or 65,0,100); applyShadowLook(); refreshShadowUI() end
...
task.spawn(function()
    ...
    pcall(function()
        game:GetService("StarterGui"):SetCore("SendNotification",{
            Title="Kimqetras HC",
            Text="v2.104 ACTIVE ♡ • corner shadows ready",
            Duration=7,
        })
    end)
