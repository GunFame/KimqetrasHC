-- Kimqetras HC v2.9 • performance pass • smooth camera swap • persistent local bullet color
-- Re-running this SAME build only reopens the current GUI instead of stacking another copy.
-- If an older build is already active, v2.9 refuses to stack and asks for one clean rejoin.
do
    local SINGLE_KEY = "KimqHC_v21_SingleRuntimeOptimized"
    local BUILD = "v2.9-performance-camera-persistent-bullets"
    ...
    -- Older builds did not expose a complete disconnect/cleanup handle. Starting v2.9
    ...
    if rawget(_G, SINGLE_KEY) and rawget(_G, "KimqHC_CurrentBuild") ~= BUILD then
        pcall(function()
            game:GetService("StarterGui"):SetCore("SendNotification", {
                Title = "Kimqetras HC",
                Text = "Older runtime detected. Rejoin once, then run v2.9 so only one version is active.",
                Duration = 7,
            })
        end)
        warn("[Kimqetras HC v2.9] Older runtime detected. Rejoin once before running v2.9; refusing to stack versions.")
        return
    end
...
-- v2.9 keeps the working v2.8 bullet behavior while removing the hot paths that caused equip/shoot/config lag.
...
task.spawn(function()
    ...
    local function setupWeaponExtras()
        ...
        local function styleVerifiedRoot(root,serial)
            ...
            for _,delayTime in ipairs({0.020,0.070}) do
        ...
        local function captureWithShortRetry(obj,serial)
            ...
            for _,delayTime in ipairs({0.015,0.055}) do
                task.delay(delayTime,function()
        ...
        local function hookShotTool(tool)
            ...
            tool.Equipped:Connect(function()
                if likelyBulletGun(tool) then task.defer(function() styleLocalToolTemplates(tool) end) end
            end)
