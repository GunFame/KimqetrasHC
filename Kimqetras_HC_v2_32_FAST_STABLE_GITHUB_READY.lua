-- Kimqetras HC v2.32 • fast stable UI + register fix + exact source macro
...
local KIMQ_BUILD = "v2.32-fast-stable-ui"
    ...
    if running and runningGui and runningBuild ~= KIMQ_BUILD then
        pcall(function()
            game:GetService("StarterGui"):SetCore("SendNotification", {
                Title = "Kimqetras HC",
                Text = "Another Kimqetras HC version is still active. Rejoin once, then run v2.32.",
                Duration = 8,
            })
        end)
        warn("[Kimqetras HC v2.32] Another runtime is active; rejoin once before switching builds.")
        return
    end
...
_G.KimqThemeEngineReady = false
    ...
    pcall(function() game:GetService("RunService"):UnbindFromRenderStep("KimqPermanentTimeV232") end)
end
...
task.spawn(function()
    ...
    pcall(function()
        RunService:BindToRenderStep("KimqPermanentTimeV232",Enum.RenderPriority.Last.Value,function()
    ...
    pcall(function()
        game:GetService("StarterGui"):SetCore("SendNotification", {Title="Kimqetras HC",Text="v2.32 loaded ♡",Duration=4})
    end)
    main.Visible=true
...
task.spawn(function()
    ...
    -- v2.32: section navigation does not repaint/rescan the entire GUI.
    ...
    -- v2.32 reveal happens only after this initial theme pass is complete.
end)


-- v2.32: delayed post-show cleanup passes removed.
...
-- ========================================================
-- v2.32 branding is deterministic at construction time; no delayed scrub pass.

-- ========================================================
-- v2.32 DETERMINISTIC REVEAL
...
task.spawn(function()
    ...
    local t0=tick()
    while not _G.KimqThemeEngineReady and tick()-t0<15 do task.wait(.03) end
